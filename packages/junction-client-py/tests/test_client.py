"""Tests for junction_client — mirrors TS @junction/client test coverage."""
...
from junction_client import JunctionClient, JunctionError
from junction_client.errors import ErrorCode, http_status_to_code, http_error
...
class TestErrors:
    ...
    def test_to_dict(self):
        err = JunctionError(ErrorCode.NOT_FOUND, "not found", status=404)
        d = err.to_dict()
...
class TestAuthCheck:
    def test_localhost_no_token_ok(self):
        mc = JunctionClient(base_url="http://localhost:5476")
        # Should not raise on construction
        assert mc.base_url == "http://localhost:5476"

    def test_remote_no_token_raises(self):
        mc = JunctionClient(base_url="http://example.com:5476")
        with pytest.raises(JunctionError) as exc_info:
            mc._check_auth()
        assert exc_info.value.code == ErrorCode.AUTH_REQUIRED

    def test_remote_with_token_ok(self):
        mc = JunctionClient(base_url="http://example.com:5476", token="test")
...
class TestMessageValidation:
    ...
    async def test_rejects_long_message(self):
        mc = JunctionClient(message_length_limit=100)
        with pytest.raises(JunctionError) as exc_info:
...
class TestContextBuffer:
    ...
    async def test_flush_preserves_failed_entries(self):
        mc = JunctionClient(max_retries=0)
...
class TestAppDataDir:
    def test_contains_app_name(self, monkeypatch):
        ...
        # JUNCTION_HOME set (e.g. an isolated pod home) must not shadow it.
