"""JunctionClient — async Python client for the Junction Gateway.

Standalone package (no dependency on junction main package).
Uses aiohttp for HTTP and WebSocket communication.

Mirrors the TypeScript @junction/client API surface.
"""
...
from junction_client.errors import JunctionError, ErrorCode, http_error
...
class JunctionClient:
    """Async HTTP client for the Junction Gateway.
    ...
    def __init__(
        ...
        *,
        ...
        port = os.environ.get("JUNCTION_PORT", "5476")
    ...
    async def _request(
        ...
        raise last_error or JunctionError(ErrorCode.NETWORK_ERROR, "Request failed")
    ...
    async def register_mcp_server(
        ...
        if not name or not command:
            raise JunctionError(ErrorCode.VALIDATION_ERROR, "MCP server requires name and command")
