...
Tier-based pull-request reviewer routing via `CODE_APPROVERS.yaml` in both Junction and JunctionWebsite packages. Automatically assigns reviewers based on file paths changed, with a drift validator test that fails the build if patterns don't match actual files.
...
The Code Reviewer is now a built-in app (`src/junction/apps/builtins/code_reviewer/`) with a full Python backend. It is disabled by default (`defaultEnabled: false`) and can be enabled via the App Store or config.
...
- `src/junction/apps/builtins/code_reviewer/` — built-in app backend
