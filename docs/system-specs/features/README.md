...
| [agent-interrupt-controller.md](agent-interrupt-controller.md) | `junction.irq`: masking, coalescing, epoch resets and an error backstop for script-cron pollers, so a cheap probe interrupts an expensive agent turn instead of the turn polling. Also the app-facing probe SDK. |
