...
(cursor → claude → codex → kimi → dsh → goose → grok → opencode → pi → droid) and only
...
fixed path — Junction is not the authority on where Kiro CLI is installed, and
...
**The CLI is always launched IN PLACE — never from a copy.** Junction execs the
...
The host always sends one-shot approvals (`always=False`, the default). Junction — not the agent — owns the trust scope (`slot._trust`, `slot._trust_reads`, `slot._trusted_patterns`, `safety_override`, `channel.trusted`, parent session `approval_policy`). Per-call `session/request_permission` is required so Junction's PreToolUse hooks (`auto_deny_tools`, sensitive-path checks, credential redaction) fire on every tool invocation. The `always=True` path is reserved for a future "skip Junction hooks for this exact tool" feature; no caller passes it today.
...
- Run `junction setup --agent-only` after editing

Note: there IS a top-level `agents/` directory used at runtime for project-level overrides, but the bundled source lives in `src/junction/config/`.

Default model: `claude-opus-4.8`. Default tools: `execute_bash`, `fs_read`, `fs_write`, `code`, `grep`, `glob`, `use_aws`, `web_fetch`, `web_search`, `introspect`, `session`, `report`, `@junction-cron`, `@junction-core`.
...
therefore removes that one Junction-authored legacy key and preserves every
unknown key; an unfamiliar key may belong to a newer kiro-cli schema or to the
user. Foreign specs, prefix lookalikes such as `junction-custom.json`, and app
...
strict filter for newly generated Junction specs, where Junction owns the whole
output schema.
...
- **Model**: `set_model` is skipped for custom agents — kiro-cli uses the
  agent's own `model` field. Only the default junction agent gets Junction's
...
- **MCP servers**: backend-dependent.
  - **kiro-cli**: `session/new` passes `mcpServers: []` — kiro-cli loads
    ...
    file). Non-junction agents (e.g. AIM-installed) load only their own
...
set (i.e., for the default junction agent).  Custom agents skip this so
...
`junction-lite` background runtime, whose config Junction itself writes with
...
log since kiro-cli's cancellation is a permission decision outside Junction's control.
...
### Model-wait watchdog (silent-retry harnesses)

Some harnesses retry a failed model call (a provider rate limit, an overload) inside the harness and send nothing over ACP while they do: OpenCode keeps its retry state in its own TUI status. The turn then goes silent before any output, or between a tool result and the next model call, and never ends. Neither `AcpClient` watchdog above covers that: the stale-turn check needs streamed text, and the tool-stall check needs a tool in flight. Such a turn used to hold until `_DEFAULT_PROMPT_TIMEOUT` (2 hours).

For a backend in `ACP_BACKENDS_SILENT_RETRY` (opt-in membership in `acp/types.py`; OpenCode only, since it is the one shown to do this), `_prompt_loop` runs a third check. When nothing arrives for `_MODEL_WAIT_STALL_TIMEOUT` (300 s), with no tool in flight and no permission request awaiting our answer, it kills the child (`_kill_process(force=True)`, so the next prompt cold-starts) and raises `AcpTurnStalled`, a non-transient `AcpError`. The watchdog state lives in `_prompt_loop` so every consumer (streaming, `send_message`, commands) gets the same view:

- **In flight** is read from raw frames (`_track_model_wait`): a `tool_call` id stays in `_tools_in_flight` until a `tool_call`/`tool_call_update` reports it `completed` or `failed`. A harness that never reports a terminal status keeps the watchdog quiet rather than risk stopping real work. A `session/request_permission` id stays in `_awaiting_answer` until `_send_response`/`_send_error` answers it (`_note_answered`).
- **Silence** is measured from the moment the consumer last handed control back after a frame, or from our last answer (`_last_answer_ts`), whichever is later. It never uses `_last_activity`, which every stderr line refreshes, so a harness that logs its retries to stderr is still caught; and time the consumer spent on a frame (a hook, a broadcast, a human approval) is never counted as harness silence.

The Kiro path never evaluates it (H13). Callers: the dashboard shows the error card (its text names a likely rate limit; no re-queue, because the error is not retry-eligible); the task runner counts it as an ordinary failed attempt (`MAX_RETRIES`); the harness router classifies it as `rate_limit` (`limits.classify_exception`), so a routed run rests the lane and fails over.
...
- **Sandbox ownership**: `_spawn()` calls `sandbox.wrap_argv()` to wrap the command with platform-native isolation (Linux: two-stage `unshare -rm` → `unshare -U` bind-mounts + UID drop; macOS: `sandbox-exec` Seatbelt profile). On Windows, where Junction has no native OS wrapper, an explicitly classified official Kiro backend delegates to Kiro CLI's built-in sandbox; every other backend retains the no-backend fail-closed policy. The parent passes a fully scrubbed child environment on every platform, which is the enforcement point for raw Windows delegation. Configurable via `sandbox_mode` constructor param (`"auto"` default, `"off"` to disable). See `docs/system-specs/modules/security.md`.
...
- **Sign-in is fully delegated to `kiro-cli`.** `kiro-cli login
  ...
  credential store, exactly as it does from a terminal. Junction stages no
  ...
  OS sandbox posture ACP already uses, with the Junction data home hidden.
