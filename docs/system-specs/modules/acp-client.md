...
- `"cursor"` / `"codex"` / `"dsh"` / `"pi"` / `"kimi"` / `"goose"` / `"grok"` / `"droid"`: argv from the runtime registry (`cursor-agent acp`, `npx @agentclientprotocol/codex-acp`, Buzz's DeepSeek Harness launcher, `pi-acp`, `kimi acp`, `goose acp`, `grok agent stdio`, `droid exec --output-format acp`). Handshake is ACP v1 (`PROTOCOL_VERSION_CLAUDE`). `session/load` `_meta` is omitted (not kiro's `_kiro.dev/session_file`).
...
- `~/.junction/agent.json` — user overrides (optional)
