...
| Target is a Multitask Mode session (`mode == "multitask"`) | 403 | A multitask session's turn lifecycle is not the dashboard's: `/api/chat` routes its input to `state.multitask.ingest`, which makes a durable queue entry and fans it out to topic sub-sessions. Refused rather than emulated — a target whose lifecycle differs needs its own handling, not a second copy of the orchestrator's rules |
...
- **No delivery to a target outside the addressable set.** `session_send` writes
  ...
  guard admits: a channel-linked, channel-mirrored, multitask-mode, incognito,
