...
- **Crew data-home secret/trust-root leaves are covered under EVERY known home prefix.** The default data home is `~/.junction`. Older directories stay covered. Each Junction secret / governance trust-root leaf (`.env`, `browser-cookies.txt`, `playwright-storage-state.json`, `sel_hmac.key`, `trust`, `security_events.jsonl`, `app_admission.json`, `security_policy.json`, `profiles`, `admission_policy.json`, `denied_commands.json`, `crons.json`, `cron-history`, `oauth_endpoints.json`, `live_target.json`, `token_signing.key`, `refresh_chains.json`, `.local_secret`, `routing`, `run`) is expanded onto `_SENSITIVE_HOME_DIRS` under each entry of `_CREW_HOME_PREFIXES = (".junction", ".kiro/crew", ".kirocrew")`. So the same leaf is read+write-blocked in (1) the current home `~/.junction`, (2) the previous home `~/.kiro/crew`, and (3) an older `~/.kirocrew` directory. Those older directories are not deleted; they stay on the floor because they can still hold secrets. A new secret is added to `_CREW_SECRET_LEAVES` once and is covered in both locations.
...
- **Bash gate**: the marker leaf is also in `_WRITE_PROTECTED_BASH_LEAVES`, and `is_sensitive_bash_command` matches it **verb-independently** (any command naming the home-anchored marker path, including a trailing-`/` subpath so `mkdir -p …/.data-home-ready/x` — which also materializes it — is caught). This mirrors the verb-independent backstop the sensitive-dir matcher uses, so quoted redirects / `cp` / `python open()` / novel write verbs cannot bypass an enumerated allowlist. Bash *reads* are incidentally blocked too — harmless, since the marker holds no secret (it is deliberately NOT in `_SENSITIVE_HOME_DIRS`, so file-read tools and `is_sensitive_path` are unaffected) and the only legitimate readers (`junction doctor`, the migration code) use Python `os` calls, not bash.
...
`~/.kiro/crew/denied_commands.json` (respecting `JUNCTION_HOME`) — NOT in the
...
user's real home and environment with only the Junction data homes — the
