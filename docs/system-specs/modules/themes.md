...
installer art) is generated by `assets/brand/build.py`. The frontend
pack-author contract (the CSS-var
surface and the
...
4. **Promote** — the validated snapshot is atomically renamed into
   `~/.junction/themes/<slug>/`. Concurrent installs of the same slug are
...
- **CSS containment** — install-time denylist (no `@import`, external `url()`,
  ...
  (milestone-bound to the first L1-pack release). Until then, drift between the two parsers — and between the scoper
