...
Storage: `~/.junction/security_events.jsonl` (append-only JSONL with HMAC-SHA256 chain).
...
| `downstream_service` | MCP server name if applicable (`junction-core`, `junction-cron`, `internal-mcp`) |
...
- **Second protocol anchored on this key — domain-separated:** `session_pid_sig.py`
  ...
  (`HMAC(sel_hmac.key, "junction.session_pid.sig.v1")`) so the sidecar MAC and
...
```
junction security events [-n 20]   # Show recent events
junction security verify            # Verify HMAC chain integrity
```
