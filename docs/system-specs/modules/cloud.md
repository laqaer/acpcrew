...
`src/junction/cloud/` runs Junction on the user's **own** AWS EC2 instance with a
...
- (2) the shell `deniedCommands` in `config/defaults.json` (kiro-cli
  ...
  the `junction cloud destroy|stop|start|launch|connect|tunnel|login|logout` wrappers
  ...
  to `junction-src-*` would be an agent exfiltration path. It also blocks the
...
- (3) an in-layer chokepoint — `run_aws` calls `assert_chokepoint_allowed()`,
  which under an agent session (`JUNCTION_SESSION_KEY` set) allows only an
...
`del os.environ['JUNCTION_SESSION_KEY']` before an in-process call, or — since
...
| `ec2.py` | `deploy`/`status`/`stop`/`start`/`destroy` via `aws cloudformation` + `ec2`; AZ- **and egress-**aware `discover_network` + `resolve_explicit_subnet` (`--subnet` pin, same guarantees); tag-based stateless discovery; `_validate_cidr`. `find_stack` verifies BOTH `junction:managed=true` AND `junction:instance==<tag>` before status/stop/start/destroy touch a stack — so a same-prefix managed stack with a different instance tag can't be acted on by the wrong `--tag`. |
| `iam.py` | Least-privilege launcher policy generator (applied by the user, never by Junction) + read-only reachability check + the **content-fixed instance permissions-boundary document** (`boundary_policy_document`/`boundary_arn`) and its constants (`BOUNDARY_NAME`). |
...
| `templates/junction-ec2.yaml` | The CloudFormation stack. |
...
(`junction-src-<account>-<region>`); the instance downloads it with its own IAM
...
- **IMDSv2 enforced** on the instance (`HttpTokens: required`, hop limit 1):
  Junction runs a prompt-injectable agent, so an SSRF/injection that reaches the
...
- **Least-privilege, tag-/prefix-scoped IAM.** The RCE-adjacent SSM verbs
  ...
  require `junction:managed=true`; CloudFormation stack mutation/delete is scoped
  to `stack/junction-*/*`, and the change-set verbs (which `aws cloudformation
  deploy` authorizes on the **changeSet ARN**, not just the stack ARN) are a
  separate statement scoped to both `changeSet/junction-*/*` and
  `stack/junction-*/*` (scoping to `stack/*` alone would deny the launch under
  the generated policy); only enumerate/`GetTemplateSummary` stay on `*`.
  `iam:PassRole` is scoped to `junction-ec2-*`; S3 is scoped to `junction-src-*`.
  Command-history
...
- **Create verbs require the managed request-tag (per-resource-ARN split).**
  ...
  `aws:RequestTag/junction:managed=true`, so a leaked launcher credential can't
...
- **Escalation primitives constrained + immutable, pre-created permissions
  ...
  can't tag an *existing* resource `junction:managed=true` to pull it under the
  ...
  self-authorship gap:
  - The boundary is a **single** managed policy named `junction-ec2-boundary`
    ...
    **not** per-launch CloudFormation. It is create-if-not-exists (tolerates
    ...
    `junction-src-<account>-*/*` (region-agnostic — IAM is global; the
  ...
  - The template no longer creates the boundary; the `InstanceRole` references it
    ...
    `^arn:aws:iam::[0-9]{12}:policy/junction-ec2-boundary$`), which the launcher
    fills with `arn:aws:iam::<account>:policy/junction-ec2-boundary`.
  ...
  - `iam:CreateRole` remains gated on `ArnLike iam:PermissionsBoundary ==
    arn:…:policy/junction-ec2-boundary` (`ArnLike`, NOT `StringEquals` — the
  ...
  - **Residual (first-write race), tracked in as-built:** the very first
    ...
    hole. Operators who want it gone entirely run `junction cloud iam-boundary`
  ...
  launcher path (`junction-src-${AccountId}-${Region}/${StackTag}/…`), not the
  ...
  tag-gated (`aws:ResourceTag/junction:managed=true`) so a leaked credential
...
- **`PutRolePolicy`/`PassRole` tag-gated (not just name-prefix).** Both
  ...
  `aws:ResourceTag/junction:managed=true` on the target role — not just the
  `junction-ec2-*` name prefix. Without the tag gate, a leaked launcher credential
  could target a **pre-existing** `junction-ec2-*` role that a third party created
  ...
  template's `InstanceRole.Tags`) — carries `junction:managed=true`, and the tag
...
- **`iam:TagRole` gated so the tag gate can't be self-defeated.** The tag gate
  ...
  unbounded `junction-ec2-*` role `junction:managed=true`, then inline admin +
  pass it. `iam:TagRole` is therefore **not** unconditioned in the role-management
  statement; it is its own statement gated on
  `aws:ResourceTag/junction:managed=true` (`IamTagRoleOnManaged`). `TagRole` is
...
- **Anti-squat bucket pin, end to end.** The launcher source bucket name is
  deterministic (`junction-src-<account>-<region>`) and thus globally
  ...
  verification; if the account can't be derived (the `junction-src-unknown-*`
  ...
  reused — not just on create, so a pre-existing `junction-src-*` bucket whose
...
`junction cloud launch`. They install *client* prerequisites only — the gateway
...
`junction cloud launch` runs `python -m junction`, which imports the whole CLI —
