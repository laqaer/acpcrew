...
The subagent module (`junction/subagent.py`) spawns isolated background agents for parallel task execution. Each subagent gets its own LLM session via `SessionManager`, runs a focused task, and announces the result via callback.
...
| `INJECTION_TIMEOUT` | 900 | Inner cap: max seconds for a single `stream_and_collect` call (15 minutes); default `_DEFAULT_INJECTION_TIMEOUT = 900.0`, tunable via `JUNCTION_INJECTION_TIMEOUT` (float seconds, clamped to `_ON_DONE_TIMEOUT`) |
...
3. If still empty **and subagent has no parent session key** → use the cached `JunctionConfig.agent.approval_mode` (snapshotted at `SubagentManager` init); if `"auto"` → `"auto"`
...
| Inner cap | `gateway.py _subagent_done()` | 900s (`INJECTION_TIMEOUT`, tunable via `JUNCTION_INJECTION_TIMEOUT`) | Single `stream_and_collect` call |
...
`skills/subagent/SKILL.md` (project-level) triggers on keywords: `background`, `spawn`, `bg`, `subtask`, `parallel`, `separately`, `concurrently`. Instructs the LLM to use `junction spawn "task"` via bash to spawn subagents.

### CLI: `junction spawn "task"`
...
Exposed via `junction-core` MCP server. Always fire-and-forget — results
...
Exposed via `junction-core` MCP server. Unlike fire-and-forget `spawn_run`,
...
Each sub-agent runs as its own Junction-owned ACP session (via
...
- `max_wait` defaults to 7200s (2 hours), clamped to `[60, 7200]`, and is configurable via the `JUNCTION_SPAWN_SUB_AGENTS_MAX_WAIT` environment variable. The deadline uses `time.monotonic()`.
...
Unknown `agent.completion_keep` values cause `junction gateway` to fail
...
[`src/junction/docs/configuration.md`](../../../src/junction/docs/configuration.md),
[`src/junction/docs/subagents.md`](../../../src/junction/docs/subagents.md),
