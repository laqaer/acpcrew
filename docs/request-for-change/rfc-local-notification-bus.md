...
Notifications are generated by `DashboardState.notify(kind, title, body, *, meta=None)` (`src/junction/dashboard/state.py:1508`). The method appends to an in-memory log, increments an unread counter, broadcasts to SSE/WS clients, and persists to `~/.kirocrew/notifications.jsonl`.
