...
Full schemas in `steward_ledger_spec.md`. The shape, and the three things that are
easy to get wrong:
...
fixtures via `scripts/lib/issue-radar-stewards-fixtures.mjs`.
...
it through `components/StewardPlate.tsx`; the agent roster uses the same generator
through `components/AgentAvatar.tsx`. The generator emits SVG as a string and the
...
| Brief size | 25,781 chars ≈ 6.4k tokens | `steward_brief.md` |
