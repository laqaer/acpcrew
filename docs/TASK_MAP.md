...
| CLI chrome | Primary `junction`; aliases `acpcrew` / `junction` | CLI packaging / entry | Done via #23 | [#19](https://github.com/laqaer/junction/issues/19) |
| Model-router | Observe + compose: health/status, catalog, role DAG; advertised ids actually applied; sidecar slugs skipped until M2 | `src/junction/model_router/`, [model-router spec](system-specs/modules/model-router.md) | Catalog via #24; apply via this PR | [#20](https://github.com/laqaer/junction/issues/20) |
