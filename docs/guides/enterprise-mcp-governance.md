# Junction behind enterprise MCP governance
...
Junction starts, the dashboard works, chat works — and a large part of the
...
no tools to work with. Nothing errors. `junction doctor` reports the MCP servers
healthy.
...
- Junction's own probe **spawns each server directly** and completes an MCP
...
### 1. Declare registry mode on the Junction side
...
Junction then stamps `"type": "registry"` on the servers it manages, so they
...
Verify with `junction doctor`, which grows an `MCP Governance (enterprise)`
section whenever the local identity came from Identity Center.

### 2. Have the administrator allow-list the servers

Junction needs three servers, and they must appear in the registry file under
**exactly** these names:
...
Set `version` to the Junction version your fleet runs.
...
organisation pins Junction centrally, that pin now governs the MCP side too.
