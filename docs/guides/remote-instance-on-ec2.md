...
2. To let the dashboard manage it, add it in **Settings → Remote Instances → Add
   remote instance → Connection method = SSH tunnel**, giving the **SSH host / alias** and the
   **remote port**.
...
- **Manual (a box you already have):** **Settings → Remote Instances → Add remote
  instance → Connection method = AWS SSM Session Manager**, then fill **SSM target** (the EC2
