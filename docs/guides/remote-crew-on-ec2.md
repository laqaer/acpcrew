# Setting up a remote gateway on an EC2 instance
...
registered in **Settings → Remote gateways**.
...
2. To let the dashboard manage it, add it in **Settings → Remote gateways → Add remote
   gateway → Connection method = SSH tunnel**, giving the **SSH host / alias** and the
   **remote port**.
...
- **Manual (a box you already have):** **Settings → Remote gateways → Add remote gateway →
