# `junction pod` — Requirements

## Overview

`junction pod` gives developers **kubectl-style, throwaway, full-stack test
instances**, one per feature worktree. A *pod* is an ephemeral Junction gateway
booted from a worktree's **own** `.venv`, on its **own** deterministic port, with
its **own** `JUNCTION_HOME` (isolated DB / sessions / memory), **no Slack tunnel**,
...
As a contributor, I want `junction pod up <worktree>` to boot that worktree's full
...
- WHEN `--provision` (or `junction pod provision <worktree>`) is given, THE SYSTEM
...
- `junction pod ls` SHALL list running pods with port + health (≈ `kubectl get pods`).
- `junction pod status <wt>` SHALL report up/down + port + health.
- `junction pod url <wt>` SHALL print the pod base URL.
- `junction pod token <wt> [--ttl]` SHALL (re)mint a dashboard token for a running pod.
- `junction pod logs <wt> [-n N]` SHALL tail the pod's journal.
- `junction pod down <wt>` SHALL evict the pod and delete its isolated HOME with
  zero residue, leaving the live plane untouched.

### US-4 — One-time install
As a contributor, I want `junction pod install` to lay down the `systemd --user`
...
- A configured fixed worktree root as the *primary* resolver — git is the primary
  ...
  (`JUNCTION_POD_WORKTREES_ROOT`) is only an optional fallback for hermetic planes.
