...
so a `pip install` of Junction is complete. Mirrors the thin-verb / runtime split
used elsewhere in the CLI.

```
src/junction/pod/
  __init__.py     # exports PodConfig, PodError, derive_port, resolve_checkout, pod_home, pod_unit
  config.py       # PodConfig dataclass — every path/knob, JUNCTION_POD_*-overridable
...
Wiring into the existing CLI (`src/junction/cli.py`):
...
- implement `_pod(args)` in `cli_commands.py` → `from junction.pod.cli import dispatch; dispatch(args)`
...
- **Control plane** — the `junction pod` verbs (worktree resolution, port
  derivation, unit management, token mint, boot *prep*). These run from the
  **stable, globally-installed** `junction`, so they never break because a
  worktree's code is broken.
- **Payload** — the booted pod **is** the worktree's `.venv/bin/junction gateway`.
...
Junction worktrees are **plain git worktrees** (flat repo root — no nested package
...
2. **git** — `git -C <ref> worktree list --porcelain`, where `<ref>` is
   `JUNCTION_POD_REPO` if set else the invoking `cwd` (git lists ALL linked
...
3. **root fallback** — `JUNCTION_POD_WORKTREES_ROOT/name`, only if that env is set
...
| | value |
|---|---|
| venv binary | `checkout / ".venv" / "bin" / "junction"` |
| built dist | `checkout / "src" / "junction" / "static" / "dist"` |
...
- `ensure_venv(checkout)`: `python3.12 -m venv .venv` then `.venv/bin/pip install -e .`
  (editable install → provides `.venv/bin/junction`).
- `build_dist(checkout)`: `npm run build` in `<checkout>/website` (→ `website/dist`),
  then stage `website/dist` into the served `src/junction/static/dist`. Progress
  goes to **stderr** so a concurrent `pod up --json` keeps a clean stdout.

## Boot (the `ExecStart` body)

`junction pod _run <name>` (hidden verb) → `runtime.boot()`:
...
6. `os.execve(bin_path, ["junction","gateway","--no-crons"], env)` with `build_pod_env(...)`.

`build_pod_env` sets `JUNCTION_HOME`, `JUNCTION_PORT`, `JUNCTION_PROJECT_DIR`, a
...
`junction pod install` writes `~/.config/systemd/user/junction-pod@.service`:
...
- `Environment=` lines pin every non-default `JUNCTION_POD_*` (roots/ports/prefix/
  PATH, plus `JUNCTION_POD_REPO`/`JUNCTION_POD_WORKTREES_ROOT` when set) so the
  systemd-booted gateway resolves the same plane the installing CLI used (systemd
  starts with a clean env). `JUNCTION_POD_BIN` overrides the booted binary.
...
| `JUNCTION_POD_PATH` | generic PATH | PATH handed to a booted pod gateway |
