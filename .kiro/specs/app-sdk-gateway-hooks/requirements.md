...
This feature extends the Junction App SDK and gateway to provide backend integration hooks, enabling apps (both built-in and external) to register API routes, manage cron jobs, and hook into gateway lifecycle without modifying Junction core files. The goal is to eliminate the pattern where every new app must hardcode itself into `routes.py`, `server.py`, and `manager.py`.

## Glossary

- **Gateway**: The Junction aiohttp server process that hosts the dashboard, API routes, and app lifecycle management.
...
- **Builtin_App**: An app bundled with Junction source code, located in `src/junction/apps/builtins/`.
...
**User Story:** As an app developer, I want to declare HTTP route handlers in my app manifest and implementation, so that Junction automatically mounts them without requiring changes to core gateway files.
