...
Implement gateway-side hooks enabling apps to register routes, manage crons, and participate in lifecycle events without modifying Junction core files. Implementation is in Python (aiohttp) within the Junction package.
...
- [x] 2. Implement App Context, Cron SDK, EventBus, and AppStorage
  - [x] 2.1 Create `src/junction/apps/context.py` with `AppContext` dataclass and `AppHealthStatus`
  ...
  - [x] 2.2 Create `src/junction/apps/cron_sdk.py` with `CronSDK` class
  ...
  - [x] 2.3 Create `src/junction/apps/event_bus.py` with `EventBus` class
    ...
    - _Requirements: 5.2_
  - [x] 2.4 Create `src/junction/apps/app_storage.py` with `AppStorage` class
...
- [x] 4. Implement Module Loader and Route Registry
  - [x] 4.1 Create `src/junction/apps/module_loader.py` with isolated module loading
    - Implement `load_app_module(app_name, app_dir, module_path)` using `spec_from_file_location`
    - Register modules as `_junction_app_{app_name}.{dotted_path}` in sys.modules
  ...
  - [x] 4.2 Create `src/junction/apps/route_registry.py` with `RouteRegistry` class
...
- [x] 5. Implement Lifecycle Hook Dispatcher
  - [x] 5.1 Create `src/junction/apps/lifecycle.py` with `LifecycleDispatcher` class
...
- [x] 6. Implement Builtin Auto-Discovery
  - [x] 6.1 Create `src/junction/apps/discovery.py` with `discover_builtin_apps()` function
