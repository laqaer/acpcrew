...
def download_then_parse(url):
    ...
    # ok: junction.download-then-exec
    resp = requests.get(url)
    return resp.json()
...
def download_then_subprocess(fetch):
    ...
    # ruleid: junction.download-then-subprocess
    download_path = fetch()
...
def run_pinned_binary(config_path):
    ...
    # ok: junction.download-then-subprocess
