...
``JUNCTION_PORT`` env var, not a config key.)
"""
...
from junction import __version__, model_registry, platform_compat
...
# ``junction`` — so this cannot create an import cycle with the loader, and the
# ``computer_use`` package's ``__init__`` pulls in only ``platform_compat`` /
# ``executors`` (both stdlib-only), never ``config``.
from junction.computer_use.types import DEFAULT_ATTACH_SCREENSHOT as _CU_DEFAULT_ATTACH_SCREENSHOT
from junction.computer_use.types import DEFAULT_MAX_TREE_DEPTH as _CU_DEFAULT_MAX_TREE_DEPTH
from junction.computer_use.types import DEFAULT_MAX_TREE_NODES as _CU_DEFAULT_MAX_TREE_NODES
from junction.computer_use.types import (
    DEFAULT_SCREENSHOT_JPEG_QUALITY as _CU_DEFAULT_SCREENSHOT_JPEG_QUALITY,
)
from junction.computer_use.types import DEFAULT_SCREENSHOT_MAX_PX as _CU_DEFAULT_SCREENSHOT_MAX_PX
from junction.computer_use.types import DEFAULT_TEXT_LIMIT as _CU_DEFAULT_TEXT_LIMIT
from junction.computer_use.types import MAX_SCREENSHOT_MAX_PX as _CU_MAX_SCREENSHOT_MAX_PX
from junction.computer_use.types import MAX_TEXT_LIMIT as _CU_MAX_TEXT_LIMIT
from junction.computer_use.types import MAX_TREE_DEPTH_LIMIT as _CU_MAX_TREE_DEPTH
from junction.computer_use.types import MAX_TREE_NODES_LIMIT as _CU_MAX_TREE_NODES
from junction.computer_use.types import MIN_SCREENSHOT_MAX_PX as _CU_MIN_SCREENSHOT_MAX_PX

# Pure path primitives live in the leaf module ``config.paths`` (stdlib-only,
# no ``junction`` imports) so the modules that only need ``config_dir()`` can
...
# these from ``junction.config.loader``.
#
...
# ``patch("junction.config.loader.config_dir", ...)`` (used across the suite).
from junction.config.paths import (  # noqa: F401, kiro_agents_dir
    _WORKSPACE_DIR_NAME,
    CONFIG_DIR_NAME,
    OUTBOX_DIR_NAME,
...
from junction.config.superseded_defaults import drift_summary, superseded_default_drift
...
# reference these as ``junction.config.loader.X`` (e.g. the cache tests patch
# ``junction.config.loader._validate_config_data``). ``validate_config_data``
...
from junction.config.validation import (  # noqa: F401
    _CONFIG_CACHE,
    _CONFIG_CACHE_LOCK,
    _HAS_JSONSCHEMA,
    _actual_type_name,
...
from junction.config.validation import validate_config_data as _validate_config_data  # noqa: F401
from junction.effort import EFFORT_LEVELS, is_valid_effort, model_supports_effort
from junction.instances.constants import CONNECT_TIMEOUT_CEILING_SECS as _CONNECT_TIMEOUT_CEILING
from junction.instances.constants import DEFAULT_CONNECT_TIMEOUT_SECS as _DEFAULT_CONNECT_TIMEOUT
from junction.instances.constants import DEFAULT_MAX_RECOVERY_ATTEMPTS as _DEFAULT_MAX_RECOVERY
from junction.instances.constants import DEFAULT_MINT_TIMEOUT_SECS as _DEFAULT_MINT_TIMEOUT
from junction.instances.constants import DEFAULT_PROBE_FAILURE_THRESHOLD as _DEFAULT_PROBE_FAILS
from junction.instances.constants import DEFAULT_RECOVER_BACKOFF_MAX_SECS as _DEFAULT_BACKOFF_MAX
from junction.instances.constants import DEFAULT_SSH_COMPRESSION as _DEFAULT_SSH_COMPRESSION
from junction.instances.constants import DEFAULT_TUNNEL_BASE_PORT as _DEFAULT_TUNNEL_BASE_PORT
from junction.instances.constants import DEFAULT_WARM_SET_CAP as _DEFAULT_WARM_SET_CAP
from junction.instances.constants import MAX_RECOVERY_ATTEMPTS_CEILING as _MAX_RECOVERY_CEILING
from junction.instances.constants import MINT_TIMEOUT_CEILING_SECS as _MINT_TIMEOUT_CEILING
from junction.instances.constants import MINT_TIMEOUT_FLOOR_SECS as _MINT_TIMEOUT_FLOOR
from junction.instances.constants import (
    RECOVER_BACKOFF_MAX_CEILING_SECS as _RECOVER_BACKOFF_CEILING,
)
from junction.mcp_gateway.rewriter import default_overlay_dir, default_socket_path
from junction.model_router.routing import ROUTE_ROLE_KEYS as ROLE_MODEL_KEYS
...
# must exclude them, or Junction warns the user about a key it wrote itself.
...
# JunctionConfig._extra_sections and re-emitted by to_dict() so an
...
CRED_OWNER_ID = "JUNCTION_OWNER_ID"
...
# JUNCTION_PORT is validated at CLI entry (cli.py main()).
# By the time loader.py is imported the env var is a valid int or absent.
DASHBOARD_PORT: int = int(os.environ.get("JUNCTION_PORT", _DEFAULT_PORT))
...
# ``patch("junction.config.loader.config_dir", ...)`` test seam working.


def _workspace_dir_file() -> Path:
    """Return the path to the saved workspace_dir file, respecting JUNCTION_HOME."""
...
def workspace_root() -> Path:
    ...
    Resolution order:
    1. ``JUNCTION_WORKSPACE`` env var (used as-is, no subdirectory appended)
    2. Saved path in ``config_dir()/workspace_dir`` (written by ``junction setup``)
    3. Platform default with ``junction-workspace`` subdirectory
    ...
    """
    override = os.environ.get("JUNCTION_WORKSPACE")
    if override:
...
def denied_commands_path() -> Path:
    ...
    which do not route through the agent tool gate. Respects ``JUNCTION_HOME``.
    """
...
def computer_use_state_path() -> Path:
    ...
    agent tool gate. Respects ``JUNCTION_HOME``.
...
def oauth_endpoints_path() -> Path:
    ...
    the operator hand-edits the file out-of-band. Respects ``JUNCTION_HOME``.
    """
...
def aws_consent_path() -> Path:
    ...
    ``junction aws-consent`` CLI, both of which open the path directly rather
    than through this gate. Respects ``JUNCTION_HOME``.
    """
...
def read_config_for_update(path: Path | None = None) -> dict:
    ...
    Pair this with :func:`junction.atomic_write.atomic_write` on the way out so
...
#: Marker used in :attr:`JunctionConfig.degraded_sections` for "a whole config
...
def write_config_atomically(path: Path, data: dict, *, fsync: bool = False) -> None:
    ...
    **This deliberately does NOT call ``platform_compat.restrict_to_owner``.**
    ...
    ``JunctionConfig.save()`` — so invoking it here would put a blocking subprocess
...
def default_project_dir(workspace: str | None = None) -> str:
    ...
    """
    from junction.security import is_sensitive_path  # circular import
...
def read_env_file_credential(key: str, env_file: Path | None = None) -> str:
    ...
    Same line format :meth:`JunctionConfig.load_credentials` parses (one pair
...
def inject_kiro_cli_api_key(env: MutableMapping[str, str]) -> MutableMapping[str, str]:
    ...
    consumed in-process from :meth:`JunctionConfig.load_credentials`, but this
...
def resolve_agent_config_path() -> Path:
    ...
    """
    proj = os.environ.get("JUNCTION_PROJECT_DIR")
    if proj:
...
class AgentConfig:
    ...
    mcp_registry_mode: bool = field(
        ...
        metadata=_meta(
            ...
            "and junction-computer in the registry by those exact names.",
        ),
    )
    ...
    sandbox_allow_unsandboxed_exec: bool = field(
        ...
        metadata=_meta(
            ...
            "Windows host, a Linux kernel refusing user namespaces) `junction "
    ...
    jail: str = field(
        ...
        metadata=_meta(
            ...
            "it. Disable per-invocation with --no-jail or JUNCTION_NO_JAIL=1.",
    ...
    log_level: str = field(
        ...
        metadata=_meta(
            "Log Level",
            "Persistent log level for the junction logger. "
    ...
    def __post_init__(self) -> None:
        ...
        # Clamp to [0.5, 60.0] to match ``JunctionConfig.load()`` behavior
...
class MemoryConfig:
    ...
    embed_model_path: str = field(
        ...
        metadata=_meta(
            ...
            "JUNCTION_EMBED_MODEL_PATH env var wins over this.",
        ),
    )
...
class KnowledgeConfig:
    ...
    doc_ingest_hosts: list[str] = field(
        ...
        metadata=_meta(
            ...
            "Exact hostnames whose links may be fetched by JUNCTION ITSELF and "
...
class SlackConfig:
    ...
    use_tunnel_url: bool = field(
        ...
        metadata=_meta(
            ...
            "When true, dashboard links posted to Slack (e.g. via /junction dashboard) "
...
class DashboardConfig:
    ...
    qr_session_until_restart: bool = field(
        ...
        metadata=_meta(
            ...
            "whether or not the gateway is still running. Either way `junction "
...
class JunctionAgentConfig:
    ...
    source: str = field(
        default="junction",
        metadata=_meta("Source", "Agent origin: junction or builtin."),
    )
...
class SkillsConfig:
    ...
    # ``junction config set skills.auto_create_from_sessions true`` or the
...
class TelemetryConfig:
    ...
    otlp_endpoint: str = field(
        ...
        metadata=_meta(
            ...
            "junction[otlp] package extra to be installed "
    ...
    beacon_enabled: bool = field(
        ...
        metadata=_meta(
            ...
            "and for a non-default JUNCTION_HOME. Opt out with "
            "JUNCTION_TELEMETRY_DISABLED=1 or by turning this off; an "
...
# ---------------------------------------------------------------------------
# Validation helpers — used by JunctionConfig.load()
# ---------------------------------------------------------------------------
...
# GET /api/config/junction response (which serializes a freshly loaded config)
...
def _cached_validated_data() -> dict | None:
    ...
    Thin wrapper over the :class:`~junction.config.validation.ConfigCache`:
    ...
    test suite patches as ``junction.config.loader.config_path``).
    """
...
def _normalize_jail(value: object) -> str:
    ...
    ``off`` per-invocation is expressed via ``--no-jail`` / ``JUNCTION_NO_JAIL``,
...
def _normalize_acp_backend(value: object) -> str:
    ...
    ``junction.acp.types`` executes the ``junction.acp`` package init.
    """
...
class ResolvedBindings:
    ...
    kiro_agent: str
    # The Junction agent's own default model, "" when it pins none. Ranks below
    ...
    # The Junction ALIAS whose bindings these are ("" when no alias applied). A
...
class McpGatewayConfig:
    ...
    apps_enabled: bool = field(
        ...
        metadata=_meta(
            ...
            "Apps off stays off (tightest-wins: it beats JUNCTION_MCP_APPS=1, and an "
    ...
    socket_path: str = field(
        ...
        metadata=_meta(
            ...
            "$JUNCTION_HOME/mcp-gateway/gateway.sock. A unix socket at this path "
    ...
    overlay_dir: str = field(
        ...
        metadata=_meta(
            ...
            "Empty -> $JUNCTION_HOME/mcp-gateway/agents.",
        ),
    )
...
class InstancesConfig:
    ...
    remote Junction instances over SSH tunnels. Off by default — opt-in only,
    ...
    ``junction.instances.constants`` so their canonical values cannot drift
    from this dataclass.
    """
    ...
    mint_timeout_secs: float | None = field(
        ...
        metadata=_meta(
            "Mint Timeout (secs)",
            "How long to wait for the remote `junction token` mint to return "
...
class TunnelConfig:
    ...
    name_mode: str = field(
        ...
        metadata=_meta(
            "Name Mode",
            "Tunnel naming: 'username' uses 'junction', "
            "'hash' uses 'junction-<hostHash>' for multi-host disambiguation.",
            enum=["username", "hash"],
        ),
    )
...
def _parse_telegram_accounts(raw: object) -> dict[str, "TelegramAccountConfig"]:
    ...
    through :meth:`JunctionConfig.save` with its tokens and allow-lists intact;
...
class JunctionConfig:
    ...
    def load(cls) -> JunctionConfig:
        ...
        # than read there because junction.env.mcp_search_path is reached from
        ...
        try:
            from junction.env import publish_config_path_dirs
    ...
    @classmethod
    def _load_resolved(cls) -> JunctionConfig:
        ...
        if cached_data is not None:
            ...
            # the default "junction" agent in-memory (matching the on-disk
            ...
            if not loaded_base and not local_data:
                ...
                kiro = cfg.agent.default_agent or "junction"
                cfg.agents["default"] = JunctionAgentConfig(
                    kiro_agent=kiro,
                    workspace="default",
        ...
        # normalized form (see JunctionConfig.degraded_sections).
        ...
        # Parse agents section into dict[str, JunctionAgentConfig]
        raw_agents = data.get("agents", {})
        agents: dict[str, JunctionAgentConfig] = {}
            ...
            for name, entry in raw_agents.items():
                if isinstance(entry, dict):
                    ...
                    agents[name] = JunctionAgentConfig(
                        ...
                        source=entry.get("source", "junction"),
        ...
        cfg = cls(
            agent=AgentConfig(
                ...
                # ride on the `junction-dashboard` server an operator may already
            ...
            slack=SlackConfig(
                ...
                command=slack_data.get("command", "junction"),
            ...
            mcp=McpConfig(
                ...
                # junction.env.augmented_path, so the ONE gate the built-in
    ...
    def _resolve_agent_model() -> str:
        ...
        """
        agent_json = kiro_agents_dir() / "junction.json"
    ...
    def load_credentials(self) -> dict[str, str]:
        ...
        # credentials from the process environ (setting _JUNCTION_CREDS_SCRUBBED=1),
        ...
        scrubbed = bool(os.environ.get("_JUNCTION_CREDS_SCRUBBED"))
    ...
    def create_provider_factory(self) -> Callable:
        ...
        Junction is KiroACP-only: the sole provider is the ACP adapter driving
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            if agent in ("junction-lite", "junction-heartbeat"):
...
def build_provider_factory(cfg: "JunctionConfig") -> Callable:
    ...
    """
    from junction.platform.context import current_context, safe_context_call
...
def _workspace_name_for_dir(config: JunctionConfig, ws_dir: Path) -> str:
...
def _materialized_kiro_agent(agent_name: str | None, project_dir: str | None = None) -> str:
    ...
    Junction alias. Without this lookup :func:`resolve_agent_bindings` would fall
...
def _read_hardened_agent_spec(path: Path) -> dict | None:
    ...
    ``agent_discovery`` imports ``junction.hooks``, whose closure reaches
...
def _project_declares_agent(agent_name: str, project_dir: str) -> bool:
    ...
    * **On the loop** — read the cache and nothing else, via a helper that makes no
    ...
    """
    try:
        # circular import: agent_discovery imports junction.hooks (the hardened
        ...
        # junction.config.loader in sys.modules. A module-scope import here would
...
def resolve_agent_bindings(
    ...
    *project_dir* is the session's active project directory, which widens step 2 to
    ...
    # An app agent is resolvable by kiro-cli but is not a Junction alias, so it
...
def resolve_effective_model(
    ...
    2. the bound kiro agent's pinned ``model`` (skipped for the built-in
       ``junction`` agent, which tracks the global by design)
    3. the global ``agent.model`` default
    4. the installed ``junction.json`` / bundled ``defaults.json`` model
    ...
    if kiro_agent and kiro_agent != "junction":
