"""Config validation + the validated-data cache for Junction.
...
- ``_config_fingerprint()`` — it reads ``config_path()`` / ``config_local_path()``,
  which the test suite patches via ``junction.config.loader.config_path``; keeping
...
``junction.config.loader`` logger — not this module's ``__name__`` logger — to
...
#
# JunctionConfig.load() is called on per-message / per-request hot paths (skill
...
# name. Exposed so any lingering `junction.config.loader._CONFIG_CACHE_LOCK`
...
def validate_config_data(data: dict) -> dict:
    ...
    # circular import: schema.py imports JunctionConfig from config.loader, which
    ...
    from junction.config.loader import CONFIG_RESERVED_TOP_KEYS
    from junction.config.schema import JSON_SCHEMA, SCHEMA_REGISTRY
    ...
    # must be excluded — otherwise every load of a config Junction has ever
    # saved warns about Junction's own bookkeeping.
