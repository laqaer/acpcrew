"""Pure filesystem path primitives for Junction configuration.
...
factory) the way ``from junction.config.loader import config_dir`` does.
...
``outbox_dir``, ``env_path``, …) remain in :mod:`junction.config.loader` so that
their ``config_dir()`` lookups resolve in the loader namespace — preserving the
``patch("junction.config.loader.config_dir", ...)`` test seam used across the
suite.

All names here are also re-exported from ``junction.config.loader`` for
...
# Override: JUNCTION_WORKSPACE env var or <config_dir>/workspace_dir
# macOS: /Volumes/workplace/junction-workspace (fallback ~/workplace)
# Linux: ~/workplace/junction-workspace
_WORKSPACE_DIR_NAME = "junction-workspace"
...
# Memo for ``config_dir()``: ``(raw JUNCTION_HOME, _resolved_home at the time,
...
# (``JUNCTION_HOME`` is repointed per test by the suite's isolation fixture, and
...
def _write_recovery_breadcrumb(data_home: Path) -> None:
    ...
    try:
        ...
        content = (
            "Junction data-home location pointer (safe to delete).\n"
            "\n"
...
# ``JUNCTION_HOME=/etc/junction`` accepted on macOS only, so the two platforms
...
def _is_unsafe_home(p: Path) -> bool:
    ...
    through, and Junction would then create agent JSON inside ``/etc``.
    ...
    Shared by :func:`_valid_override_home` (``JUNCTION_HOME``) and
...
def _valid_override_home() -> Path | None:
    """Return the resolved ``JUNCTION_HOME`` override iff it is set AND valid.
...
def shared_kiro_settings_writable() -> bool:
    ...
    Keyed on ``JUNCTION_POD`` rather than on the presence of a data-home override,
    because a custom ``JUNCTION_HOME`` is a normal single-instance install (the
    ...
    what it sees.
    """
    return not os.environ.get("JUNCTION_POD")


def config_dir() -> Path:
    global _config_dir_memo
    override_raw = os.environ.get("JUNCTION_HOME")
    ...
    if p is not None:
        ...
        return p
    if os.environ.get("JUNCTION_HOME"):
        logger.warning(
            "JUNCTION_HOME=%s is a system directory, ignoring",
            os.environ.get("JUNCTION_HOME"),
        )
...
def data_home() -> Path:
    ...
    1. A **valid** ``JUNCTION_HOME`` override -> delegate to :func:`config_dir`
...
def config_package_dir() -> Path:
    """Return the installed ``junction/config/`` directory.
...
def kiro_home() -> Path:
    ...
    but Junction currently resolves the host ``~/.kiro`` for most of those readers
    ...
    moving where Junction READS them, which breaks session resume. Only the agents
...
def kiro_sessions_dir() -> Path:
    ...
    so an instance that redirects its agent specs and then has Junction read
...
def isolated_agents_dir(data_home: Path) -> Path:
    ...
    happens to be an ancestor of it (``JUNCTION_HOME=$HOME`` is enough), which
...
def kiro_agents_dir() -> Path:
    ...
    Lives in this leaf module so :mod:`junction.config.loader` can locate
    installed agent JSONs without importing :mod:`junction.agent` — which
