...
AWS Transcribe is supported as an optional extra (``pip install 'junction[voice]'``).
"""
...
from junction import aws_consent, dep_sync, platform_compat
from junction.executors import stt_executor
...
def _own_scripts_dir() -> str:
    ...
    * ``shutil.which`` only sees ``PATH``, and a venv is on ``PATH`` only after
      ``activate``. The gateway is launched as ``<venv>/bin/junction``, which does
...
def is_available(stt_config=None) -> bool:  # type: ignore[no-untyped-def]
    ...
    if stt_config is None:
        ...
        stt_config = JunctionConfig.load().stt
...
def _load_stt_config() -> Any:
    ...
    return JunctionConfig.load().stt
    ...
    def __init__(self, profile: str) -> None:
        if boto3 is None:  # pragma: no cover — optional 'aws' extra not installed
            raise RuntimeError(
                ...
                "dependencies (pip install 'junction[voice]')."
            )
...
async def _transcribe_aws(audio_path: str, stt_config) -> str | None:  # type: ignore[no-untyped-def]
    ...
    if boto3 is None:
        logger.error("AWS Transcribe not available: install 'junction[voice]'")
        return None
    try:
...
async def _transcribe_faster(audio_path: str, stt_config) -> str | None:  # type: ignore[no-untyped-def]
    ...
    # junction.executors.stt_executor for why sharing the PTY-teardown pool was
    # the wrong bulkhead.
...
async def _transcribe_apple(audio_path: str, stt_config) -> str | None:  # type: ignore[no-untyped-def]
    ...
    Delegates to :mod:`junction.apple_speech`, which owns the Swift-helper seam.
