...
Feature-flagged via env ``JUNCTION_AUTONUDGE`` (on by default; set to ``0`` to disable).
"""
...
#    ``junction.messaging.link`` runs ``messaging/__init__``, which pulls the
...
#    ``junction`` modules to obtain one tuple of string literals.
...
def enabled() -> bool:
    """Feature flag — on by default. Set ``JUNCTION_AUTONUDGE=0`` to disable."""
    return os.environ.get("JUNCTION_AUTONUDGE", "1").lower() not in ("0", "false", "no")
...
class AutoNudgeService:
    ...
    async def start(self) -> None:
        if not enabled():
            logger.info("AutoNudge disabled (JUNCTION_AUTONUDGE not set)")
            return
