# `junction pod` — isolated worktree test instances

Spin up a **throwaway, full-stack Junction gateway** for any feature worktree —
its own port, its own `JUNCTION_HOME` (own DB / sessions / memory), no Slack
...
junction pod up   <wt> [--json]   # bring up an isolated pod → {base_url, token, port}
junction pod up   <wt> --provision# provision (if needed) then bring it up
junction pod up   <wt> --approval reads  # boot its gateway in an approval mode
junction pod up   <wt> --crons          # boot its gateway with the cron scheduler on
junction pod ls                   # what's running (≈ kubectl get pods) + orphaned HOMEs (with age)
junction pod prune [--all] [--dry-run]  # bulk-reclaim orphaned HOMEs (default: older than 3d; --all for every age)
junction pod status <wt>          # up/down + health
junction pod token  <wt> [--ttl]  # (re)mint a dashboard token for a running pod
junction pod url    <wt>          # print its base_url
junction pod logs   <wt> [-n N]   # tail its journal
junction pod down   <wt>          # evict → delete its HOME, verified (zero residue)
```

`<wt>` is a friendly worktree name. It is resolved to a checkout **git-natively**:
`junction pod up <name>` matches a linked worktree by its directory basename, its
branch (`<name>` or `feat/<name>`), or an exact path — run it from inside any
Junction checkout (or set `JUNCTION_POD_REPO`). The resolved path is pinned so the
...
`.venv/bin/junction` and a built SPA bundle (`src/junction/static/dist`). These
...
- **Control plane** — the `junction pod` verbs (resolution, port derivation, unit
  management, token mint, boot *prep*). These run from the **stable, globally
  installed** `junction`, so they never break just because a worktree's code is broken.
- **Payload** — the booted pod *is* the worktree's `.venv/bin/junction gateway`. If
...
`junction pod install` writes a template unit `junction-pod@.service` whose
`ExecStart` re-enters `junction pod _run <wt>` (boot logic lives in
`junction.pod.runtime.boot`). `MemoryMax`/`CPUQuota` cap a runaway pod;
...
of its sessions and config. So `junction pod down` owns reclamation on every
...
| `JUNCTION_POD_BIN` | (auto) | the `junction` binary the unit boots |
...
systemd **system** unit — which is how `junction service install` runs the
...
that names neither cause nor remedy. `junction doctor` reports the same three
