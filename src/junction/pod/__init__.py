"""Junction pod — isolated, throwaway, full-stack test instances per worktree.

A *pod* is an ephemeral Junction gateway booted from one feature worktree's own
``.venv``, on its own deterministic port, with its own ``JUNCTION_HOME`` (own DB /
...
gateway or the shared ``~/.junction`` data**. Think ``kubectl`` for local worktree
test rigs.
...
The user-facing surface is ``junction pod <verb>`` (see :mod:`junction.pod.cli`):
...
:func:`junction.pod.runtime.resolve_checkout`) and pinned so the systemd-booted
gateway never re-resolves. Mechanism, per platform:

* **Linux (``systemd --user``)**: one template unit ``junction-pod@<wt>.service``
  whose ``ExecStart`` re-enters ``junction pod _run <wt>`` (boots the worktree's own
...
* **macOS (``launchd``)**: one agent plist per pod under the pod plane's own directory (not
  ...
  enforced** — see :mod:`junction.pod.launchd` for why a weaker key is
...
from junction.pod.runtime import (
    PodError,
    derive_port,
    pod_home,
    pod_unit,
