...
pod's ``.local_secret``) lives here so :mod:`junction.pod.cli` stays a thin verb
...
from junction.platform_compat import IS_LINUX, IS_MACOS
...
# ``junction gateway --approval``. This tuple is the ENFORCEMENT point: the env
...
def resolve_checkout(
    ...
    (from ``JUNCTION_POD_REPO`` else *cwd*), matching a worktree's basename, then
    its branch (``name`` or ``feat/<name>``), then an exact path → optional
    ``JUNCTION_POD_WORKTREES_ROOT/name`` fallback → :class:`PodError`.
    """
...
def _systemctl_env() -> dict[str, str]:
    ...
    a systemd SYSTEM unit — which is how ``junction service install`` runs the
...
def unit_state(cfg: PodConfig, name: str) -> tuple[str, int]:
    ...
    :func:`junction.pod.launchd.unit_state` for how the crash signal is
...
def _write_and_load_unit(cfg: PodConfig) -> subprocess.CompletedProcess | None:
    ...
    systemd.* :func:`junction.pod.unit.unit_is_current` reads the file, but what
...
def start_pod(cfg: PodConfig, name: str) -> subprocess.CompletedProcess:
    ...
    with pod_name_mutex(cfg, name):
        ...
        # an absolute junction path at install time (a pruned worktree leaves it
...
def orphan_homes(cfg: PodConfig) -> list[str]:
    ...
    :func:`cleanup_home`'s re-validation via ``junction pod down <name>``.
    """
    try:
...
def build_pod_env(cfg: PodConfig, home_dir: Path, port: int, checkout: Path) -> dict[str, str]:
    ...
    env = {
        **os.environ,
        ...
        "JUNCTION_HOME": str(home_dir),
        "JUNCTION_PORT": str(port),
        "JUNCTION_PROJECT_DIR": str(checkout),
        ...
        # keys on THIS marker rather than on "has an isolated JUNCTION_HOME",
        ...
        # real lessons). Safe only because every Junction reader of the transcripts
        # dir now resolves through ``kiro_sessions_dir()``; without that the pod
        # would write sessions somewhere Junction never looks and lose resume.
        ...
        # finds no `JUNCTION_WORKSPACE` and no `config_dir()/workspace_dir` file in
        ...
        # WRITE the live workspace. `JUNCTION_WORKSPACE` is the documented override
        ...
        "JUNCTION_WORKSPACE": str(home_dir / "workspace"),
        # The pod's OWN venv leads PATH, ahead of cfg.gateway_path (which starts
        # with ~/.local/bin). Without this a bare `junction` inside a pod — an
        # agent bash turn, a subprocess, `_junction_bin()`'s "console-script on
    ...
    # gateway's JUNCTION_BOUND_PORT (dashboard.server._export_bound_port).
    # Inside a pod env it would name the wrong plane — the pod's own
    # JUNCTION_PORT above is the target — so drop it unconditionally rather
    # than rely on resolution precedence alone.
    env.pop("JUNCTION_BOUND_PORT", None)
    return env
...
def write_pod_config(home_dir: Path, seed: str) -> None:
    ...
    # The pod's own workspace root (see build_pod_env's JUNCTION_WORKSPACE).
...
def pod_context(cfg: PodConfig, name: str) -> tuple[Path, dict[str, str]]:
    """Resolve pod *name* to ``(its own junction binary, its isolated env)``.
    ...
    if not (bin_path.exists() and os.access(bin_path, os.X_OK)):
        raise PodError(f"no junction venv at {bin_path} (provision {name} first)")
    env = build_pod_env(cfg, cfg.home_dir(name), derive_port(cfg, name), checkout)
    return bin_path, env


# `pod exec` forwards to a real junction, so it inherits the WHOLE CLI — including
...
# `service`): every verb below acts only on `JUNCTION_HOME` state, which
...
#                    HOST registry, which is NOT covered by JUNCTION_HOME or
...
#   browse         — writes browser auth state outside JUNCTION_HOME
...
#                    `JUNCTION_PORT`. A pod's config names no dashboard port, so it
...
#                    `JUNCTION_PORT` — so this hazard is confined to `_tui`.
...
#                    `junction pod logs NAME` reads the pod's own unit.
...
def exec_in_pod(cfg: PodConfig, name: str, argv: list[str]) -> int:
    """``exec`` *argv* as the pod's own junction, in the pod's isolated env.
    ...
    # edit land outside the pod even with JUNCTION_WORKSPACE set correctly.
    workspace = Path(env["JUNCTION_WORKSPACE"])
    try:
...
# --------------------------------------------------------------------------- #
# Boot — the ExecStart body. Re-entered as ``junction pod _run <name>`` by the
...
def boot(cfg: PodConfig, name: str) -> int:
    ...
    if not (bin_path.exists() and os.access(bin_path, os.X_OK)):
        print(f"FATAL: no junction venv at {bin_path} (provision {name} first)")
        return 3
    if not (checkout / "src" / "junction" / "static" / "dist").is_dir():
    ...
    if approval and approval not in APPROVAL_MODES:
        ...
        print(
            f"junction-pod: ignoring unknown APPROVAL={approval!r} "
    ...
    if crons_raw and not crons:
        ...
        print(
            f"junction-pod: ignoring unrecognised CRONS={crons_raw!r} "
    ...
    print(f"junction-pod: name={name} port={port} home={home_dir} checkout={checkout}")
