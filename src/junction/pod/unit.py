...
The unit's ``ExecStart`` re-enters the installed ``junction`` binary as
``junction pod _run %i``, so the boot logic lives in Python (see
:func:`junction.pod.runtime.boot`) and nothing is shipped outside the package.
...
:func:`junction.pod.runtime.stop_pod`, which runs it after the service is
...
from junction.pod.config import PodConfig, environment_vars
from junction.service.common import systemd_quote

_UNIT_TEMPLATE = """\
[Unit]
Description=Junction pod (%i) — isolated worktree gateway, throwaway
# Multi-active: many run at once, one per worktree, each on its own port + own
# JUNCTION_HOME. Orthogonal to the live gateway; refuses to bind the live port.
...
# junction binary; boot logic is junction.pod.runtime.boot (no shipped shell).
ExecStart={junction_bin} pod _run %i
...
# bringing the pod back on a wiped home. `junction pod down` owns reclamation
# (junction.pod.runtime.stop_pod); `junction pod ls` reports what a crash left.
...
def _junction_argv() -> tuple[str, ...]:
    """Argv prefix that re-enters the junction entry point the unit should boot.

    Resolution order:
      1. ``JUNCTION_POD_BIN`` — explicit override (used when installing a unit that
         must boot a specific build, e.g. a worktree's own ``.venv``).
      2. the console-script on PATH.
      3. ``<this python> -m junction`` so it works from a bare checkout.
    """
    override = os.environ.get("JUNCTION_POD_BIN")
    if override:
        return (override,)
...
def _environment_block(cfg: PodConfig) -> str:
    ...
    The *selection* lives in :func:`junction.pod.config.environment_vars` so the
...
def render_unit(cfg: PodConfig) -> str:
    return _UNIT_TEMPLATE.format(
        junction_bin=" ".join(systemd_quote(arg) for arg in _junction_argv()),
...
def unit_exec_ok(cfg: PodConfig) -> bool:
    ...
    The unit bakes an absolute junction path at install time (often a
