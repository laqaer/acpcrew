...
Every value is ``JUNCTION_POD_*``-overridable so a test harness can stand up a
...
from junction.config.paths import _default_home
...
DEFAULT_UNIT_PREFIX = "junction-pod"
...
def environment_vars(cfg: PodConfig) -> dict[str, str]:
    ...
    from the built-in default are emitted (plus ``JUNCTION_POD_PATH``, always
    ...
    candidates: list[tuple[str, str, str | None]] = [
        ...
        ("JUNCTION_POD_BASE_PORT", str(cfg.base_port), str(DEFAULT_BASE_PORT)),
        ("JUNCTION_POD_LIVE_PORT", str(cfg.live_port), str(DEFAULT_LIVE_PORT)),
        ("JUNCTION_POD_UNIT_PREFIX", cfg.unit_prefix, DEFAULT_UNIT_PREFIX),
        ("JUNCTION_POD_PATH", cfg.gateway_path, None),  # always pin PATH
    ]
    out = {
        key: val
    ...
    if cfg.repo_hint is not None:
        out["JUNCTION_POD_REPO"] = str(cfg.repo_hint)
    if cfg.worktrees_root is not None:
        out["JUNCTION_POD_WORKTREES_ROOT"] = str(cfg.worktrees_root)
    return out
...
class PodConfig:
    ...
    checkout by :func:`junction.pod.runtime.resolve_checkout` (git-native).
    """
    ...
    def load(cls) -> "PodConfig":
        ...
        artifacts_dir = _env_path("JUNCTION_POD_ARTIFACTS_DIR", pod_root / ".e2e-artifacts")
        ...
        return cls(
            ...
            base_port=_env_int("JUNCTION_POD_BASE_PORT", DEFAULT_BASE_PORT),
            live_port=_env_int("JUNCTION_POD_LIVE_PORT", DEFAULT_LIVE_PORT),
            unit_prefix=os.environ.get("JUNCTION_POD_UNIT_PREFIX", DEFAULT_UNIT_PREFIX),
            gateway_path=os.environ.get("JUNCTION_POD_PATH", default_path),
            repo_hint=_env_path_opt("JUNCTION_POD_REPO"),
            worktrees_root=_env_path_opt("JUNCTION_POD_WORKTREES_ROOT"),
        )

    # ---- derived locations -------------------------------------------------
    def home_dir(self, name: str) -> Path:
        """Isolated ``JUNCTION_HOME`` for pod *name* (nuked on stop)."""
