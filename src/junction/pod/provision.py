...
A pod boots the worktree's OWN ``.venv/bin/junction gateway`` serving its OWN
...
module collapses them into one command (``junction pod provision`` /
``pod up --provision``).
...
from junction.env import find_node_tool, node_augmented_path
...
def venv_bin(checkout: Path) -> Path:
    """Path to the worktree venv's ``junction`` entry point.
    ...
    """
    name = "junction.exe" if platform_compat.IS_WINDOWS else "junction"
    return venv_bin_dir(checkout) / name


def dist_dir(checkout: Path) -> Path:
    return checkout / "src" / "junction" / "static" / "dist"
...
def _npm_bin() -> str | None:
    ...
    _say(
        ...
        "Node, or set JUNCTION_NODE_BIN_DIR=/abs/path/to/node/bin."
    )
    return None
...
def build_dist(checkout: Path) -> bool:
    ...
    ``src/junction/static/dist``. Returns True if the dist exists afterward."""
