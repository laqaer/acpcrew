"""macOS ``launchd`` backend for pods — the darwin twin of :mod:`junction.pod.unit`.
...
(``junction-pod@.service``) and instantiates it per pod via ``%i``. launchd has
no such concept, so each pod gets its own plist under the pod plane's own directory
(``cfg.pods_dir/dev.junction.pod.<name>.plist`` — deliberately NOT
...
absolute ``junction`` path at *install* time, which goes stale when the worktree
...
stop half of a ``Restart=``, so :func:`junction.pod.runtime.stop_pod` owns
...
:func:`junction.pod.runtime.orphan_homes` exists so ``pod ls`` can REPORT those
...
Every other isolation property is unchanged on macOS: own ``JUNCTION_HOME``, own
...
from junction.pod.unit import _junction_argv as _shared_junction_argv
...
def pod_label(cfg: PodConfig, name: str) -> str:
    ...
    (``JUNCTION_POD_UNIT_PREFIX``) cannot collide with a developer's real pods,
    exactly as it cannot on systemd.
    """
    # EVERY plane carries its prefix segment — including the default one.
    # Omitting it for the default made "dev.junction.pod." a strict prefix of
...
def _junction_argv() -> list[str]:
    """Return the shared entry-point prefix in launchd's list form."""
    return list(_shared_junction_argv())
...
def render_plist(cfg: PodConfig, name: str) -> dict[str, object]:
    ...
    body: dict[str, object] = {
        ...
        # boot logic stays in Python (junction.pod.runtime.boot), so nothing
        # shell-shaped is shipped, same as the systemd unit.
        "ProgramArguments": [*_junction_argv(), "pod", "_run", name],
...
def stop(cfg: PodConfig, name: str, *, timeout: float = 15.0) -> subprocess.CompletedProcess:
    ...
    **A per-pod plist must not outlive its pod.** systemd's template unit is
    ...
    it behind makes :func:`junction.pod.runtime.orphan_homes` classify the leftover HOME as "installed,
...
def active_names(cfg: PodConfig) -> set[str]:
    ...
    prefix = pod_label(cfg, "")  # e.g. "dev.junction.pod." (trailing dot kept)
    names: set[str] = set()
