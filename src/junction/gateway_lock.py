"""Single-writer guard for a ``JUNCTION_HOME``.

Two ``junction gateway`` processes bound to the same home each open the same
...
Isolated homes (``--test-mode``/``--seed`` with a distinct ``JUNCTION_HOME``)
...
class GatewayLockError(RuntimeError):
    """Raised when another process already owns this ``JUNCTION_HOME``."""
    ...
    def __init__(self, home: Path, holder_pid: int | None, diagnosis: str | None = None) -> None:
        ...
        super().__init__(f"{detail}; stop it first or set JUNCTION_HOME to an isolated directory")


class GatewayLock:
    """Process-lifetime exclusive lock on a single ``JUNCTION_HOME``.
    ...
    matching the single-host scope of ``JUNCTION_HOME``.
    ...
    *port* is diagnostic only. When given, a refusal reports whether the holder
    ...
    def _diagnose(self, recorded_pid: int | None) -> tuple[int | None, str | None]:
        ...
        return recorded_pid, (
            ...
            "Stop the running gateway, or set JUNCTION_HOME to an isolated directory."
        )
    ...
    def _describe_live_owner(self, pid: int, recorded_pid: int | None) -> str:
        ...
        message = (
            ...
            f"{self._home}; stop it first ({CLI_BIN} stop) or set JUNCTION_HOME to an "
            "isolated directory"
        )
...
def _port_answers_http(port: int, timeout: float = 1.5) -> bool:
    ...
    mirrors :func:`junction.dashboard.port_reclaim._probe_gateway_healthy` in a
