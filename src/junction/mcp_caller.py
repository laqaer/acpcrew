"""Caller-identity protocol extension for Junction MCP servers.

BACKGROUND
----------
The base MCP protocol (Anthropic spec 2024-11-05) models a 1:1 stdio
transport: one client, one server. When Junction introduces a broker that
...
Previously, the backend read this identity from the ``JUNCTION_SESSION_KEY``
...
3. **Typed context** — tool handlers receive a ``CallerContext`` dataclass
   ...
   constructed from the ambient ``JUNCTION_SESSION_KEY`` env var so
...
any future Junction-owned MCP server must go through this module.
"""
...
CALLER_META_KEY = "junction.caller"

#: Capability key backends advertise in ``initialize`` response under
#: ``capabilities.experimental``. Presence indicates the backend
#: understands the ``junction.caller`` ``_meta`` block and is safe to pool.
CALLER_CAPABILITY_KEY = "junction.caller-identity"
...
def _parent_pid(pid: int) -> int:
    ...
    Delegates to :func:`junction.platform_compat.get_ppid`, which reads
...
class CallerContext:
    """Identity of the originating Junction session for a single MCP call.
    ...
    def from_env(cls) -> "CallerContext":
        """Synthesize a context from the ``JUNCTION_SESSION_KEY`` env var, or
        ...
        Fallback order:
          1. ``JUNCTION_SESSION_KEY`` env var (primary, per-session mode)
...
# ``junction.caller`` block from every inbound frame (``backend.py``
...
_CURRENT_CALLER: ContextVar["CallerContext | None"] = ContextVar(
    "junction_current_caller", default=None
)
