...
def _is_managed_agent_process(pid: int) -> bool:
    """Check if a PID belongs to an agent process managed by Junction (guards against PID recycling)."""
...
# Catches Junction-spawned MCP subtrees that escaped PID-file tracking.
...
# JUNCTION_SPAWNED environ marker — see _is_sweepable_orphan_work). Work
...
# Entrypoints that positively identify a Junction-spawned MCP/worker process.
# Each marker MUST be unique to a process Junction itself launches — the sweep
...
_MCP_ENTRYPOINT_MARKERS = (
    b"junction_sandbox_",  # sandbox wrapper script (session-spawned)
    b"junction.mcp_gateway.stub",  # gateway pool worker (not gatewayd itself)
)
...
_GATEWAYD_MODULE = b"junction.mcp_gateway.gatewayd"
_GATEWAY_MARKERS = (
    _GATEWAYD_MODULE,
    b"junction.cli",
    b"junction.__main__",
...
# MCP launcher cmdline shapes that carry NO Junction fingerprint (a user's own
# shell can produce identical cmdlines), so matching them requires the
# ``JUNCTION_SPAWNED`` environ marker as positive identity — the public fork's
...
# its argv0 (``npx``/``node``) nor its args mention Junction, so a grandchild
...
def _is_orphan_mcp(cmdline: bytes) -> bool:
    """True if cmdline matches a Junction MCP entrypoint (not a peer gateway)."""
...
def _is_marked_mcp_launcher(cmdline: bytes) -> bool:
    ...
    :func:`_env_has_junction_marker` because a user's own shell produces
...
def _env_has_junction_marker(pid: int) -> bool:
    """True if *pid*'s environment carries the ``JUNCTION_SPAWNED`` marker.
    ...
    if sys.platform != "linux":
        return False
    needle = f"{JUNCTION_SPAWNED_ENV}={JUNCTION_SPAWNED_VALUE}".encode()
    try:
...
def _is_sweepable_orphan_gatewayd(cmdline: bytes) -> bool:
    ...
    1. a structural ``-m junction.mcp_gateway.gatewayd`` argv pair — never
       ``junction.cli`` / ``junction.__main__``, which stay unconditionally
...
def _is_sweepable_orphan_work(pid: int, cmdline: bytes, age_seconds: float) -> bool:
    ...
    return _env_has_junction_marker(pid)
    ...
    for pid in pids:
        ...
        try:
            ...
            if gw_age >= _ORPHAN_MIN_AGE_SECONDS and _is_sweepable_orphan_gatewayd(cmdline):
                ...
                continue
            # Work-class orphan (JUNCTION_SPAWNED marker, no launcher shape).
            ...
            try:
                ...
                # a module-level `from junction.sel import sel` would be circular.
...
def _kill_orphan_work_tree(pid: int, cmdline: bytes, age_seconds: float, budget: int) -> int:
    ...
    of a marked work orphan inherited the ``JUNCTION_SPAWNED`` environment
        ...
        try:
            ...
            if target == pid:
                logger.info(
                    ...
                    "reason=JUNCTION_SPAWNED work orphan (reparented to init)",
                    target,
                    basename,
                    int(age_seconds),
                )
    ...
    try:
        ...
        sel().log_tool_invocation(
            ...
            metadata={
                ...
                "reason": "JUNCTION_SPAWNED orphan work process",
            },
        )
    except Exception:
