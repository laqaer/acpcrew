...
with Junction's existing file locking patterns.  Read-modify-write cycles
...
#: Junction can neither install nor clean them up. Their ledger rows must SURVIVE
...
def declared_capability_keys(deps_data: dict[str, Any]) -> list[str]:
    ...
    alias (see :class:`junction.apps.manifest.Dependencies`).  Shared by the
        ...
        for dep in result["removable"]:
            ...
            if dep["id"] in keep or uncleanable:
                ...
                if entry:
                    ...
                    # was installed at all (Junction has no operation to remove
