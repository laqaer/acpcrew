"""App management REST API endpoints for the Junction dashboard.
...
from junction.apps.dependencies import clean_dependencies
...
from junction.apps.lifecycle_scripts import run_lifecycle_script as _run_lifecycle_script
...
from junction.apps.manifest import Dependencies, PlatformConfig
from junction.apps.official_category_order import load_category_order
from junction.apps.official_editorial import load_sections
...
from junction.apps.teardown import teardown_app_runtime
from junction.apps.version import check_min_version as _check_min_version_str
...
from junction.publish_governance import DEPLOY_WEB_PROVIDER_ID, publish_denied_reason
...
def _check_min_version(manifest_data: dict[str, Any]) -> str | None:
    """Check if the app requires a newer Junction version.

    Returns an error message if the current version is too old, or None if OK.
    """
    return _check_min_version_str(manifest_data.get("minJunctionVersion", ""))
...
async def handle_publish_providers(request: web.Request) -> web.Response:
    ...
    * the publish-governance chokepoint denies its destination id (governance
    ...
    try:
        from junction.deploy import profiles as _deploy_profiles
...
async def _register_app_off_loop(name: str) -> RegistrationResult:
    ...
    ``JUNCTION_HOME`` — manifest reads, skill-dir symlink walks, agent JSON
...
async def handle_install_app(request: web.Request) -> web.Response:
    ...
    # Check minJunctionVersion before installing
...
async def handle_register_external(request: web.Request) -> web.Response:
    ...
    Junction only tracks metadata so the dashboard can display them.
...
async def handle_uninstall_app(request: web.Request) -> web.Response:
    ...
    if is_registry_source(info.get("source", "")):
        ...
        if app_reg_name:
            from junction.apps.registry import app_source_dir
...
async def handle_disable_app(request: web.Request) -> web.Response:
    ...
    async with app_lifecycle_lock(name):
        ...
        if module_name in BUILTIN_NAMES:
            try:
                mod = importlib.import_module(f"junction.apps.builtins.{module_name}")
...
async def handle_app_config(request: web.Request) -> web.Response:
    ...
    Reads/writes ``~/.junction/apps/{name}/data/config.json``.
    ...
    from junction.apps.manager import app_data_dir
    from junction.atomic_write import atomic_write
...
async def handle_app_ui_file(request: web.Request) -> web.Response:
    ...
    from junction.apps.dev_mode import is_dev_mode_cached
...
async def handle_app_dev_mode(request: web.Request) -> web.Response:
    ...
    """
    from junction.apps.dev_mode import set_dev_mode
...
async def _fetch_git_blob(
    ...
    *,
    ...
    (mirroring how :mod:`junction.apps.registry` already clones), read the
    ...
    from junction.apps.registry import is_clone_host_trusted
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="junction-blob-")
...
async def handle_blob_proxy(request: web.Request) -> web.Response:
    ...
    Caches fetched blobs to ~/.junction/cache/blobs/{repo}/{ref}/{path}.
...
# Strip sensitive auth headers — app backends use X-Junction-Proxy HMAC, not user cookies
...
async def handle_app_api_proxy(request: web.Request) -> web.StreamResponse:
    ...
    # Header format: X-Junction-Proxy: <timestamp>:<hmac-sha256>
    ...
    try:
        ...
        headers["X-Junction-Proxy"] = f"{ts}:{sig}"
    except OSError as exc:
...
async def handle_registries_refresh(request: web.Request) -> web.Response:
    ...
    """
    from junction.apps.registry import refresh_registries
