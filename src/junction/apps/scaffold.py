"""App scaffolding — generate a new app skeleton with `junction app init`.
...
_BACKEND_TEMPLATE = '''"""Minimal backend for {name} — a Junction app.

Run with: python backend/server.py
Or let Junction manage it via the app manifest backend section.
"""
import json
import os
...
APP_NAME = os.environ.get("JUNCTION_APP_NAME", "{name}")
  ...
  build: {{
    ...
    rollupOptions: {{
      external: [
        ...
        '@junction/app-sdk', '@junction/app-sdk/ui', 'lucide-react',
      ],
    }},
  }},
}})
...
import {{ useAppApi }} from '@junction/app-sdk'
...
```bash
junction app install /path/to/{name}
junction app enable {name}
```
