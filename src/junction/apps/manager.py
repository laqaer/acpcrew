...
from junction.apps.manifest import AppManifest, app_name_error
...
class InstalledApp:
    ...
    ``origin`` — where the app came from (read-only, set at install time):
      - ``"builtin"``: baked into the Junction dashboard
    ...
    ``resources`` — who manages agent/skill/cron registration:
      - ``"gateway"``: Junction manages via bridges.py symlinks
      - ``"app"``: the app manages its own resource registration

    ``lifecycle`` — who manages updates and uninstall:
      - ``"gateway"``: Junction handles updates and uninstall
...
def _credential_free_source_metadata(value: str) -> str:
    ...
    from junction.apps.registry import _strip_git_target_userinfo
...
def _validate_source_path(source: Path) -> list[str]:
    ...
    if manifest.minJunctionVersion:
        ver_err = _check_min_version(manifest.minJunctionVersion)
        if ver_err:
...
def _check_min_version(min_version: str) -> str | None:
    """Return error string if current Junction version is too old, else None."""
...
_REGISTRY_SOURCE_REPOSITORY: ContextVar[str | None] = ContextVar(
    "junction_registry_source_repository", default=None
)
...
def install_app(
    ...
    *,
    ...
    from junction.dashboard.token_auth import generate_app_secret, write_app_secret
...
def uninstall_app(name: str, *, keep_data: bool = True) -> AppResult:
    ...
    # because this runs under `junction app uninstall` in a DIFFERENT process:
    #
    ...
    try:
        from junction.apps.dev_mode import remove_dev_app
...
def _drop_trust_grant(name: str) -> None:
    ...
    endpoints do: ``JunctionConfig.load()`` degrades a corrupt file to defaults, so
    ...
    #
    # `JunctionConfig.load()` deep-merges `config.local.json` OVER `config.json`,
    ...
    # base branch has two dozen writers in it, `junction config set` among them, so
    ...
    # endpoint emits its own SEL event, but this path runs from `junction app
...
def register_external_app(
    ...
    *,
    ...
    """Register a self-managed app with Junction's app system.

    Self-managed apps (``resources="app"``) handle their own agent/skill/MCP
    registration.  Junction only tracks metadata so the dashboard can display them.
...
# Built-in apps are features baked into the Junction dashboard that we
...
# routes stay fully intact and it is enabled with ``junction app enable channels``.
...
def resolve_mcp_backend_url(mcp_servers: Any) -> str | None:
    ...
    if not isinstance(mcp_servers, dict):
        return None
    gateway_port = int(os.environ.get("JUNCTION_PORT", "5476"))
    ...
    for app_data in all_builtins:
        ...
        if _app_declares_backend(app_data):
            ...
            try:
                ...
                # junction.apps.routes imports from junction.apps.manager
                # at module load, so we cannot import routes at the top of
                # this file without creating a cycle.
                from junction.apps.routes import invalidate_app_secret_cache
