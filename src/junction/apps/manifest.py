"""App manifest — static metadata for Junction apps.

An app manifest (``app.json``) declares an app's identity, resources, and
requirements without executing any app code.  Junction reads it during
install to register agents, skills, crons, UI pages, and backend config.

Design follows the same pattern as :class:`junction.plugins.manifest.PluginManifest`
...
from junction.cron import is_valid_skip_date, is_valid_timezone
...
@dataclass
class Dependencies:
    """External dependencies that Junction should resolve during install.

    ``managedBy`` controls the default installation strategy:
      - ``"gateway"``: Junction resolves each dependency through the edition's
        ``CapabilityManager`` seam
      - ``"app"``: Junction only checks existence, does not install
...
class ClientInstallConfig:
    ...
    Used when Junction runs remotely (e.g. cloud desktop) and the app
...
class PlatformConfig:
    ...
    - ``"server"`` (default): Junction clones + installs on the server.
    - ``"client"``: Must be installed on the user's local machine.
      When Junction is on an incompatible platform, the App Store shows
    ...
    (``window.junction.isElectron``, set by the shell's preload) is client-side
    ...
    #
    # ``windows`` is in both directions because Junction itself supports Windows
...
@dataclass
class AppManifest:
    """Static metadata for a Junction app — readable without executing app code.

    Parsed from ``app.json`` at the root of an app package.  Follows the same
    pattern as :class:`~junction.plugins.manifest.PluginManifest`: dataclass
    ...
    author: str = ""
    license: str = ""
    minJunctionVersion: str = ""  # noqa: N815
    ...
    def validate(self, app_root: Path | None = None) -> list[str]:
        ...
        # 'junction.apps.builtins.x.server' has no '..' and is not absolute, so
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.minJunctionVersion:
            d["minJunctionVersion"] = self.minJunctionVersion
        if self.signer:
    ...
    def from_dict(cls, data: dict[str, Any]) -> AppManifest:
        ...
        return cls(
            ...
            minJunctionVersion=str(data.get("minJunctionVersion", "")),  # noqa: N815
