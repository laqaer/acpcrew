"""Registration bridges — wire app resources into Junction's runtime.

When an app is installed or enabled, its agents, skills, and cron jobs need
to be registered with Junction's existing systems.  This module provides
...
from junction.apps.interpreter import resolve_app_python, venv_provided_command
...
from junction.cron_script import resolve_script_path
from junction.env import emit_env
from junction.executors import maintenance_executor
from junction.platform.governance import may_skip_gate_now, strip_ungoverned_auto_approve
...
# Where Junction loads skills from
...
def _ceiling_filtered_allowed(refs: object, agent_name: str = "") -> list[Any]:
    ...
    :func:`~junction.platform.governance.may_skip_gate`, because this is one of
...
_HOST_CLI_COMMAND = "junction"


def _pin_host_cli_command(app_name: str, cfg: dict[str, Any]) -> dict[str, Any]:
    """Rewrite ``command: "junction"`` to THIS gateway's interpreter.
    ...
    * the launcher on PATH may belong to a DIFFERENT Junction install, so the
    ...
    * that launcher may simply be broken (a stale wrapper whose interpreter is
    ...
    Pinning ``sys.executable -m junction`` plus PYTHONPATH and JUNCTION_HOME makes
    ...
    pkg_parent = str(Path(junction_file()).parent.parent)
    env = dict(cfg.get("env") or {})
    existing = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{pkg_parent}{os.pathsep}{existing}" if existing else pkg_parent
    env.setdefault("JUNCTION_HOME", str(app_dir(app_name).parent.parent))
    cfg["command"] = sys.executable
    cfg["args"] = ["-s", "-m", "junction", *list(cfg.get("args") or [])]
    cfg["env"] = env
    return cfg


def junction_file() -> str:
    """Path to the running ``junction`` package's ``__init__``."""
    import junction

    return str(junction.__file__)
...
def _own_mcp_servers(app_name: str) -> dict[str, Any]:
    ...
    reference: ``_register_mcp_servers`` writes the server into Junction's own
...
#: config must name a RUNTIME path — one under the data home, which ``JUNCTION_HOME``
...
def _placeholder_values(app_name: str) -> dict[str, str]:
    ...
    try:
        from junction.apps.builtins.pptx_maker.backend import paths as pptx_paths
        from junction.apps.builtins.pptx_maker.backend import provision as pptx_provision
...
def _register_agents(app_name: str, manifest: AppManifest, app_root: Path) -> list[str]:
    ...
    * a copy is regenerated from the template on every registration, and the
    ...
    for agent_path_str in manifest.agents:
        ...
        # Drop a legacy SYMLINK from an older Junction (which pointed at a file
        ...
        try:
            ...
            # (@junction-cron / @junction-core) in `tools`. Those specs live in
...
async def register_app_crons_with_service(app_name: str, cron_service: Any) -> list[str]:
    ...
    from junction.mcp_cron import _vet_script_file, _vet_shell_command
...
# App MCP servers are written to Junction's OWN agent config, not to the shared
...
# HTTP entry there breaks EVERY kiro session, see backend.py's warning). Junction
...
def _mcp_json_path() -> Path:
    """Junction's own agent config. A function, not an import-time constant:
    ...
    return _kiro_agents_dir() / "junction.json"
...
def _mcp_lock(*, exclusive: bool = True, target: Optional[Path] = None) -> Iterator[None]:
    ...
    WHICH file's sidecar to lock (default: Junction's own agent config); pass the
...
def _resolve_live_mcp_url(app_name: str, url: str, live_port: int | None = None) -> str:
    ...
    try:
        if live_port is None:
            ...
            from junction.apps.backend import get_app_backend_port
...
#: one guaranteed to import ``junction``.
...
def resolve_stdio_command(cfg: dict, app_root: Path | None = None) -> dict:
    ...
    ``junction`` at all. ``mcp_gateway/rewriter.py`` bakes ``sys.executable`` into its stub
    ...
    (see :mod:`junction.apps.interpreter`): prefer the app's own venv interpreter — that is
...
def _targets_gateway_module(cfg: dict) -> bool:
    """True when a python stdio entry launches a ``junction``-owned module.
    ...
    - a non-dash token: the script operand — everything after belongs to the
      SCRIPT (``python3 server.py -m junction.mode``), never to CPython.
    ...
    (``-sm junction``) reads here as an unknown flag followed by an operand
        ...
        if arg == "-m":
            if i + 1 < len(args):
                ...
                return isinstance(module, str) and (
                    module == "junction" or module.startswith("junction.")
                )
            return False
        if arg.startswith("-m") and len(arg) > 2:
            module = arg[2:]
            return module == "junction" or module.startswith("junction.")
...
def _register_mcp_servers(
    ...
    """Register app-provided MCP servers into Junction's agent config.
...
def _scrub_legacy_shared_mcp(app_name: str) -> int:
    ...
    try:
        ...
        # Junction process), and an unlocked stale read-modify-write here would
        ...
        # junction.json), so pass the legacy path explicitly.
