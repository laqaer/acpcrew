...
def builtin_app_names() -> frozenset[str]:
    ...
    from junction.apps.manager import builtin_owns_installed
...
def shipped_builtin_module_path(app_name: str, module_name: str) -> Path | None:
    ...
    # root. This supports both core modules (``junction.apps.builtins.*``)
...
def third_party_execution_allowed() -> bool:
    ...
    try:
        ...
        value = getattr(JunctionConfig.load().agent, "apps_allow_third_party", False)
...
def trusted_app_names() -> frozenset[str]:
    ...
    try:
        ...
        raw = getattr(JunctionConfig.load().agent, "apps_trusted", [])
    except Exception as exc:  # noqa: BLE001 - unreadable policy must fail closed
        logger.error(
...
def _repository_grant_denied_for_binding(
    ...
    *,
    ...
    if not binding:
        ...
        if repository is not None and repository.strip():
            ...
            try:
                ...
                from junction.apps.registry import resolve_installed_trust_repository
    ...
    try:
        ...
        from junction.apps.registry import _same_git_target
