...
from junction.apps.context import AppContext, build_app_context
...
class LifecycleDispatcher:
    ...
    def _resolve_hook(app_name: str, hook_path: str):
        ...
        if shipped_root is not None:
            ...
            mod = importlib.import_module(f"junction.apps.builtins.{shipped_root.name}.{dotted}")
    ...
    async def _invoke(self, app_name: str, hook_path: str, ctx: AppContext, *, phase: str) -> bool:
        ...
        try:
            ...
            # module object (`_junction_app_<app>.<mod>`), so an on_startup hook
