...
from junction import hooks, model_registry
...
logger = logging.getLogger("junction.app.code-review-sage")
...
async def _handle_settings(request: web.Request) -> web.Response:
    ...
    async def _audit_settings(outcome: str) -> None:
        def _emit() -> None:
            from junction.sel import sel  # circular import: sel->config->apps cycle
            sel().log_api_access(
