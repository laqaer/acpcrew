...
_FALLBACK_AGENT = "junction"     # default agent when the reviewer agent isn't installed
...
try:
    from junction.config.loader import DEFAULT_MODEL as _SYSTEM_DEFAULT_MODEL
...
# Valid concrete effort levels — sourced from junction.effort (single source of
# truth), not a hardcoded list. "" (inherit default) is handled separately.
try:
    from junction.effort import EFFORT_LEVELS as VALID_EFFORTS
...
def _resolve_review_agent(preferred: str = REVIEW_AGENT) -> str:
    ...
    `junction` agent. GitHub posting runs the `gh` CLI, so the chosen agent needs
...
def _write_effort_overlay(work_dir: str, model: str, effort: str = REVIEW_EFFORT) -> None:
    ...
    ``junction/providers/acp.py:_write_cli_overlay``)::
...
class _BatchRuntimeHolder:
    ...
    async def _ensure_runtime_locked(self) -> "AcpRuntime":
        ...
        if AcpRuntime is None:
            raise RuntimeError("AcpRuntime unavailable (junction.acp.runtime not importable)")
