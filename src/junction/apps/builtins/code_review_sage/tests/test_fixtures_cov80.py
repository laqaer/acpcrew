...
discarded by ``--cov=junction`` (the same measurement problem ``setup.cfg``
...
from junction.apps.builtins.code_review_sage.tests import fixtures


class TestSensitiveFiles:
    def test_a_gateway_path_carries_a_guard_removal_and_an_import_add(self):
        server = fixtures.SENSITIVE_FILES[0]
        assert server["path"] == "src/junction/gateway/server.py"
    ...
    def test_the_pair_mixes_a_sensitive_and_a_non_sensitive_file(self):
        ...
        assert paths == ["src/junction/gateway/server.py", "src/junction/util/format.py"]
