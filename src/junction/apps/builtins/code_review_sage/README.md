...
A self-evolving deep code reviewer packaged as a built-in Junction app. Reviews
...
Code Review Sage is a built-in app (listed in `junction.apps.builtins`). Enable
it from the dashboard Apps page, or:

```bash
junction app enable code-review-sage
```
