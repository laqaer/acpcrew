...
pytestmark = pytest.mark.skipif(
    not __import__('junction.sandbox', fromlist=['userns_available']).userns_available(),
...
class _TwoInstances(unittest.IsolatedAsyncioTestCase):
    ...
    def tearDown(self) -> None:
        ...
        # ``junction.apps.manager`` also SETS ``manager`` as an attribute on the
        # ``junction.apps`` package object, and that package was never evicted — so
        ...
        # So a later test doing ``import junction.apps.manager as manager`` patched the
    ...
    def _use(self, home: Path):
        ...
        """
        os.environ["JUNCTION_HOME"] = str(home)
        # `bridges` must go WITH `manager`: it does `from junction.apps.manager import …`,
        ...
        for name in list(sys.modules):
            if (
                ...
                or name.startswith("junction.apps.manager")
                or name.startswith("junction.apps.bridges")
            ):
                del sys.modules[name]
...
class TestWhatGetsCommitted(_TwoInstances):
    async def test_the_rotation_schedule_syncs(self):
        ...
        from junction.apps.builtins.ops_mission_control.backend.providers import (  # noqa: E501
            schedule_file,
        )
