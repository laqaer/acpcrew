...
from junction.apps.builtins.ops_mission_control.backend import dispatch, notify_out
from junction.apps.builtins.ops_mission_control.backend.models import Signal
from junction.notifications.bus import NotificationPayload, NotificationValidationError
...
class _NotifyBase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Patched ON THE MODULE UNDER TEST, not as "junction.apps.manager.is_app_enabled".
...
class TestNothingIsPushedOnAClaim(unittest.TestCase):
    ...
    def test_no_claim_path_calls_the_notifier(self) -> None:
        for module in (dispatch, __import__(
            "junction.apps.builtins.ops_mission_control.backend.routes",
            fromlist=["routes"],
        )):
