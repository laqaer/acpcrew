...
class TestCrossPlatform(unittest.TestCase):
    ...
    def test_timezone_lookup_degrades_instead_of_raising(self):
        ...
        tmp = Path(tempfile.mkdtemp())
        prev = os.environ.get("JUNCTION_HOME")
        os.environ["JUNCTION_HOME"] = str(tmp)
        try:
...
class TestTheScheduleIsWriteProtectedButReadable(unittest.TestCase):
    ...
    def _path(self) -> str:
        from junction.apps.builtins.ops_mission_control.backend.providers import schedule_file
    ...
    def test_the_shell_path_is_closed_too(self):
        ...
        That asymmetry means a custom-`JUNCTION_HOME` install (a pod, `dev-backend.sh`) has
...
class TestEveryRedactionSinkUsesTheSameSeam(unittest.TestCase):
    ...
    def _source(self, name: str) -> str:
        ...
        mod = importlib.import_module(
            f"junction.apps.builtins.ops_mission_control.backend.{name}"
        )
