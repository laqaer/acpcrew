...
from junction.apps.builtins.ops_mission_control.backend import routes
...
class TestSettingsRoute(_HomeIsolatedAsync):
    ...
    async def test_the_team_memory_repo_is_settable_over_the_api(self):
        ...
        """
        from junction.apps.builtins.ops_mission_control.backend import ledger_sync
...
class TestManifestCrons(unittest.TestCase):
    ...
    def test_secrets_live_outside_the_app_dir_so_uninstall_cannot_reach_them(self):
        ...
        from junction.apps.builtins.ops_mission_control.backend.secrets import secrets_path

        prev = os.environ.get("JUNCTION_HOME")
        with tempfile.TemporaryDirectory() as tmp:
            os.environ["JUNCTION_HOME"] = tmp
            try:
    ...
    def test_settings_discloses_that_uninstall_keeps_credentials(self):
        ...
        # junction/src/<root>). Resolved by counting rather than guessed — an
...
class TestSkillDelivery(unittest.TestCase):
    ...
    def _skill_root(self) -> Path:
        ...
        return Path(junction.__file__).resolve().parent / "builtin_skills" / "ops-mission-control"
    ...
    def _sop_calls(self):
        ...
        # parents[4] is src/junction (routes.py -> backend -> app -> builtins ->
        # apps -> junction). Counted, not guessed: an off-by-one makes this SKIP,
    ...
    def test_readme_companion_contract_names_the_real_symbols(self):
        ...
        from junction.apps.builtins.ops_mission_control.backend import companion
    ...
    def test_no_sop_instructs_a_blocked_or_raw_auth_path(self):
        ...
        cli_mint = re.compile(r"junction\s+(?:pod\s+)?tok" + "en" + r"\b")
