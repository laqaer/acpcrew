...
**No system prerequisite.** ``pip install junction`` must leave nothing for the
...
Follows :mod:`junction.embeddings` and
:mod:`junction.apps.builtins.papyrus.backend.tectonic`, the two in-tree
...
#: accepted.
ENGINE_URL_ENV = "JUNCTION_PPTX_ENGINE_URL"

#: Escape hatch for tests and CI: a test run must never reach the network.
SKIP_DOWNLOAD_ENV = "JUNCTION_PPTX_SKIP_ENGINE_DOWNLOAD"
...
def _reject_member_name(name: str) -> str | None:
    ...
    * an empty name.
    ...
    Same rules as
    :func:`junction.apps.builtins.papyrus.backend.tectonic._reject_member_name`.
    """
    if not name:
