...
from junction.apps.builtins.pptx_maker.backend import engine_source, paths
from junction.apps.manager import app_dir
from junction.atomic_write import atomic_write
from junction.sandbox import cgroup_scope_argv, run_limited, sandboxed_spawn_argv

logger = logging.getLogger("junction.app.pptx-maker")
...
def resolve_uv() -> str | None:
    ...
    ``pip install junction`` always has the binary — but not necessarily on
...
def mcp_tools_path() -> str:
    ...
    from junction.apps.builtins.pptx_maker.backend import paths as _paths
...
def _run(argv: list[str], *, cwd: str, timeout: int) -> tuple[int, str]:
    ...
    Uses the :func:`junction.sandbox.sandboxed_spawn_argv` chokepoint so the
...
def _render_agents(install_dir: Path, log: list[str]) -> int:
    ...
    which is itself overridable via ``JUNCTION_HOME``).
...
def _stage_static(install_dir: Path, log: list[str]) -> None:
    ...
    ``src/junction/builtin_skills/pptx-maker/``, which the gateway copies into
...
def provision() -> ProvisionOutcome:
    ...
    try:
        ...
        from junction.apps.builtins.pptx_maker.backend import preview_tools
