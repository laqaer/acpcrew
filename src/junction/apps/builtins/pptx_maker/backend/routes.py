...
from junction.apps.builtins.pptx_maker.backend import (
    decks,
    engine,
    library,
    paths,
...
# (:mod:`junction.messaging.raster`). Verifying the first few bytes is enough
...
# `JUNCTION-BITMAP-…` in the artifact text restores to nothing (its index is out
...
_BITMAP_PLACEHOLDER_RE = re.compile(
    r"\x00JUNCTION-BITMAP-" + _BITMAP_PLACEHOLDER_NONCE + r"-(\d+)\x00"
)
...
async def off_loop(fn: Callable[..., _T], *args: Any, **kwargs: Any) -> _T:
    ...
    DNS — saturating it stalls the gateway (see ``junction.executors``).
    """
...
def _deps_status() -> dict:
    ...
    which is what lets the UI say "installed by Junction" rather than implying
    the host changed.
...
def _redact_artifact(raw: bytes) -> bytes:
    ...
    def _stash(match: re.Match[str]) -> str:
        ...
        return f"\x00JUNCTION-BITMAP-{_BITMAP_PLACEHOLDER_NONCE}-{len(stash) - 1}\x00"
