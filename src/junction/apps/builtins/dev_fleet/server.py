"""Dev Fleet — standalone aiohttp backend for Junction feature worktrees.

Manages Junction feature worktrees (git worktrees of the main repo) and their
isolated pod test instances. Runs as a subprocess spawned by the Junction app
backend system (apps/backend.py). The gateway proxies /apps/dev-fleet/api/* to
this process with X-Junction-Proxy HMAC signing; HMAC middleware validates
...
from junction import dep_sync, frontend, hooks, platform_compat
from junction.apps.builtins.dev_fleet import gateway_service
from junction.apps.proxy_auth import raw_request_target
from junction.config.loader import JunctionConfig
from junction.env import find_node_tool, node_bin_dirs
...
APP_NAME = os.environ.get("JUNCTION_APP_NAME", "dev-fleet")
...
def _own_source_checkout() -> str | None:
    ...
    for parent in here.parents:
        if parent.name == "src" and (parent / "junction").is_dir():
...
# A candidate is adopted solely when it passes _is_junction_checkout, so an
...
def _configured_main_repo() -> str:
    ...
    by a discovered one.
    """
    explicit = os.environ.get("JUNCTION_DEVFLEET_REPO", "").strip()
    if explicit:
        return explicit
...
def _repo_source_hint() -> str:
    ...
    """
    if os.environ.get("JUNCTION_DEVFLEET_REPO", "").strip():
        return "It is set by the JUNCTION_DEVFLEET_REPO environment variable."
    configured = _load_dev_fleet_cfg().get("repo_path")
...
def _discover_main_repo() -> str:
    ...
    if configured:
        return configured
    for candidate in (
        os.environ.get("JUNCTION_PROJECT_DIR", ""),
        _own_source_checkout() or "",
        *_candidate_checkouts(),
    ):
        if _is_junction_checkout(candidate):
            return candidate
    return ""
...
def _default_main_repo() -> str:
    ...
    if explicit:
        return explicit
    for candidate in (os.environ.get("JUNCTION_PROJECT_DIR", ""), _own_source_checkout() or ""):
        if _is_junction_checkout(candidate):
            return candidate
    return ""
...
async def _upstream_remote() -> str:
    ...
    'junction' instead of 'origin') are honoured automatically. Cached at
...
def _build_pending() -> bool:
    ...
    try:
        ...
        dist = Path(_repo()) / "src" / "junction" / "static" / "dist"
        if not dist.exists():
            return False
...
#   _POD_IMPORTED  — the ``junction.pod`` modules are importable, so the
...
def _find_cli() -> list[str]:
    """Invoke the junction CLI as a module of OUR interpreter.

    Never resolved through the filesystem: a `junction` shim planted in an
    ...
    Targets the ``junction`` PACKAGE (its ``__main__``), NOT ``junction.cli``:
    ``cli.py`` has no ``if __name__ == "__main__"`` guard, so
    ``python -m junction.cli <cmd>`` imports the module, runs no ``main()`` and
    ...
    before ``junction.cli`` is imported, so it is the only correct ``-m`` entry.
    """
    return [sys.executable, "-m", "junction"]
...
def _own_checkout_path() -> str | None:
    """Checkout root the RUNNING process's junction package resolves into.
    ...
    ``<checkout>/src/junction/__init__.py``.
    """
    try:
        import junction as _pkg
...
def _launchd_live_worktree() -> str | None:
    ...
    the system-wide ``junction``. ``None`` correctly means "no row is live"
    ...
    # <checkout>/.venv/bin/junction -> <checkout>
...
def _bin_override_var(name: str) -> str:
    ...
    """
    return f"JUNCTION_DEVFLEET_BIN_{name.upper().replace('-', '_')}"
...
def _load_dev_fleet_cfg() -> dict:
    ...
    directly rather than through JunctionConfig (a separate process owns the
...
def _pod_env() -> dict:
    ...
    return {**_build_env(), "JUNCTION_POD_REPO": _repo()}
...
async def _sync_start_locked() -> dict:
    ...
    if npm_bin is None:
        ...
        return {"ok": False, "error": (
            ...
            "JUNCTION_NODE_BIN_DIR=/abs/path/to/node/bin in the gateway's "
    ...
    # that is incidental. `-m junction...dep_sync` would import the module from
    ...
    if locked_scripts:
        ...
        snapshot_dir = Path(tempfile.mkdtemp(prefix="junction-dep-sync-"))
    ...
    # JUNCTION_EDITION_DIR and JUNCTION_ALLOW_EDITION, so on an edition
    ...
    # which strips JUNCTION_EDITION_DIR, so the guard would read "stock" on every
    # install and never fire. apps/backend.py therefore propagates that one var
    # explicitly, the same way it already propagates JUNCTION_PROJECT_DIR.
    if frontend.edition_configured():
        ...
        raw_steps += [
            ...
            ([sys.executable, "-c",
              "import sys;from junction.frontend import build_and_stage;"
...
_DISABLE_BACKGROUND_ENV = "JUNCTION_DEVFLEET_NO_BACKGROUND"
...
def _sel():
    ...
    junction.sel directly (no _handlers_pkg indirection needed)."""
    from junction.sel import sel as _sel_singleton
...
async def hmac_proxy_middleware(request: web.Request, handler) -> web.Response:
    """Verify X-Junction-Proxy HMAC on every request except /health.
    ...
    header = request.headers.get("X-Junction-Proxy")
    if not header:
        return _deny("missing X-Junction-Proxy header")

    parts = header.split(":", 1)
    if len(parts) != 2:
        return _deny("malformed X-Junction-Proxy header")
...
_LIVE_GATEWAY_UNIT = "junction-gateway.service"
# The launchd counterpart of the live systemd unit. Same agent
# `junction service install` writes (junction.service.common.LAUNCHD_LABEL);
...
_LIVE_GATEWAY_LABEL = "dev.junction.gateway"
...
def _gateway_label() -> str:
    ...
    try:
        from junction.config.loader import config_dir
        from junction.pod.config import DEFAULT_UNIT_PREFIX
...
async def _live_user_unit_status() -> str:
    ...
    unit. A ``junction service install`` SYSTEM unit
    (``/etc/systemd/system/junction.service``) is NOT controllable that way:
...
def _manual_restart_command() -> str:
    ...
    Always the service-aware ``junction restart``: it resolves whatever manager
...
async def _make_live(path: str, dry_run: bool = False,
    ...
    ``.venv/bin/junction`` (else Provision -> ``missing_venv`` when absent,
    ...
    is what keeps a ``junction service install`` host (a SYSTEM unit, needing
    ...
    # manager. Restarting still does, and that is the one thing a `junction
    ...
    # drivable at all, a detached `junction restart` can still finish the
    ...
    def _validate_artifacts_sync() -> tuple[str, str] | None:
        ...
        if not dist_index.is_file():
            return ("missing_dist", (
                ...
                "(src/junction/static/dist/index.html) — run Pull+Build "
        ...
        if not can_restart:
            ...
            if foreground is not None:
                ...
                if restarted:
                    ...
                    # while the detached `junction restart` is acting on it.
                    _MAKE_LIVE_COMMITTED = True
