...
LLM-influenced CLI arguments. Direct open() would bypass junction path
controls, so every path is vetted through the CENTRALIZED sensitive-path
gate (junction.security.is_sensitive_path) -- never a local copy of the
denylist, which would drift (miss .docker, .npmrc, .netrc, trust roots...).

Fail-closed: these scripts record Junction demos, so the worktree venv they
run in has junction installed. If the import fails, path safety cannot be
...
def _sensitive_gate(real_path, write=False):
    ...
    try:
        ...
        print(
            "FATAL: junction is not importable -- path safety cannot be "
...
def read_json_input(path):
    ...
    try:
        ...
        print(
            "FATAL: junction is not importable -- input reads require the "
...
def open_media_input(path, workdir=None):
    ...
    Delegates to junction.hooks.safe_copy_file_nolink: open(O_NOFOLLOW) ->
    ...
    Fails closed when junction is not importable. Returns
    ...
    try:
        ...
        print(
            "FATAL: junction is not importable -- media input reads require "
