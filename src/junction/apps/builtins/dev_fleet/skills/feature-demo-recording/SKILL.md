...
1. **Use an interpreter that can import `junction`.** The gate consults the
   ...
   is NOT guaranteed to exist. Do NOT trust `command -v junction` to name one: on
   ...
   `<py> -c "import junction"`. Exit 78 with "cannot import junction" means the
...
import `junction`. Two interpreters is the normal case, and a
`ModuleNotFoundError: playwright` there is not the exit-78 gate.

```bash
PY=<worktree>/.venv/bin/python      # must satisfy: $PY -c "import junction"
