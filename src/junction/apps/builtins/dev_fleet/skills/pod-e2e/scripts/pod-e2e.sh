...
#
#   junction pod up --json  →  health poll  →  auth check  →  API tests  →  Playwright  →  pod down
#
# Everything runs on the pod's own port + its own JUNCTION_HOME. The live
...
if [ -z "$JUNCTION_CLI" ]; then
  for _cand in "$HOME/.local/bin/junction" "/usr/local/bin/junction"; do
    if [ -x "$_cand" ] && "$_cand" pod --help >/dev/null 2>&1; then
      JUNCTION_CLI="$_cand"; break
    fi
  done
fi
[ -n "$JUNCTION_CLI" ] || { echo "FATAL: junction CLI with pod subcommand not found on PATH" >&2; exit 65; }

# Resolve the checkout path for $NAME via `git worktree list --porcelain`.
# We search from either JUNCTION_POD_REPO or the script's own directory.
#
# This MUST mirror pod/runtime.py resolve_checkout(), or the harness can test a
# different checkout than `junction pod up` booted — a silently wrong QA verdict.
...
_resolve_checkout() {
  local name="$1"
  local repo_hint="${JUNCTION_POD_REPO:-$HERE}"
  local wt_path=""
  ...
  # Final fallback: JUNCTION_POD_WORKTREES_ROOT/<name>
  if [ -z "$wt_path" ] && [ -n "${JUNCTION_POD_WORKTREES_ROOT:-}" ] && [ -d "$JUNCTION_POD_WORKTREES_ROOT/$name" ]; then
    wt_path="$JUNCTION_POD_WORKTREES_ROOT/$name"
  fi
  echo "$wt_path"
}
...
if [ -z "$CHECKOUT" ] || [ ! -d "$CHECKOUT" ]; then
  ...
  echo "  Ensure a git worktree with basename '$NAME' exists, or set JUNCTION_POD_REPO / JUNCTION_POD_WORKTREES_ROOT" >&2
  exit 66
fi
...
# that skips the FE phase (--api-only, unhealthy pod, no JUNCTION_PW_PY) or
...
MANIFEST=""
for _m in "$CHECKOUT/.pod-test.sh" "$CHECKOUT/src/junction/.pod-test.sh"; do
...
_pod_down_best_effort() {
  if [ "$ALREADY_UP" -eq 0 ] && [ "$KEEP" -eq 0 ] && [ "$NO_STOP" -eq 0 ] && [ -n "$NAME" ]; then
    "$JUNCTION_CLI" pod down "$NAME" >/dev/null 2>&1 || true
  fi
}
...
_pod_status_json=$("$JUNCTION_CLI" pod status "$NAME" --json 2>/dev/null || echo '{}')
...
if [ "$_pod_is_up" = "yes" ]; then
  ...
  POD_JSON=$("$JUNCTION_CLI" pod up "$NAME" --json 2>"$ARTIFACT_DIR/pod-up.log")
  if [ $? -ne 0 ]; then
...
if [ -z "$BASE_URL" ] || [ -z "$TOKEN" ]; then
  ...
  TOKEN=$("$JUNCTION_CLI" pod token "$NAME" 2>/dev/null | tail -1)
  BASE_URL=$("$JUNCTION_CLI" pod url "$NAME" 2>/dev/null | tail -1)
fi
...
if [ "$HEALTHY" -eq 0 ]; then
  "$JUNCTION_CLI" pod logs "$NAME" -n 50 > "$ARTIFACT_DIR/boot-fail.log" 2>&1 || true
...
if [ "$RUN_API" -eq 1 ] && [ "$HEALTHY" -eq 1 ]; then
  ...
  if [ ! -x "$API_PY" ]; then
    fail "api-tests — worktree venv python not found at $API_PY (run: junction pod provision $NAME)"
  else
...
if [ "$RUN_FE" -eq 1 ] && [ "$HEALTHY" -eq 1 ]; then
  if [ -z "$PW_PY" ] || [ ! -x "$PW_PY" ]; then
    ...
    log "FAIL: Playwright python not found (set JUNCTION_PW_PY)"
    log "  python3 -m venv <path> && <path>/bin/pip install playwright==1.61.0"
    log "  export JUNCTION_PW_PY=<path>/bin/python"
    log "  (or re-run with --api-only to skip the frontend phase deliberately)"
    fail "playwright — no usable JUNCTION_PW_PY, so zero screenshots were captured (see fix above)"
    ...
    JUNCTION_POD_TOKEN="$TOKEN" "${PW_CMD[@]}" > "$ARTIFACT_DIR/playwright.log" 2>&1
    PW_RC=$?
    if [ "$PW_RC" -eq 0 ]; then
