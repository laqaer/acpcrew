#!/usr/bin/env python3
"""pod-playwright.py — headless-chromium frontend check for a Junction pod.

Run by pod-e2e.sh with a Playwright venv python (Playwright + bundled
chromium at ~/.cache/ms-playwright/chromium-*). The Junction gateway serves its
own SPA bundle (src/junction/static/dist), so we point chromium straight at the
...
def main():
    ...
    ap.add_argument("--token", default=None,
                    help="Dashboard token (prefer JUNCTION_POD_TOKEN env — argv is world-readable)")
    ...
    token = os.environ.get("JUNCTION_POD_TOKEN") or args.token
    if not token:
        ap.error("token required: set JUNCTION_POD_TOKEN env or pass --token")
