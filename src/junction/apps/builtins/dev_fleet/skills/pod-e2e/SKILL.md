---
name: pod-e2e
description: "ONLY for developing Junction itself -- if the project you are working on is anything else, ignore this skill: it drives Junction's own pod tooling, which does not exist in another repository. Runs end-to-end tests (backend API + frontend Playwright) for a Junction feature worktree against an ISOLATED throwaway pod instance, without touching the live gateway. Use when asked to e2e-test / smoke-test / verify a Junction worktree's feature hands-off, run API + browser tests on a pod, or prove a new backend route + UI work together. NOT for testing the live instance, and NOT a general-purpose e2e or smoke-test skill."
...
on one port** — exactly like a Docker container. The `junction pod` CLI is the
...
junction pod down <wt>
```

Other verbs: `ls` (list running pods) · `status <wt>` · `token <wt>` · `url <wt>`
· `logs <wt>` · `provision <wt>`. Run `junction pod --help` for the full list.

**Isolation guarantees** (enforced by the pod runtime):
own `JUNCTION_HOME`, own port, **no tunnel** (can't grab the real Slack identity),
...
1. **up** — `junction pod up <wt> --json`. If already active, reuses it (and
...
3. **auth** — proves auth: `/api/sessions` → 200 with token, 403 without.
   Token comes from `junction pod up --json` output (no manual minting needed).
...
6. **collect** — all logs + screenshots land in
   ...
   screenshot.
7. **stop** — `junction pod down <wt>`: stops the service, waits for its process
...
`<worktree>/.pod-test.sh` then `<worktree>/src/junction/.pod-test.sh`.
...
The **pod** isolates the gateway under test (own `JUNCTION_HOME`, own port,
...
```
spawn_run(task="""
You are a QA engineer verifying the Junction feature in worktree '<wt>'.
...
`.venv/bin/junction` (editable install) + a built SPA bundle (`static/dist`).
The pod boot refuses without them.

```bash
junction pod up <wt>                 # auto-builds the venv; FAILS LOUD if no dist
junction pod up <wt> --provision     # full on-ramp: venv + build, then up
junction pod provision <wt>          # just the on-ramp (venv + dist)
junction pod provision <wt> --venv-only
```

Every failure teaches the next step: no worktree → create one; no venv → auto;
no dist → build-or-`--provision`.

- Playwright venv: controlled by env `JUNCTION_PW_PY`.
  ...
  export JUNCTION_PW_PY=<path>/bin/python
  ```
...
3. **On approval, attach to the PR automatically -- do NOT ask again.**
   - Copy the approved files into `<worktree>/temp-screenshots/<feature>/`
     ...
     for the full convention; NEVER under `docs/` or `src/junction/**` --
...
junction-worktree-dev skill, "Rule 9 -- Leave the worktree clean (so prune can
reap it)."
