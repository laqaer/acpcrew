...
that indirection (``junction service install`` from a build that has it).
...
``junction restart`` instead of a service manager. Strictly ordered
...
from junction.service.macos import (
    PLIST_PATH,
...
#                          be a silent no-op. Remedy: `junction service install`
...
#: run-marker (see :mod:`junction.instances.run_marker`).
...
def default_confinement() -> "str | None":
    ...
    ``junction-agents.slice`` cgroup scope. Namespaces and cgroups are
    INHERITED by children regardless of ``start_new_session``, so a
    ``junction restart`` spawned from here would produce a replacement gateway
    ...
    (c) — because ``cli.main()`` clears the inherited ``JUNCTION_SANDBOX_ACTIVE``
    ...
    * ``JUNCTION_SANDBOX_ACTIVE`` — exported only by the namespace launcher /
      seatbelt env prefix, i.e. only ever present INSIDE the sandbox.
    * ``junction-agents.slice`` in ``/proc/self/cgroup`` — the transient scope
    ...
    try:
        ...
        return None
    if "junction-agents.slice" in cgroup:
        return "the Dev Fleet backend runs inside the junction-agents cgroup scope"
    return None
...
def default_detached_spawn(argv: list[str]) -> None:
    ...
    The command must OUTLIVE us — it is ``junction restart``, which kills the
...
class ForegroundBackend:
    ...
    telling the operator to run ``junction restart`` themselves. This backend
    ...
    **Identity.** There is no ``ExecMainStartTimestampMonotonic`` here, so
    ...
    replacement) — the same identity ``junction restart`` itself waits on, and
    ...
    **FAIL SAFE.** This class never signals any process itself. Its only
    mutation is establishing the detached ``junction restart``; when the binary
    ...
    **Confinement gate.** The Dev Fleet backend usually runs inside the
    gateway-applied OS sandbox and ``junction-agents.slice`` cgroup scope,
    ...
    def _usable_launcher(path: "str | None") -> "str | None":
        """*path* when it is an absolute, executable ``junction`` file."""
        if not path:
            return None
        p = Path(path)
        ...
        # PATH entry happens to name. .stem so a Windows ``junction.exe`` would
        # also pass, though callers currently construct this backend POSIX-only.
        if p.stem != "junction" or not p.is_absolute():
            return None
        try:
    ...
    def _resolve_binary(self, port: int) -> "str | None":
        """The ``junction`` launcher to run the restart with, or ``None``.
        ...
        can have planted. A ``shutil.which("junction")`` fallback is
        ...
        no launcher (a source-tree ``python -m junction`` launch) keeps the
    ...
    async def restart_detached(self) -> "tuple[bool, str]":
        """Establish a detached ``junction restart``. ``(ok, error)``.
