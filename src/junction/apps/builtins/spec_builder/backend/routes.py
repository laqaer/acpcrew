...
from junction.platform_compat import RENAME_NOREPLACE_AVAILABLE, rename_noreplace
...
try:
    from junction.hooks import _fd_real_path, safe_read_file_bytes_nolink
...
try:
    from junction.autonudge import AutoNudgeService as _AutoNudgeService
...
# circular import: junction.dashboard.server imports the builtins to register
...
logger = logging.getLogger("junction.app.spec-builder")
...
def _safe_dir(raw: str, *, must_exist: bool = True) -> Path | None:
    ...
    must pass through, so the guarantees hold uniformly:
      ...
      * must not be a sensitive path (credential stores, ``.ssh``, ``.aws``,
        policy files) per ``junction.security.is_sensitive_path``;
...
def _dispatch_turn(
    ...
    *,
    ...
    try:
        ...
        from junction.dashboard.turn_dispatch import bounded_chat_turn
...
async def _git(cwd: str, *args: str) -> tuple[int, str, str]:
    ...
    try:
        ...
        # gateway's ~100 threads (see junction.sandbox and issue #935).
...
def register_routes(app: web.Application) -> None:
    ...
    event loop, and a ``JUNCTION_HOME`` on stalled network storage would freeze
