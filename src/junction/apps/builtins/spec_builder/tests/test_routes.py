...
from junction.apps.builtins.spec_builder.backend import routes
...
def _live_state_snapshot() -> dict[str, int]:
    ...
    module-level ``routes._state_dir()`` freezes whichever ``JUNCTION_HOME``
...
def test_security_helper_is_imported_at_module_scope():
    ...
    assert "    from junction.security import is_sensitive_path" not in src
...
def test_gateway_helpers_are_imported_at_module_scope():
    ...
    for node in ast.walk(tree):
        ...
        for inner in ast.walk(node):
            if isinstance(inner, ast.ImportFrom) and (inner.module or "").startswith("junction"):
    ...
    for mod in local_imports:
        assert mod.startswith("junction.dashboard."), f"undocumented local import: {mod}"
...
def test_route_registration_touches_no_filesystem():
    ...
    loop during start_dashboard, so a JUNCTION_HOME on stalled network storage
...
async def test_delete_restores_the_spec_when_archiving_fails(tmp_path, monkeypatch):
    ...
    import junction.dashboard.chat_persistence as cp
...
async def test_delete_discards_queued_work_before_cancelling(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "junction.dashboard.chat_persistence",
