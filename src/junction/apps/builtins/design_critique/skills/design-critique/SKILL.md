...
- **Code or markup file** (HTML, JSX/TSX, Vue, Svelte, a CSS+HTML pair, etc.): **render it, then
  ...
  printed by the resolution command (`python3 -c "import junction, pathlib; print(pathlib.Path(junction.__file__).parent / 'apps/builtins/design_critique/skills/design-critique')"`), and the scripts live in `<skill-dir>/scripts`:
