...
function detect() {
  ...
  // Junction app with an embedded UI — no routes of its own.
  ...
  if (ajRaw) {
    try {
      ...
      if (aj.ui || aj.mcpServers || aj.agents) {
        ...
        notes.push('This is a Junction app with an embedded UI — it has no routes to crawl by itself. Run the whole Junction dashboard and open its page' + (hostPage ? ` (${hostPage})` : '') + ', or give a screenshot.')
        return { framework: 'Junction app (embedded UI)', routing: 'host-embedded', routes: [], hostPage }
      }
