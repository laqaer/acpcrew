...
before dispatch (see `junction.apps.proxy_auth`); only `/health` is left
...
from junction.platform_compat import (
...
APP_NAME = os.environ.get("JUNCTION_APP_NAME") or "design-tweak"
...
# $JUNCTION_HOME. Creation is deferred to `main()`, the actual process entry
...
def _is_junction_internal(target: Path) -> bool:
    ...
    for base in _JUNCTION_INTERNAL_DIRS:
...
# `JUNCTION_PROXY_SECRET` is the whole of this backend's authentication: the
# gateway HMACs every proxied request with it (`junction.apps.proxy_auth`), and
...
# (`JUNCTION_HOME`, `JUNCTION_APP_*`, operator-declared `JUNCTION_DEVFLEET_BIN_*`,
...
def _child_env(bin_dir: Path) -> dict:
    ...
    `_CHILD_ENV_STRIP` for why `JUNCTION_PROXY_SECRET` and `PORT` in particular
    are disqualifying.
    """
    env = {
        k: v
...
def _find_entry(folder: Path, root: Path | None = None):
    ...
    for rel in _ENTRY_CANDIDATES:
        ...
        if not p.is_file():
            continue
        if is_sensitive_path(str(p)) or _is_project_secret(base, p) or _is_junction_internal(p):
            continue
        return p
    return None
...
class Handler(BaseHTTPRequestHandler):
    server_version = "Junction-SelectToEdit/" + VERSION  # brand-ok: Server-header token
    timeout = _CLIENT_READ_TIMEOUT  # see `_CLIENT_READ_TIMEOUT`
