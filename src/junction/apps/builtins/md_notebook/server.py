"""Notes backend — git-backed markdown vaults for Junction.
...
from junction import hooks, platform_compat, security
from junction.apps.builtins.md_notebook import git_ops
from junction.apps.builtins.md_notebook import notes as notes_mod
from junction.apps.proxy_auth import raw_request_target, verify_proxy_request
from junction.atomic_write import atomic_write, replace_with_retry
from junction.config.paths import config_dir
from junction.loop_lock import LoopBoundLock
from junction.platform_compat import restrict_to_owner
...
async def proxy_auth_middleware(request: web.Request, handler: Callable) -> web.StreamResponse:
    ...
    if not verify_proxy_request(
        request.headers.get("X-Junction-Proxy", ""),
...
def create_app() -> web.Application:
    ...
    from junction.apps.builtins.md_notebook import syncer
