...
A stock machine has no LaTeX compiler, so ``pip install junction`` shipped this
...
follows :mod:`junction.embeddings`, the in-tree precedent for exactly this
...
logger = logging.getLogger("junction.app.papyrus")
...
SKIP_DOWNLOAD_ENV = "JUNCTION_PAPYRUS_SKIP_TECTONIC_DOWNLOAD"
...
TECTONIC_URL_ENV = "JUNCTION_PAPYRUS_TECTONIC_URL"
...
def reset_provision_state() -> None:
    """Forget the job state (tests, and ``JUNCTION_HOME`` changes)."""
    global _job_thread
    with _state_lock:
...
def _reject_member_name(name: str) -> str | None:
    ...
    * an empty name.
    ...
    Mirrors :func:`junction.apps.builtins.papyrus.backend.store.safe_child`'s
...
def _make_ssl_context() -> ssl.SSLContext:
    ...
    Same shape and reason as :func:`junction.embeddings._make_ssl_context`: a
...
def provision_in_background(root: Path | None = None) -> bool:
    ...
    reasoning as :func:`junction.embeddings._run_download_on_daemon_thread`. The
...
def _provision_job(root: Path | None) -> None:
    ...
    if installed:
        ...
        from junction.apps.builtins.papyrus.backend import latex
