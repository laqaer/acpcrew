...
def is_reparse_link(path: Path) -> bool:
    ...
    Junction owns has to refuse a junction too, or the refusal is POSIX-only.
...
def safe_project_dir(name: str, root: Path | None = None) -> Path:
    ...
    # reasoning as `_config_path`: this is a directory Junction owns and creates.
...
def _config_path(project: Path) -> Path | None:
    ...
    # file is one Junction owns and writes, so a link at that name is illegitimate
    # wherever it points.
    #
