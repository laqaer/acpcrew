...
* **Sandbox chokepoint.** The repository is agent- and user-influenced content
  ...
  :func:`junction.sandbox.sandboxed_spawn_argv` (OS isolation + scrubbed env)
  and spawns through :func:`junction.sandbox.create_subprocess_limited`.
...
from junction.apps.builtins.papyrus.backend import procio, store
from junction.atomic_write import atomic_write
from junction.executors import subprocess_executor
from junction.git_divergence import divergence_count_args, parse_divergence_counts
...
def _pin_attributes_sync(cwd: Path) -> None:
    ...
    **Appended, never written over.** Overwriting with
    ...
    # written. Both are paths Junction owns and writes BY NAME, so a link there is
    ...
    # Verified before the fix. The rule has to hold for every segment Junction
    ...
    if (
        ...
        # security DECISION (a link where Junction owns the name), and a denial that
