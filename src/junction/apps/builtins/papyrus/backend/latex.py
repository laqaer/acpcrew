...
* ``-no-shell-escape`` is passed **explicitly** on every pdflatex invocation.
  ...
  ask.
* The compiler spawn is routed through :func:`junction.sandbox.sandboxed_spawn_argv`
  — the OS-level sandbox + credential-scrubbed env chokepoint — and carries
  :func:`junction.sandbox.create_subprocess_limited`, so a runaway macro expansion
...
from junction.apps.builtins.papyrus.backend import procio, store, tectonic
...
def _sensitive_hidden_dirs() -> tuple[str, ...]:
    ...
    Resolved per call, not cached at import: ``JUNCTION_HOME`` is set per test and
...
async def compile_project(project: Path, main_file: str) -> CompileResult:
    ...
    # mount (`JUNCTION_HOME` on NFS/SMB) those block for as long as the mount takes to
