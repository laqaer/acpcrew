...
def _credentials_are_unconfined() -> str:
    ...
    try:
        ...
        mode = str(getattr(JunctionConfig.load(), "sandbox", "") or "").strip().lower()
...
class RunSupervisor:
    ...
    def _build_runner(self, *, stop_check) -> Any:
        ...
        ``import junction.acp`` FIRST: there is a known circular import that only
        ...
        """
        import junction.acp  # noqa: F401  # circular import — must precede the factory
        ...
        if SessionAgentRunner.available():
            ...
            # silently activates the DEFAULT agent — which carries the full junction-core
