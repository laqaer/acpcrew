...
def _write_protected_targets() -> tuple[str, ...]:
    ...
    * The PARENT DIRECTORY is masked, not the file. ``sandboxed_spawn_argv``'s
      ...
      ``~/.junction/config.json`` and exit 0; passing the parent blocked it.
    * Only EXISTING directories are returned — the launcher mounts over each target, and a
    ...
    Masking the whole ``~/.junction`` directory is safe *for this caller* specifically: the
...
def _run(
    ...
    # `~/.junction/config.json` and exited 0. Those paths are `security.
    ...
    # under `.junction`), i.e. the one-way doors that
