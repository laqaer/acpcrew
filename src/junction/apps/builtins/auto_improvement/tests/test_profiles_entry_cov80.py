...
from junction.apps.builtins.auto_improvement import profiles
...
class TestBuildProfile:
    ...
    def test_a_configured_clone_builds_the_github_repo_profile(
        ...
        clone.mkdir()
        monkeypatch.setattr(
            "junction.apps.builtins.auto_improvement.backend.store.data_dir",
            lambda: tmp_path / "data",
        )
        monkeypatch.setattr(
            "junction.apps.builtins.auto_improvement.backend.store.workspace_dir",
            lambda: tmp_path / "data",
        )
