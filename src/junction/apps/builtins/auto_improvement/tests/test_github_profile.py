...
from junction.apps.builtins.auto_improvement.spine.profile import (
    BugRunner,
    BuildGate,
    CalibrationParams,
    EditAllowlist,
...
@pytest.mark.skipif(not __import__('junction.sandbox', fromlist=['userns_available']).userns_available(), reason="requires unprivileged user namespaces (sandbox backend)")
class TestIsolation:
...
class TestMeasureEnvDoesNotUndoTheSandboxScrub:
    ...
    def test_credential_shaped_names_are_stripped_by_shape(self) -> None:
        ...
        `GITHUB_TOKEN` survives `junction.sandbox.scrub_env` (its list covers
...
class TestGateChildEnvironmentMeasuredEndToEnd:
    ...
    def test_no_credential_reaches_the_child_and_the_suite_can_still_run(
        ...
        from junction.apps.builtins.auto_improvement.profiles.github_repo import profile as prof
...
class TestGateRunsInStrictSandboxMode:
    ...
    def test_the_gate_requests_strict_mode(self, monkeypatch, tmp_path) -> None:
        ...
        # `sandboxed_spawn_argv` at module scope, so patching `junction.sandbox` leaves the
