...
def _module_name_for(rel: str) -> str:
    ...
    ``src/junction/apps/foo/bar.py`` → ``bar`` (we grep on the leaf + a couple parents so a
...
def _build_prompt(
    *,
    ...
    return (
        ...
        "  {\n"
        '    "file":       "<repo-relative path, e.g. src/junction/foo/bar.py>",\n'
...
def discover_surfaces_via_agent(
    ...
    *,
    ...
    src_dir = Path(clone) / "src" / "junction"
    src_label = "src/junction" if src_dir.exists() else "src"
