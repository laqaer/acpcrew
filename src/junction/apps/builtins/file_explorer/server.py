...
from junction.sandbox import cgroup_scope_argv, run_limited, wrap_argv
...
APP_NAME = os.environ.get("JUNCTION_APP_NAME", "file-explorer")
...
# Junction data-home subdirectories that are safe for the file explorer to
...
def _is_sensitive(p: Path) -> bool:
    ...
    if after != -1:
        ...
        if next_part not in _JUNCTION_SAFE_SUBDIRS:
            return True
...
def _junction_safe_children(junction_dir: Path) -> list[dict]:
    ...
    junction_dir = _contain_in_allowed_roots(junction_dir, operation="tree_list")
    out: list[dict] = []
    try:
        children = sorted(junction_dir.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower()))
    except (OSError, PermissionError):
        return out
    for child in children:
        if child.name in _JUNCTION_SAFE_SUBDIRS and child.is_dir():
...
def _search_rg(root: Path, query: str, include: str, exclude: str) -> list[dict]:
    ...
    # separately from Junction itself, which is why the same folder gets prompted
...
class FileExplorerHandler(BaseHTTPRequestHandler):
    server_version = "Junction-FileExplorer/0.1"
    ...
    def _authorized_or_health(self, method: str) -> bool:
        """Verify the gateway's X-Junction-Proxy HMAC before dispatch (CWE-306).
        ...
        if verify_proxy_request(
            self.headers.get("X-Junction-Proxy", ""),
            method=method,
            target=self.path,
            body=b"",
        ):
    ...
    def _dispatch(self, method: str) -> None:
        ...
        # Junction's reverse proxy forwards requests as /api/<path>;
    ...
    def _h_tree(self, qs: dict[str, list[str]]) -> None:
        ...
        # read (``.exists()``/``.is_dir()``/``_junction_safe_children``), so no
        ...
        if _is_data_home_root(expanded):
            ...
            if resolved.exists() and resolved.is_dir():  # lgtm[py/path-injection]
                ...
                entries = _junction_safe_children(resolved)
                return self._json(
