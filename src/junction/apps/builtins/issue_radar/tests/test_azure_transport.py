...
``junction.dashboard.handlers.source_providers``; the spawn tests patch
...
from junction.apps.builtins.issue_radar.backend import azure_client
from junction.apps.builtins.issue_radar.backend.errors import (
    ProviderCliError,
    ProviderPermissionError,
    ProviderSetupError,
)
from junction.dashboard.handlers import source_providers
from junction.github_runner import STRICT_PROVIDER_BIN_ENV
...
class TestAzBinResolution(unittest.TestCase):
    ...
    def test_the_override_is_validated_and_its_canonical_path_is_returned(self):
        ...
        with mock.patch.dict(os.environ, {"JUNCTION_ISSUE_RADAR_AZ": "/opt/az/az"}, clear=False):
        ...
        for error in (ValueError("not canonical"), OSError("unreadable")):
            with self.subTest(error=type(error).__name__):
                ...
                with mock.patch.dict(
                    os.environ, {"JUNCTION_ISSUE_RADAR_AZ": "/tmp/az"}, clear=False
                ):
                ...
                self.assertIn("JUNCTION_ISSUE_RADAR_AZ", str(caught.exception))
    ...
    def test_a_failed_resolution_is_not_cached(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JUNCTION_ISSUE_RADAR_AZ", None)
    ...
    def test_with_no_candidates_at_all_the_error_names_the_remedies(self):
        ...
        self.assertIn("JUNCTION_ISSUE_RADAR_AZ", message)
