...
from junction.apps.builtins.issue_radar.backend import github_client, provider, store, watch
from junction.apps.manager import is_app_enabled
from junction.config.loader import JunctionConfig
from junction.context import ui_language_tag
...
async def _handle_connect(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # ``dashboard.gitlab_hosts`` allowlist, and ``JunctionConfig.load()`` is
...
def _ui_language() -> str:
    ...
    unshipped stored value — see :func:`junction.context.ui_language_tag`. The
    ...
    **Call this OFF the event loop** (``asyncio.to_thread``): ``JunctionConfig
    ...
    """
    try:
        return ui_language_tag(JunctionConfig.load())
    except Exception:
...
async def _run_oneshot_model(request: web.Request, key: str, prompt: str) -> str:
    ...
    ``junction-lite`` background agent — the same lever workflows / title-gen /
    ...
    so no kiro-cli subprocess leaks. It reuses the user's own Junction backend, so
    ...
    provider, _is_new, _resumed = await state.sessions.get_or_create(key, agent="junction-lite")
    try:
...
async def _compute_issue_ai(
    ...
    *,
    ...
    from junction.llm_helpers import parse_llm_json
...
#
# "Investigate" opens a Junction chat session (seeded with an investigation
...
async def _compute_label_recommendations(
    ...
    Mirrors :func:`_compute_issue_ai` exactly (``junction-lite`` background agent,
    ...
    from junction.llm_helpers import ToolApprovalPolicy, parse_llm_json, stream_and_collect
    from junction.security import redact
