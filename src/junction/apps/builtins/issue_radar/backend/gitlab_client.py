...
GitLab App, no PAT stored by Junction, no hosted backend. Every call shells out
...
def _glab_bin() -> str:
    ...
    same binary is untrusted. Set ``JUNCTION_ISSUE_RADAR_GLAB`` to an absolute
    path to override (still validated), or ``JUNCTION_PROVIDER_BIN_STRICT=1`` to
    ...
    override = os.environ.get("JUNCTION_ISSUE_RADAR_GLAB")
    if override:
        try:
            ...
            raise ProviderSetupError(
                f"JUNCTION_ISSUE_RADAR_GLAB={override!r} failed validation: {exc}",
    ...
    raise ProviderSetupError(
        ...
        "manager) and run `glab auth login`, or set JUNCTION_ISSUE_RADAR_GLAB to "
...
def allowed_hosts() -> frozenset[str]:
    ...
    """
    try:
        return frozenset(JunctionConfig.load().dashboard.gitlab_hosts)
