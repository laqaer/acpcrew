...
No GitHub App, no PAT, no Junction-hosted credential storage: every call
...
_GH_OVERRIDE_ENV = "JUNCTION_ISSUE_RADAR_GH"
...
def _gh_bin() -> str:
    ...
    ``JUNCTION_ISSUE_RADAR_GH`` to an absolute path to override (still
    validated), or ``JUNCTION_PROVIDER_BIN_STRICT=1`` to require a root-owned
