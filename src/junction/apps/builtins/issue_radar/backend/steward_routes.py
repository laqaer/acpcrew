...
``steward_store.CrewStoreError`` maps to **409**, never 500: every raise is a
...
logger = logging.getLogger("junction.app.issue-radar.crews")
...
def _crew_conflict(handler):
    ...
    async def _wrapped(request: web.Request) -> web.Response:
        ...
        except steward_store.CrewStoreError as exc:
...
def _crew_for_slot(slot_keys: tuple[str, ...]) -> tuple[provider.RepoKey, dict[str, Any]] | None:
    ...
    for entry in store.list_connected_repos():
        ...
        for crew in steward_store.list_crews(owner, repo, routes._scope(key), include_retired=True):
...
async def _require_crew(
    ...
    if not steward_store.is_crew_id(crew_id):
        return {}, web.json_response(
            {"error": f"invalid crew id {crew_id!r}", "code": "invalid_crew_id"},
            status=400,
        )
    crew = await routes._st(key, steward_store.read_crew, key.owner, key.repo, crew_id)
    if crew is None:
        return {}, web.json_response(
...
def _crews_page(owner: str, repo: str, root: Any) -> dict[str, Any]:
    ...
    """
    crews = steward_store.list_crews(owner, repo, root)
    ...
    return {"crews": crews, "settings": steward_store.read_settings(owner, repo, root), "counts": counts}
...
def _crew_page(owner: str, repo: str, crew_id: str, limit: int, root: Any) -> dict[str, Any]:
    ...
    return {
        "crew": steward_store.read_crew(owner, repo, crew_id, root),
        "items": steward_store.list_work_items(owner, repo, crew_id, root),
        "events": steward_store.read_events(owner, repo, root, crew_id=crew_id, limit=limit),
        ...
        "counts": {"open": steward_store.open_slot_count(owner, repo, crew_id, root)},
    }
...
async def _handle_crew_create(request: web.Request) -> web.Response:
    ...
    crew = await routes._st(key, steward_store.create_crew, key.owner, key.repo, body)
...
def _auto_approve_armed(crew: dict[str, Any]) -> bool:
    ...
    """
    return bool(crew.get("unattended")) and steward_runtime.is_live(crew)
...
async def _handle_crew_update(request: web.Request) -> web.Response:
    ...
    crew = await routes._st(key, steward_store.update_crew, key.owner, key.repo, crew_id, body)
...
async def _handle_crew_retire(request: web.Request) -> web.Response:
    ...
    if missing is not None:
        return missing
    crew = await routes._st(key, steward_store.retire_crew, key.owner, key.repo, crew_id)
...
async def _revoke_execution(
    ...
    closes it; :func:`steward_runtime.revoke_crew_execution` documents the two grants
    ...
    if state is None:
        return
    try:
        await steward_runtime.revoke_crew_execution(state, crew, reason)
    except Exception:
        logger.warning(
