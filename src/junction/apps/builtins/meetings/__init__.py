...
# ``importlib.import_module("junction.apps.builtins.meetings")`` then checks
# ``hasattr(_mod, "register_routes")`` on the PACKAGE itself (not the
# backend.routes submodule) — same as issue_radar/__init__.py.
from junction.apps.builtins.meetings.backend.routes import register_routes  # noqa: F401
