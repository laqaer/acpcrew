...
from junction.apps.builtins.mochi.pinned_files_service import pins_mutation
...
def _data_dir() -> Path:
    ...
    server, a separate short-lived process, and ``junction.apps.manager``
...
def _call_tool(name: str, args: dict[str, Any]) -> str:
    ...
    return call_tool_with_logging(
        ...
        downstream_service=f"junction-{SERVER_NAME}",
    )
...
def run_mcp_server() -> None:
    """Run the stdio MCP server. Entry point for ``junction app mcp mochi``."""
    # Keep logging off stdout — stdout is the JSON-RPC channel.
    logging.basicConfig(level=os.environ.get("JUNCTION_LOG_LEVEL", "WARNING"), stream=None)
