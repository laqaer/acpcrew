"""App registry — curated list of available Junction apps.
...
own ``app.json`` — they never need to update the Junction registry JSON
...
from junction.apps import install_receipt, official_catalog
from junction.apps.admission import app_admission_denied, verified_signer
...
from junction.apps.manager import list_apps as list_installed_apps
from junction.apps.manager import (
...
from junction.sandbox import cgroup_scope_argv, create_subprocess_limited, wrap_argv
from junction.sel import sel

try:
    from junction.sel import sel as _sel_fn
except ImportError:
...
from junction.platform import PlatformCompositionError, current_context
...
# :func:`junction.service.common` for the same reason). This is a
...
def _pinned_registries() -> list[Any]:
    ...
    from junction.config.loader import ExternalRegistryConfig
...
def _effective_registries() -> list[Any]:
    ...
    from junction.config.loader import (
        JunctionConfig,  # circular import: loader.py imports from apps/ at module level; deferring avoids ImportError
    )

    try:
        configured = list(JunctionConfig.load().registries or [])
...
async def _fetch_app_manifest(
    ...
    *commit*: a pinned commit. When set, the manifest is read from THAT tree
    ...
    # is_clone_host_trusted() loads config from disk (JunctionConfig.load), so
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="junction-manifest-")
...
def _enrich_with_install_status(
    ...
    *detected* is a set of app names that were found via ``detectInstalled``
    shell commands (installed outside Junction's app manager).
    """
...
def _apply_trust_fields(
    ...
    *,
    ...
    - ``verified``: ``True`` only when provenance is NOT ``"external"`` AND
      ...
      repo publishing ``"author": "junction"`` in its manifest does not
...
def _read_external_registry_cache(
    ...
    *,
    ...
    try:
        ...
        from junction.apps.manifest import KEBAB_RE
...
async def _fetch_external_registry_index(
    ...
    *repo* is a git-cloneable URL (https/ssh/git/scp-style).  The repo is
    ...
    tmp_root: str | None = None
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="junction-registry-")
...
async def list_registry() -> list[dict[str, Any]]:
    ...
    5. Enrich with install status from Junction's app manager
    ...
    # the app author's — a repo publishing ``"author": "junction"`` in its
...
def _read_git_metadata_bounded(path: Path, limit: int) -> str | None:
    ...
    :func:`junction.hooks.safe_read_prefix`, which canonicalizes via ``realpath``
...
async def _run_app_build(
    ...
    if (build_dir / "package.json").is_file():
        ...
        # PATH, which is routinely a different interpreter: `bin/junction` execs
        # `.venv/bin/junction` WITHOUT putting the venv's `bin/` on PATH, and
...
async def _retained_startup_refusal(
    ...
    from junction.apps.hooks_integration import stop_retained_startup_hooks
...
async def install_from_registry(
    ...
    # Bundled (curated, Junction-shipped) entries have no ``_registry`` marker
    ...
    # the bundled (Junction-shipped) catalog, which is itself a distinct source.
    ...
    # Junction is running on an incompatible platform, return client install
    ...
    # Validate minJunctionVersion if declared
    min_version = (manifest or {}).get("minJunctionVersion", "")
    if min_version:
        from junction.apps.version import check_min_version
    ...
    # the junction-managed path below already uses `result` for the
    ...
    try:
        ...
        if deps_data and isinstance(deps_data, dict):
            from junction.apps.dependencies import resolve_dependencies as _resolve_deps
            from junction.apps.manifest import Dependencies as _Deps
