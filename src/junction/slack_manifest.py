...
* ``cli_setup`` (``junction manifest``) prints the manifest or the deep link,
* the dashboard's ``GET /api/slack/manifest`` serves both to Settings → Slack,
* ``security._junction_slack_app_link_alias`` decides whether a deep link is
...
def raw_template() -> str:
    ...
    return str(_pkg_files("junction").joinpath("slack-manifest.yaml").read_text("utf-8"))
