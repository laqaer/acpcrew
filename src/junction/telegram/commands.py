...
Commands:
  ...
  /junction dashboard [<N>h|<N>m] — send a presigned dashboard login link
...
from junction.messaging.conversation import ConversationState  # noqa: F401
...
# Two-token command: ``/junction dashboard [<TTL>]``. The bare ``/junction``
# token is deliberately NOT a command on its own (see parse_command).
_DASHBOARD_ALIASES = frozenset(("/junction",))
...
def _strip_bot_mention(cmd: str, bot_username: str) -> str:
    ...
    ``/new@JunctionBot`` instead of bare ``/new``. This is standard,
...
def parse_command(text: str, bot_username: str = "") -> str | None:
    ...
    *bot_username* (this bot's own registered username, from ``getMe``) gates
    ...
    if cmd in _STOP_ALIASES:
        return "stop"
    # /junction dashboard [<TTL>] -- requires the explicit "dashboard"
    # subcommand, so a bare "/junction" (typo or menu tap) falls through as
...
def parse_dashboard_argument(text: str) -> str:
    """Return the argument of ``/junction dashboard [<N>h|<N>m]`` (``""`` if none).
    ...
    :func:`junction.messaging.commands.parse_dashboard_ttl`, which takes this
...
#: documented in the help card's footer instead. ``/junction`` is absent for
...
_HELP_FOOTER = (
    ...
    "/junction dashboard [<N>h|<N>m] — get a dashboard login link (DM only)\n"
    "\n"
