...
(channel-neutral) so this module never imports ``junction.slack``.
"""
...
from junction.config.loader import ACTIVATION_MENTION, ACTIVATION_OFF
...
from junction.messaging.attachments import IngestLimits, append_attachment_context
...
from junction.messaging.dispatch import build_directive_consumer, delivery_is_muted
from junction.messaging.driver import APPROVAL_INTERACTIVE, TurnDriver
...
from junction.messaging.renderer import SilentRenderer, display_safe
from junction.messaging.session_trust import add_trusted_session, is_session_trusted
from junction.messaging.sessions_view import collect_recent_sessions_audited
...
from junction.security import redact, redact_local_paths
from junction.sel import sel
from junction.stats import Stats
from junction.telegram.attachments import process_telegram_attachments
...
from junction.voice_reply import synthesis_settings, synthesize_and_deliver
...
if TYPE_CHECKING:
    ...
    from junction.telegram.client import TelegramCallback, TelegramClient

from junction.messaging.queue_receipt import MAX_COLLAPSE as _MAX_COLLAPSE
from junction.messaging.queue_receipt import STEER_ACK_EMOJI as _STEER_ACK_EMOJI
from junction.messaging.queue_receipt import (
    ReceiptQueue,
    ReceiptSurface,
)
...
# Canonical kiro-cli agent fallback so Telegram sessions load junction-core
...
# path's _DEFAULT_JUNCTION_AGENT.
_DEFAULT_JUNCTION_AGENT = "junction"
...
def _mention_re(handle: str) -> "re.Pattern[str]":
    ...
    ``(?![A-Za-z0-9_])`` is the whole point: without it `@junctionbot` matches
    inside `@junctionbot2`, so a message aimed at another bot in the same Topic
...
class TelegramDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        # junction.telegram.commands._strip_bot_mention for why an unset
    ...
    async def handle_message(
        ...
        *privacy_request* carries a modifier that was parsed off an EARLIER copy of
        ...
        # "junction" agent — so the session loads junction-core (spawn_run)
    ...
    async def _handle_dashboard(
        ...
        Mirrors the Slack ``/junction dashboard`` implementation: calls
        ``generate_token`` directly (never via shell) and builds the URL from
        the ``dashboard.url`` config (``JUNCTION_PORT`` overrides the port,
        ...
        from junction.dashboard.urls import dashboard_origin, parse_dashboard_url
        ...
        try:
            ...
            if not origin:
                ...
                # (parse_dashboard_url applies the JUNCTION_PORT override).
            ...
            logger.warning("telegram /junction dashboard: token generation failed", exc_info=True)
            try:
                sel().log_api_access(
                    caller=str(user_id),
                    operation="telegram.dashboard_token",
    ...
    async def _handle_stop(self, route: tuple[str, str], chat_id: int) -> None:
        ...
        :func:`~junction.messaging.commands.stop_running_turn`; this supplies
    ...
    async def _handle_yolo(
        ...
        :func:`~junction.messaging.commands.run_yolo_command`. Reachable only by
    ...
    def _addresses_this_bot(self, msg: InboundMessage) -> bool:
        ...
        * the message replies to one sent by this bot, matched on ``bot_id``.
        ...
        # makes it exact — Telegram usernames extend one another (`@junctionbot`,
        # `@junctionbot2`, `@junctionbot_dev` may all sit in one Topic), and an
        ...
        # `_strip_bot_mention` uses, so `@junctionbot` does not match inside
        # `@junctionbot2`.
    ...
    async def _uploads_restricted(self, session_key: str) -> bool:
        ...
        :func:`junction.messaging.upload_gate.uploads_restricted`, shared with the
        ...
        transports.
        """
        from junction.dashboard.handlers._shared import _probe_persisted_session
    ...
    async def _handle_sessions(
        ...
        *caller* is the requesting user's id, and the audit record's subject. It was
        ...
        lines.append("")
        lines.append("Open one with /junction dashboard.")
    ...
    def _configured_agent(self) -> str:
        ...
        return self.agent or self.cfg.agent.default_agent or _DEFAULT_JUNCTION_AGENT
    ...
    async def _require_direct_chat(
        ...
        *,
        ...
        were never allow-listed at all. Same rule `/junction dashboard` follows.
    ...
    def _bind_origin_mirror(self, session_key: str, route: tuple[str, str], chat_id: int) -> None:
        ...
        :func:`~junction.messaging.link.bind_origin_mirror`, shared with the
    ...
    async def _handle_link(self, route: tuple[str, str], chat_id: int) -> None:
        ...
        :func:`~junction.messaging.link.rebind_conversation_location`, the
