...
Populated `$JUNCTION_HOME` used by tests that need a non-empty
...
Running `junction gateway --seed minimal` against a fresh
`$JUNCTION_HOME` lands you on a clickable dashboard: memory tab shows
the workspace content, cron tab shows the two jobs, sessions tab shows
the pinned **Welcome to Junction** starter. Useful as the default
...
`$JUNCTION_HOME` but are intentionally **not** included here. Text and
