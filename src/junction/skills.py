...
from junction import skill_trust
from junction.atomic_write import atomic_write
from junction.config.loader import JunctionConfig, config_dir
from junction.cron import referenced_skill_names
from junction.frontmatter import SKILL_LOADER, parse_frontmatter
...
from junction.skill_usage import SKILL_USAGE_FILENAME, SkillUsageLedger
from junction.skills_script_validator import validate_scripts
...
# across all three sources (junction builtin, workspace, extra paths).
...
# Bundled fallback — inside the junction package
...
def _project_skills_dir() -> Path | None:
    """Return project-level skills/ dir from JUNCTION_PROJECT_DIR, or None."""
...
def _trusted_skill_roots() -> tuple[str, ...]:
    ...
    * the installed ``junction`` package — built-in apps keep their skills
...
_RELOCATED_SKILLS: dict[str, str] = {
    "prepare-pr": "junction-dev/prepare-pr",
    "babysit": "junction-dev/babysit",
    "junction-worktree-dev": "junction-dev/junction-worktree-dev",
}
...
def _skill_tree_fingerprint(root: Path) -> str | None:
    ...
    :func:`junction.hooks.safe_read_file_bytes_nolink` with the tree root as
...
def _ensure_builtin_skills(base: Path) -> None:
    ...
    stale_builtins = {"learn", "subagent", "cron", "junction-core"} - source_names
    if base.exists():
...
class SkillsLoader:
    ...
    def __init__(
        ...
        config: JunctionConfig | None = None,
    ):
        ...
        cfg = config or JunctionConfig.load()
        ...
        # Co-located with the skills root's parent (the Junction home) so it
    ...
    def _owning_app(self, name: str, skill_file: Path) -> str | None:
        ...
        * the flat ``skills/<skill>`` link, whose name says nothing — so the
        ...
        if head != name:
            return head
        try:
            from junction.apps.manager import apps_dir
    ...
    def _emit_lazy_load_metric(t0: float, *, hit: bool) -> None:
        ...
        try:
            ...
            get_recorder().histogram(
                "junction.skill.lazy_load.duration",
                elapsed_ms,
                unit="ms",
                attrs=attrs,
            )
            get_recorder().counter("junction.skill.lazy_load.count", attrs=attrs)
    ...
    def _repo_scope_satisfied(relpath: str, project_dir: str | Path | None) -> bool:
        ...
        *project_dir* is the SESSION's active project — the same value the
        ...
        The rule itself lives in ``junction.project_scope`` because lessons are
    ...
    def strip_frontmatter(content: str) -> str:
        ...
        ``junction.frontmatter``. Its closer grammar is stricter than
