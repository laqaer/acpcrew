...
from junction import platform_compat, slack_manifest
...
def _fix_shell_profiles() -> None:
    """Remove stale Junction PATH entries from shell profiles."""
    home = Path.home()
    profiles = [
        home / ".zshrc",
        home / ".bashrc",
...
def _find_electron_dir() -> Path | None:
    ...
    only resolves from a source checkout: first via JUNCTION_PROJECT_DIR, then
...
def _setup_electron() -> None:
    """Build and install the Junction desktop app (macOS only)."""
    ...
    app_src = electron_dir / "dist" / arch / "Junction.app"
    if not app_src.is_dir():
        for candidate in ("mac-arm64", "mac", "mac-x64"):
            app_src = electron_dir / "dist" / candidate / "Junction.app"
            if app_src.is_dir():
                break
    if not app_src.is_dir():
        print("  ❌ Build succeeded but Junction.app not found in dist/")
        return

    app_dest = Path.home() / "Applications" / "Junction.app"
    app_dest.parent.mkdir(parents=True, exist_ok=True)
    if app_dest.is_dir():
        shutil.rmtree(app_dest)
    shutil.copytree(str(app_src), str(app_dest))
    print("  ✅ Junction.app installed to ~/Applications")
    print("     Launch via Spotlight (⌘+Space → Junction) or Finder → ~/Applications")
...
def _setup_impl(
    ...
    from junction.agent import install_agent  # circular import: agent imports cli
    from junction.cli import _project_dir_file  # circular import: cli -> cli_setup -> cli
    ...
    # 0. Save project dir so junction works from anywhere
    proj = os.environ.get("JUNCTION_PROJECT_DIR")
    if proj:
    ...
    # 2a. Ensure `junction` is reachable on PATH (the packaged Electron app
    ...
    from junction.agent import ensure_junction_on_path
    from junction.mcp_cleanup import clean_stale_managed_mcp

    # `claim_existing`: this is the explicit setup path, so the user has named
    # THIS install as the one they want `junction` to mean. Gateway startup
    # deliberately does not, so a background start never takes the command away
    # from a working install (see ensure_junction_on_path).
    shim = ensure_junction_on_path(claim_existing=True)
    if shim:
    ...
    # 2b. Ensure config.json has default Junction agent for fresh installs
...
def _maybe_setup_cloud() -> None:
    """Offer to launch Junction on the user's own AWS EC2.

    A thin delegating step — all AWS/CloudFormation/SSM logic lives in the
    testable ``junction.cloud`` module. This just asks and hands off to the
    ...
    try:
        ...
        from junction.cli_cloud import handle_cloud
...
def _setup_slash_command() -> None:
    ...
    print("── Slash Command ──\n")
    current = cfg.get("slack", {}).get("command", "junction")
