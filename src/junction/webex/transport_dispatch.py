...
module never imports ``junction.slack``.
...
from junction.messaging.approval import PendingApprovals, SessionApprovalDecider
...
from junction.messaging.dispatch import (
    ChannelTurn,
    build_directive_consumer,
    drive_turn,
    inbound_permitted,
)
from junction.messaging.driver import APPROVAL_INTERACTIVE
...
from junction.messaging.queue_receipt import ReceiptQueue, ReceiptSurface
...
from junction.webex.attachments import process_webex_attachments
from junction.webex.cards import LiveChoices, read_press
...
from junction.webex.renderer import WebexRenderer, webex_display_safe
from junction.webex.transport import ROOM_DIRECT, WEBEX_CAPABILITIES
...
# Canonical kiro-cli agent fallback so Webex sessions load junction-core
...
# Telegram / WeCom paths' _DEFAULT_JUNCTION_AGENT.
...
class WebexDispatcher:
    ...
    async def _handle_dashboard(self, inbound: "WebexInbound") -> None:
        ...
        from the ``dashboard.url`` config (``JUNCTION_PORT`` overrides the port,
        ...
        if not arg.startswith("dashboard"):
            await self._reply(
                inbound,
                "Usage: `/junction dashboard [<N>h|<N>m]`",
            )
            return
        ...
        try:
            ...
            logger.warning("Webex /junction dashboard: token generation failed", exc_info=True)
            try:
                sel().log_api_access(
                    caller=caller,
                    operation="webex.dashboard_token",
