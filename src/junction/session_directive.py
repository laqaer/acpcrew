...
Some Junction MCP tools act on *the session that called them* — arm a monitor
...
(one ``junction-core`` serves the whole runtime, and the ``/proc`` walk is
...
Junction observing the tool CALL — as an MCP-served call whose CANONICAL name
...
# The MCP server name Junction registers its own tools under (kiro-cli reports
...
_SENTINEL = "[[JUNCTION_SESSION_DIRECTIVE]]"
...
_REFUSAL_SENTINEL = "[[JUNCTION_SESSION_DIRECTIVE_REFUSED]]"
...
def decode(text: str, expected_tool: str) -> dict[str, Any] | None:
    ...
    *expected_tool* (the name Junction recorded for this tool call) matches the
