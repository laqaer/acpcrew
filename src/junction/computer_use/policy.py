...
to every other security plane in Junction. ``is_sensitive_path`` cannot see a
...
CATEGORY_JUNCTION_SELF = "junction_self"
...
def denied_rule_for(app: AppRef) -> DeniedApp | None:
    ...
    * ``title_substrings`` — the resolved WINDOW TITLE. Load-bearing for the
      self-target rule: Junction's dashboard served into a browser tab presents
