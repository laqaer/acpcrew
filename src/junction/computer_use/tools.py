...
sidecar. :mod:`junction.mcp_computer` is a thin shim that resolves the caller's
...
* The refusals that DO hold for computer use are enforced HERE, in band, on the
  ...
  Junction's-own-window denylist, the secure-field and sensitive-text checks. They
...
from junction.computer_use import enable_state, gate, keymap, overlay, policy, render, service
...
# has no single target) and ``computer_end_turn`` touches Junction's own cache, so
...
def dispatch_tool(
    ...
    *session_key* is the identity the shim resolved with
    ``mcp_core._resolve_session_key_strict`` (env var, or ``JUNCTION_HOST_PID``
