...
1. deciding whether an overlay should exist at all (macOS + the ``cursor_motion``
   opt-in, default OFF);
2. the lifecycle of the ``junction.computer_use.overlay_proc`` child — lazy
   spawn, bounded retry, reap;
3. turning a "the agent is about to click here" event into a
   :class:`~junction.computer_use.cursor_motion.MotionPlan` and shipping it down
   the child's stdin.
...
from junction.computer_use.cursor_motion import MotionPlan, plan_motion
...
def cursor_motion_enabled() -> bool:
    ...
    * ``config.json``'s ``computer_use.cursor_motion`` is true — the typed
    ...
    try:
        ...
        section = getattr(JunctionConfig.load(), "computer_use", None)
    except Exception:
...
class CursorOverlay:
    ...
    the cross-loop hazard ``junction.__init__`` documents at length.
    """
    ...
    async def _spawn(self) -> "asyncio.subprocess.Process | None":
        ...
        junction.computer_use.overlay_proc``. Nothing agent-supplied enters it —
    ...
    async def _await_ready(self, proc: "asyncio.subprocess.Process") -> bool:
        """Wait (bounded) for the child's ``JUNCTION_OVERLAY_READY`` line.
