...
from junction.computer_use.policy import title_is_denied as _denied_title
    ...
    **The window we keep is the frontmost one that has a TITLE**, not simply the
    ...
    for info in macos_ffi.window_list():
        ...
        if existing is not None and _denied_title(existing.window_title):
            ...
            # window hosting Junction slip past the title rule entirely.
            continue
...
def _read_info_plist(path: str) -> dict:
    ...
    """
    try:
        from junction.hooks import safe_read_prefix
...
def pid_owns_point(pid: int, x: float, y: float) -> bool:
    ...
    denied one (Junction's own window) would sail through: the policy check passes
