...
whatever launched the tree — ``Junction.app`` when packaged, ``Terminal.app`` for a
dev ``junction gateway``, an IDE if launched from one. A probe inspects only its
...
should actually grant, because "grant Accessibility to Junction" is unactionable
