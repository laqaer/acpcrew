...
:class:`~junction.computer_use.backend.ComputerUseBackend`, which on macOS means
...
event loop. :mod:`junction.computer_use.tools` is the async half — it offloads
...
* walk a window into a :class:`Snapshot`, persist any captured pixels, and cache
  the result in the shared :class:`~junction.computer_use.index.SnapshotIndex`;
...
from junction.computer_use import index as snapshot_index
from junction.computer_use import render
from junction.computer_use.backend import ComputerUseBackend, get_shared_backend
...
def _config_section() -> object:
    ...
    try:
        ...
        return getattr(JunctionConfig.load(), "computer_use", object())
    except Exception:
