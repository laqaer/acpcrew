...
``junction`` and never touches ctypes, so it loads identically on macOS,
...
# ── MCP tool names (server ``junction-computer``) ──
...
# Keystone opt-in for the ``junction computer apps`` / ``call`` diagnostics. Those
...
SCREENSHOT_DIR_NAME = "junction-computer-shots"
...
OVERLAY_MODULE = "junction.computer_use.overlay_proc"
...
OVERLAY_READY_LINE = "JUNCTION_OVERLAY_READY"
...
class DeniedApp:
    ...
    # because a host application can display someone else's UI: Junction's own
