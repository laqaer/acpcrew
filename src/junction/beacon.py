...
DELIBERATELY SEPARATE FROM ``junction.metrics`` (the OTEL trunk). Four reasons,
each independently disqualifying:
  ...
  1. ``MetricsRecorder._guard()`` runs EVERY attribute through
     ...
     metric.
  2. OTLP egress lives in the ``junction[otlp]`` package extra, NOT the default
...
``JUNCTION_TELEMETRY_DISABLED`` is truthy, ``telemetry.beacon_enabled`` is
false, the process looks like CI, or ``JUNCTION_HOME`` is non-default (dev
...
# Opt-out env var. Truthy disables; mirrors the JUNCTION_TELEMETRY convention
# in metrics/provider.py so operators only learn one spelling.
DISABLE_ENV = "JUNCTION_TELEMETRY_DISABLED"
...
# install cannot change. ``JUNCTION_DISTRIBUTION`` is inherited by every child
...
def is_env_opted_out() -> bool:
    """Return whether ``JUNCTION_TELEMETRY_DISABLED`` pins the beacon off.
...
def is_default_home() -> bool:
    ...
    """
    raw = os.environ.get("JUNCTION_HOME", "").strip()
    if not raw:
        return True
    try:
...
def install_id(*, create: bool = True) -> str:
    ...
    ``junction telemetry status`` so merely inspecting status cannot
    ...
    try:
        ...
        # a caller inspecting state (``junction telemetry status``) needs "the
...
def telemetry_permitted(*, enabled: bool, acked: bool, audit_tool: str = "") -> Verdict:
    ...
    chapter, or an explicit ``junction telemetry enable|disable``). It gates the
    ...
    if not is_default_home():
        return Verdict(
            False,
            "non-default JUNCTION_HOME (dev home / pod / preview)",
            "non_default_home",
        )
...
def status(endpoint: str, *, enabled: bool, app_version: str, acked: bool) -> dict[str, object]:
    ...
    (which the Privacy panel refetches) and from ``junction telemetry status``.
