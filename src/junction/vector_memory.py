...
Storage: ~/.junction/memory.db (SQLite, WAL mode)
FAISS index: ~/.junction/memory.faiss (optional, for vector search)
...
from junction.embeddings import PRIORITY_BULK, PRIORITY_INTERACTIVE, PRIORITY_NORMAL
...
from junction.metrics.db_metrics import timed
from junction.project_scope import (
    canonical_scope,
...
# existing `from junction.vector_memory import _MAX_*` paths keep working.
from junction.vector_memory_constants import (  # noqa: F401
    _INJECTION_PATTERNS,
...
class LessonWriteOutcome(str, Enum):
    ...
    The vocabulary matches :meth:`junction.learn.LessonStore.save_or_enrich`, which
...
class VectorMemoryStore:
                ...
                if self.embed_fn is None:
                    ...
                    if (
                        ...
                        if candidate is not None:
                            ...
                            try:
                                probe = candidate("_junction_embed_probe")
                            except Exception:
                                probe = None
    ...
    def migrate_from_markdown(self) -> dict[str, int]:
        ...
        # Honor JUNCTION_HOME via config_dir() so the source directory matches
