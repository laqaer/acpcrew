...
The kiro agents directory (:func:`junction.config.paths.kiro_agents_dir`) is
...
surface so it can be wired into ``junction doctor`` and gateway boot in one or
...
#: ``junction doctor``, the command you run *when something is broken*, is one of
...
def sweep_agents_dir(
    ...
    *,
    ...
    :param dry_run: when true, identify eligible files and populate
        ...
        ``junction doctor``) REPORT what a sweep would reclaim without mutating
