...
from junction.feishu.client import CHAT_GROUP
...
if TYPE_CHECKING:
    ...
    from junction.feishu.client import LarkClient, LarkInbound
    from junction.history import ConversationLog
    from junction.session import SessionManager

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so Feishu sessions load junction-core
# (spawn_run etc.) instead of kiro-cli's bare built-in default.  Mirrors the
# Slack / Telegram / WeCom paths' _DEFAULT_JUNCTION_AGENT.
...
class FeishuDispatcher:
    ...
    async def handle_message(self, inbound: "LarkInbound") -> None:
        ...
        async def _surface_new_session() -> None:
            from junction.dashboard.channel_slots import (  # noqa: PLC0415
        ...
        await drive_turn(
            ChannelTurn(
                ...
                # dashboard toggle and Slack's `/junction yolo` drive, so it needs no
