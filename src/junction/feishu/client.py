...
``lark-oapi`` is an OPTIONAL dependency (``pip install "junction[feishu]"``).
It is imported lazily inside this module's methods so that every other module
in the package -- including :mod:`junction.feishu.transport` and the channel
roster in :mod:`junction.channels` -- imports cleanly on a build that does not
