...
# nothing from ``junction`` and never touches ctypes), so there is no cycle and
...
from junction.computer_use import types as _cu_types
from junction.constants import AWS_PROFILE_NAME_RE, WINDOWS_DEVICE_STEMS
...
# Artifact slug grammar — mirrors junction.artifacts._SLUG_RE (kept here so
...
def infer_use_case(session_key: str) -> str:
    """Map a Junction session_key to a categorical useCase label.
...
# stdio loop, killing the whole junction-core MCP server for the session.
...
# Artifact tools — slug pattern matches junction.artifacts._SLUG_RE.
...
from junction.artifacts import MAX_CONTENT_BYTES as ARTIFACT_CONTENT_MAX  # noqa: E402
...
MCP_CORE_SCHEMAS: dict[str, ToolSchema] = {
    ...
    # the whole junction-core server for the session.
...
# ── Tool Schemas (MCP Computer Use — server ``junction-computer``) ──
#
...
# ── Tool Schemas (MCP Dashboard — server ``junction-dashboard``) ──
#
