...
# tree Junction spawns (the ACP provider, MCP probes, gateway pool backends).
...
# identify escaped MCP launcher processes whose *cmdline* carries no Junction
...
JUNCTION_SPAWNED_ENV = "JUNCTION_SPAWNED"
JUNCTION_SPAWNED_VALUE = "1"

# Canonical truthy set for boolean environment variables (JUNCTION_NO_JAIL,
# JUNCTION_DEV_MODE, …).  Use ``env_flag_enabled`` rather than ``bool(os.environ
# .get(...))`` — a bare bool() treats ``"0"``/``"false"`` as truthy, which for a
# security toggle (e.g. JUNCTION_NO_JAIL) is a silent-bypass footgun.
ENV_TRUTHY = frozenset({"1", "true", "yes", "on"})


# Minimum supported Node.js MAJOR version for every Python-side check
# (``junction doctor``, the frontend-build probe in ``cli.py``, the TUI
