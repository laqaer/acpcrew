...
Jobs are created via MCP tools (``cron_add``) or the CLI (``junction cron add``).
...
from junction import cron_script, platform_compat, sel, shutdown_event
from junction.config.loader import JunctionConfig, config_dir, data_home
from junction.constants import env_flag_enabled
from junction.cron_history import CronHistoryStore, CronRunRecord
from junction.executors import _CRON_QUEUE_WAIT_SECS, cron_gate_budget, subprocess_executor
from junction.resource_status import admission_check
from junction.validation import CHANNEL_MAX_LEN, MAX_CRON_MESSAGE, MAX_SHORT_STRING
...
#
# Gating mirrors the repo's other strict rails (e.g. JUNCTION_STRICT_ON_LOOP_
...
# JUNCTION_STRICT_LOOP_SAFETY=1 to escalate the warning to a hard failure fleet-
# wide.
_STRICT_LOOP_SAFETY_ENV = "JUNCTION_STRICT_LOOP_SAFETY"
...
class CronLoopSafetyError(RuntimeError):
    ...
    mode (``JUNCTION_STRICT_LOOP_SAFETY``); otherwise the guard warns.
    """
...
def format_schedule(schedule: CronSchedule, tz_name: str = "") -> str:
    ...
    if not tz_name:
        try:
            tz_name = JunctionConfig.load().timezone
        except Exception:
            pass
...
def get_local_tz() -> tuple[str, ZoneInfo]:
    ...
    try:
        tz_name = JunctionConfig.load().timezone or "UTC"
...
def _job_tz(job: CronJob) -> ZoneInfo:
    ...
    try:
        tz_name = job.timezone or JunctionConfig.load().timezone or "UTC"
...
class CronService:
    ...
    def __init__(
        ...
        *,
        ...
        _cfg = JunctionConfig.load().cron_history
    ...
    def _guard_off_event_loop() -> None:
        ...
        loop-park hazard. Under strict mode (``JUNCTION_STRICT_LOOP_SAFETY``)
        ...
        :class:`~junction.apps.cron_sdk.CronSDK` facade offloads to a worker
