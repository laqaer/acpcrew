...
Junction has aggregate duration histograms (``junction.metrics``) and a stall
...
def profiling_enabled(env: dict[str, str] | None = None) -> bool:
    ...
    stray ``JUNCTION_DEBUG=0`` reads as disabled rather than as "the name is
...
class StackSampler:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="junction-perf-sampler", daemon=True
        )
        self._thread.start()
...
def pyspy_unavailable_message() -> str:
    ...
    return (
        ...
        "Install it with:  pip install 'junction[perf]'   (or: pip install py-spy)\n"
