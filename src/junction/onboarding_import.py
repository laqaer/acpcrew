...
from junction.embeddings import make_sync_embed_fn
from junction.frontmatter import BLOCK_SCALAR_INDICATORS, ONBOARDING_IMPORT, split_frontmatter
...
def _add_instruction_files(
    ...
    """Project user-authored instruction documents onto Junction's memory tiers.
    ...
    deliberately NOT imported: Junction's persona surface is theme-pack persona,
...
def _sqlite_snapshot(
    ...
    snapshot_dir = Path(tempfile.mkdtemp(prefix="junction-import-sqlite-"))
    try:
...
def _row_is_workspace_scoped(value: Any) -> bool:
    ...
    Junction's own memory tables have no workspace column, so a genuinely
...
def _mcp_lock(_path: Path) -> Iterator[None]:
    """Coordinate with dashboard and app writers of the Junction MCP file."""
    ...
    from junction.dashboard.handlers.mcp import _get_mcp_lock_sync
...
def _write_mcp(
    ...
    *,
    ...
    with _mcp_lock(path):
        ...
        from junction.mcp_discovery import configured_mcp_aliases
