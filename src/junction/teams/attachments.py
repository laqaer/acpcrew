...
* inbound classification, size limits, image-signature validation, text/document
  ...
  ownership -> :mod:`junction.messaging.attachments`
* outbound reference scanning, the security floor, the raster sniff and the
  per-message budgets -> :mod:`junction.messaging.outbound_files`
...
from junction.messaging.renderer import _default_redactor
...
#: Budgets for :func:`junction.messaging.outbound_files.extract_local_refs`.
