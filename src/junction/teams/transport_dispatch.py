...
from junction.teams.attachments import append_attachment_context, process_teams_attachments
...
from junction.teams.commands import (
    DIRECTIVE_USAGE,
    HELP_TEXT,
    command_argument,
    parse_command,
...
from junction.teams.session_resume import (
    ResumeReleaseError,
    RoutingDecision,
    TeamsSessionResume,
)
...
if TYPE_CHECKING:
    ...
    from junction.teams.client import TeamsClient, TeamsInbound

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so Teams sessions load junction-core
...
# Telegram / WeCom / Webex paths' _DEFAULT_JUNCTION_AGENT.
...
class TeamsDispatcher:
    ...
    async def _handle_dashboard(self, inbound: "TeamsInbound", arg: str) -> None:
        ...
        ``/junction dashboard 2h`` cannot disagree about durations.
        """
        ...
        try:
            ...
            if not origin:
                ...
                # parse_dashboard_url resolves with the JUNCTION_PORT override.
    ...
    def _bind_origin_mirror(self, session_key: str, inbound: "TeamsInbound") -> None:
        ...
        :func:`~junction.messaging.link.bind_origin_mirror`; this only supplies
