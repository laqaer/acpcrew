...
from junction.config.loader import JunctionConfig, config_dir, read_local_secret
...
# server (junction-core, junction-dashboard, junction-cron, junction-computer)
...
def _resolve_excluded_tools(caller_session: str = "") -> set[str]:
    ...
    if (
        ...
        sel().log_api_access(
            caller=caller_session or os.environ.get("JUNCTION_SESSION_KEY", "mcp"),
    ...
    try:
        ...
        session_key = caller_session or os.environ.get("JUNCTION_SESSION_KEY", "")
        if not session_key:
        ...
        try:
            ...
            if http_exc.code == 404:
                _last_startup_race_time = now
                sel().log_api_access(
                    caller=os.environ.get("JUNCTION_SESSION_KEY", "mcp"),
    ...
    *,
    ...
    def _sel_audit(
        ...
        try:
            sel().log_tool_invocation(
                session_key=session_key
                or os.environ.get("JUNCTION_SESSION_KEY", "mcp"),
                source="mcp",
                tool_name=tool_name,
    ...
    while True:
        ...
        if method == "initialize":
            ...
            if advertise_caller_identity:
                ...
                # gatewayd injects the per-call ``_meta.junction.caller`` block
            ...
            # any client-forged ``junction.caller`` block and injects its own
            ...
            if tool_name in excluded:
                sel().log_tool_invocation(
                    session_key=(
                        ...
                        if _caller_ctx
                        else os.environ.get("JUNCTION_SESSION_KEY", "mcp")
                    ),
                    source="mcp",
                    tool_name=tool_name,
                    tool_kind=server_name,
