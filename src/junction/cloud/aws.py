...
Every AWS interaction in :mod:`junction.cloud` goes through :func:`run_aws`.
...
Credentials are resolved by the ``aws`` CLI's own provider chain — Junction
...
(:func:`junction.sandbox.wrap_argv`, ``mode="standard"``) exactly like the
...
from junction.sandbox import cgroup_scope_argv, popen_limited, wrap_argv
...
class CloudActionDenied(Exception):
    ...
    ``deniedCommands`` block the ``aws`` CLI verbs and the ``junction cloud``
    wrappers, but an agent could still import ``junction.cloud`` and call a
...
def _in_agent_session() -> bool:
    ...
    return bool(os.environ.get("JUNCTION_SESSION_KEY"))


def assert_human_action(action: str) -> None:
    """Refuse a destructive/sensitive cloud mutation from an agent session.

    The gateway injects ``JUNCTION_SESSION_KEY`` into every agent/ACP subprocess
    (and MCP tools key off it). A human running ``junction cloud …`` in their own
    ...
    ``JUNCTION_SESSION_KEY`` is an ordinary env var the agent's own code can
    ...
    credentials (see :mod:`junction.cloud.iam`); to truly wall the agent off
...
def assert_chokepoint_allowed(args: list[str]) -> None:
    ...
    in-process ``import junction.cloud`` path. All three are bypassable by a
    determined, code-executing agent (obfuscated shell string; ``del
    os.environ['JUNCTION_SESSION_KEY']``; raw ``aws``/boto3 when ``~/.aws`` is
    ...
    credentials (:mod:`junction.cloud.iam` — tag/ARN-scoped, no IAM writes), plus
