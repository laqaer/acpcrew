...
Same philosophy as ``deploy_web/iam.py``: **Junction never performs an IAM
...
from junction.cloud import aws

# Resource tag every launcher-created resource carries (mirrors deploy-web's
# junction:managed). Scopes the tag-conditioned statements below.
MANAGED_TAG_KEY = "junction:managed"

# The IAM role name pattern the template uses; PassRole is scoped to it.
ROLE_NAME_PREFIX = "junction-ec2-"
...
# permissive. iam:CreateRole is gated on this name so a junction-ec2-* role MUST
...
BOUNDARY_NAME = "junction-ec2-boundary"
...
def boundary_policy_document(account: str = "*") -> dict[str, Any]:
    ...
    launcher bucket prefix (``junction-src-<account>-*/*``) instead of the
    ...
    resource is region-agnostic (``junction-src-<account>-*``) — a byte-stable
    ...
    return {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Effect": "Allow",
                "Action": ["s3:GetObject"],
                "Resource": f"arn:aws:s3:::junction-src-{account}-*/*",
            },
        ],
    }
...
def policy_document() -> dict[str, Any]:
    ...
    statements: list[dict[str, Any]] = [
        {
            # Stack-mutating verbs scoped to junction-* stacks so the applied
        ...
        },
        {
            ...
            # BOTH the changeSet and stack ARN forms, still junction-*-scoped.
        ...
        },
        {
            ...
            # is evaluated per-resource, so requiring junction:managed=true on the
        ...
        },
        {
            ...
            # CreateSecurityGroup creates the SG (tagged junction:managed=true at
        ...
        {
            # SG rule mutation is gated to Junction-tagged security groups: the
            # stack's SG carries junction:managed=true from creation
        ...
        },
        {
            ...
            # junction:managed=true and thereby bring it under the tag-gated
        ...
        {
            # Destructive verbs, gated to Junction-tagged resources so a leaked
            # launcher credential can't delete/retag security groups it never
            # created. The stack's SG carries junction:managed=true from creation.
        ...
        },
        {
            ...
            # iam:CreateRole is the enforcement point: a junction-ec2-* role can
            # ONLY be created WITH our permissions boundary (iam:PermissionsBoundary
            # must ArnLike-match arn:...:policy/junction-ec2-boundary). Because the
        ...
        {
            # PutRolePolicy is scoped to the junction-ec2-* role ARN AND gated on
            # the role carrying our creation tag (aws:ResourceTag/
            # junction:managed=true). The name-prefix scope alone let a leaked
            # launcher credential target a PRE-EXISTING junction-ec2-* role that a
            ...
            # tagged junction:managed=true, and CreateRole is boundary-gated
        ...
        },
        {
            ...
            # unbounded junction-ec2-* role as junction:managed=true, then inline
        ...
        },
        {
            ...
            # 403 and the launch can't tag the role junction:managed=true (which the
            ...
            # junction-ec2-* role junction:managed=true, then inline admin + pass it
            # to EC2 (the tag is only non-spoofable if the same policy can't apply
            # it to an arbitrary role).
            #
            # Gate: aws:ResourceTag/junction:managed=true — the role must ALREADY be
            ...
            #     junction:managed=true tag is already "present" in context → match.
            #   * (b) A STANDALONE tag-role on a pre-existing unbounded victim
            #     (which does NOT yet carry junction:managed) is DENIED — the key is
        ...
        },
        {
            ...
            # admin (junction cloud iam-boundary) and drop this statement — the
        ...
        },
        {
            ...
            # a junction-ec2-* role and pass it to EC2 — a full escalation. The
        ...
        {
            # PassRole is scoped to the junction-ec2-* role ARN, gated on the EC2
            # service (iam:PassedToService) AND on our creation tag
            # (aws:ResourceTag/junction:managed=true) — the same non-spoofable
            # constraint as PutRolePolicy. Without the tag, a leaked launcher
            # credential could pass a PRE-EXISTING, unbounded junction-ec2-* role
        ...
        },
        {
            ...
            # and SendCommand (effectively RCE) — are gated to Junction-tagged
