"""Ship the local Junction source to the instance via S3 (private-repo safe).

The public Junction repo may be private, so the EC2 box can't ``git clone`` it
...
is created once per account/region (``junction-src-<account12>-<region>``) and
reused; each launch uploads to ``<tag>/junction-src.tar.gz``.
"""
...
SOURCE_KEY_NAME = "junction-src.tar.gz"
_BUCKET_PREFIX = "junction-src-"
...
# keep out of the git-archive path — notably the Junction dev-mode data dir
...
def repo_root() -> Path:
    """The Junction source root to package (the installed package's repo).
...
def _use_git_archive(root: Path) -> Optional[Path]:
    ...
    try:
        out = tempfile.NamedTemporaryFile(  # noqa: SIM115 - handed to caller
            prefix="junction-src-", suffix=".tar.gz", delete=False
        )
        out.close()
...
def _tracked_tree_is_dirty(root: Path) -> bool:
    ...
    tracked file without committing, ``junction cloud launch`` would silently
...
def _tar_fallback(root: Path) -> Path:
    ...
    a custom ``JUNCTION_HOME`` under the repo is dropped. If the tracked-file
...
def _account_from_bucket(bucket: str) -> str:
    """Extract the 12-digit account id embedded in a ``junction-src-<account>-<region>`` name.
    ...
    shape (e.g. the ``junction-src-unknown-*`` fallback), so the caller fails
...
def ensure_bucket(profile: str, region: str) -> str:
    ...
    if not account:
        ...
        # closed rather than mint an unprotected `junction-src-unknown-*` bucket.
        raise aws.AWSError(
    ...
    # A pre-existing junction-src-* bucket (older Junction version, or one whose
...
def ensure_instance_boundary(profile: str = "", region: str = "") -> str:
    ...
    ``junction-ec2-boundary`` (see :mod:`cloud.iam`) — the launcher CODE creates
    ...
    **fetch its default version and compare it to the expected content-fixed
    ...
    (``junction cloud iam-boundary``) and drop the ``iam:CreatePolicy`` grant.
    """
    ...
    create = [
        ...
        iam.BOUNDARY_NAME,
        "--description",
        "Junction EC2 instance permissions ceiling (SSM core + launcher source read).",
        "--policy-document",
...
def _verify_instance_boundary_content(arn: str, account: str, profile: str, region: str) -> None:
    ...
    An existing ``junction-ec2-boundary`` must ONLY be reused if its content is
...
def delete_instance_boundary(profile: str = "", region: str = "") -> dict:
    ...
    call this only after every junction-ec2-* role is gone.
    """
    from junction.cloud import iam
...
def delete_source(tag: str, profile: str = "", region: str = "") -> dict:
    ...
    # back to `junction-src-unknown-*` because the account didn't resolve), fail
