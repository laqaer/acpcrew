"""Junction cloud launcher — provision + run Junction on the user's own EC2.
...
- **Bring-your-own-AWS, store nothing.** Every AWS call shells to the ``aws``
  ...
  Junction persists only a profile name, region, and stack name.
...
- Provisioning is a **human/installer action, never an LLM tool** — the cloud
  ...
  ``aws cloudformation delete-stack``) **and** the ``junction cloud
...
- ``templates/junction-ec2.yaml`` — the CloudFormation template.
"""

from __future__ import annotations

from junction.cloud.aws import AWSError, run_aws
