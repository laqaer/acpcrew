"""The interactive ``junction cloud launch`` wizard.

One command → answer at most a question or two → Junction running on a
...
from junction.cloud import ec2, iam, login, sizes, ssm, ui
...
def _deploy_with_progress(
    *,
    ...
    exists, the tail of ``/var/log/junction-setup.log`` over SSM. This replaces
...
def aws_start_instance(instance_id: str, profile: str, region: str) -> None:
    ...
    from junction.cloud import aws as _aws
...
def _fetch_bootstrap_log(
    ...
    try:
        res = ssm.run_command(
            instance_id,
            "tail -n %d /var/log/junction-setup.log 2>/dev/null "
            "| tr -d '\\r' "
            "| grep -avE 'Installing (npm|junction and) depend|building React app' || true" % lines,
            profile,
            region,
            run_as="root",
            total_wait=30,
...
def launch(
    ...
    *,
    ...
    wizard is embedded in a larger flow (``junction setup``) that still has
    ...
    if result is None:
        ...
        # A resumed stack may be STOPPED (after `junction cloud stop`). All the
    ...
    # When embedded in `junction setup` (hold_tunnel=False) we must return so
