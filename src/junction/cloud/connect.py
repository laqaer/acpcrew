"""Connect to a launched Junction instance — SSM tunnel + token + open browser.
...
2. Mint a short-lived dashboard token on the instance (``junction token``) over
...
# `junction token` prints a URL like http://localhost:5476/?token=<jwt>;
...
def mint_token(instance_id: str, profile: str = "", region: str = "", *, ttl: str = "6h") -> str:
    ...
    Runs ``junction token --ttl <ttl>`` on the box and parses the token out of
    ...
    res = ssm.run_command(
        ...
        f"junction token --ttl {_safe_ttl(ttl)} 2>/dev/null | grep -m1 'token=' || true",
        profile,
        region,
        total_wait=90,
    )
...
def connect(
    ...
    *,
    ...
    if not ready:
        ...
        if not token:
            # The tunnel is up but we couldn't mint a token (e.g. `junction token`
...
def _safe_ttl(ttl: str) -> str:
    """Constrain a TTL string to the ``<n>[hm]`` shape junction token accepts."""
...
def _browser_open_supported() -> bool:
    ...
    if os.environ.get("JUNCTION_NO_BROWSER"):
        return False
