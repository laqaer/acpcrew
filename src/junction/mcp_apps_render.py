...
The gateway daemon (``src/junction/mcp_gateway/``) writes a UI payload to a
spool file at ``$JUNCTION_HOME/mcp-apps/<uuid4hex>.json`` and injects an opaque
marker ``[junction-mcp-app:<uuid4hex>]`` into the tool result *text* that flows
...
Security posture:
  ...
  * This side is flag-independent: if a marker appears we handle it. The
    ``JUNCTION_MCP_APPS`` opt-in gate lives on the gateway (producer) side.
"""
...
# Anchored to exactly 32 hex chars so a stray "[junction-mcp-app:...]" with the
# wrong shape is simply ignored rather than treated as a spool id.
MARKER_RE = re.compile(r"\[junction-mcp-app:([0-9a-f]{32})\]")
...
def _spool_dir() -> Path:
    """Directory holding spool files. ``JUNCTION_MCP_APPS_SPOOL`` overrides the
    default ``$JUNCTION_HOME/mcp-apps``."""
    override = os.environ.get("JUNCTION_MCP_APPS_SPOOL")
    if override:
...
def _claim_render(spool_id: str, producing_session_key: str) -> dict[str, Any] | None:
    ...
    sets ``JUNCTION_SESSION_KEY`` to the prefixed form while ``slot.key`` is
