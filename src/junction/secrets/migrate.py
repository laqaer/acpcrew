...
from junction.config.loader import _JIRA_TOKEN_RE, CRED_JIRA_API_TOKEN, config_dir, env_path
...
def _env_lock_path(ep: Path) -> Path:
    ...
    Used by:
      ...
      * :func:`junction.dashboard.handlers.weixin_qr._commit_credential_and_config`
        — the QR sign-in handler (the only other in-tree ``.env`` writer)
      * :func:`junction.dashboard.handlers.messaging._write_env_updates`
...
class MigrationReport:
    ...
    *would* migrate). ``already_referenced`` names keys whose ``.env`` value was
    ...
    via ``junction secrets rm <key>`` is recommended.
    """
...
def _parse_env_lines(text: str) -> list[tuple[str, str]]:
    ...
    Matches :meth:`JunctionConfig.load_credentials` parsing: strips whitespace,
...
def migrate_env_secrets(
    *,
    ...
    monkeypatching :func:`junction.secrets.migrate.env_path` and
    :func:`junction.secrets.migrate.config_dir`.
    """
    ep = env_path()
