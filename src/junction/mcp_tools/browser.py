...
``junction-core`` is a separate stdio shim, so the handler cannot reach the bus
...
from junction.security import canonicalize_ip, redact_credentials, redact_exfiltration_urls
...
def _navigate_target_is_public(url: str) -> bool:
    ...
    dashboard) so the ``junction-core`` shim stays free of dashboard deps.
...
def _use_builtin_browser() -> bool:
    ...
    """
    try:
        return bool(getattr(JunctionConfig.load().dashboard, "use_builtin_browser", True))
    except Exception:
        return True
...
def browser(name: str, args: dict[str, Any]) -> str:
    ...
    # off, so there is no per-call caller context and no JUNCTION_SESSION_KEY,
