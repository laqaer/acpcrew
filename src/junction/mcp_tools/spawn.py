...
from junction.subagent import UNADVERTISED_AGENTS, resolve_max_subagents
from junction.subagent_persistence import agent_dir_for_display
...
def schemas() -> list[dict[str, Any]]:
    ...
    try:
        _max_sub = resolve_max_subagents(JunctionConfig.load())
    except Exception:
        _max_sub = 0
    _cap_hint = (
...
def spawn_run(name: str, args: dict[str, Any]) -> str:
    ...
    # JUNCTION_SESSION_KEY/JUNCTION_CHANNEL_ID) so a cron running with
    ...
    # responder for.
    approval_mode = os.environ.get("JUNCTION_APPROVAL_MODE", "")
...
def spawn_sub_agents(name: str, args: dict[str, Any]) -> str:
    ...
    poll_interval = 2.0
    try:
        max_wait = float(os.environ.get("JUNCTION_SPAWN_SUB_AGENTS_MAX_WAIT", "7200"))
