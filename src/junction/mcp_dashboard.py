...
Deliberately NOT part of ``junction-core``. Core is the tool surface EVERY
...
matching ``@junction-dashboard`` reference in ``tools`` — kiro-cli loads a server
...
server's requests carry ``X-Internal-Caller: junction-dashboard`` (attached
...
read the agent does not already have (``list_sessions`` in ``junction-core`` is
...
``junction.caller-identity`` so gatewayd injects one; see
...
SERVER_NAME = "junction-dashboard"
