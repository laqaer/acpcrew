...
Both modes live in :mod:`junction.messaging.privacy_mode`, keyed by session key,
...
from junction.acp.client import AcpError, AcpProcessDied, AcpPromptBusy, AcpTimeoutError
from junction.acp.types import STOP_REASON_CANCELLED, STOP_REASON_END_TURN
from junction.agent_discovery import project_agent_files, project_agent_name
...
from junction.messaging.identity import channel_inbound_permitted, publish_turn_identity
from junction.messaging.link import canonical_key
from junction.messaging.session_trust import _trusted_sessions as _shared_trusted_sessions
from junction.messaging.session_trust import add_trusted_session as _add_trusted_session
from junction.messaging.session_trust import clear_trusted_sessions, is_session_trusted
...
from junction.session import SessionClosingError, SessionManager
from junction.session_map import SessionMap
from junction.slack.blocks import build_working_blocks, deprecation_warning_block
...
from junction.task import Task
from junction.taskrunner import TaskRunner
from junction.voice_reply import DEFAULT_PROVIDER, VALID_PROVIDERS
from junction.voice_reply import is_available as _tts_available
from junction.voice_reply import validate_length_scale as _validate_length_scale
from junction.voice_reply import voice_reply as _voice_reply_fn

logger = logging.getLogger(__name__)

# Mapping of bang commands to their /junction slash equivalents.
_BANG_TO_SLASH: dict[str, str] = {
    "!yolo": "/junction yolo",
    "!stop": "/junction stop",
    "!voice": "/junction voice",
    "!agent": "/junction agent",
    "!dashboard": "/junction dashboard",
    "!ta": "/junction agent",
    # "!allowlist" removed — multi-user access disabled for security
    "!channel": "/junction channel",
    "!link-to-dashboard": "/junction link-to-dashboard",
    "!restart": "/junction restart",
}
...
try:
    _overrides = JunctionConfig.load().slack.reactions
except Exception:
...
# Falls back to single JUNCTION_OWNER_ID for backward compatibility.
...
_orch_cfg: JunctionConfig | None = None
...
def _is_slack_restricted(session_key: str) -> bool:
    ...
    :func:`junction.messaging.privacy_mode.is_restricted`); the Slack spelling
...
def _conv_state_map(sessions: object) -> "SessionMap | None":
    ...
    Thin wrapper over :func:`junction.messaging.privacy_mode.conv_state_map`,
...
def _get_default_agent() -> str:
    ...
    if _cached_default_agent is None:
        _cached_default_agent = JunctionConfig.load().agent.default_agent
...
def _list_all_agent_names(cc_plugins_dir: Path | None = None) -> str:
    ...
    :func:`_iter_cc_agent_names`. The internal ``junction-lite`` variant is
    ...
    agents_dir = kiro_agents_dir()
    if agents_dir.is_dir():
        # Hide the internal junction-lite variant from BOTH sources — a
        # ~/.kiro/agents/junction-lite.json would otherwise leak into the list.
        names.extend(f.stem for f in sorted(agents_dir.glob("*.json")) if f.stem != "junction-lite")
    seen = set(names)
    for agent_name in _iter_cc_agent_names(cc_plugins_dir):
        if agent_name not in seen and agent_name != "junction-lite":
...
def set_orch_cfg(cfg: JunctionConfig) -> None:
...
def load_voice_reply_config(cfg: "JunctionConfig | None" = None) -> None:
...
def get_orch_cfg() -> "JunctionConfig | None":
...
def is_allowed_user(user_id: str) -> bool:
    ...
    (JUNCTION_OWNER_ID) is authorized to interact via Slack.
    """
    if not user_id:
        return False
...
async def _handle_slash_command(
    ...
    if cmd == "!stop":
        ...
        from junction.slack.blocks import build_stopping_blocks
    ...
    if cmd == "!voice":
        from junction.voice_reply import VALID_ENGINES, _validate_pitch, _validate_rate
    ...
    if cmd == "!agent":
        parts = cmd_text.split()
        if len(parts) == 1:
            name = _get_default_agent() or "junction"
            await slack.post_message(
                channel,
    ...
    if cmd == "!dashboard":
        ...
        from junction.slack.allowlist import send_dashboard_link
    ...
    if cmd == "!link-to-dashboard":
        ...
        from junction.slack.interactions import _import_thread_to_slot
    ...
    if cmd == "!channel":
        if not is_owner(user_id):
            ...
            return ""
        from junction.config.loader import _VALID_ACTIVATIONS
    ...
    await slack.post_message(
        channel,
        f"❌ Unknown command `{cmd}`. Type `/junction help` for available commands.",
        reply_ts,
    )
    return ""
...
def _append_footer_actions(
    ...
    *staleness_token* must be minted by the caller, which is async and can do the
    ...
    if thread_ts and not linked_session_key and dashboard_state:
        from junction.slack.format import build_link_dashboard_button
...
async def maybe_route_linked_thread(
    ...
    ``/junction link-to-dashboard`` behaves identically on both.
...
async def handle_message(
    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    # @mention: "<@UBOT|junction> !agent foo"   → "!agent foo"      (strip prefix)
    ...
    status_ctrl = StatusReactionController(
        ...
        channel,
        msg_ts,
        enabled=JunctionConfig.load().slack.reactions_enabled,
    )
    ...
    _show_thinking = JunctionConfig.load().slack.show_thinking
    ...
    if channel_activation == ACTIVATION_REVIEW:
        from junction.slack.blocks import review_draft_blocks
...
async def handle_interaction(
    ...
    if not pending:
        ...
        if action_id == _ACTION_TRUST and thread_ts:
            ...
            # ``junction.session.SessionMap`` to drive the fail-closed path, and
...
async def _handle_sessions_command(
    ...
    *,
    ...
    Delegates to
    :func:`junction.slack.sessions_view._collect_recent_sessions_off_loop`
    and :func:`junction.slack.sessions_view._build_sessions_blocks` so the
