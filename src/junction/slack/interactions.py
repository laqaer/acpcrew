...
from junction.security import redact_and_truncate, redact_credentials, redact_exfiltration_urls
...
from junction.slack.outbound import (
    PostedOptions,
...
def _probe_tracked_channel_scope(channel_ids: set[str]) -> None:
    ...
    the probe (see :mod:`junction.slack.scope_probe`) turns that silent-dead
...
async def dispatch(payload: dict) -> None:
    ...
    # ── Cron acknowledge ──
    from junction.slack.format import CRON_ACK_ACTION_PREFIX
    ...
    from junction.slack.format import SUBAGENT_ACK_ACTION_PREFIX
...
async def _refresh_channels_modal(view_id: str) -> None:
    ...
    if not _orch or not _orch.slack:
        return
    from junction.slack.blocks import channels_modal
...
async def _handle_ch_activation(payload: dict, action: dict) -> None:
    ...
    from junction.slack.handler import _persist_channel_config

    await run_config_write(_persist_channel_config, cid, activation=new_mode)
    if _orch:
        from junction.config.loader import JunctionConfig

        _orch._cfg = JunctionConfig.load()
    sel().log_api_access(
        caller=caller,
...
async def _handle_ch_remove(payload: dict, action: dict) -> None:
    ...
    from junction.slack.allowlist import persist_tracking_channel
...
async def _handle_users_select(
    ...
    from junction.slack.handler import is_owner, set_allowed_users
...
async def _handle_channels_select(
    ...
    # Imported at call time on purpose: tests patch ``handler.is_owner`` to drive
    # the non-owner rejection, and only a call-time rebind observes that patch.
    from junction.slack.handler import is_owner, set_tracking_channels
...
async def _handle_stop_confirm(payload: dict, channel: str, msg_ts: str, user_id: str) -> None:
    ...
    if has_session or active_task:
        ...
        async def _on_soft() -> None:
            from junction.slack.blocks import build_stopped_blocks
        ...
        async def _on_hard() -> None:
            from junction.slack.blocks import build_stop_failed_blocks
...
async def _handle_allowlist_remove(
    ...
    from junction.slack.blocks import allowlist_list_block
...
async def _handle_channel_remove(
    ...
    from junction.slack.blocks import channel_list_block
...
async def _handle_resume_choice(
    ...
    async with lock:
        ...
        try:
            from junction.config.loader import data_home
...
async def _handle_session_end(
    ...
    # Also try treating value as a direct session key (from /junction sessions buttons)
...
async def _handle_review_approve(payload: dict, action: dict) -> None:
    ...
    from junction.slack.handler import _review_drafts_get, _review_drafts_pop
...
async def _handle_review_edit(payload: dict, action: dict) -> None:
    ...
    from junction.slack.blocks import review_edit_modal
    from junction.slack.handler import _review_drafts_get
...
async def _handle_review_revise(payload: dict, action: dict) -> None:
    ...
    from junction.slack.blocks import review_revise_modal
    from junction.slack.handler import _review_drafts_get
