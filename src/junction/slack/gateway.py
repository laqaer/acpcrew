"""Slack Socket Mode gateway orchestrator for Junction.
...
import junction
import junction.crash_guard as crash_guard
from junction import agent_scratch, beacon, dep_sync, platform_compat, shutdown_event
from junction.acp.client import AcpError, AcpProcessDied
from junction.agents_janitor import sweep_agents_dir
from junction.autonudge import (
    APPROVAL_STALL_REASON,
    AutoNudgeService,
    NudgeLoop,
)
from junction.autonudge import enabled as autonudge_enabled
from junction.autonudge import (
    is_channel_key,
    runtime_budget_exceeded,
)
from junction.beacon import distribution
...
from junction.cron_script import run_command_sandboxed, run_script_sandboxed
from junction.dashboard import cautious_boot, start_dashboard
...
from junction.dashboard.handlers.messaging import _rehydrate_slot_from_history
from junction.dashboard.handlers.updates import remediation_command as _remediation_command
...
from junction.dashboard.token_auth import MAX_SESSION_TTL_SECS, generate_token
from junction.dashboard.turn_dispatch import bounded_chat_turn, spawn_guarded_turn
...
from junction.executors import (
    CronQueueTimeout,
    cron_gate_budget,
...
from junction.frontend import build_frontend_async
from junction.gateway_shutdown_budget import GRACEFUL_SHUTDOWN_SECS
...
from junction.hooks import HookManager, HooksConfig, hooks_config_from_config_dict
...
from junction.mcp_gateway import is_gateway_supported
from junction.mcp_gateway.manager import (
    GatewayManager,
    GatewaySpec,
)
from junction.mcp_gateway.resolve_once import prefetch as resolve_prefetch
...
from junction.messaging.display_safety import redact_for_display
from junction.messaging.identity import publish_turn_identity
...
from junction.messaging.renderer import chunk_for_transport
from junction.messaging.transport import InboundMessage, delivery_confirmed
from junction.platform import boot_platform
...
from junction.platform.update_governance import (
    commits_ahead,
    git_command_env,
...
from junction.sandbox import ensure_agents_slice_limits, warm_backend
from junction.security import redact, redact_credentials, redact_exfiltration_urls
from junction.sel import sel
from junction.service.common import restart_command_hint
from junction.session import HEARTBEAT_KEY, SessionManager
...
from junction.slack.retry import open_dm_with_retry
from junction.slack.scope_probe import warn_unreadable_tracked_channels
...
from junction.wecom.gateway import warn_if_channel_uncredentialed

if TYPE_CHECKING:
    from junction.dashboard.state import _ChatSlot
    from junction.discord.client import DiscordClient
    from junction.imessage.client import IMessageClient
    ...
    from junction.whatsapp.client import WhatsAppClient
...
async def _persist_turn_row(
    ...
    *,
    ...
    so a cold-cache ``JunctionConfig.load()`` neither runs on every turn nor
...
# Tunable via JUNCTION_SUBAGENT_DIGEST_CHUNK_SIZE. Guarded parse: a malformed
...
def _digest_chunk_size() -> int:
    try:
        return max(1, min(int(os.environ.get("JUNCTION_SUBAGENT_DIGEST_CHUNK_SIZE", "10")), 1000))
...
# junction.subagent) instead of adding a third hand-maintained copy next to
...
# JUNCTION_INJECTION_TIMEOUT) which only covers a single injected continuation turn.
...
HEARTBEAT_SAFE_TOOLS = frozenset(
    {
        ...
        "WorkspaceSearch",
        # Junction-core reads (no side effects)
        "learn_list",
        "cron_list",
        "spawn_list",
        "spawn_status",
...
class CronVetOverran(Exception):
    ...
    :data:`~junction.cron._SUBPROC_CLEANUP_ALLOWANCE_SECS`, so the deadline
...
def claim_vet_bound(job: CronJob) -> float:
    ...
    ``junction.cron._vet_allowance`` adds to the run deadline.  Read from
...
def _claim_backstop(job: CronJob, subprocess_bound: int) -> float:
    ...
    :data:`~junction.cron._SUBPROC_CLEANUP_ALLOWANCE_SECS`; reading the constant
...
class GatewayOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # client handle is owned by the channel registry (``junction.channels``),
        ...
        # The rewriter's ``JUNCTION_MCP_TARGET_*`` mapping from the last broker
    ...
    async def _init_services(self) -> None:
        ...
        try:
            from junction.agent import rebuild_agent_config  # circular import
            ...
            # desktop app launches the gateway but never runs `junction setup`.
            #
        ...
        try:
            from junction.agent import missing_required_agent_specs  # circular import
        ...
        from junction.config.loader import ACTIVATION_OBSERVE
    ...
    async def _init_cron(self) -> None:
        ...
        async def _cron_callback(job: CronJob) -> str | None:
            ...
            def _cron_extra_env() -> dict[str, str] | None:
                """job.env plus JUNCTION_APPROVAL_MODE when the job runs auto.
                ...
                JUNCTION_SESSION_KEY/JUNCTION_CHANNEL_ID) lets the spawn_run MCP
                tool (mcp_core.py) read and forward its own approval_mode
                explicitly, independent of whether parent_session resolution
                succeeds.

                JUNCTION_APPROVAL_MODE is a RESERVED control var: it is stripped
                ...
                """
                env = {k: v for k, v in (job.env or {}).items() if k != "JUNCTION_APPROVAL_MODE"}
                if job.approval_mode == "auto":
                    env["JUNCTION_APPROVAL_MODE"] = "auto"
                return env or None
            ...
            if len(agents) > 1:
                ...
                for agent in agents:
                    ...
                    try:
                        ...
                        # JUNCTION_SESSION_KEY and macOS sets no
                        # JUNCTION_HOST_PID, so the ancestor PID-walk over the
            ...
            try:
                ...
                # laptop + a cloud desktop both running Junction) can tell which
        ...
        if self._no_crons:
            ...
            from junction.apps.bridges import reconcile_app_crons_for_execution
    ...
    async def _init_heartbeat(self) -> None:
        ...
        async def _heartbeat_task(task_text: str, deliver: str) -> str | None:
            ...
            _acquired = False
            try:
                # Use the dedicated ``junction-heartbeat`` agent — minimal
                # MCP surface (junction-core only on public installs) so cycle
                ...
                await _persist_turn_row(
                    ...
                    surface="heartbeat",
                    agent_fallback=lambda: "junction-heartbeat",
                    t0=_turn_t0,
                )
            except asyncio.TimeoutError:
    ...
    async def _fire_webex_nudge(self, loop: NudgeLoop) -> bool:
        ...
        from junction.webex.transport import ROOM_DIRECT
    ...
    async def _deliver_result(
        ...
        if not deliver:
            try:
                if JunctionConfig.load().heartbeat.default_deliver == "dashboard":
                    deliver = "dashboard"
            except Exception:
    ...
    def _init_mcp_discovery(self) -> None:
        ...
        """
        try:
            from junction.mcp_discovery import list_servers  # circular import
    ...
    async def _init_api_server(self) -> None:
        ...
        from junction.dashboard import start_api_server
    ...
    async def _auto_migrate_memory(self) -> None:
        ...
        """
        from junction.memory import legacy_memory_present
    ...
    async def _set_memory_migrated(self, value: bool) -> None:
        ...
        from junction.dashboard.handlers.memory import _set_migrated
    ...
    async def _shutdown(self) -> None:
        ...
        if self.dashboard_state:
            from junction.dashboard.chat import save_all_slots_to_history
        ...
        # clean Junction exit it and its pooled MCP subprocesses would
    ...
    async def _check_for_updates(self) -> None:
        ...
        Delegates to the resolved :class:`~junction.platform.update_provider.CommandProvider`
        ...
        provider = None
        try:
            from junction.platform.update_provider import resolve_provider
    ...
    def _publish_provider_update_state(self, result: object) -> None:
        ...
        """
        from junction.dashboard.handlers.updates import _update_info
    ...
    async def _check_for_updates_via_provider(self, provider: object) -> None:
        ...
        from junction import __version__ as _running_version
        from junction.config import JunctionConfig
        from junction.platform.update_governance import min_version, update_required
        from junction.platform.update_provider import UpdateProvider
    ...
    async def _restart_after_update(self) -> None:
        ...
        platform_compat.reexec_python_module("junction", sys.argv[1:])

    async def _check_for_updates_legacy(self) -> None:
        """Legacy update check — the existing layout-aware logic."""
        try:
            from junction import __version__ as _running_version
            from junction.dashboard.handlers import _do_update_check, _update_info
    ...
    async def _auto_apply_update(self) -> None:
        ...
        The public OSS flow is the same one used by ``junction update`` and the
        ...
        from junction.platform.update_provider import _kill_and_reap
        ...
        try:
            ...
            # needs a deliberate `junction update`, and a detached HEAD has no
            ...
            # `junction update`, where a human is deciding.
            ...
            #    up already refuses for COMMITTED work and defers to `junction
            ...
            if pip_rc != 0:
                ...
                #     `-m junction` so it would survive, but the next restart
                #     through the service manager runs `junction` and does not.
                #
            ...
            if self.sessions:
                await self.sessions.close_all()
            # Use -m junction rather than sys.argv[0] so the restart resolves
    ...
    async def _auto_apply_wheel_update(self) -> None:
        ...
        * The install is NOT a git checkout (no ``can_apply``) and NOT externally
        ...
        :func:`junction.platform.update_layout.wheel_update_command` from a
        ...
        """
        from junction.dashboard.handlers import _update_info
        ...
        # piped installer interceptable. Same gate `junction update` applies.
        ...
        from junction.platform_compat import trusted_system_bin
    ...
    def _write_marker_worker(self, run_marker: Any, port: int) -> None:
        ...
        ``run_marker`` is the :mod:`junction.instances.run_marker` module,
    ...
    async def run(self) -> None:
        ...
        from junction.session import cleanup_orphaned_sessions
        ...
        from junction.slack.events import SeenCache, init_socket_mode
        from junction.slack.interactions import init as init_interactions
        ...
        # a stale ~/.local/bin/junction that may point at an uninstalled
        # worktree. See junction.instances.run_marker. Written for headless
        # API-only gateways too: the marker's filename is what lets a client
        # or MCP child discover a non-default port when neither JUNCTION_PORT
        ...
        if self._json_ready:
            ...
            print(f"JUNCTION_READY:{json.dumps(ready_payload)}", flush=True)
        ...
        if not self._no_dashboard:
            try:
                ...
                # the service, and `junction token` still produces a URL.
        ...
        from junction.dashboard.handlers import _bg_mcp_probe
    ...
    async def _start_channel_transports(
        ...
        :func:`junction.channels.builtin_channel_descriptors` and the loop
        lives in :mod:`junction.messaging.registry` — adding a channel no
        ...
        # (issues #304, #5418). Runs after JUNCTION_READY, outside the
...
async def run_gateway(
    ...
    *,
    ...
    # kiro agents directory (see junction.agents_janitor). Scheduled as a
    ...
    # (see junction.agent_scratch). Liveness-keyed, never age-keyed, so a
    ...
    if not test_mode:
        with contextlib.suppress(Exception):
            threading.Thread(
                target=beacon.send,
                args=(cfg.telemetry.beacon_endpoint, junction.__version__),
                kwargs={
                    "enabled": cfg.telemetry.beacon_enabled,
                    "acked": cfg.dashboard.privacy_acked,
                },
                name="junction-beacon",
                daemon=True,
            ).start()
