...
``junction.messaging.TurnDriver`` onto Slack's streaming + Block Kit surface
...
* **Length splitting** belongs to
  :func:`junction.messaging.split.split_markdown_safe`, the shared fence-safe
...
* **Outbound local-image extraction** belongs to
  :mod:`junction.messaging.outbound_files`, with Slack's per-file ceiling, count
  cap and ``files_upload_v2`` call in :mod:`junction.slack.files`. Extraction
...
from junction.messaging.renderer import Renderer, chunk_text
...
from junction.slack.files import UPLOAD_LIMITS, upload_outbound_files
from junction.slack.format import SLACK_MSG_LIMIT, extract_options, strip_thinking_tags
