...
def _read_allowlist() -> tuple[set[str] | None, str]:
    ...
    **One reader.** The same validated read decides BOTH "is this config
    ...
    disagree.  Asking ``JunctionConfig.load()`` for the value while probing the
...
def _load_allowed_team_ids() -> bool:
    ...
    own ``JunctionConfig.load()`` -- may widen the allowlist afterwards, or the
...
def validate_enterprise(
    ...
    *,
    ...
    pass their own ``JunctionConfig.load()`` snapshot of
    ...
    :class:`~junction.platform.interfaces.SlackEnterpriseGate` protocol, which
    ...
    if extra and degraded:
        ...
        # ``JunctionConfig.load()``-derived value -- which degrades a torn
        ...
        # ``JunctionConfig.load()`` snapshot of the same
