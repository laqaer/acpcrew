...
from junction.cron import format_schedule
from junction.dashboard.chat_utils import run_config_write
from junction.dashboard.handlers import get_update_info
from junction.dashboard.token_auth import LINK_WINDOW_SECS, MAX_SESSION_TTL_SECS, parse_duration
from junction.executors import subprocess_executor
from junction.hooks import safe_read_file
from junction.mcp_discovery import list_servers
from junction.messaging.identity import channel_inbound_permitted
...
from junction.session import unlink_queued_temp_paths
from junction.skills import SkillsLoader
from junction.slack.allowlist import prompt_track_channel, send_dashboard_link
...
from junction.slack.interactions import dispatch as dispatch_interactive
...
from junction.slack.transport_dispatch import handle_message_transport
from junction.stats import Stats
from junction.transcribe import is_available as stt_available
from junction.transcribe import transcribe_audio
...
def register_slash_command(name: str, handler: SlashHandler, description: str = "") -> None:
    """Register a sub-command for ``/junction <name>``."""
    SLASH_REGISTRY[name] = (handler, description)


def _build_help_text(cmd_name: str = "junction") -> str:
...
async def _handle_agent(
    ...
    from junction.slack.handler import (  # circular import: handler.py imports events.py for command dispatch, creating runtime circular dependency
        _get_default_agent,
        _resolve_agent_name,
        _set_default_agent,
        is_owner,
...
def _get_agent_names() -> list[str]:
    ...
    for f in agents_dir.glob("*.json"):
        try:
            ...
            # `/junction channels` handler / channel-modal refresh task before
...
def init_socket_mode(orch: GatewayOrchestrator, seen: SeenCache) -> None:
    ...
    if not orch._owner_id:
        logger.error("JUNCTION_OWNER_ID is not set — Slack disabled for security")
...
async def _publish_home_tab(orch: GatewayOrchestrator, user_id: str) -> None:
    ...
    try:
        ...
        if orch.cron_svc is not None:
            ...
            if jobs:
                try:
                    _tz = JunctionConfig.load().timezone
                except Exception:
                    _tz = ""
...
async def _handle_slash(orch: GatewayOrchestrator, payload: dict) -> None:
    """Route ``/junction <sub-command>`` via :data:`SLASH_REGISTRY`.
    ...
    try:
        if _use_transport:
            await handle_message_transport(
                ...
                reactions_enabled=JunctionConfig.load().slack.reactions_enabled,
                show_thinking=JunctionConfig.load().slack.show_thinking,
...
async def _route_message(
    ...
    # ── !restart: bang alias for /junction restart — intercept here so it
    ...
    # the native path. (Junction has no challenge-redirect path, so this simply
