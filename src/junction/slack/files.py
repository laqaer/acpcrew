...
onto :class:`~junction.messaging.attachments.Attachment`, and supplying an
...
in :mod:`junction.messaging.attachments` so other channels reuse them instead of
...
The OUTBOUND half is the mirror image: :mod:`junction.messaging.outbound_files`
...
from junction.platform_compat import restrict_dir_to_owner
...
#: the blocks :func:`junction.messaging.attachments.transcribe_audio_attachments`
...
async def upload_outbound_files(
    ...
    **The bytes travel, not the path.** ``OutboundFile.data`` is what every gate
    in :mod:`junction.messaging.outbound_files` was applied to, and the
...
async def _upload_one(
    ...
    """
    staging = await asyncio.to_thread(tempfile.mkdtemp, prefix="junction-slack-upload-")
    try:
