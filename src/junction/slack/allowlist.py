...
1. **User allowlist** — when a user joins a tracked channel
   (``member_joined_channel``) or is nominated via ``/junction @user``,
   the owner gets a DM with Allow / Deny buttons.
2. **Tracking channel** — ``/junction #channel`` sends an Add / Ignore
...
from junction.dashboard.token_auth import LINK_WINDOW_SECS, MAX_SESSION_TTL_SECS, generate_token
from junction.sel import sel
from junction.slack.handler import is_allowed_user, is_tracked_channel
from junction.tunnel import get_tunnel_url
...
async def prompt_allowlist(
    ...
    When the user is already allowed (manual ``/junction @user`` recall),
...
async def send_dashboard_link(
    ...
    if cfg.slack.use_tunnel_url and local_only and not tunnel_url:
        ...
        from junction.platform.context import async_safe_context_call
...
# inline. `send_dashboard_link()` above (the explicit `/junction dashboard`
