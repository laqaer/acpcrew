...
from junction.hooks import HOOK_REPLY, TOOL_AUTO_APPROVE, TOOL_DENY
from junction.llm_helpers import save_conversation_turn_off_loop
from junction.messaging import auto_title
from junction.messaging.dispatch import build_directive_consumer
...
from junction.slack.renderer import SlackApprovalDecider, SlackRenderer
...
#: Canonical kiro-cli agent name for Junction. Used as the final agent
#: fallback so the transport session loads the same MCP surface (incl. the
#: junction-core server that provides ``spawn_run``) as the dashboard/native
#: path. Without it, an empty ``agent.default_agent`` config makes kiro-cli
#: launch under its bare built-in default (no junction-core -> no spawn_run).
#: Mirrors the native path's fallback (``handler.py``: ``_get_default_agent()
#: or "junction"``) and the many ``agent="junction"`` call sites.
...
async def _refresh_dashboard_tab(session_key: str) -> None:
    ...
    try:
        ...
        from junction.dashboard.channel_slots import surface_channel_state
...
async def handle_message_transport(
    ...
    *,
    ...
    # /junction link-to-dashboard routes into its dashboard slot (with the same
    ...
    try:
        ...
        # default win; otherwise fall back to the canonical "junction" agent so
        # the session loads junction-core (spawn_run) rather than kiro-cli's bare
