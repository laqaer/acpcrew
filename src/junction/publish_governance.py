...
``JunctionConfig.load()`` degrades to defaults on a malformed file, which would
...
def _read_allowed_destinations() -> tuple[list[str], str | None]:
    ...
    ``JunctionConfig.load().publish.allowed_destinations``. A ``config.json`` of
...
def publish_denied_reason(request: web.Request, provider_name: str) -> str | None:
    ...
    """
    # The three ``junction.platform`` imports in this function stay FUNCTION-LOCAL
    ...
    # every gateway boot and invert that direction. ``sel`` and ``JunctionConfig``
