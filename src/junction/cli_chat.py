...
from junction.acp._dispatch import is_shell_kind
from junction.acp.client import AcpError, AcpTimeoutError
...
from junction.constants import BANNER, DATA_WARNING, MIN_NODE_MAJOR
...
#: the same value so a task-bound ``junction`` profile cannot be skipped merely
#: because the CLI relied on the provider's default.
_DEFAULT_KIRO_AGENT = "junction"
...
def _build_tool_gate(agent: str = "") -> _ToolGate:
    ...
    """
    return _ToolGate(
        hooks=HookManager(hooks_config_from_config_dict(JunctionConfig.load().hooks)),
        agent=agent,
    )
...
def _tui(args: argparse.Namespace) -> None:
    ...
    if not tui_js:
        ...
        for _ in range(15):
            p = p.parent
            bundle = p / "src" / "Junction" / "tui" / "dist" / "bundle.mjs"
            if bundle.is_file():
                tui_js = bundle
                break
            idx = p / "src" / "Junction" / "tui" / "dist" / "index.js"
            if idx.is_file() and (p / "src" / "Junction" / "tui" / "node_modules").is_dir():
                tui_js = idx
                break
    ...
    home_override = getattr(args, "home", None) or os.environ.get("JUNCTION_HOME", "")
    if home_override:
...
async def _chat(message: str | None, model: str | None, agent: str | None = None) -> None:
    ...
    channel_id = os.environ.get("JUNCTION_CHANNEL_ID") or None
    # Keep the provider and governance gate on one canonical identity.  ACP
    # resolves an omitted agent to ``junction``; passing an empty string only to
...
async def _read_line(prompt: str) -> str:
    ...
    threading.Thread(target=_worker, daemon=True, name="junction-permission-prompt").start()
    try:
        return await future
...
async def _interactive(
    provider: LLMProvider, cfg: JunctionConfig, *, gate: _ToolGate | None = None
) -> None:
...
def _ensure_default_agent_in_config() -> None:
    """Ensure config.json includes a default Junction agent for fresh installs."""
    p = config_path()
    try:
        data = read_config_for_update(p)
    except ConfigReadError:
