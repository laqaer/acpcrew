...
from junction import __version__ as _mc_version
from junction import agent_state, dep_sync, diagnostics, platform_compat, sandbox
from junction._bootstrap import _source_checkout_root
from junction.acp import kas_assets, kas_auth
from junction.acp.client import KIRO_CLI_BIN
...
from junction.discord import install_url, intent_probe
from junction.doctor_deadpath import doctor_dead_paths
...
from junction.kiro_cli import mcp_governance_may_apply
from junction.mcp_cleanup import ALWAYS_ON_BIN_MCP_SERVERS as _ALWAYS_ON_MCPS
from junction.mcp_cleanup import JUNCTION_BIN_MCP_SERVERS as _MANAGED_MCPS
from junction.mcp_cleanup import OPT_IN_BIN_MCP_SERVERS as _OPT_IN_MCPS
from junction.mcp_discovery import McpServerInfo, probe_server
...
from junction.platform.governance import CU_MCP_SERVER, may_skip_gate_now
...
from junction.service import common as common_service
from junction.service import controller as service_controller
from junction.service import linux as service_linux
from junction.session_pid_sig import signing_health
...
# attribute directly (``patch("junction.cli_doctor.KIRO_AGENTS_DIR", tmp)``),
...
def _doctor_effective_model(cfg: JunctionConfig, project_dir: str, issues: list[str]) -> None:
    ...
    # only after the global defers (tier 4). Reading junction.json in both cases
    ...
    try:
        ...
        bound = bindings.kiro_agent or "junction"
    ...
    if decided_by.startswith("bound agent pin"):
        ...
        pinned_agent, pinned_value = "junction", default_model
    else:
    ...
    if pinned_value:
        # pinned_agent is either the literal "junction" or a configured kiro
...
# Junction's agent backend is kiro-cli (the sole public ACP backend). The
...
def _doctor_mcp_tools(agent_path: Path, issues: list[str]) -> None:
    ...
    Two passes scoped to the managed servers (`junction-core`,
    `junction-cron`, `junction-computer`):
...
def _doctor_mcp_governance(agent_path: Path, issues: list[str]) -> None:
    ...
    """
    try:
        declared = JunctionConfig.load().agent.mcp_registry_mode
    except Exception:
...
def _doctor_path_launcher() -> None:
    """Report which install the ``junction`` command on PATH actually belongs to.

    A gateway never takes the name from another install's working launcher (see
    ``agent.ensure_junction_on_path``), which is the right call — but it leaves a
    ...
    ``junction`` can run a different install, at a different version or channel,
...
#: Mirrors ``mcp_core._STRICT_IDENTITY_SERVERS``; ``junction-dashboard`` is
#: opt-in per agent, so it is reported only when an agent actually references it.
_STRICT_IDENTITY_SERVERS = ("junction-core", "junction-dashboard")


def _doctor_strict_identity(cfg: JunctionConfig) -> None:
    ...
    launcher exports ``JUNCTION_HOST_PID``, so routing is not what decides
...
def _process_apparmor_confinement() -> str:
    ...
    ``junction-userns (enforce)``. The per-LSM path is tried first; the bare
...
def _doctor_sandbox_apparmor(reason: str, issues: list[str]) -> None:
    ...
    * profile installed, the service unit applies it, and this process is
    ...
    print(f"{_INDENT}  sudo systemd-run --pipe --unit=junction-sandbox-test \\")
    print(f"{_INDENT}    --property=AppArmorProfile=-{apparmor.PROFILE_NAME} \\")
    print(f"{_INDENT}    --uid=$(id -u) --gid=$(id -g) \\")
    print(f'{_INDENT}    {quoted_python} -c "import junction.sandbox as sb; \\')
...
def _doctor_pod_session_bus(issues: list[str]) -> None:
    ...
    Junction to point at — every pod verb then fails with "Failed to connect to
    ...
    from junction.pod.runtime import has_session_bus, session_bus_socket
...
def _doctor_model_url_reachable(issues: list[str]) -> None:
    ...
    """
    del issues  # advisory-only diagnostic; keeps the call-site signature uniform
    from junction.embeddings import redact_model_url  # circular-safe (no loader)
    ...
    try:
        ...
        print("               Fix: set JUNCTION_EMBED_MODEL_URL (or memory.embed_model_url)")
...
def _kas_version_label(script: "Path") -> str:
    ...
    :mod:`junction.acp.kas_assets`), so the version is the path component whose
...
def _doctor_kas(issues: list[str]) -> None:
    ...
    if JunctionConfig.load().agent.acp_backend == ACP_BACKEND_KAS:
...
def _report_kas_backend(issues: list[str]) -> None:
    ...
    if not (node and script):
        ...
        print("               KAS bundle (or set JUNCTION_KAS_NODE / JUNCTION_KAS_SCRIPT).")
...
def _discord_intent_grants(token: str) -> intent_probe.IntentGrants:
    ...
    :func:`~junction.discord.intent_probe.probe_intent_grants`; the guard here
...
def _doctor_discord(
    cfg: JunctionConfig, creds: dict[str, str], port: int | None, issues: list[str]
) -> None:
...
def _doctor_whatsapp(cfg: JunctionConfig, issues: list[str]) -> None:
    ...
    # every `junction` invocation, since cli.py imports this module at its own
    ...
    if store.exists():
        ...
        # failing here would break the documented `junction doctor && junction
...
def _doctor(
    ...
    # trivially. Walks EVERY spec in the agents dir (not just junction.json),
    ...
    try:
        from junction._sqlite_compat import fts5_available
    ...
    # Gated on the checkout markers themselves (setup.cfg + src/junction, via
    ...
    # Windows box and neither is a Junction dependency there. Reporting them as
    ...
    if stt_active and cfg.stt.provider == "transcribe":
        try:
            ...
            print("               Install: pip install 'junction[voice]'")
    ...
    print("\nSlack Integration")
    if _has_slack:
        has_owner = bool(creds.get("JUNCTION_OWNER_ID"))
        print("  tokens:      ✅ configured")
        if has_owner:
            print(f"  owner:       ✅ {creds['JUNCTION_OWNER_ID']}")
        else:
            print("  owner:       ⚠️  JUNCTION_OWNER_ID not set")
