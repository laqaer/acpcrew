...
the existing bucket + distribution by the ``junction:site`` tag and reuses them
(idempotent), so the public URL stays stable. The physical bucket name is an
opaque ``junction-web-<random>`` (no account id); idempotency never relies on
re-deriving it.
"""
...
BUCKET_PREFIX = "junction-web-"
TAG_MANAGED = "junction:managed"
TAG_SITE = "junction:site"
...
def resolve_aws_bin() -> str:
    ...
    :func:`junction.github_runner.validate_provider_executable` — the repo's
    ...
    if found:
        from junction.github_runner import validate_provider_executable
...
def run_aws(args: list[str], profile: str, timeout: int = 30) -> tuple[int, str, str]:
    ...
    resolved by the CLI's own provider chain; Junction passes only the profile name.
...
def find_site_by_tag(site_id: str, profile: str, region: str = DEFAULT_REGION) -> Optional[dict[str, str]]:
    """Resolve an existing site's bucket + distribution by the junction:site tag.
    ...
    (1) require BOTH junction:site=<id> AND junction:managed=true (filters
    ...
    managed naming scheme (junction-web-*); (3) multiple matching buckets or
...
def list_sites(profile: str, region: str = DEFAULT_REGION) -> list[dict[str, Any]]:
    ...
    Groups managed resources by the junction:site tag; status/domain read live
