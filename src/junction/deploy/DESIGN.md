# deploy-web — Junction App Design Doc (v1)

**Status:** ✅ Implemented (v1) — built-in app at `junction/apps/builtins/deploy_web/`
(engine/render/scan/iam/handlers) + UI `JunctionWebsite/src/apps/deploy-web/DeployWebPage.tsx`,
...
Extend Junction's scope — including **public usage** — by collaborating with AWS,
**without Junction ever managing AWS accounts or credentials**, to **safely deploy
...
your own AWS" is the deliberate bar. The *user-owns-their-own-cloud* model (Junction
...
1. **One-time AI-guided setup** (§12): the agent walks the user through AWS *access* —
   ...
   **once ever**, reused for everything after. Junction stores only the profile name.
...
### Packaging — built-in app, ships with Junction
deploy-web is a **built-in app that ships *with* Junction** (like Research Lab / Team
...
Junction package and **disabled / opt-in by default** so users without AWS never see AWS
...
**Flagship use case:** publish an existing Junction artifact — **HTML, markdown file,
...
**YES — build a `deploy-web` built-in Junction app** (ships with Junction, opt-in/disabled
...
Junction's code-verified posture.

### One-picture flow
> User authenticates **once** (`aws configure sso`) → Junction stores only the
> **profile name** → `deploy-web --artifact <dir>` runs a deterministic 6-step flow
> on the user's credentials → returns a live HTTPS URL. Junction never sees or stores a key.
...
deploy-web mostly **extends code Junction already ships**:
...
upload the module runs the rendered output through Junction's **existing
...
- **AWS account is the source of truth.** Junction holds **no** local record — **no
  SQLite cache in v1** (Q8). `list-sites` queries the Resource Groups Tagging API live
  every time (`tag:GetResources Key=junction:managed`), so there is zero drift.
- **Identity vs naming (Q2):**
  - **Logical site id** = the artifact slug (or user-supplied name) → drives the
    `junction:site=<id>` tag and the UI label. This is what makes re-deploy idempotent.
  - **Physical bucket name** = `junction-web-<random ~12hex>` — opaque, **no account
...
- Mandatory tags on every resource: `junction:managed=true` + `junction:site=<id>`,
...
- Junction **never persists AWS credentials** — every key reference in source is
...
of the following is ever done by Junction:
...
- ❌ Running `aws configure`, `aws sso login`, `ada`, or any credential-establishing
  command on the user's behalf — these remain **user-run, outside Junction**.
- ❌ Assuming roles, minting STS tokens, or brokering cross-account access.

What Junction **does**, and the entire trust surface, is narrow:
...
- ✅ Pass `--profile <name>` to the **local** AWS CLI and let the OS-resident provider
  chain resolve credentials entirely outside Junction's process boundary.
...
This makes the credential/account boundary identical to Junction's existing,
...
Two scoping levers: an **S3 name prefix** (`junction-web-*`) and a **resource tag**
(`aws:ResourceTag/junction:managed=true`) confining CloudFront mutate/delete actions.
...
## 8. Build shape — built-in app (ships with Junction)
...
- **Backend: a Python builtin module** — runs the §4 6-step deploy flow + recall/destroy
  ...
  credential resolution entirely outside Junction's process, preserving the §6.1
...
- **No per-app update cron.** As a **built-in** app, deploy-web is versioned and updated
  with the Junction package itself (`junction update`) — there is no independent upstream
...
- **Built-in packaging:** ships inside the Junction package (registered like Research
...
- Junction **never reads, stores, or persists** AWS credentials — verified across the
...
- Scoped customer-managed policy (§7): S3 confined to the `junction-web-*` prefix;
  CloudFront mutate/delete confined by the `junction:managed=true` resource tag.
...
- The approval prompt **states the public nature** of the URL being created
  (a world-readable site), per Junction's security-awareness norm.
...
- **One-time AWS prep (~10–15 min, once ever):** `aws configure sso` (~3 min) +
  create the IAM role/policy (~5–10 min). Junction stores only the profile name.
...
- **Chat-native path:** saying "deploy this artifact publicly" in chat triggers the same
  deploy-web **skill workflow** in junction-core (no custom tool involved) — same backend
  endpoint, same gates.
...
is good at: Junction already has a live agent in the loop, so "installation" doesn't
...
- Generate the cycle-006 scoped IAM policy JSON (S3 + CloudFront only, confined to
  `junction-web-*` + `junction:managed` tag) and show it to the user.
- The **user applies it themselves** (console or their own `aws iam` command) and
  attaches it to their assumable role. Junction does **not** create, attach, or modify
...
- **Verify (read-only reachability only, Q3):** after the user applies it, Junction runs
...
Step 2/3 — Permissions
   ...
   (You apply it — Junction never edits your IAM.)
...
- **Safety boundary intact.** The agent guides, *generates* policy text, and
  *read-only verifies* — it never reads/stores keys and **never performs an IAM write**.
  ...
  themselves; Junction never creates/attaches/modifies roles or policies.
- **No account management.** Junction does not create AWS accounts, IAM users/roles,
  or credentials, and does not rotate or transmit them. All account/IAM mutation is the
  user's own action, outside Junction.
...
- Built as a **Python builtin module** (aws CLI subprocess, not boto3) + UI page +
  thin chat skill (§8) — no per-app update cron (built-in; updated with Junction).
