...
- **One shared base stack per account** (`junction-deploy-base`): a private S3
...
- Cost lives in the **user's** account and is scale-to-zero (S3 storage +
  CloudFront requests). Idle ≈ $0. AWS earns the consumption; Junction pays
  nothing.
...
- **M3 lifecycle** — DONE: `list.sh` / `persist.sh` / `detach_backend.py`, plus the
  **in-account scheduled reaper** (`install-reaper.sh` → EventBridge-timed Lambda;
  ...
  role scoped to `junction-deploy-app-*`. Runs in-account with no local creds —
