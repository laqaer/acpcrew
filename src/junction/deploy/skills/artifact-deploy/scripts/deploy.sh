#!/usr/bin/env bash
# Junction One-Click Deploy - deploy a pre-built static site under a slug.
#
# Usage:
...
# Falls back to a builtin grep set ONLY when python3/junction is unavailable;
# the fallback's failure is also fatal (no `|| true`).
_SCAN_RC=0
if python3 -c "import junction.security" 2>/dev/null; then
  CRED_HITS=$(python3 - "$SNAPSHOT_DIR" <<'PYEOF'
import sys, os, re
from pathlib import Path
from junction.security import get_credential_patterns, is_sensitive_path
...
PYEOF
  ...
  # Fallback: python3/junction unavailable — use grep (failure = abort).
...
if [[ "$TTL_HOURS" != "0" ]]; then
  if ! "${AWS[@]}" cloudformation describe-stacks --stack-name "junction-deploy-reaper" >/dev/null 2>&1; then
    echo ""
    echo "ERROR: The reaper stack (junction-deploy-reaper) is NOT deployed."
