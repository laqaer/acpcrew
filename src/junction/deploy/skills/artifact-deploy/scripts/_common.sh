#!/usr/bin/env bash
# Shared helpers for Junction One-Click Deploy scripts.
#   source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_common.sh"
# Provides: STACK_NAME, PROFILE, REGION, aws_cli(), read_base_outputs(), validate_slug(),
#           scan_source_dir()
STACK_NAME="junction-deploy-base"
PROFILE="${PROFILE:-}"
...
# scan_source_dir <dir>
# Fail-closed pre-package/upload scan. Uses junction.security (canonical) with
...
validate_source_dir() {
  ...
  # Canonical check (preferred): junction.security.is_sensitive_path.
