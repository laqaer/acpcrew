#!/usr/bin/env bash
# Junction One-Click Deploy - TTL reaper. Deletes non-persistent deploys whose
# expires_at is in the past (S3 prefix + invalidation, and any backend stack).
# Safe to run on a schedule (Junction cron / in-account EventBridge).
  ...
  if [[ "$_arch" == "engine" ]]; then
    ...
    if [[ -n "$_site_dist" ]]; then
      ...
      if [[ "$_enabled" == "GONE" ]]; then
        ...
        # attacker-writable — verify the distribution's junction:site tag
        ...
        _dist_site_tag="$(aws_cli cloudfront list-tags-for-resource --resource "$_dist_arn" \
          --query "Tags.Items[?Key=='junction:site'].Value | [0]" --output text 2>/dev/null || echo "TAG_LOOKUP_FAILED")"
        if [[ "$_dist_site_tag" != "$slug" ]]; then
          echo "   error: distribution $_site_dist junction:site tag ('$_dist_site_tag') != slug '$slug' -- identity mismatch, quarantining manifest, skipping" >&2
          continue
        fi
      fi
      if [[ "$_enabled" == "GONE" ]]; then
    ...
    if [[ -n "$_site_bucket" ]]; then
      # R27 F1 (twin): verify the bucket's junction:site tag matches the slug
      ...
      _btag="$(aws_cli s3api get-bucket-tagging --bucket "$_site_bucket" \
        --query 'TagSet[?Key==`junction:site`].Value | [0]' --output text 2>/dev/null || echo '')"
      if [[ "$_btag" != "$slug" ]]; then
        if aws_cli s3api head-bucket --bucket "$_site_bucket" 2>/dev/null; then
          echo "   bucket $_site_bucket identity tag mismatch (junction:site='$_btag') -- skipping"
...
" "$slug"; then
  ...
  if [[ "$_reaping" == "true" ]]; then
    ...
    stack_name="junction-deploy-app-$slug"
    ...
    if [[ "$stack_status" == "GONE" || "$stack_status" == "DELETE_COMPLETE" ]]; then
      ...
      _stack_tag="$(aws_cli cloudformation describe-stacks --stack-name "$stack_name" \
        --query 'Stacks[0].Tags[?Key==`junction:site`].Value | [0]' --output text 2>/dev/null || echo '')"
      if [[ "$_stack_tag" != "$slug" ]]; then
        echo "   stack $stack_name tag mismatch (junction:site='$_stack_tag') -- skipping retry"
        continue
      fi
  ...
  if aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$slug" >/dev/null 2>&1; then
    # R21 F1: verify the stack's junction:site tag matches the slug being
    ...
    _stack_tag="$(aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$slug" \
      --query 'Stacks[0].Tags[?Key==`junction:site`].Value | [0]' --output text 2>/dev/null || echo '')"
    if [[ "$_stack_tag" != "$slug" ]]; then
      echo "   stack junction-deploy-app-$slug tag mismatch (junction:site='$_stack_tag') -- skipping"
      continue
    fi
    # Two-phase: initiate stack deletion and write reaping marker.
    if ! aws_cli cloudformation delete-stack --stack-name "junction-deploy-app-$slug"; then
      echo "   stack delete failed for $slug -- will retry next run"
      continue
    fi
    echo "   backend stack junction-deploy-app-$slug scheduled for deletion"
