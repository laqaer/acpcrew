#!/usr/bin/env bash
# Junction One-Click Deploy - per-app backend (M2.2). API Gateway HTTP API ->
...
BOUNDARY_ARN="arn:aws:iam::${ACCT_ID}:policy/junction-deploy-app-boundary"
if ! aws_cli iam get-policy --policy-arn "$BOUNDARY_ARN" >/dev/null 2>&1; then
  ...
  echo "  then run:" >&2
  echo "    aws iam create-policy --policy-name junction-deploy-app-boundary \\" >&2
...
_STACK_STATUS="$(aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" \
  --query 'Stacks[0].StackStatus' --output text 2>/dev/null || echo "DOES_NOT_EXIST")"
if [[ "$_STACK_STATUS" == "ROLLBACK_COMPLETE" ]]; then
  echo "error: stack junction-deploy-app-$SLUG is in ROLLBACK_COMPLETE state." >&2
  echo "  This dead stack blocks new deployments. Delete it first:" >&2
  echo "    aws cloudformation delete-stack --stack-name junction-deploy-app-$SLUG \\" >&2
  echo "      ${PROFILE:+--profile $PROFILE }--region $REGION" >&2
  exit 1
fi
...
echo ">> deploying backend stack junction-deploy-app-$SLUG ($([[ $TABLE == 1 ]] && echo 'API GW + Lambda + DynamoDB' || echo 'API GW + Lambda')) ..."
...
aws_cli cloudformation deploy \
  --stack-name "junction-deploy-app-$SLUG" \
  --template-file "$TEMPLATE" \
  --capabilities CAPABILITY_NAMED_IAM \
  --no-fail-on-empty-changeset \
  --parameter-overrides "file://$_PARAMS_FILE" \
  --tags "junction:site=$SLUG" "junction:managed=true"

APIDOMAIN="$(aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" \
...
echo "   API Gateway origin; Lambda not world-accessible.$([[ $TABLE == 1 ]] && echo ' DynamoDB table: junction-deploy-app-'$SLUG || true)"
