#!/usr/bin/env bash
# Junction One-Click Deploy - remove one deployed slug (S3 prefix + invalidation).
...
if "${AWS[@]}" cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" >/dev/null 2>&1; then
  ...
  # junction:site=<slug> + junction:managed=true; a stack without matching
  # tags is not provably this deployment's -- abort rather than delete.
  _STACK_TAGS="$("${AWS[@]}" cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" \
...
slug = sys.argv[1]
ok = tags.get("junction:site") == slug and tags.get("junction:managed") == "true"
print("ok" if ok else "mismatch")
' "$SLUG")"
  if [[ "$_TAG_OK" != "ok" ]]; then
    echo "error: stack junction-deploy-app-$SLUG identity tags do not match this deployment -- refusing to delete" >&2
    exit 1
  fi
  echo ">> deleting backend stack junction-deploy-app-$SLUG ..."
  "${AWS[@]}" cloudformation delete-stack --stack-name "junction-deploy-app-$SLUG"
fi
