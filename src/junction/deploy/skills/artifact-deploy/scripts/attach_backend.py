#!/usr/bin/env python3
"""Attach a per-app backend origin to the shared Junction CloudFront distribution.
...
OAC_NAME = "junction-deploy-lambda-oac"


def aws(profile, region, *args):
    # Every AWS spawn from these LLM-facing helpers MUST route through
    # the sandbox chokepoint. Failing open when junction is not importable
    ...
    # invocation), never bare python3 without junction on sys.path.
    try:
        from junction.deploy.engine import resolve_aws_bin
        from junction.sandbox import run_limited, sandboxed_spawn_argv
    except ImportError:
        sys.stderr.write(
            "error: junction package not importable — refusing to spawn AWS "
            "commands unsandboxed. Run this script with the Junction venv "
...
def ensure_lambda_oac(profile, region):
    ...
    cfg = {
        "Name": OAC_NAME,
        "Description": "Junction deploy Lambda OAC",
...
def _validate_args(profile: str, region: str, dist_id: str, slug: str) -> None:
    ...
    # VERBATIM copy of junction.constants.AWS_PROFILE_NAME_PATTERN (#6063):
...
def main():
    ...
    ap.add_argument(
        ...
        default="",
        help="random secret for x-junction-origin-verify header (API GW authorizer)",
    )
    ap.add_argument(
    ...
    if a.origin_verify_secret_file:
        ...
        # Preferred path: junction.hooks.safe_read_file does is_sensitive_path +
        # O_NOFOLLOW open in one shot, closing the TOCTOU between validation and
        # read. Fallback: standalone execution where junction is not importable
        ...
        try:
            ...
            # junction.hooks.safe_read_file. Reading a user-controlled secret
            ...
            sys.stderr.write(
                "error: junction.hooks is unavailable — cannot read "
                "--origin-verify-secret-file without the shared sensitive-path "
                "gate. Run this script from a junction environment (fail-closed).\n"
            )
            sys.exit(2)
    # Argument validation — FieldSpec patterns with ImportError fallback for
    # standalone operator execution where junction is not on sys.path.
    ...
    if origin_verify_secret:
        custom_headers = {
            ...
            "Items": [
                {
                    "HeaderName": "x-junction-origin-verify",
