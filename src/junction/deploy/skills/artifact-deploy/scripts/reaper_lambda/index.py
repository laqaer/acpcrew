...
NON-persistent deploy whose `expires_at` is in the past, reaps it:
  ...
  3. delete its backend CloudFormation stack junction-deploy-app-<slug>
...
def _stack_site_tag_ok(name, slug):
    """A stack may only be deleted when its junction:site tag equals
    ...
    try:
        ...
        return False
    if tags.get("junction:site") != slug:
        print(json.dumps({"app_reap_stack_tag_mismatch": slug, "stack": name,
                          "actual": tags.get("junction:site", "<missing>")}))
        return False
    return True
...
def _del_stack(slug):
    name = f"junction-deploy-app-{slug}"
...
def _reap_engine_arch(man, slug, now):
    ...
    if dist_id:
        # Verify junction:site tag matches the slug being reaped BEFORE
        # any destructive operation. The tag VALUE is the site_id (NOT "true" —
        # that's on junction:managed). engine.py sets this at creation via
        ...
        try:
            ...
            expected_site = slug
            if tags.get("junction:site") != expected_site:
    ...
    if site_bucket:
        ...
        if not _owner:
            ...
            return "reaping"
        # Verify junction:site tag matches the slug being reaped.
        # engine.py tags buckets at creation: junction:site=<site_id>.
        try:
            ...
            # which deployment's resources the reaper targets.
            expected_site = slug
            if bucket_tags.get("junction:site") != expected_site:
                print(json.dumps({"engine_reap_bucket_tag_mismatch": slug,
                                  "bucket": site_bucket,
                                  "expected": expected_site,
                                  "actual": bucket_tags.get("junction:site", "<missing>")}))
                _quarantine_manifest(slug)
            ...
            if code == "NoSuchBucket":
                ...
                # point at any untagged junction-web-* bucket (e.g. someone
...
def handler(event, context):
    ...
    for slug in _slugs():
        ...
        if man and man.get("reaping"):
            ...
            stack_name = f"junction-deploy-app-{slug}"
            try:
        ...
        # (tagged junction:site). The existing shared-bucket reap path doesn't touch them.
        if man.get("arch") == "engine":
            result = _reap_engine_arch(man, slug, now)
            if result == "reaped":
                reaped.append(slug)
        ...
        if has_stack:
            # Verify the stack's junction:site tag matches the slug
            # being reaped BEFORE deletion — same identity gate as the
            # engine-arch path. deploy-backend.sh tags stacks at
            # creation (junction:site=<slug>); a stack without a matching tag
            # is not provably this deployment's — skip destructive ops.
            if stack_tags.get("junction:site") != slug:
                print(json.dumps({
                    "app_reap_stack_tag_mismatch": slug,
                    "stack": stack_name,
                    "actual": stack_tags.get("junction:site", "<missing>"),
                }))
