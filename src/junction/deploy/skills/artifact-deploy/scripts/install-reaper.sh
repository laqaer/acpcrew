#!/usr/bin/env bash
# Junction One-Click Deploy - install the in-account scheduled reaper ONCE per
...
_STACK_STATUS="$(aws_cli cloudformation describe-stacks --stack-name junction-deploy-reaper \
  --query 'Stacks[0].StackStatus' --output text 2>/dev/null || echo "DOES_NOT_EXIST")"
if [[ "$_STACK_STATUS" == "ROLLBACK_COMPLETE" ]]; then
  echo "error: stack junction-deploy-reaper is in ROLLBACK_COMPLETE state." >&2
  echo "  This dead stack blocks new deployments. Delete it first:" >&2
  echo "    aws cloudformation delete-stack --stack-name junction-deploy-reaper \\" >&2
  echo "      ${PROFILE:+--profile $PROFILE }--region $REGION" >&2
  exit 1
fi
...
echo ""
echo "✅ reaper installed: Lambda junction-deploy-reaper runs $RATE in-account."
...
echo "   Test one run:  aws lambda invoke --function-name junction-deploy-reaper /tmp/reap.json --profile <p> --region <r> && cat /tmp/reap.json"
