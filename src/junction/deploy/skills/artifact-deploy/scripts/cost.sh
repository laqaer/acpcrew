#!/usr/bin/env bash
# Junction One-Click Deploy - cost ESTIMATE from usage metrics (NOT the AWS bill;
...
echo "Junction deploy cost - ESTIMATE from usage (NOT the AWS bill; billing lags ~24h)"
...
if [[ -n "$SLUG" ]]; then
  ...
  if aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" >/dev/null 2>&1; then
    lfn="$(aws_cli cloudformation describe-stacks --stack-name "junction-deploy-app-$SLUG" --query 'Stacks[0].Outputs[?OutputKey==`FunctionName`].OutputValue' --output text 2>/dev/null)"
