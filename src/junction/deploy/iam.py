...
Junction never performs an IAM write. Verification is **read-only reachability only**
...
# The bucket template creates junction-deploy-${AccountId}-${Region}; the
# engine.py HTTP-publish path creates junction-web-<random>. The IAM policy
# must cover BOTH prefixes to work with both code paths.
S3_PREFIX = "junction-deploy-*"
...
BOUNDARY_POLICY_NAME = "junction-deploy-app-boundary"
...
def boundary_policy_document() -> dict[str, Any]:
    ...
    --policy-name junction-deploy-app-boundary --policy-document file://...
    """
    return {
        "Version": "2012-10-17",
        "Statement": [
            ...
            },
            {
                ...
                "Resource": "arn:aws:dynamodb:*:*:table/junction-deploy-app-*",
            },
            {
            ...
            },
            {
                ...
                # junction-managed resources only.
            ...
            },
            {
                ...
                "Resource": "arn:aws:cloudformation:*:*:stack/junction-deploy-app-*/*",
            },
            {
                "Sid": "ReaperStsCap",
                "Effect": "Allow",
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    "arn:aws:dynamodb:*:*:table/junction-deploy-app-*",
                ],
            },
            {
                ...
                "Resource": [
                    "arn:aws:logs:*:*:log-group:/junction-deploy-app/*",
                    "arn:aws:logs:*:*:log-group:/junction-deploy-app/*:*",
                ],
            },
            {
            ...
            },
            {
                ...
                "Resource": "arn:aws:iam::*:role/junction-deploy-app-*",
            },
            {
                "Sid": "ReaperApiGwCap",
                "Effect": "Allow",
...
def policy_document(*, include_custom_domain: bool = False, tier: str = "static") -> dict[str, Any]:
    ...
    Tiers:
      ...
      - fullstack: adds scoped Lambda, API Gateway, DynamoDB, IAM PassRole
        for junction-deploy-app-* stacks, and CloudFront invalidation.
    """
    statements: list[dict[str, Any]] = [
        {
            ...
            "Action": [
                ...
                # and the junction-deploy-logs-* target bucket match the
                # junction-deploy-* resource prefix below.
        ...
        },
        {
            ...
            # junction-deploy-* wildcard, which also matches the append-only
            # audit bucket junction-deploy-logs-* (CloudTrail data events + S3
            ...
            "Resource": [
                "arn:aws:s3:::junction-deploy-logs-*",
                "arn:aws:s3:::junction-deploy-logs-*/*",
            ],
        },
        {
        ...
        },
        {
            ...
            # so the junction:managed tag condition can never match and no
        ...
        },
        {
            ...
            "Resource": "arn:aws:cloudformation:*:*:stack/junction-deploy-*/*",
        },
        {
        ...
        },
        {
            ...
            # creates (junction-deploy-trail-*) — all these actions support the
            # trail resource type.
            ...
            "Resource": "arn:aws:cloudtrail:*:*:trail/junction-deploy-trail-*",
        },
    ]
    statements.append({
        ...
        statements += [
            {
                ...
                "Resource": "arn:aws:lambda:*:*:function:junction-deploy-app-*",
            },
            {
                # CWE-778 access logging: the app-apigw*.yaml stages create an
                # AWS::Logs::LogGroup (/junction-deploy-app/<slug>/apigw*) and
            ...
            {
                # Mutations gated on the junction:managed tag — the
                # templates tag every AWS::ApiGatewayV2::Api at creation, so
                # only Junction-created APIs are mutable/deletable. An
                ...
                "Condition": {
                    "StringEquals": {"aws:ResourceTag/junction:managed": "true"}
                },
            },
            {
            ...
            },
            {
                ...
                # the junction-deploy-app-boundary permissions boundary — this
            ...
            },
            {
                ...
                "Resource": [
                    "arn:aws:iam::*:role/junction-deploy-app-*",
                    "arn:aws:iam::*:role/junction-deploy-reaper*",
                ],
            },
            {
            ...
            },
            {
                ...
                "Resource": "arn:aws:lambda:*:*:function:junction-deploy-reaper*",
            },
            {
                ...
                "Resource": "arn:aws:cloudformation:*:*:stack/junction-deploy-reaper/*",
            },
            {
                ...
                "Effect": "Allow",
                "Action": ["iam:CreateRole"],
                "Resource": "arn:aws:iam::*:role/junction-deploy-reaper*",
                "Condition": {
                    "ArnLike": {
                        "iam:PermissionsBoundary": (
                            f"arn:aws:iam::*:policy/{BOUNDARY_POLICY_NAME}"
            ...
            },
            {
                ...
                "Resource": "arn:aws:events:*:*:rule/junction-deploy-reaper*",
            },
            {
