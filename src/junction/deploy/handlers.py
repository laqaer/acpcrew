...
from junction.deploy.render import render_standalone
from junction.deploy.scan import Finding, is_credential_finding, scan_content, summarize
...
def _reaper_remediation(profile: str, region: str) -> str:
    ...
    (the stack creates an IAM role, which Junction never does itself).
    """
...
def _stage_tree_safe(source: Path, staging_root: Path) -> Path:
    ...
    """
    from junction.hooks import FileTooLargeError, safe_read_file_bytes_nolink
        ...
        try:
            base_rc, base_out, _ = await asyncio.to_thread(
                ...
                ["cloudformation", "describe-stacks",
                 "--stack-name", "junction-deploy-base",
        ...
        if not manifest_bucket and ttl_hours != 0:
            ...
            return 409, {
                "error": (
                    ...
                    "(junction-deploy-base). Use ttl_hours=0 for persistent "
        ...
        if ttl_hours != 0:
            try:
                reaper_rc, _, _ = await asyncio.to_thread(
                    engine.run_aws,
                    ["cloudformation", "describe-stacks",
                     "--stack-name", "junction-deploy-reaper",
            ...
            if reaper_rc != 0:
                return 409, {
                    "error": (
                        ...
                        "(junction-deploy-reaper). Install the reaper "
...
def _check_reaper_installed(profile: str, region: str) -> bool:
    """Check if the junction-deploy-reaper stack exists in the account.
...
async def _handle_pending_list(request: web.Request) -> web.Response:
    ...
    """
    from junction.deploy.pending import list_pending
...
async def _handle_pending_confirm(request: web.Request) -> web.Response:
    ...
    from junction.deploy.pending import add_pending, claim_pending
...
async def _handle_pending_dismiss(request: web.Request) -> web.Response:
    ...
    if denied:
        return denied
    entry_id = request.match_info["id"]
    from junction.deploy.pending import remove_pending
