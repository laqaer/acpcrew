...
from junction.agent import _prompt_path
from junction.agent_discovery import agent_skill_globs
from junction.config.loader import JunctionConfig, workspace_dir_for
from junction.config.paths import kiro_agents_dir
from junction.constants import PRODUCT_NAME
from junction.cron import get_local_tz
...
from junction.quick_prompts import expand_quick_prompt
...
# Docs directory bundled inside the junction package
...
def _role_description(cfg: "JunctionConfig") -> str:
...
def _build_user_profile_section(cfg: "JunctionConfig") -> str:
...
def ui_language_tag(cfg: "JunctionConfig") -> str:
...
def _build_ui_language_section(cfg: "JunctionConfig") -> str:
...
def _load_steering_resources() -> str:
    ...
    """
    try:
        cfg_path = kiro_agents_dir() / "junction.json"
...
async def compress_thread_history(
    ...
    *exclude_last_n* is forwarded to ``conversation_log.recent`` to drop
    ...
    """
    from junction.llm_helpers import (  # circular import
        background_turn,
        stream_and_collect,
    )
    ...
    try:
        async with background_turn(
            sessions, task="thread_compress", agent="junction-lite"
        ) as client:
...
def build_session_replay(
    ...
    *,
    ...
    """Build session replay from Junction's conversation_log.
...
def _skills_injection_plan(agent: str | None, *, is_cc: bool) -> tuple[bool, list[str]]:
    ...
    is_custom = bool(agent) and agent != "junction"
    ...
    *marks* is an ordered list of ``(label, monotonic)`` checkpoints. The first
    ...
    try:
        ...
        for label, ms in timings.items():
            recorder.histogram(
                "junction.context.section.duration",
                ms,
                unit="ms",
...
class ContextBuilder:
    ...
    def _resolve_prompt_templates(prompt: str, session_key: str) -> str:
        ...
        if "{{MAX_SUBAGENTS}}" in prompt:
            # Lazy import: junction.subagent imports this module, so a
            # top-level import would cycle.
            try:
                from junction.subagent import (  # circular import: subagent -> context
                    resolve_max_subagents,
                )

                cap = resolve_max_subagents(JunctionConfig.load())
            except Exception:
                cap = 0
        ...
        if density == "more":
            widget_block = (
                ...
                "and iterate on them via `@junction-core/artifact_save`, "
            ...
            widget_block = (
                ...
                "genuinely warranted.\n\n"
                "## Artifacts\n\n"
                "Persist widgets via `@junction-core/artifact_save` (and friends). "
    ...
    def build_session_context(
        ...
        *context_groups* selects which switchable groups are injected (see
        ...
        For custom agents (non-junction), skills and workspace identity are
        ...
        if minimal_context:
            ...
            parts.append(_build_ui_language_section(JunctionConfig.load()))
            logger.debug(
        ...
        if is_custom:
            ...
            logger.debug("Building session context for junction agent")
        ...
        # links). These are the built-in junction assistant's dashboard/Slack UI
        ...
        # and injecting the junction assistant's mandates on top both
        ...
        # Honour JunctionConfig.timezone (e.g. "Asia/Tokyo") so the LLM sees
        ...
        # Workspace identity — junction-only (custom agents don't use workspaces)
        if not is_custom:
        ...
        # Documentation pointer — junction-only, lightweight reference
        ...
        #    Junction injects the mapped set itself, scoped by ``only=``.
        # 2. No mapping + the junction agent -> the whole catalog (unchanged).
        ...
        # Mapped: CC only (kiro loads them natively). Unmapped: junction only.
            ...
            if lessons_ctx:
                if len(lessons_ctx) > caps.lessons:
                    ...
                    parts.append(
                        ...
                        "also suggest they run `junction learn remove <substring>` from "
                        "their terminal.\n"
    ...
    def build_message(
        ...
        *,
        ...
        if is_new_session:
            ...
            if is_cc:
                # CC gets the SAME Junction persona prompt as kiro — including
            ...
            # history from Junction's conversation_log — provider-agnostic.
            if compressed_history:
                parts.append(
        ...
        if not minimal_context:
            try:
                from junction.resource_status import probe as _probe_resources
