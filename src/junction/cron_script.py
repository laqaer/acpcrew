...
Usage:
    ...
    def run(ctx):
        data = ctx.call_tool("junction-core", "local_knowledge_search", {"query": "..."})
        if not ready(data):
...
from junction.config.loader import config_dir, read_local_secret
from junction.config.paths import kiro_agents_dir
from junction.github_runner import prevalidated_gh_env
...
from junction.security import is_sensitive_path, redact
...
# JUNCTION_OWNER_ID; JUNCTION_INTERNAL_SECRET is handed to scripts via a 0600
# temp file instead of the env (defense-in-depth item 4).
_CRON_ENV_DENY: frozenset[str] = frozenset({"JUNCTION_INTERNAL_SECRET", *_AGENT_DENIED_ENV_KEYS})
...
if TYPE_CHECKING:
    from junction.cron import CronJob
...
class ScriptContext:
    ...
    def __post_init__(self) -> None:
        ...
        # keeps credential and dial target from one resolution; JUNCTION_PORT is the
        ...
        self._port = int(
            os.environ.pop("_JUNCTION_DIAL_PORT", "")
            or os.environ.get("JUNCTION_PORT", "5476")
        )
        ...
        secret_file = os.environ.pop("_JUNCTION_SECRET_FILE", "")
        if secret_file and Path(secret_file).exists():
            ...
            self._secret = os.environ.pop("JUNCTION_INTERNAL_SECRET", "")
        ...
        try:
            self._rpc(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "junction-cron-script", "version": "0.1"},
                },
            )
...
def _resolve_mcp_server(name: str) -> tuple[str, ...] | None:
    ...
    if not cfg_path.exists():
        # Fall back to any junction-named agent spec in the same agents dir.
        for p in kiro_agents_dir().glob("*junction*.json"):
            cfg_path = p
            break
    if not cfg_path.exists():
        return None
...
def _resolve_internal_secret(port: int) -> str:
    ...
    home-wide fallback); the ``JUNCTION_INTERNAL_SECRET`` env var is normally unset,
    ...
    ``_JUNCTION_DIAL_PORT``. Resolving twice -- once for the credential, once for the
    ...
    unrepresentable, which is what the ``_JUNCTION_DIAL_PORT`` mechanism promised.
    """
    env_secret = os.environ.get("JUNCTION_INTERNAL_SECRET", "")
    if env_secret:
        return env_secret
...
def run_script_sandboxed(
    ...
    launcher = (
        ...
        # /tmp dir from sys.path[0] before importing json/os/types/junction —
        ...
        "from junction.cron_script import ScriptContext, Skip, Done, Report\n"
    ...
    fd, launcher_path = tempfile.mkstemp(suffix=".py", prefix="junction_cron_")
    sandbox_cleanup: str | None = None
    # Resolve the dial port ONCE: the credential written below and the
    # _JUNCTION_DIAL_PORT the child dials must come from the same resolution, or a
    ...
    secret_fd, secret_path = tempfile.mkstemp(prefix="junction_secret_")
    try:
        try:
        ...
        clean_env["_JUNCTION_SECRET_FILE"] = secret_path
        ...
        clean_env["_JUNCTION_DIAL_PORT"] = str(dial_port)
