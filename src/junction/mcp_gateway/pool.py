...
* It is redundant with a mechanism that already works. ``gatewayd`` stamps
  ``_meta.junction.caller`` (session key, session type, principal, channel)
...
here historically, but nothing ever populated its ``JUNCTION_PRINCIPAL``
...
# ``JUNCTION_MCP_READ_LIMIT``. Asyncio's stdlib default is 64 KiB which is
...
def _resolve_read_buffer_limit() -> int:
    ...
    Precedence: env var ``JUNCTION_MCP_READ_LIMIT`` (bytes) → config key
    ...
    """
    raw = os.environ.get("JUNCTION_MCP_READ_LIMIT")
    if raw:
        try:
            val = int(raw)
            if val >= 1024:
