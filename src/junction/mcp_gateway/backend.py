...
advertised the ``junction.caller-identity`` capability (pooled operation),
...
from junction.executors import image_executor, maintenance_executor
from junction.mcp_caller import (
...
from junction.mcp_gateway.backend_tmp import (
...
from junction.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES, RESPONSE_SPILL_THRESHOLD_BYTES
from junction.mcp_gateway.spill import maybe_spill_response
...
#    "pool": "example-mcp::junction::...", "pid": 12345, "ok": true}
#
...
# than this. However, shared backends (junction-core) host legitimately long
...
# would kill a high-refcount junction-core backend that is still answering
# fresh pings.
...
def _strip_caller_meta(msg: dict[str, Any]) -> dict[str, Any]:
    ...
    ``params._meta.junction.caller`` unconditionally removed.
    ...
    ``_meta.junction.caller`` in the request. This function is called on
    ...
    # ("junction.caller") — exactly the shape build_caller_meta writes and
    # CallerContext.from_meta reads. Strip that flat key. (An earlier nested
    # "_meta[junction][caller]" strip was a no-op against the real wire format,
...
MCP_APPS_ENV_FLAG = "JUNCTION_MCP_APPS"
...
def _mcp_apps_enabled() -> bool:
    ...
    1. ``JUNCTION_MCP_APPS`` off -> disabled. Absolute kill switch, for tests,
    ...
    Read per-call (``JunctionConfig.load`` is fingerprint-cached, so this is a
    ...
    try:
        ...
        gw = JunctionConfig.load().mcp_gateway
...
def _inject_caller_meta(msg: dict[str, Any], caller: CallerContext) -> dict[str, Any]:
    """Return a shallow copy of ``msg`` with ``params._meta.junction.caller``
...
class Backend:
    ...
    async def forward_from_stub(
        ...
        *,
        ...
        3. Caller-identity injection — ``tools/call`` requests get a
           ``params._meta.junction.caller`` block when the backend
           advertised the capability at initialize time. The block is
           built via :func:`junction.mcp_caller.build_caller_meta` so
    ...
    async def _handle_initialize(
        ...
        # path — without this a stub could forge _meta.junction.caller at init.
        forward_msg = _strip_caller_meta(msg)
        # MCP Apps: advertise the ui extension to the backend (no-op unless
        # the JUNCTION_MCP_APPS flag is on). Must follow the strip so the
    ...
    async def run_stdout_pump(self) -> None:
        ...
        try:
            while True:
                ...
                # session (see junction.imaging MAX_IMAGE_EDGE_PX). Two
    ...
    def _audit_visibility_withhold(
        ...
        in :mod:`junction.mcp_gateway.app_call`) audits every outcome — so the
    ...
    async def _heartbeat_once(self, now: float) -> str:
        ...
        * ``"wedged"`` -- a stub is attached AND BOTH: (1) an in-flight request
          ...
          of ping freshness. Shared backends (junction-core) host long tools
...
async def spawn_backend(
    ...
    1. ``command`` is taken verbatim from ``JUNCTION_MCP_TARGET_<SERVER>`` env
       vars populated at Junction startup by the rewriter from the user's
    ...
    spawn_env = dict(env)
    spawn_env[JUNCTION_SPAWNED_ENV] = JUNCTION_SPAWNED_VALUE
...
async def send_initialize(
    ...
    *,
    ...
    ``capabilities.experimental.junction.caller-identity`` in the response.
    ...
    spawn for them (no cross-tenant injection of ``_meta.junction.caller``).
    ...
    request = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": dict(client_info or {"name": "junction-gateway", "version": "0"}),
        },
    }
...
async def _pump_stderr(reader: asyncio.StreamReader, label: str) -> None:
    ...
    while True:
        ...
        # stack trace) does not land verbatim in the Junction log.
        logger.debug(
            "backend[%s] stderr: %s",
            label,
