"""Warm-pool prewarming for the Junction MCP gateway.
...
A PoolKey carries no channel dimension (see :mod:`junction.mcp_gateway.pool`),
...
#: gateway socket (``$JUNCTION_HOME/mcp-gateway/``) so it shares the
...
class HotKeyStore:
    ...
    def record_outcome(self, *, hit: bool) -> None:
        ...
        get_recorder().counter(
            "junction.mcp.warm_pool.acquire",
