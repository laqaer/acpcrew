...
PoolKey invariant (same reasoning as the ``JUNCTION_SPAWNED_ENV`` marker in
:func:`junction.mcp_gateway.backend.spawn_backend`): the injected value is
...
:mod:`junction.agents_janitor`: ``os.lstat`` snapshots, symlinks are never
