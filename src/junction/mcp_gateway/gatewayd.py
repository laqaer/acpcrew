"""Asyncio unix-socket server for the Junction MCP gateway.

This module is the entry point for ``python -m
junction.mcp_gateway.gatewayd`` and for in-process use by
:class:`junction.mcp_gateway.manager.GatewayManager`.
...
from junction.config.loader import config_dir as _config_dir
from junction.executors import maintenance_executor, subprocess_executor
from junction.mcp_caller import CallerContext
from junction.mcp_caller import _parent_pid as _ppid_fn
from junction.mcp_gateway import credwatch, hazards, socketsec, transport
from junction.mcp_gateway.apps import sweep_spool as apps_sweep_spool
from junction.mcp_gateway.backend import Backend, BackendGone, spawn_backend
from junction.mcp_gateway.backend_tmp import sweep_all_backend_tmp
from junction.mcp_gateway.breaker import CircuitBreaker
from junction.mcp_gateway.hashing import hash_effective_env, non_secret_env
from junction.mcp_gateway.manager import _scrub_sensitive_env, is_credential_env_key
...
from junction.mcp_gateway.resolve_once import resolved_launch
...
from junction.mcp_gateway.shutdown_budget import DRAIN_SECS, POOL_SHUTDOWN_SECS
from junction.mcp_gateway.spill import cleanup_old_spill_files
from junction.mcp_gateway.stub import fallback_counts as stub_fallback_counts
...
from junction.platform_compat import count_open_fds as _shared_count_open_fds
from junction.platform_compat import get_process_start_id as _get_process_start_id
from junction.platform_compat import proc_rss_bytes as _proc_rss_bytes
from junction.sandbox import _PYTHON_ENV_PREFIXES, warm_backend
...
def _emit_backend_acquire_metric(acquire_ms: float, *, warm: bool) -> None:
    """Emit junction.mcp.backend.acquire.duration (best-effort).
...
def _emit_lazy_load_metrics(elapsed_ms: float, *, warm: bool) -> None:
    ...
    try:
        rec = get_recorder()
        rec.counter("junction.mcp.lazy_load.count", attrs={"transport": "stdio"})
        rec.histogram(
...
# ``mcp_gateway.read_buffer_limit_bytes`` / ``JUNCTION_MCP_READ_LIMIT``. Anything
...
_DEFAULT_SOCKET_SUBDIR = "junction"
...
#: ``JUNCTION_MCP_TARGET_<SERVER>`` env vars (accepts legacy ``MC_MCP_TARGET_<SERVER>``
...
def _default_cli_socket_path() -> Path:
    ...
    This is used ONLY when ``python -m junction.mcp_gateway.gatewayd`` is
    invoked without an explicit ``--socket`` flag — a rare operator path,
    typically ad-hoc debugging. The Junction production path always
    derives the socket from ``McpGatewayConfig.socket_path`` / the
    ``default_socket_path()`` in :mod:`junction.mcp_gateway.rewriter`,
    which returns ``$JUNCTION_HOME/mcp-gateway/gateway.sock``.

    Preference order for this CLI fallback:
    1. ``$XDG_RUNTIME_DIR/junction/mcp-gateway.sock`` when XDG is set.
    2. ``$JUNCTION_HOME``/config-dir ``/mcp-gateway/mcp-gateway.sock``.
...
async def run_gatewayd(
    ...
    *,
    ...
    try:
        ...
        # window. Defense-in-depth on top of the 0700 $JUNCTION_HOME directory;
...
def _declared_env_pairs(pool_key: PoolKey, identity_keys: Collection[str]) -> dict[str, str]:
    ...
    """
    try:
        overlay_dir = resolve_overlay_dir(JunctionConfig.load().mcp_gateway.overlay_dir)
    except Exception:
...
def env_target_resolver(pool_key: PoolKey) -> Optional[tuple[str, list[str], dict[str, str], str]]:
    """Look up ``JUNCTION_MCP_TARGET_<SERVER>`` in the process env and return the
    spawn tuple, or ``None`` if no mapping is set.

    Wire format: ``JUNCTION_MCP_TARGET_SLACK_MCP="slack-mcp --stdio"``.
    The server name is upper-cased with ``-`` replaced by ``_``. Env is
    inherited from the gateway process with ``JUNCTION_CHANNEL_ID``
    ...
    :func:`junction.mcp_gateway.manager._scrub_sensitive_env` so even if
    the gateway process somehow inherited credential vars, backends won't.
    """
    base = "JUNCTION_MCP_TARGET_" + pool_key.server_name.upper().replace("-", "_")
    ...
    for key in _PYTHON_ENV_PREFIXES:
        env.pop(key, None)
    # No JUNCTION_CHANNEL_ID is exported into the backend env. It used to be
    ...
    # _meta.junction.caller (see _inject_caller_meta).
...
def _audit_peer_denied(reason: str) -> None:
    ...
    (:mod:`junction.sel`) in addition to the WARNING log line. Wrapped
...
def _audit_peer_allowed(caller: str, pool_label: str) -> None:
    ...
    HMAC-chained security event log (:mod:`junction.sel`) alongside the
...
def _audit_caller_rekey(caller: str, pool_label: str) -> None:
    ...
    connection from effectively unauthorized (no ``_meta.junction.caller`` on
...
def _resolve_peer_identity(peer_pid: int) -> tuple[str, list[int]]:
    ...
    Delegates to the shared :func:`junction.peer_resolve.resolve_peer_identity`
...
async def _handle_connection(
    ...
    if register.get("type") == "app-call":
        ...
        from junction.mcp_gateway.app_call import _audit, handle_app_call
        from junction.mcp_gateway.backend import _mcp_apps_enabled
    ...
    try:
        while True:
            ...
            # for subsequent forwards so ``_meta.junction.caller`` carries the
    ...
    *,
    ...
    if target is None:
        raise _TargetUnknown(
            ...
            "set JUNCTION_MCP_TARGET_<SERVER> env var or pass a target_resolver"
        )
    command, args, env, work_dir = target
...
def _zombie_diagnostic_path() -> Path:
    ...
    ``$JUNCTION_HOME/logs/gatewayd_zombie_diagnostic.jsonl`` so a single
...
def _count_open_fds() -> int:
    ...
    ``junction.process.open_fds`` gauge — so this diagnostic cannot drift
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="Unix socket path to bind. Default: $XDG_RUNTIME_DIR/junction/mcp-gateway.sock",
    )
    p.add_argument(
        "--max-backends",
        dest="max_backends",
