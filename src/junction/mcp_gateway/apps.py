...
pattern-matches ``[junction-mcp-app:<id>]`` on the wire, reads the spool file
...
Everything here is invoked only behind the ``JUNCTION_MCP_APPS`` feature flag
that lives in :mod:`junction.mcp_gateway.backend`; this module has no flag of
...
from junction.mcp_apps_render import MAX_SPOOL_BYTES, SPOOL_SCHEMA_VERSION

logger = logging.getLogger(__name__)

# Env override for the spool directory (tests point this at a tmp dir so they
# never touch a developer's real ~/.junction). Read per-call, never cached.
SPOOL_ENV = "JUNCTION_MCP_APPS_SPOOL"
...
MARKER_PREFIX = "[junction-mcp-app:"
MARKER_SUFFIX = "]"
...
def spool_dir() -> Path:
    """Return the spool directory (``JUNCTION_MCP_APPS_SPOOL`` override, else
    ``$JUNCTION_HOME/mcp-apps``). Not created here — :func:`write_spool` does the
    ...
    reader) — both go through :func:`junction.config.paths.config_dir` so a
    ``JUNCTION_HOME`` override (pods, tests) moves writer and reader together.
