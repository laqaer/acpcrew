...
* The probe connects as ``clientInfo: {"name": "junction-probe"}`` while the
  gateway connects as ``junction-gateway``. A server that negotiates its
...
------------------------------------------------------------------
``junction.caller-identity`` is an extension this project invented; the MCP base
...
CALLER_IDENTITY_CAPABILITY = "junction.caller-identity"
...
def assess(
    ...
    #    ``junction-computer`` advertises yet stays session-bound deliberately
