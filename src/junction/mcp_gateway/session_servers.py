...
from junction.mcp_gateway.rewriter import _WRAPPER_MARKER, _WRAPPER_MARKER_LEGACY
...
def _acp_server_entry(
    ...
    ``-m junction.mcp_gateway.stub`` and anything inserted ahead of that would
...
def pooled_session_servers(
    ...
    ``_meta.junction.caller``. It is deliberately NOT a pool dimension (see
    :mod:`junction.mcp_gateway.pool`), so passing it does not split backends —
