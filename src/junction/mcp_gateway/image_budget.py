...
:mod:`junction.acp.prompt_blocks` entirely -- and because kiro-cli replays the
...
(:func:`junction.imaging.downscale_image_block`) to every image block in a
relayed tool result:
...
unmodified, mirroring :mod:`junction.mcp_gateway.spill`: those frames carry no
