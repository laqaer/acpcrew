"""Junction MCP stub — shim between kiro-cli and the gateway daemon.
...
record to ``$JUNCTION_HOME/logs/stub_fallback.jsonl`` and ``execvpe``\u200bs
...
from junction.mcp_caller import CallerContext, _parent_pid
from junction.mcp_gateway import transport
from junction.mcp_gateway.hashing import hash_command, hash_effective_env
from junction.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES, PoolKey
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    :func:`junction.mcp_gateway.rewriter.rewrite_agents`. ``--real-stub``
    ...
    p.add_argument(
        "--socket",
        default=os.environ.get("JUNCTION_MCP_SOCKET") or os.environ.get("MC_MCP_SOCKET") or _default_socket_path(),
    )
...
def _resolve_channel_id(cli_value: Optional[str]) -> Optional[str]:
    """``--channel-id`` wins; else ``JUNCTION_CHANNEL_ID`` env; else None."""
    return cli_value or os.environ.get("JUNCTION_CHANNEL_ID") or None
...
def _ancestor_pids() -> list[int]:
    ...
    Uses :func:`junction.mcp_caller._parent_pid`, which delegates to
...
def _build_caller_block(channel_id: Optional[str]) -> dict[str, str]:
    """Assemble caller-identity from ``JUNCTION_*`` env vars.

    ``session_key`` is resolved via :meth:`CallerContext.from_env`, which
    reads ``JUNCTION_SESSION_KEY`` first and falls back to the warm-pool PID
...
async def run_bridge(
    ...
    *,
    ...
    tasks = {
        asyncio.create_task(stdin_pump(), name="junction-mcp-stub-stdin"),
        asyncio.create_task(stdout_pump(), name="junction-mcp-stub-stdout"),
        asyncio.create_task(stop_event.wait(), name="junction-mcp-stub-stop"),
        ...
        # check.
        asyncio.create_task(
            writer_failed_evt.wait(), name="junction-mcp-stub-writer-failed"
        ),
    }
    ...
    if peer_supports_ping:
        tasks.add(
            asyncio.create_task(
                _liveness_monitor(), name="junction-mcp-stub-liveness"
            )
        )
...
async def _amain(argv: Optional[list[str]] = None) -> int:
    ...
    _log_level = os.environ.get("JUNCTION_MCP_LOG", os.environ.get("MC_MCP_LOG", "warning")).upper()
    ...
    if not payload.get("session_key"):
        recaller_task = asyncio.create_task(
            ...
            name="junction-mcp-stub-recaller",
        )
    try:
...
def main() -> None:
    """Sync entry point for ``python -m junction.mcp_gateway.stub``."""
    try:
