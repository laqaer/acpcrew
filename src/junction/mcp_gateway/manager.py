"""Supervise the Junction MCP gateway subprocess.

Lifecycle:

1. :meth:`GatewayManager.start` — spawn ``python -m
   junction.mcp_gateway.gatewayd``, wait until the unix socket appears,
...
Gateway failures are non-fatal for Junction. If the daemon crashes, the
...
from junction.env import resolve_krb5_ccname
from junction.mcp_gateway import transport
from junction.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES
from junction.mcp_gateway.shutdown_budget import TOTAL_SHUTDOWN_BUDGET_SECS
from junction.sandbox import _SENSITIVE_ENV_PREFIXES as _SANDBOX_SENSITIVE_ENV_PREFIXES
...
_GATEWAYD_MODULE = "junction.mcp_gateway.gatewayd"
...
# (SLACK_BOT_TOKEN/APP/USER, JUNCTION_OWNER_ID). config/loader.py seeds those
...
class GatewayManager:
    ...
    async def _spawn_once(self) -> None:
        ...
        # Capture gatewayd stdout/stderr to the canonical Junction log path.
