...
``JUNCTION_SESSION_KEY`` stay unwrapped only because nothing lists them in the
stub allowlist.
...
from junction import __version__, platform_compat
from junction.atomic_write import atomic_write
from junction.config.paths import config_dir
from junction.env import mcp_search_path, spec_path_key
from junction.mcp_gateway import STUB_MODULE
from junction.mcp_gateway.hashing import hash_command, is_secret_env_key
...
# — a backend that does not advertise ``junction.caller-identity`` in its
...
_WRAPPER_MARKER = "_junction_mcp_gateway_wrapped"
...
#: The stub is launched as a module by the interpreter running Junction.
...
#: that can import ``junction`` is guaranteed.
#:
...
def _resolve_target_command(
    ...
    :func:`junction.env.mcp_search_path` — literally the same composition the
...
def _build_stub_entry(
    *,
    ...
    wrapped.update({
        ...
        # ``-m junction.mcp_gateway.stub`` leads; the stub's own flags follow.
    ...
    *,
    ...
    for name, entry in servers.items():
        ...
        if name in UNPOOLABLE_SERVERS:
            # Leave unchanged — these bind to JUNCTION_SESSION_KEY.
            new_servers[name] = entry
            continue
        ...
        # would put filesystem IO behind every ``JunctionConfig.load()`` (244
    ...
    for name, entry in inject.items():
        ...
        # command from JUNCTION_MCP_TARGET_<SERVER>, keyed only by server name with
        ...
        if target_env is not None:
            env_key = "JUNCTION_MCP_TARGET_" + alias.replace("-", "_").upper()
...
def _rewrite_inputs_fingerprint(
    ...
    * ``path_env`` / ``pathext`` / ``path_augment`` — feed the
      ...
      :func:`junction.env.mcp_search_path` over an empty spec PATH — the
    ...
    *,
    ...
    Args:
        ...
        sandbox_mode: Value from ``config.agent.sandbox`` — fed through
            so the stub's PoolKey matches Junction's sandbox policy.
    ...
    Returns:
        ...
        * ``target_env``: mapping ``{JUNCTION_MCP_TARGET_<SERVER>: "cmd arg arg"}``
    ...
    # bypasses the gateway unless wrapped (the "junction-lite bypass" class of
...
def _collect_target_env(
    ...
    """Populate ``target_env`` with ``JUNCTION_MCP_TARGET_<SERVER>`` entries
    for every wrapped server in ``mcp_servers``.

    Two kinds of entry are written per wrapped server:

    * ``JUNCTION_MCP_TARGET_<SERVER>`` — first-wins across calls, kept as a
      backward-compatible fallback for any pool key whose
      ``command_args_hash`` has no disambiguated entry.
    * ``JUNCTION_MCP_TARGET_<SERVER>__<command_args_hash>`` — one per distinct
    ...
    for server_name, entry in mcp_servers.items():
        if not isinstance(entry, dict) or not (
            ...
            continue
        env_key = "JUNCTION_MCP_TARGET_" + server_name.replace("-", "_").upper()
        ...
        if target_cmd:
            ...
            if existing is not None and existing != spec:
                logger.warning(
                    "mcp-gateway rewriter: JUNCTION_MCP_TARGET env-key collision on "
...
def default_overlay_dir() -> Path:
    """Return ``$JUNCTION_HOME/mcp-gateway/agents`` (follows ``config_dir``)."""
...
def forward_declared_env_enabled() -> bool:
    ...
    try:
        ...
        return bool(JunctionConfig.load().mcp_gateway.forward_declared_env)
    except Exception:
...
def pool_identity_env_keys() -> frozenset[str]:
    ...
    try:
        ...
        declared = JunctionConfig.load().mcp_gateway.pool_identity_env or []
    except Exception:
