"""Publish-provider interface for sharing Junction artifacts to external
destinations.
...
Mirrors Junction's ``LLMProvider`` ABC pattern.
...
- ``push_version`` is **best-effort** and NEVER raises for upstream failures —
  ...
  fails the Junction update" invariant.
"""
...
class SharingModel:
    ...
    #: Can Junction set sharing via the provider API? When False the UI shows an
...
class SyncModel:
    ...
    - ``authority="junction"`` + ``concurrency="token"`` → MIRROR_GUARDED
      (sha256 guard).
    - ``authority="junction"`` + ``concurrency="lww"`` → MIRROR_LWW
      (blind last-write-wins; no Force-push).
    - ``authority="remote"`` + ``concurrency="crdt"`` → LIVE_CRDT
      (Junction is a participant; no version-conflict UI).
    """

    #: "junction" (MIRROR / collab_mode=mirror) | "remote" (LIVE / collab_mode=live)
    authority: str = "junction"
