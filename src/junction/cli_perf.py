...
output. The sampling itself lives in :mod:`junction.perf_sampler`.
...
from junction import cli_help, platform_compat
from junction.atomic_write import atomic_write
from junction.config.loader import config_dir
from junction.gateway_lock import LOCK_FILENAME
from junction.perf_sampler import (
...
def _read_gateway_pid() -> int | None:
    """PID of a **live** gateway from ``$JUNCTION_HOME/gateway.lock``, or None.
...
def _write_artifact(output: Path, text: str) -> bool:
    ...
    symlink, so a planted ``junction-profile.folded`` link would have the linked
...
def _sample_in_process(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # sys.exit on their normal path (`--call junction.cli_doctor:_doctor`
...
def _sample_out_of_process(args: argparse.Namespace, pid: int) -> int:
    ...
    * py-spy's raw output has to be re-read and redacted anyway (it embeds
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="junction-perf-") as tmpdir:
...
def register_perf_parser(sub: argparse._SubParsersAction) -> None:
    ...
    sample = perf_sub.add_parser(
        ...
        description=(
            "Debug-only stack sampler. Requires JUNCTION_DEBUG=1. With no "
    ...
    sample.add_argument(
        "--output",
        type=Path,
        default=Path("junction-profile.folded"),
        help="Where to write the profile (default: ./junction-profile.folded)",
    )
