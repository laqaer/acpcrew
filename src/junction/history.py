...
from junction.config.loader import JunctionConfig, config_dir
from junction.executors import run_in_embed_pool
from junction.frontmatter import SKILL_UPDATE, frontmatter_value
...
from junction.messaging.link import canonical_key, legacy_key
...
from junction.skills import AUTO_SKILL_MAX_PROCEDURE_CHARS, AutoSkillProvenance
...
from junction.skills_script_validator import validate_skill_script
from junction.vector_memory_constants import (
...
class OnLoopPersistError(AssertionError):
    ...
    ``JUNCTION_STRICT_ON_LOOP_PERSIST`` or ``JUNCTION_DEV_MODE``) any on-loop
...
_allow_on_loop_persist: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "junction_allow_on_loop_persist", default=False
)
...
def _on_loop_persist_strict() -> bool:
    ...
    - ``JUNCTION_STRICT_ON_LOOP_PERSIST`` is truthy (explicit opt-in — the knob
    ...
    - ``JUNCTION_DEV_MODE`` is truthy (developer gateway runs — where all
    ...
    A truthy ``JUNCTION_STRICT_ON_LOOP_PERSIST`` wins; an explicit falsy value
    ...
    """
    explicit = os.environ.get("JUNCTION_STRICT_ON_LOOP_PERSIST", "").strip().lower()
    if explicit in _ON_LOOP_TRUTHY:
        return True
    if explicit in _ON_LOOP_FALSY:
        return False
    return os.environ.get("JUNCTION_DEV_MODE", "").strip().lower() in _ON_LOOP_TRUTHY
...
def on_loop_persist_strict() -> bool:
    ...
    The strictness knob (``JUNCTION_STRICT_ON_LOOP_PERSIST`` /
    ``JUNCTION_DEV_MODE``) governs the on-loop persistence discipline for every
...
def _resolve_retention_days() -> int:
    ...
    """
    try:
        return int(JunctionConfig.load().session.archive_retention_days)
    except Exception:
...
def _cleanup_old_archives(retention_days: int | None = None, base: Path | None = None) -> int:
    ...
    # the expensive JunctionConfig.load() disk read + parse.
    now = _time.time()
    ...
    # re-run the expensive JunctionConfig.load().
    _last_cleanup = now
...
def _strip_skill_frontmatter(text: str | None) -> str:
    ...
    field parser — deliberately outside ``junction.frontmatter``; editing
...
class HistoryConsolidator:
    ...
    def consolidate_session(self, key: str) -> None:
        ...
        and the ``junction consolidate`` CLI command.  Skips if the session
            ...
            if has_vector and self._vector_store is not None:
                ...
                def _prompt_value(e: dict) -> object:
                    ...
                    if str(e.get("key", "")).startswith("lesson."):
                        from junction.vector_memory import _lesson_display_text
    ...
    async def _dedupe_judge(self, prompt: str) -> str:
        ...
        try:
            async with background_turn(
                self._sessions, task="skill_dedupe", agent="junction-lite"
            ) as client:
    ...
    async def _merge_skill_update(
        ...
        try:
            async with background_turn(
                self._sessions, task="skill_merge", agent="junction-lite"
            ) as client:
                text = await stream_and_collect(
                    client, prompt, approval_policy=ToolApprovalPolicy.REJECT_ALL
                )
        ...
        async with contextlib.AsyncExitStack() as stack:
            try:
                client = await stack.enter_async_context(
                    background_turn(
                        self._sessions, task="consolidation", agent="junction-lite"
                    )
                )
            ...
            # scopes the junction-lite session to tools:[] via set_mode, but the
            # Claude Code backend skips set_mode and injects the full
            # junction-core/cron toolset — without REJECT_ALL a background
