...
of consumer import this module at module scope; :mod:`junction.cli_server`
...
does not pull in ``junction.cli_server``.

Patch-namespace contract
------------------------

A large body of tests patches these symbols in the ``cli_server`` namespace
(``junction.cli_server._gateway_owns_port`` and friends). Chain-internal
...
from junction.config.loader import JunctionConfig  # noqa: F401
from junction.config.loader import _DEFAULT_PORT
from junction.constants import CLI_CONSOLE_STEMS
from junction.dashboard.origin import parse_dashboard_url
...
def _patchable(name: str) -> Any:
    ...
    (so ``mock.patch("junction.cli_server.<name>")`` intercepts calls made
...
def _config_url_port() -> int | None:
    ...
    try:
        ...
        # chain functions: tests rebind ``cli_server.JunctionConfig`` wholesale
        # (``patch("junction.cli_server.JunctionConfig")``), which a direct
        # global read here would silently miss.
        cfg = _patchable("JunctionConfig").load()
    ...
    try:
        ...
        # URLs and applies the JUNCTION_PORT override; re-split only to learn
...
def _gateway_owns_port(port: int) -> bool:
    ...
    listener launched as ``/tmp/junction gateway`` would pass it. The proof used
    ...
    **Fails closed** at every step: no sidecar, no recorded pid, a pid that does
    ...
    ``--port`` and ``JUNCTION_PORT`` remain available on such hosts.
    ...
    **Non-POSIX denies outright.** ``process_owner_uid`` cannot report an owner
    ...
    misconfigured ``JUNCTION_HOME``) would let them replace both the marker and
    ...
    ``JUNCTION_PORT``, which is precisely where they were before this fallback
    ...
    if owner is None or owner != os.getuid():
        return False
    return _patchable("_is_junction_process")(recorded)
...
def _marker_port() -> int | None:
    ...
    (see :mod:`junction.instances.run_marker`), so a client with no ``--port``,
    no ``JUNCTION_PORT`` and no port in ``dashboard.url`` can read that instead
    ...
    * **Ambiguity.** With several gateways up there is no basis to pick one, so
    ...
    if len(owned) > 1:
        print(
            ...
            "Pass --port or set JUNCTION_PORT to target a specific gateway.",
            file=sys.stderr,
        )
    return None
...
def resolve_client_port(cli_port: int | None) -> int:
    ...
    2. ``JUNCTION_PORT`` env var if set to a valid integer. Deliberately ABOVE
       the bound-port export: an explicitly-set ``JUNCTION_PORT`` is how a
       caller retargets a child at a DIFFERENT gateway — ``pod exec`` builds a
       client env with ``JUNCTION_PORT=<pod-port>`` precisely so the command
       reaches the pod, while the inherited ``JUNCTION_BOUND_PORT`` still
    ...
    3. ``JUNCTION_BOUND_PORT`` env var if set to a valid integer — the port the
    ...
    that ``junction token`` / ``status`` / ``logout`` / ``stop`` all hit the
    ...
    (e.g. started with ``junction gateway --port 6776``): the parent's own
...
def resolve_client_port_src(cli_port: int | None) -> tuple[int, str]:
    ...
    ``"env"`` (``JUNCTION_PORT``), ``"bound"`` (``JUNCTION_BOUND_PORT``),
    ...
    if cli_port is not None:
        return cli_port, "cli"
    env_port = os.environ.get("JUNCTION_PORT")
    if env_port:
        try:
            ...
            # another path.
            pass
    bound_port = os.environ.get("JUNCTION_BOUND_PORT")
    if bound_port:
        try:
...
def resolve_serving_port() -> int:
    ...
    ``JUNCTION_BOUND_PORT`` is consulted BEFORE ``JUNCTION_PORT``. The client
    resolver reads ``JUNCTION_PORT`` first, which is correct for a CLI client --
    ...
    and ``JUNCTION_PORT`` is a request that may be stale or merely inherited. A
    shell that exported ``JUNCTION_PORT=5476`` and then started a second gateway
    ...
    stay separate. After the bound port, the remaining precedence (``JUNCTION_PORT``
...
def _basename_stem(tok: str) -> str:
    ...
    Lets the venv launchers ``python.exe`` / ``junction.exe`` match the same
    checks as their POSIX ``python`` / ``junction`` counterparts. ``shlex.split``
    ...
    ``D:\\...\\junction.exe`` path and never match. This is host-independent — a
    basename is whatever follows the last ``/`` or ``\\``.

    Module scope rather than nested in :func:`_args_look_like_junction` so
    :func:`junction.cli_server._own_console_script` shares the one definition.
    """
    cleaned = tok.strip('"')
...
def _args_look_like_junction(args: str) -> bool:
    ...
    **precise** (never match an unrelated process that merely mentions
    "junction") while still recognising *every* way the gateway can be spawned.

    Instead of enumerating brittle substring variants (``junction.gateway`` vs
    ``junction gateway`` vs ``junction gateway`` …), we parse the command line
    *structurally* and key on the real module/binary name plus a known server
    subcommand (:data:`_JUNCTION_SERVER_SUBCOMMANDS`). This is deterministic and
    robust to interpreter path, Python version suffix, and whitespace. Two spawn
    shapes are recognised:

    * **Module invocation** — ``<python> -m junction <subcmd>`` (the form used by
    ...
    * **Console script** — the executable token (argv0, after wrappers such
    ...
    Examples::
        ...
        >>> _args_look_like_junction("/x/python3.10 -m junction gateway")
        True
        >>> _args_look_like_junction("python3 -m junction.dashboard")
        True
        >>> _args_look_like_junction("/usr/local/bin/junction up")
        True
        >>> _args_look_like_junction("/usr/local/bin/junction start")
        True
        >>> _args_look_like_junction("python -m junction run /tmp/spec.md")  # task runner
        False
        >>> _args_look_like_junction("vim /tmp/junction-notes.txt")
        False
    """
    ...
    for index, token in enumerate(tokens):
        ...
        if token == "-m" and index + 1 < len(tokens):
            ...
            # misread (e.g. "grep -m junction gateway file"). The match is
            ...
            if interpreter_seen:
                # "junction.gateway" -> ("junction", "gateway"); a bare
                # "junction" -> ("junction", "").
                package, _, dotted_subcmd = tokens[index + 1].partition(".")
                if package == "junction":
                    # Dotted submodule form: ``-m junction.gateway``.
                    if dotted_subcmd in _JUNCTION_SERVER_SUBCOMMANDS:
                        return True
                    # Subcommand-as-argument form: ``-m junction gateway``. The
                    ...
                    # ``-m junction run gateway`` ("gateway" is a file argument
                    # to the task runner) must NOT be treated as a server.
                    if (
                        index + 2 < len(tokens)
                        and tokens[index + 2] in _JUNCTION_SERVER_SUBCOMMANDS
                    ):
                        return True
...
def _is_junction_process(pid: int) -> bool:
    ...
    Windows ``Win32_Process`` WMI — the venv ``junction.exe`` re-execs
    ``python.exe`` so the image name alone is ambiguous there) and defers
    classification to :func:`_args_look_like_junction`.
    ...
    if not out:
        return False
    return _args_look_like_junction(out)
