...
from junction.acp.types import PROVIDER_LABEL_CLAUDE, PROVIDER_LABEL_DEFAULT
...
from junction.agent_discovery import cached_project_agent_names, list_agents
from junction.config.loader import DEFAULT_MODEL, JunctionConfig
from junction.constants import SUBAGENT_COMPLETION_PREFIX
...
from junction.mcp_gateway import STUB_MODULE
from junction.metrics.events import CHILD_PERMISSION_DENIED, emit_counter
...
from junction.resource_status import cached_admission_check
...
from junction.session_surface import has_dashboard_surface
from junction.session_workspace import result_path as _ws_result_path
from junction.slack.format import extract_options
...
from junction.subagent_cost import (
    append_cost_sample,
    compact_cost_log,
    read_learned_cost,
)
...
UNADVERTISED_AGENTS = frozenset({"junction", "junction-conductor"})
...
# Default raised to 900s and made tunable via ``JUNCTION_INJECTION_TIMEOUT``
...
def _resolve_injection_timeout() -> float:
    ...
    val = _env_float("JUNCTION_INJECTION_TIMEOUT", _DEFAULT_INJECTION_TIMEOUT)
...
def _subagent_default_model() -> str:
    ...
    """
    try:
        from junction.config.loader import JunctionConfig, normalize_agent_model

        return normalize_agent_model(JunctionConfig.load().agent.role_models.get("subagent", ""))
    except Exception:
        return ""
...
def _subagent_default_effort() -> str:
    ...
    try:
        ...
        val = JunctionConfig.load().agent.role_efforts.get("subagent", "")
...
#
# Tunable via ``JUNCTION_SUBAGENT_DIGEST_HOLD_SECS``; 0/negative disables the
...
def _digest_hold_secs() -> float:
    try:
        val = float(os.environ.get("JUNCTION_SUBAGENT_DIGEST_HOLD_SECS", ""))
...
def compute_max_subagents(cfg: JunctionConfig) -> int:
...
def resolve_max_subagents(cfg: JunctionConfig) -> int:
...
class SubagentManager:
    ...
    def __init__(
        ...
        try:
            self._global_approval_mode = JunctionConfig.load().agent.approval_mode
        except Exception:
            logger.warning(
                "Failed to load JunctionConfig for approval_mode; defaulting to interactive",
                exc_info=True,
            )
        ...
        try:
            self._result_ttl_secs = int(JunctionConfig.load().agent.subagent_result_ttl_secs)
        except Exception:
        ...
        try:
            self._spawn_stagger_secs = max(
                0.0, float(JunctionConfig.load().agent.subagent_spawn_stagger_secs)
            )
        except Exception:
    ...
    def update_completion_keep(self, mode: str, max_chars: int) -> None:
        ...
        Called from ``api_junction_config_patch`` after the user changes
    ...
    async def _stall_verdict(self, info: SubagentInfo) -> tuple[str, str]:
        ...
        uses (:mod:`junction.acp.liveness`) for ``/proc`` evidence.
    ...
    async def _sigkill_session(self, session_key: str) -> None:
        ...
        :func:`junction.executors.subprocess_executor` via
    ...
    def spawn(
        ...
        Args:
            ...
            agent (str): Agent name override (default: "junction").
        ...
        try:
            min_mem = JunctionConfig.load().agent.spawn_min_memory_gb
        except Exception:
            min_mem = 4.0
        ...
        if cwd:
            try:
                allowed_roots = JunctionConfig.load().agent.subagent_cwd_allowed_roots
            except Exception:
    ...
    async def _run_inner(self, info: SubagentInfo, session_key: str) -> None:
        ...
        # Mirrors junction.dashboard.chat_runner._pending_tools.
        ...
        try:
            # circular import: reached while junction.slack.handler is still
    ...
    def _provider_label_of(provider: object) -> str:
        ...
        from junction.providers.acp import provider_label
