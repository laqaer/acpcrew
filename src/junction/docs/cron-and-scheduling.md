...
```bash
junction cron add "pipeline-check" "check pipeline health" --every 1800
junction cron add "weekday-9am" "check tickets" --cron "0 9 * * MON-FRI" --approval-mode auto
junction cron update <job-id> --name "new name" --message "new prompt" --approval-mode auto
junction cron list
junction cron remove <id>
```
...
```bash
junction cron add "auto-check" "check pipeline" --every 300 --approval-mode auto
```
...
```bash
junction cron add "standup-prep" "prepare standup notes" --cron "0 9 * * MON-FRI" --strict-schedule
junction cron update <job-id> --strict-schedule
```
...
```
junction cron adopt <id> --session-of chat-3-1712793600   # hand it to a session
junction cron adopt <id> --session-of dashboard:chat-3-1712793600
junction cron adopt <id> --release                        # back to operator-only
```
