...
PATCH  /api/webhooks/tokens/{source_id}  {agent?, enabled?, label?}
DELETE /api/webhooks/tokens/{source_id}
```
...
curl -X POST http://127.0.0.1:5476/api/hooks/agent \
  ...
  -H "Authorization: Bearer $JUNCTION_WEBHOOK_TOKEN" \
...
`X-Junction-Token: <token>` is accepted as an equivalent.
...
| `401` | `{"error": "…"}` | Signing failure on a token that requires signatures: `X-Junction-Timestamp` or `X-Junction-Signature` missing, timestamp unparseable or more than 300 seconds from now, digest mismatch, or a signature already seen inside the window (replay). Each cause has its own `error` string. |
| `403` | `{"error": "Forbidden"}` | Not produced by this endpoint any more. If you see it, you are hitting a different path or a proxy in front of the gateway. |
...
```
X-Junction-Timestamp: 1785372000            # unix seconds, integer
X-Junction-Signature: sha256=<hex hmac>     # lowercase hex
```
...
SIG=$(printf '%s.%s' "$TS" "$BODY" \
  | openssl dgst -sha256 -hmac "$JUNCTION_WEBHOOK_SIGNING_SECRET" -hex \
  | sed 's/^.* //')
