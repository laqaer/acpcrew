...
The agent calls the `suggest_followup` MCP tool (junction-core) with up to three
items:
  ...
  "items": [
    {
      ...
      "prompt": "In src/junction/dashboard/handlers/files.py, add a per-caller token-bucket limiter to api_file_upload ... (full standalone instruction)",
...
| Layer | Path |
| --- | --- |
| Tool declaration + dispatch | `src/junction/mcp_core.py` |
| Arg schema | `src/junction/validation.py` |
| Card endpoint | `src/junction/dashboard/chat_handlers.py` |
| Worktree endpoint | `src/junction/dashboard/handlers/worktree.py` |
