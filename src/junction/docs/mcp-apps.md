...
| Condition | Result |
|---|---|
| `JUNCTION_MCP_APPS` = `0`/`false`/`no`/`off` | disabled (explicit kill-switch, wins over everything) |
| stored `mcp_gateway.apps_enabled` = `false` | disabled, even with `JUNCTION_MCP_APPS` on — a released opt-out is still honoured |
| config unreadable | disabled (fails closed — the stored preference cannot be confirmed) |
| `JUNCTION_MCP_APPS` = `1`/`true`/`yes`, no stored opt-out | enabled (explicit override — tests, e2e harness) |
...
Check `JUNCTION_MCP_APPS` is not set to an off value and that
...
3. The gateway writes the payload to a spool file at
   `$JUNCTION_HOME/mcp-apps/<uuid4hex>.json` and injects an opaque marker
   `[junction-mcp-app:<uuid4hex>]` into the tool result *text*.
...
|---|---|
| Tool output renders as plain text | the server has no stub in MCP Management, or `JUNCTION_MCP_APPS` is set to an off value |
