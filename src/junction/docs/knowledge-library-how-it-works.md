...
Each chunk is sent independently to the `junction-knowledge` agent (Claude Haiku 4.5, cheapest model at 0.4x rate). The LLM returns structured JSON:
...
Embeddings share the vector-memory setting in `~/.junction/config.json` —
