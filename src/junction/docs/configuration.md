...
```bash
junction config get                    # print full config
junction config get agent.model        # print a specific value
junction config set agent.model auto   # set a value (auto type detection)
junction config set --local agent.model auto   # write config.local.json instead
...
The dashboard port is **not** a config key: set `JUNCTION_PORT` instead.
...
`JUNCTION_KAS_NODE` / `JUNCTION_KAS_SCRIPT` to point elsewhere.
...
| `agent.log_level` | Persistent log level for the `junction` logger, applied at startup. The `--verbose` CLI flag overrides it | `"WARNING"` |
...
| `slack.command` | Slash-command name | `"junction"` |
...
Only the owner (`JUNCTION_OWNER_ID`) is authorized to interact over Slack.
