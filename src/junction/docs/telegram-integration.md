...
- `/yolo [on|off|renew]` — report or change the auto-approve grant. This is the
  SAME process-wide grant the dashboard toggle and Slack's `/junction yolo`
...
- `/sessions` — the ten most recent conversations, newest first, with a mark for
  whichever is live. Read-only: opening one is `/junction dashboard`. **Direct
  message only**, like `/junction dashboard`: the listing names every conversation
