...
3. Click **Get IAM policy** and apply the generated least-privilege policy **yourself** to a
   ...
   also emits a permissions-boundary policy that must exist as `junction-deploy-app-boundary`
...
behind one shared per-account base stack (`junction-deploy-base`: one private bucket plus one
...
The reaper only touches resources that carry both the `junction:site=<id>` and
`junction:managed=true` tags and match the managed naming scheme. It verifies the tag against the
...
- Per-site bucket names are random and opaque (`junction-web-<hex>`) and are hidden from the
...
`aws:ResourceTag/junction:managed=true`, and Junction tags each resource at creation. An unrelated
...
scoped to the `junction-deploy-*` prefix, and the audit-log bucket is covered by an explicit Deny.
