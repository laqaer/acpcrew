...
This regenerates `~/.kiro/agents/junction.json` while preserving your own
...
1. Check `~/.kiro/agents/junction.json` for `junction-core`, `junction-cron`,
   and `junction-computer` under `mcpServers`, and for the matching `@`-prefixed
   entries under `tools`
...
```bash
junction config set agent.mcp_registry_mode true
junction restart
```

Your administrator also has to add `junction-core`, `junction-cron` and
`junction-computer` to the registry under those exact names. `junction doctor`
...
```bash
junction status                          # is the gateway running?
...
on another port with `JUNCTION_PORT`.
...
- Mirrored or airgapped hosts: point `JUNCTION_EMBED_MODEL_URL` (or
...
- If `JUNCTION_SKIP_MODEL_DOWNLOAD=1` is set, the model never downloads. Unset
...
Point `memory.embed_model_path` (or `JUNCTION_EMBED_MODEL_PATH`) at an absolute
...
`JUNCTION_EMBED_MODEL_PATH` is set, the dashboard refuses to change the model,
...
```bash
junction config set agent.completion_keep tail        # keep the conclusion
...
```bash
junction config set agent.subagent_result_ttl_secs 21600   # 6 hours
```
