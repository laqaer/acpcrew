...
the reverse. `junction.messaging` imports nothing from `junction.slack`.
...
`junction/messaging/transport.py`. One implementation per channel.
...
`junction/messaging/driver.py`. Shared by every channel — you do **not**
...
`junction/messaging/renderer.py`. One implementation per channel. The
...
`junction/messaging/link.py`. Session keys are **namespaced by channel** so two
...
| Path | Role |
|---|---|
| `src/junction/messaging/transport.py` | `MessagingTransport`, `TransportCapabilities`, `InboundMessage` |
| `src/junction/messaging/driver.py` | `TurnDriver` + approval ladder + redaction |
| `src/junction/messaging/renderer.py` | `Renderer` ABC, `OutputEvent`, `chunk_text` |
| `src/junction/messaging/link.py` | `ChannelLink`, `session_key`, `canonical_key` |
| `src/junction/slack/transport.py` | Slack `MessagingTransport` |
| `src/junction/slack/renderer.py` | Slack `Renderer` |
| `src/junction/slack/transport_dispatch.py` | Slack dispatch glue |
| `src/junction/config/loader.py` | `MessagingConfig` (`use_transport`) |
