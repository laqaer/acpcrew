...
Every core or app metric flows through this facade so Junction's namespace and
privacy guardrails (``validate_name`` / ``validate_attrs`` / ``redact`` from
``junction.metrics.schema``) run *before* the value reaches an OTEL instrument.
...
the first-party ``junction.metrics.schema`` guardrails. No Amazon-internal
imports.
"""
...
from junction.metrics.schema import validate_attrs, validate_name
