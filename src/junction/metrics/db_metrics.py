"""Per-query timing for Junction's SQLite stores.

``junction.db.query.duration`` — a histogram (ms) of how long individual store
operations take, labelled by which store and which kind of operation.

Why this exists: route latency (``junction.gateway.request.duration``) tells you
...
``telemetry.enabled`` (or ``JUNCTION_TELEMETRY``) is set, so an unopted host pays
...
DB_METRIC = "junction.db.query.duration"
