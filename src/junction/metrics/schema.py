...
OSS-CLEAN: stdlib only, plus the first-party ``junction.security`` scrubbers.
...
RESOURCE_ATTR_PROCESS_START_TIME = "junction.process.start_time"
...
def redact(value: str) -> str:
    ...
    """
    # First-party scrubbers (junction.security): scan for exfiltration URLs
...
def validate_name(name: str, *, app_id: Optional[str] = None) -> str:
    ...
    Args:
        ...
        app_id: If provided, the caller is an app; the name MUST start with
                ``app.<app_id>.`` and MUST NOT start with ``junction.`` or
                ``gen_ai.``.
    ...
    if app_id is not None:
        ...
        if name.startswith(NS_CORE):
            raise ValueError(f"App {app_id!r} cannot emit metrics in the junction.* namespace")
        ...
        # Core callers: must use junction.* or gen_ai.*
