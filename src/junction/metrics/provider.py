...
# Junction declares opentelemetry-sdk as a required dependency, so
...
# top-level import here would brick the ENTIRE gateway (and `junction
...
# subprocesses, `junction --version`) while telemetry is off by default and the
...
def _load_otel() -> bool:
    ...
    try:
        ...
        from junction.metrics.local_exporter import JsonlMetricExporter as _JsonlMetricExporter
    except ImportError:
        return False
...
# Instrument name -> boundaries. This map is the COMPLETE set of junction
...
_HISTOGRAM_BUCKETS_MS: dict[str, list[float]] = {
    "junction.gateway.request.duration": _FAST_BUCKETS_MS,
    "junction.db.query.duration": _FAST_BUCKETS_MS,
    "junction.mcp.backend.acquire.duration": _FAST_BUCKETS_MS,
    "junction.skill.lazy_load.duration": _FAST_BUCKETS_MS,
    ...
    # collapsing into +Inf.
    "junction.context.section.duration": _FAST_BUCKETS_MS,
    ...
    "junction.telegram.api.duration": _FAST_BUCKETS_MS,
    "junction.session.startup.duration": _STARTUP_BUCKETS_MS,
    ...
    "junction.chat.first_token.duration": _STARTUP_BUCKETS_MS,
    "junction.mcp.lazy_load.duration": _STARTUP_BUCKETS_MS,
    "junction.gateway.boot.duration": _STARTUP_BUCKETS_MS,
    "junction.turn.duration": _TURN_BUCKETS_MS,
    "junction.watchdog.idle.duration": _WATCHDOG_IDLE_BUCKETS_MS,
}
...
# Consent is not fixed for the life of the process. `junction config set
...
TELEMETRY_ENV_VAR = "JUNCTION_TELEMETRY"
_ENV_TRUTHY = frozenset({"1", "true", "yes", "on"})
_ENV_FALSY = frozenset({"0", "false", "no", "off"})


def env_pin() -> Optional[bool]:
    """Return the ``JUNCTION_TELEMETRY`` pin, or ``None`` when the var is unset.
...
def _build_recorder() -> _Build:
    ...
    try:
        cfg = JunctionConfig.load().telemetry
    ...
    try:
        ...
        try:
            from junction.metrics.process_gauges import register_process_gauges
...
def _build_otlp_reader(cfg: object) -> Optional["_ReaderT"]:
    ...
    in the separate ``junction[otlp]`` package extra (install with
    ``pip install "junction[otlp]"``), not the hard dependency set. If a host
...
def _consent_worker(generation: int) -> None:
    ...
    try:
        try:
            consent: Optional[bool] = _consent_enabled(JunctionConfig.load().telemetry)
        except Exception as exc:
...
def _schedule_consent_check_locked() -> None:
    ...
    threading.Thread(
        ...
        name="junction-telemetry-consent",
        daemon=True,
    ).start()
...
def _reap_readers_detached(readers: list) -> None:
    ...
    threading.Thread(
        target=_reap, name="junction-telemetry-reap", daemon=True
    ).start()
