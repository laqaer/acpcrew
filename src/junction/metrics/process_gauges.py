...
Instruments (all in the core ``junction.`` namespace, validated against
``schema.validate_name`` at registration):

===============================================  ==========  ====================
name                                             kind        source
===============================================  ==========  ====================
``junction.process.threads.python``              gauge       ``threading.active_count()``
``junction.process.threads.os``                  gauge       ``/proc/self/task`` (Linux only)
``junction.process.open_fds``                    gauge       ``/proc/self/fd`` / ``/dev/fd``
``junction.process.memory.rss_bytes``            gauge       ``platform_compat.proc_rss_bytes``
``junction.process.memory.peak_rss_bytes``       gauge       ``platform_compat.proc_peak_rss_bytes``
``junction.process.cpu.seconds``                 counter     ``platform_compat.proc_cpu_seconds``
``junction.process.gc.collections``              counter     ``gc.get_stats()`` per generation
``junction.process.gc.collected``                counter     ``gc.get_stats()`` per generation
``junction.process.gc.uncollectable``            counter     ``gc.get_stats()`` per generation
===============================================  ==========  ====================
...
GAUGE_THREADS_PYTHON = "junction.process.threads.python"
GAUGE_THREADS_OS = "junction.process.threads.os"
GAUGE_OPEN_FDS = "junction.process.open_fds"
GAUGE_RSS = "junction.process.memory.rss_bytes"
GAUGE_PEAK_RSS = "junction.process.memory.peak_rss_bytes"
COUNTER_CPU_SECONDS = "junction.process.cpu.seconds"
COUNTER_GC_COLLECTIONS = "junction.process.gc.collections"
COUNTER_GC_COLLECTED = "junction.process.gc.collected"
COUNTER_GC_UNCOLLECTABLE = "junction.process.gc.uncollectable"
