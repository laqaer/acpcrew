...
API server):
  ...
  * ``junction.gateway.boot.duration`` — a histogram (ms) of how long
  ...
  * ``junction.gateway.request.duration`` — a histogram (ms) of per-request
...
# --- Metric names (core junction.* namespace; validated by the recorder) ------
BOOT_METRIC = "junction.gateway.boot.duration"
REQUEST_METRIC = "junction.gateway.request.duration"
...
def record_boot_to_ready(
    ...
    """Emit the ``junction.gateway.boot.duration`` histogram (best-effort).
