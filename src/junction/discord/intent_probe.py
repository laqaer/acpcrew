...
This is the Discord analogue of :mod:`junction.slack.scope_probe`, and it
...
* **Never fatal.** Every failure path (no token, rejected token, HTTP error,
  ...
  reason instead of raising, because its caller is ``junction doctor``, and a
...
caller's own event loop (``junction doctor`` gives it a throwaway
``asyncio.run``), so it never borrows the gateway's loop or the live
:class:`~junction.discord.client.DiscordClient` session, whose REST ladder
