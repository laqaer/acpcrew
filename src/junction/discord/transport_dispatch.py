...
from junction.discord.attachments import (
...
from junction.discord.session_resume import (
...
from junction.messaging.commands import stop_running_turn
...
from junction.messaging.renderer import Renderer, SilentRenderer
from junction.messaging.transport import InboundMessage
from junction.messaging.upload_gate import live_dashboard_slot, uploads_restricted
from junction.safety_override import describe_grant_lifetime, safety_override
...
if TYPE_CHECKING:
    ...
    from junction.discord.client import DiscordClient, DiscordInteraction
...
# Canonical kiro-cli agent fallback so Discord sessions load junction-core
...
class DiscordDispatcher:
    ...
    def _render_config(self) -> tuple[bool, bool]:
        ...
        """
        try:
            discord_cfg = JunctionConfig.load().discord
    ...
    async def _handle_link(
        ...
        live in the shared
        :func:`~junction.messaging.link.rebind_conversation_location`; this
    ...
    async def _surface_own_session(self) -> None:
        ...
        from junction.dashboard.channel_slots import surface_dispatcher_session
    ...
    async def _handle_status(self, reply: "ReplyFn") -> None:
        """Report runtime stats, from the same source Slack's ``/junction status`` uses.
