"""Junction kiro-cli agent configuration.

Generates and installs ``junction.json`` into ``~/.kiro/agents/``.

Configuration files (edit these, then ``junction setup --agent-only``):

  ``src/junction/config/defaults.json``
...
from junction import agent_state, platform_compat
from junction.agent_discovery import _read_agent_spec
from junction.agent_files import (
    AGENT_FILENAME,
)
from junction.agent_files import CONDUCTOR_AGENT_FILENAME as _CONDUCTOR_AGENT_FILENAME
from junction.agent_files import HEARTBEAT_AGENT_FILENAME as _HEARTBEAT_AGENT_FILENAME
from junction.agent_files import KNOWLEDGE_AGENT_FILENAME as _KNOWLEDGE_AGENT_FILENAME
from junction.agent_files import LITE_AGENT_FILENAME as _LITE_AGENT_FILENAME
from junction.agent_files import (
    OWNED_KIRO_AGENT_FILES,
    REQUIRED_KIRO_AGENT_FILES,
)
from junction.agent_files import RESEARCH_AGENT_FILENAME as _RESEARCH_AGENT_FILENAME
from junction.atomic_write import replace_with_retry
from junction.config import config_dir
from junction.config import config_path as _mc_config_path
...
from junction.mcp_cleanup import purge_deleted_proxy_from_config
from junction.mcp_provenance import without_marker
from junction.mcp_utils import kiro_oauth_wire_entry, mcp_server_alias
from junction.platform import current_context
from junction.platform import redact_via_context as redact
from junction.platform import safe_context_call
...
from junction.sel import (  # circular import: sel imports config which imports agent
    SecurityEvent,
    sel,
)
from junction.validation import _AGENT_NAME_RE
...
_MAIN_AGENT_NAME = "junction"
# Cheap Claude Code model for Junction's background agents (lite / heartbeat).
...
def _background_agent_model() -> str:
    ...
    try:
        ...
        return JunctionConfig.load().agent.resolve_model("background")
    except Exception:
...
# Bundled fallback — inside the junction.config package
_BUNDLED_CFG_DIR = Path(__file__).resolve().parent / "config"


def _project_dir() -> Path | None:
    """Return the project root from JUNCTION_PROJECT_DIR, or None."""
    val = os.environ.get("JUNCTION_PROJECT_DIR")
    if val:
        p = Path(val)
        if p.is_dir():
            return p
...
# junction binary path — resolved lazily to handle gateway restarts
# where PATH may not include the virtualenv at import time.
_JUNCTION_BIN: str | None = None
...
def _bin_is_usable(path: Path) -> bool:
    ...
    makes ``ensure_junction_on_path`` publish a machine-wide ``junction`` that is
    ...
    if not head.startswith(b"#!"):
        ...
        # batch shim (`bin\junction.cmd` starts with `@`). The shim DOES name
        # its interpreter (`"%~dp0..\python.exe"`), but we choose not to parse
        # the batch body here; the consumer that spawns it
        # (`_junction_mcp_invocation`) resolves and validates that sibling
    ...
    bin_dir = path.parent
    # `<venv>/bin/junction` (already inside the venv) vs `<root>/bin/junction`
...
def _launcher_works(path: Path) -> bool:
    ...
    ``~/.local/bin/junction`` that points somewhere ELSE is a working install's
    launcher — which must be left alone — or a dead one we should replace.

    Deliberately not folded into ``ensure_junction_on_path``'s gate on its OWN
    resolved target: that gate additionally requires an absolute path, and its
    interpreter check already happened inside :func:`_resolve_junction_bin`.
    """
    return path.is_file() and os.access(path, os.X_OK) and _bin_is_usable(path)


def _junction_bin_subpath(root: Path) -> Path:
    """The console-script path under an install ``root`` for this OS.

    A venv exposes its entry points under ``bin/junction`` on POSIX but
    ``Scripts/junction.exe`` on Windows — pip generates a ``.exe`` launcher
    ...
    ``junction-cron`` / ``junction-core`` MCP servers (``command not found:
    .../bin/junction``). Branch on the platform so both layouts resolve.

    On Windows a relocatable ``bin\\junction.cmd`` shim is preferred over the
    pip-generated ``Scripts\\junction.exe`` when it exists. The desktop bundle
    ...
    in sync. Plain pip installs ship no ``bin\\junction.cmd``, so they keep
    resolving ``Scripts\\junction.exe`` via the fallback.
    """
    if platform_compat.IS_WINDOWS:
        cmd_shim = root / "bin" / "junction.cmd"
        if cmd_shim.is_file():
            return cmd_shim
        return root / "Scripts" / "junction.exe"
    return root / "bin" / "junction"


def _resolve_junction_bin() -> str:
    """Resolve the absolute path of the ``junction`` executable.

    Resolution order (first existing + executable wins):

    1. A sibling ``.venv`` entrypoint, for a source-tree install (an editable
       install next to its own venv, e.g. ``project/src/junction`` plus
       ``project/.venv``). Bounded by the first ``pyvenv.cfg`` walking up, so a
       pip-into-venv install falls through to step 2 instead.
    2. Same install as the current process: walk up from ``junction.__file__``
       looking for a sibling console script (see
       :func:`_junction_bin_subpath` for the per-OS layout). Covers venv-based
       ...
       ``site-packages``.
    3. ``shutil.which('junction')`` — respects PATH order.
    4. Bare ``"junction"`` — last resort, may fail but surfaces the problem
    ...
    """
    global _JUNCTION_BIN
    if _JUNCTION_BIN:
        return _JUNCTION_BIN
    ...
    #    install with a sibling .venv directory, e.g. project/src/junction
    #    + project/.venv/bin/junction).
    ...
    try:
        # Circular import: junction.agent is loaded during junction
        # package initialization, so importing junction at module level
        # would create a circular dependency. Deferring here resolves
        # after the package is fully loaded.
        import junction as _mc  # noqa: PLC0415  circular import

        pkg_dir = Path(_mc.__file__).resolve().parent
        for parent in pkg_dir.parents:
            venv_candidate = _junction_bin_subpath(parent / ".venv")
            if _usable(venv_candidate):
                _JUNCTION_BIN = str(venv_candidate)
                return _JUNCTION_BIN
            if (parent / "pyvenv.cfg").exists():
                break
    except Exception:
        logger.debug("junction venv bin check failed", exc_info=True)
    ...
    try:
        ...
        pkg_dir = Path(_mc.__file__).resolve().parent
        for parent in pkg_dir.parents:
            candidate = _junction_bin_subpath(parent)
            if _usable(candidate):
                _JUNCTION_BIN = str(candidate)
                return _JUNCTION_BIN
            if (parent / "pyvenv.cfg").exists():
                break  # reached venv root without finding the binary
    except Exception:
        logger.debug("junction bin walk failed", exc_info=True)

    # 3. PATH lookup (also validated)
    found = shutil.which("junction")
    ...
    logger.warning(
        "Could not resolve junction binary to an existing file; "
        "falling back to bare 'junction' (MCP probes may fail)"
    )
    return "junction"


def _managed_mcp_env() -> dict[str, str]:
    """Env every managed Junction MCP server is launched with.

    Pins ``JUNCTION_HOME`` when the gateway is running under an override, because
    ...
    return {"JUNCTION_HOME": str(override)} if override else {}
...
def _mcp_registry_mode() -> bool:
    ...
    ``config.local.json`` deep-merges over it and is where ``junction config set
    ...
    try:
        ...
        return JunctionConfig.load().agent.mcp_registry_mode is True
    except Exception:
...
def _junction_mcp_invocation(subcommand: str) -> tuple[str, list[str]]:
    """Resolve a CWD- and shebang-independent invocation for a built-in
    MCP server (``junction-cron`` / ``junction-core``).

    Prefers a standalone ``junction`` binary when one resolves. Falls back
    to ``<interpreter> -m junction <subcommand>`` when
    :func:`_resolve_junction_bin` cannot find a usable standalone binary --
    ...
    ``_resolve_junction_bin`` returns the bare ``"junction"`` sentinel, the
    command fails to validate, and the server gets dropped from
    ``junction.json`` on every config refresh.
    ...
    junction`` dispatches the same CLI as the ``junction`` console script.

    A resolved ``bin\\junction.cmd`` (the Windows bundle's relocatable shim,
    see :func:`_junction_bin_subpath`) is unwrapped to the sibling
    interpreter — ``<root>\\python.exe -P -s -m junction <sub>`` — instead of
    ...
    """
    bin_path = _resolve_junction_bin()
    if bin_path == "junction":  # unresolved sentinel from _resolve_junction_bin
        return sys.executable, ["-m", "junction", subcommand]
    if bin_path.endswith(".cmd"):
        ...
        if _interpreter_runnable(interpreter):
            ...
            # so with ``-m`` alone a cloned repo carrying a ``junction/``
            ...
            return str(interpreter), ["-P", "-s", "-m", "junction", subcommand]
        return sys.executable, ["-m", "junction", subcommand]
    return bin_path, [subcommand]


def _computer_use_spec_gate() -> bool:
    """Whether ``junction-computer`` belongs in an EMITTED agent spec.
...
_MANAGED_MCP_SERVERS: dict[str, dict] = {
    "junction-cron": {"invocation_fn": lambda: _junction_mcp_invocation("mcp-cron")},
    "junction-core": {"invocation_fn": lambda: _junction_mcp_invocation("mcp-core")},
    ...
    "junction-computer": {
        "invocation_fn": lambda: _junction_mcp_invocation("mcp-computer"),
    ...
    # entry nor an ``@junction-dashboard`` ref in ``tools`` — and kiro-cli loads a
    ...
    "junction-dashboard": {
        "invocation_fn": lambda: _junction_mcp_invocation("mcp-dashboard"),
        "opt_in": True,
    },
}
...
def ensure_junction_on_path(
    bin_dir: Path | None = None, *, claim_existing: bool = False
) -> str | None:
    """Ensure a ``junction`` launcher is reachable on the user's PATH.

    The source ``install.sh`` symlinks ``~/.local/bin/junction`` → the venv
    entry point, but install paths that don't run it (notably the packaged
    Electron app) leave no ``junction`` on PATH — breaking the ``junction``
    terminal command. This mirrors that symlink step in Python so it runs from
    ``junction setup``. Best-effort and idempotent:

    * No-op if ``junction`` already resolves on PATH to the same binary.
    ...
    * Otherwise (re)create ``<bin_dir>/junction`` → the resolved binary.
    ...
    Args:
        ...
        claim_existing: Take the name over from another install's working
            launcher. ``junction setup`` passes True because the user named this
    ...
    # OSError [WinError 1314] mid-wizard. pip's Scripts\junction.exe console
    ...
    # filesystem attempt so `junction setup` never prints a traceback for it.
    if platform_compat.IS_WINDOWS:
        return None

    target = _resolve_junction_bin()
    # Nothing concrete to point at — bare "junction" or a non-executable file.
    ...
    # `junction` stops working EVERYWHERE — not just in the tree that went away.
    # Any process running out of a worktree's venv (a pod gateway, a dev run, a
    # `junction setup` invoked from that tree) resolves its own venv entrypoint
    ...
    if _in_linked_git_worktree(Path(target).resolve()):
        logger.info(
            "Not installing a junction launcher: %s is inside a linked git worktree, "
            "which is ephemeral (removing the worktree would break `junction` "
            "machine-wide). Install from your primary clone, or link it yourself: "
            "ln -sfn <clone>/.venv/bin/junction ~/.local/bin/junction",
            target,
        )
        return None
    ...
    if _in_ephemeral_tree(Path(target).resolve()):
        logger.info(
            "Not installing a junction launcher: %s is inside an ephemeral tree (an "
            ...
            "`junction` on PATH, or link a persistent install yourself.",
            target,
        )
        return None

    # Already reachable on PATH as the same binary? Then there's nothing to do.
    existing = shutil.which("junction")
    if existing and os.path.realpath(existing) == os.path.realpath(target):
        return None

    # Ownership, checked on PATH before the target path: a working `junction`
    # ANYWHERE on PATH already belongs to some install — a pipx bin dir, a distro
    # package, /usr/local/bin — and writing <bin_dir>/junction would shadow it or
    ...
    # sitting AT <bin_dir>/junction while <bin_dir> is not on PATH at all.
    if existing and not claim_existing:
        existing_on_path = Path(os.path.realpath(existing))
        if _launcher_works(existing_on_path):
            logger.info(
                "Leaving `junction` on PATH alone: %s -> %s still works and belongs "
                "to another install. Run `junction setup` from the install you want "
    ...
    try:
        ...
        if link.is_symlink() or link.exists():
            ...
            #
            # An explicit `junction setup` DOES claim the name: the user named
            ...
            if not claim_existing and _launcher_works(existing_target):
                logger.info(
                    "Leaving the existing junction launcher alone: %s -> %s still "
                    "works and belongs to another install. Run `junction setup` "
        ...
        logger.warning("Could not create junction shim at %s: %s", link, exc)
        return None
    logger.info("Linked junction shim: %s -> %s", link, target)
    return str(link)


# One-time migrations performed automatically on gateway first-run (so the
# desktop app, which never runs `junction setup`, still gets them). Lazy
...
def run_first_run_setup() -> None:
    ...
    The Electron app only runs ``junction gateway`` — never ``junction
    ...
    * **PATH shim** — ``ensure_junction_on_path()`` is idempotent and only
      writes ``~/.local/bin/junction``, so it runs on every start. It is called
    ...
    * **Stale predecessor MCP purge** — ``clean_stale_managed_mcp()`` mutates
      ...
      guarded by a marker file, to honor the "Junction owns only the agent
    ...
    try:
        shim = ensure_junction_on_path()
        if shim:
            logger.info("First-run: linked junction shim at %s", shim)
    except Exception:
    ...
    try:
        from junction.platform.admission import seed_default_policy  # noqa: PLC0415
    ...
    try:
        from junction.apps.manager import (  # noqa: PLC0415
    ...
    if stale_marker.exists():
        return
    try:
        from junction.mcp_cleanup import clean_stale_managed_mcp  # noqa: PLC0415
...
def _all_skill_paths() -> list[str]:
    ...
    # User-created skills (Junction convention)
...
def _strip_legacy_denied_commands(config: dict) -> None:
    ...
    Denied commands are now enforced solely at Junction's hooks.py PreToolUse
    ...
    in its ``junction.json``. kiro-cli would keep enforcing those stale rules
...
def _apply_user_kiro_hooks(config: dict, mc_cfg: dict) -> None:
    """Merge user-defined kiro_hooks from junction config into *config* (additive).
...
def _refresh_dynamic_fields(config: dict, *, gated_off: "frozenset[str] | None" = None) -> None:
    ...
    Called when ``junction.json`` already exists so user customizations are
    ...
    # Self-heal: lift any stray Junction bookkeeping keys into the sidecar and
    ...
    from junction.config.loader import DEFAULT_MODEL, normalize_agent_model
    ...
    # config.json agent.model is the user-facing authority (junction config set
...
def _load_existing_config(
    ...
    """Load and refresh an existing junction.json.
...
def _apply_connection_tool_aliases(
    ...
    :mod:`junction.connections.tool_aliases` owns invariants 1-6, which decide
    ...
    * **The generated subset is read from a PERSISTED record, not inferred from a
      ...
      default. See :mod:`junction.connections.alias_record` for the generation
    ...
    * **A generated alias never lands on a name already in use.** The destination
    ...
    try:
        # Imported here, not at module scope: junction.connections.registry
        # validates the committed registry at MODULE level, so a missing or
        # malformed registry.json would raise on `import junction.agent` --
        ...
        from junction.connections.alias_record import (  # noqa: PLC0415
            emitted_from_alias_map,
            is_recorded_emission,
            spec_fingerprint,
        )
        from junction.connections.tool_aliases import (  # noqa: PLC0415
...
def migrate_agent_specs() -> int:
    """Strip Junction bookkeeping keys from kiro agent specs into the sidecar.
    ...
    if cleaned:
        logger.info("Cleaned %d kiro agent spec(s) of Junction bookkeeping keys", cleaned)
    return cleaned
...
def clear_model_pin(config: MutableMapping[str, object], name: str) -> None:
    ...
    the editor, or ``junction agent reset-model`` -- and never by a heuristic
...
def _conflicting_spec_for(name: str, chosen: Path, agents_dir: Path) -> Path | None:
    ...
    The runtime resolver (``JunctionConfig._resolve_named_agent_model``) accepts
...
def reset_agent_model(name: str) -> tuple[Path, str]:
    ...
    The explicit, narrow counterpart to ``junction setup --clean``, which also
...
def _decline_shared_agent_home(*, audit: bool = True) -> Path | None:
    ...
    An **ephemeral** Junction instance — one booted from a linked git worktree, or
    one running on its own isolated ``JUNCTION_HOME`` (a pod) — is throwaway by
    ...
    Declining mirrors ``ensure_junction_on_path``'s worktree guard: leave whatever
    ...
    * A **pod running from the primary checkout** is not in a linked worktree at
      ...
      ``JUNCTION_POD`` (set in ``build_pod_env``) and that counts as ephemeral on
      its own. Note what is deliberately NOT used as the signal: merely *having*
      an isolated ``JUNCTION_HOME``. A CI test gateway (the offline E2E suite boots
    ...
    * A globally exported ``KIRO_HOME`` moves the shared directory, so comparing
    ...
    relocates kiro-cli's session storage while Junction still reads the host path
    ...
    if own_home is not None and target == isolated_agents_dir(own_home).resolve():
        ...
        # (JUNCTION_HOME=$HOME suffices), handing an ephemeral instance the very
    ...
    # "Has an isolated JUNCTION_HOME" is NOT that: a CI test gateway and a user
    ...
    # (``Mode 'junction' not found``). ``_in_ephemeral_tree`` is the same
    ...
    # dead (every turn fails with ``Mode 'junction' not found``). #4781's harm is
...
def _seed_kas_permissions(config: dict[str, Any]) -> None:
    ...
    **Seed, never refresh.** Once the key exists it belongs to whoever edits the
    ...
    # circular import: `junction.acp.__init__` pulls in the runtime, which reaches
    ...
    from junction.acp.kas_permissions import (  # noqa: PLC0415
...
def _ceiling_filtered_spec(ref: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    that can come from outside this repo — rather than by Junction or the user.
...
def _collect_app_mcp_servers() -> dict[str, Any]:
    ...
    the app. Junction sessions only ever read the agent config
    ...
    try:
        # Imported lazily: junction.apps imports back into agent/security, so a
        # module-level import here would close a cycle.
        from junction.apps.bridges import registered_app_mcp_servers
        from junction.apps.manager import get_app_manifest, is_app_enabled, list_apps
...
def _durable_tool_aliases(path: Path) -> tuple[bool, object]:
    ...
    Returns:
        ...
        :func:`~junction.connections.alias_record.spec_fingerprint` both read as the
...
def rebuild_agent_config(*, clean: bool = False) -> Path:
    """Rebuild and write the merged junction.json to ~/.kiro/agents/.
    ...
    junction MCP server commands) are refreshed from defaults.
    ...
    # One-time (idempotent) self-heal: strip Junction bookkeeping keys from
    ...
    # config) FIRST.  Junction is kiro-first (ACP/kiro-cli only), so Kiro
    ...
    # entries.  Write-through is never done here (Junction reads globals but
    # never mutates them).
    #
    ...
    for name, spec in shared_mcp.items():
        if isinstance(spec, dict) and name not in managed_names:
            ...
            # (junction merge) must not mutate shared_mcp, which is reused as a
    ...
    junction_mcp = _load_json(_user_dir() / "mcp.json").get("mcpServers", {})
    for name, spec in junction_mcp.items():
        if isinstance(spec, dict) and name not in managed_names:
    ...
    # (junction > kiro-global > provider-global).  This prevents one source's
    ...
    def _resolve_command(cmd: str, env: dict | None) -> tuple[str | None, str]:
        ...
        config ("command not found: junction"). The value EMITTED into the spec
        ...
        fallback for pip-generated wrappers like ``junction``.
        """
        if not cmd:
            return None, ""
    ...
    for _n, _s in junction_mcp.items():
    ...
    for name, spec in _cfg_servers.items():
        ...
        if spec.get("url"):
            ...
            # in the agent config itself (``kiro-cli mcp add --agent junction``, a
            ...
            _store_entry = junction_mcp.get(name)
    ...
    #
    # ``junction_mcp`` is in this chain for the same reason: it holds every entry
    ...
    # branch above is the only place it is added — so ``junction-computer`` is
    ...
    # junction.json has TWO independent-locked writers: this regenerating one and
    ...
    # (Only for junction.json — every other agent file this may write has a single
    ...
    def _finalize_and_write() -> None:
        ...
        # generated pass does.
        #
        # Imported inside a try: `junction.connections.alias_record` is a submodule,
        # so importing it executes `junction.connections.__init__`, which eagerly
    ...
    try:
        is_junction_json = path.resolve() == _mcp_json_path().resolve()
    except OSError:
        is_junction_json = False
    if is_junction_json:
        with _mcp_lock():
    ...
    # Install Junction AIM capabilities package (includes junction-lite)
    _install_aim_capabilities()

    # Install junction-knowledge agent (used by Knowledge Library LLMPool)
    try:
        _install_knowledge_agent()
    except Exception:
        logger.debug("junction-knowledge agent install failed", exc_info=True)

    # Install junction-research agent (used by the Research Lab campaign loop)
    try:
        _install_research_agent()
    except Exception:
        logger.debug("junction-research agent install failed", exc_info=True)

    # Install junction-heartbeat agent (used by HeartbeatService for unattended polling)
    try:
        _install_heartbeat_agent()
    except Exception:
        logger.debug("junction-heartbeat agent install failed", exc_info=True)

    # Install junction-conductor agent (goal decomposition + session-control dispatch)
    try:
        _install_conductor_agent()
    except Exception:
        logger.debug("junction-conductor agent install failed", exc_info=True)
...
def ensure_agent_materialized(agent: str | None) -> bool:
    ...
    Normally ``junction setup --agent-only`` writes it, but a source checkout /
    dev launch that skips setup leaves it absent — this makes the runtime
    self-sufficient regardless.

    Only the managed default (``AGENT_FILENAME`` → ``junction.json``) is
...
def _install_aim_capabilities() -> None:
    """Write a bare ``junction-lite`` agent config.
    ...
    Amazon-internal package manager); the generic ``junction-lite`` fallback
...
def _remove_bare_lite_if_aim_installed() -> None:
    ...
    bare ``junction-lite.json`` when an AIM-installed duplicate existed; with
...
def _install_lite_agent_fallback() -> None:
    """Write a bare junction-lite config (cheap background agent)."""
    ...
    # stays schema-clean).
    agent_state.set_cc_model("junction-lite", _background_cc_model())


_KNOWLEDGE_SYSTEM_PROMPT = (
    "You are a knowledge extraction specialist for Junction's Knowledge Library. "
...
def _install_knowledge_agent() -> None:
    """Generate and install the junction-knowledge agent config.
...
_RESEARCH_SYSTEM_PROMPT = """# Junction Research Worker

You are `junction-research`, an autonomous research worker. You run ONE research
...
def _install_research_agent() -> None:
    """Generate and install the junction-research agent config.

    Derives from the junction agent (MCP servers, security, tools) but swaps in a
...
#: by one rather than as the whole ``@junction-dashboard`` server.
#:
...
#:
#: Every withheld verb stays MOUNTED (``@junction-dashboard`` is still in
...
def _install_conductor_agent() -> None:
    """Generate and install the junction-conductor agent config.

    Derives from the junction agent (resolved MCP invocations, security hooks)
    ...
    rather than of the prompt. The ``junction-dashboard`` server is the opt-in
    ...
    ``@junction-dashboard`` is MOUNTED whole but auto-approved only verb by verb,
    ...
    false — the default. Sharing the name would let ``junction setup`` erase the
    packaged skill on a stock install, and quarantine the user's delegation
    skill when the flag is on.
    """
    config = build_agent_config()
    config["name"] = "junction-conductor"
    config["description"] = (
    ...
    granted: list[str] = []
    withheld: list[str] = []
    for ref in ("session", "report", "@junction-core", *_CONDUCTOR_DASHBOARD_GRANTS):
    ...
    core_entry = mcp.get("junction-core")
    narrowed: dict = {}
    if core_entry:
        narrowed["junction-core"] = core_entry
    dash_cmd, dash_args = _junction_mcp_invocation("mcp-dashboard")
    ...
    # dead with no local error; without the `JUNCTION_HOME` pin the shim reads the
    ...
    narrowed["junction-dashboard"] = dash_entry
    ...
    # its KAS rule with it (a hand-written ``junction-core/*`` allow would have
    ...
    from junction.acp.kas_permissions import (  # noqa: PLC0415 - circular import
        allowed_tools_to_permissions,
    )
...
_HEARTBEAT_SYSTEM_PROMPT = """# Junction Heartbeat Worker

You are `junction-heartbeat`, an unattended polling worker that runs one task
...
and side-effect-free junction-core reads). Anything outside that list is
...
def _install_heartbeat_agent() -> None:
    """Generate and install the junction-heartbeat agent config.

    A dedicated agent for HeartbeatService.  Minimal MCP surface — only
    ``junction-core`` (learn/cron/spawn list, recall, artifacts read) on
    ...
    # Pull the ``junction-core`` entry from the main agent config so the
    ...
    agent_state.set_cc_model("junction-heartbeat", _background_cc_model())
...
def _sanitize_agent_hooks() -> None:
    ...
    for filename in OWNED_KIRO_AGENT_FILES:
        ...
        if not isinstance(hooks, dict):
            ...
            continue
        removed_keys = [key for key in hooks if key in _LEGACY_JUNCTION_HOOK_KEYS]
        if not removed_keys:
            _hooks_sanitized_mtimes[str(f)] = mtime
            continue
        data["hooks"] = {
            key: value for key, value in hooks.items() if key not in _LEGACY_JUNCTION_HOOK_KEYS
        }
