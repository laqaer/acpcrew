...
from junction.tips_text import truncate_summary
...
# Doc selection is allowlist-based (see junction.tips_allowlist) — internal
...
`junction config set session.pool_size 2`), or a ready-to-send message
...
def _scan_docs_catalog() -> list[CatalogEntry]:
    """Scan junction/docs/*.md for H1 + first paragraph."""
...
def _state_path() -> Path:
    # Use the shared path helper: JUNCTION_HOME tilde-expansion/resolution and
...
def _load_curated_tips() -> list[dict]:  # type: ignore[type-arg]
    ...
    These cover Junction-native features (keyboard shortcuts, Settings toggles,
...
async def generate_tips(state: DashboardState) -> list[dict]:  # type: ignore[type-arg]
    ...
    cfg = await loop2.run_in_executor(None, JunctionConfig.load)
...
async def api_tips_next(request: web.Request) -> web.Response:
    ...
    loop = asyncio.get_running_loop()
    cfg = await loop.run_in_executor(None, JunctionConfig.load)
...
async def api_tips_status(request: web.Request) -> web.Response:
    ...
    - opted_out: per-instance opt-out flag (Junction is a single-owner
