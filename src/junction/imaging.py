...
* :mod:`junction.acp.prompt_blocks` -- the ``session/prompt`` path (every
  channel's image attachments).
* :mod:`junction.mcp_gateway.image_budget` -- the gateway relay's rewrite of
  image content blocks in brokered MCP tool results.

This module is deliberately a LEAF: it imports nothing from ``junction.acp``
or ``junction.mcp_gateway``, because the gateway daemon (``gatewayd``, a
