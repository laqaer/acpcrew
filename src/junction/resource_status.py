...
* the **pressure-gated context line** injected in
  :meth:`junction.context.ContextManager.build_message` — a compact
...
The memory figure reuses :func:`junction.subagent._available_memory_gb`, the
...
def _read_available_gb() -> float:
    ...
    Lazily reuses :func:`junction.subagent._available_memory_gb` (the same
    ...
    """
    try:
        from junction.subagent import _available_memory_gb
...
def probe(cfg: object | None = None) -> ResourceStatus:
    ...
    *cfg* is an optional pre-loaded ``JunctionConfig``; when omitted it is
