...
and must be staged into ``<repo-root>/src/junction/static/dist`` so the
...
``JunctionWebsite/dist`` clone is honored as a last-resort fallback when
...
_DIR_NAME = "website"
_SIBLING_DIR_NAME = "JunctionWebsite"
...
# ``website/docs/extension-seams.md``). ``JUNCTION_EDITION_DIR`` names the
# edition's own ``extensions.tsx``; ``JUNCTION_ALLOW_EDITION=1`` is the
# fail-closed opt-in that must accompany it.
_EDITION_DIR_ENV = "JUNCTION_EDITION_DIR"
_EDITION_OPT_IN_ENV = "JUNCTION_ALLOW_EDITION"
...
def edition_sources_missing() -> bool:
    ...
    Absent ``JUNCTION_EDITION_DIR`` this is ``False`` and the stock path is
    untouched.
    """
...
def _edition_build_env() -> Optional[dict[str, str]]:
    ...
    The runtime rebuild (``POST /api/update``, ``junction update``, and the
    ...
    **The opt-in is READ, never synthesized.** ``JUNCTION_ALLOW_EDITION=1`` is the
    ...
    ``JUNCTION_EDITION_DIR``-without-opt-in error still fires when it should.
...
def _repo_root(junction_pkg_dir: Path) -> Path:
    """Return the repo root given the ``junction`` package directory.

    Layout: ``<repo-root>/src/junction/`` is *junction_pkg_dir*, so two
    ``.parent`` hops land on the repo root (parent of ``src/``).
    """
    return junction_pkg_dir.parent.parent


def _resolve_website_dist(junction_pkg_dir: Path) -> Optional[Path]:
    ...
    # 2. Sibling JunctionWebsite/dist (legacy fallback).
...
def ensure_dev_dist_symlink() -> Optional[Path]:
    ...
    The dashboard serves its SPA from ``<junction>/static/dist/index.html``.
    ...
    plain source-tree run (``PYTHONPATH=src python -m junction gateway``,
    ...
    3. Missing → resolve the in-tree ``website/dist`` (or a sibling
       ``JunctionWebsite`` checkout as a last resort) and symlink to it.
    ...
    """
    junction_pkg_dir = Path(__file__).resolve().parent
    tree_dist = junction_pkg_dir / "static" / "dist"
    ...
    # local build landing in the source tree / user ran junction init --ui).
    ...
    candidate = _resolve_website_dist(junction_pkg_dir)
...
def _npm_build_and_stage_locked(
    ...
    if proc.returncode != 0:
        ...
        return False
    static_dist = proj_path / "src" / "junction" / "static" / "dist"
...
def build_and_stage(
    ...
    if not npm_bin:
        ...
        return False
    try:
        with _staging_lock(root / "src" / "junction" / "static"):
...
def build_frontend_sync(
    ...
    ``website/dist`` into ``src/junction/static/dist``. Graceful no-op when
    ...
    # needs the lock.
    try:
        with _staging_lock(proj_path / "src" / "junction" / "static"):
...
async def build_frontend_async(
    ...
    ``src/junction/static/dist``. Graceful no-op when there is no
