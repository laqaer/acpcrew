...
Resolution order (most-specific-wins):
  ...
  2. Junction markers present            -> bundled profiles/junction.json
...
Every resolved profile has the SAME shape:
  {
    "source":        "config" | "junction" | "auto-detect" | "generic",
...
def _safe_regular_file(path):
    ...
    to repos without junction installed (it cannot import the gateway helpers).
    """
...
def detect_junction(root):
    """True iff all Junction marker files are present at ``root``."""
    return all(os.path.exists(os.path.join(root, m)) for m in _JUNCTION_MARKERS)
...
def resolve(root):
    ...
    if _safe_regular_file(toml_path):
        ...
        return profile
    if detect_junction(root):
        return normalize(load_bundled("junction"), "junction")
