...
```bash
# From your main Junction clone:
git fetch origin main
git worktree add ../junction-wt-<name> -b feat/<name> origin/main
cd ../junction-wt-<name>
```
...
To clean up after merging: `git worktree remove ../junction-wt-<name>`
...
isort --check-only src/junction test
flake8 src/junction test
mypy src/junction/
...
- The gateway serves the frontend from `src/junction/static/dist/` (a compiled
...
- After frontend changes, build AND stage (the build outputs to `website/dist`;
  ...
  rm -rf src/junction/static/dist && cp -R website/dist src/junction/static/dist
  ```
...
2. **Isolated pod (no cutover, hands-off).** Preview the full stack on its own
   ...
   ```bash
   junction pod up <worktree-name> --json   # own JUNCTION_HOME, own port, no crons
   junction pod down <worktree-name>        # zero residue
   ```
   Best for QA agents and end-to-end tests. `junction pod --help` for all verbs
   (`ls`, `status`, `logs`, `provision`, …). The worktree must be built first
   (venv + dist); `junction pod up --provision` does the full on-ramp.
...
`JUNCTION_HOME` isolates config, DB, sessions and workspace. It does **not**
...
git worktree at /workplace/<you>/junction-wt-<name>: ...
```
...
- A flagged feature "missing" is usually a flag set in the wrong instance's
  `JUNCTION_HOME`, not a missing bundle (Rule 4).
