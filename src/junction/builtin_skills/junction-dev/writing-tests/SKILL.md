...
`src/junction/apps/builtins/*/tests/` gets the rootdir `conftest.py` plus that app's
...
under `JUNCTION_TMP_RESIDUE_STRICT=1`. So a leak of yours will not necessarily turn CI red;
...
- **A `MagicMock` config reads TRUTHY.** Patching `JunctionConfig.load` with a bare
  ...
  into the repo. That is why `JUNCTION_TELEMETRY=0` is forced for the whole suite. Any
