...
def _resolve_home() -> Path:
    ...
    A RELATIVE JUNCTION_HOME cannot be honored safely: this launcher is invoked
...
def resolve_device(want: str | None) -> dict:
    ...
    if booted:
        return booted[0]
    phones = [d for d in devs if d["name"].startswith(("iPhone", "Junction"))]
    if phones:
        return phones[-1]
    if devs:
        return devs[-1]
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    # here tear down a live mirror another JUNCTION_HOME owns for the device.
...
def _live_owned_pid(udid: str) -> int | None:
    ...
    JUNCTION_HOME legitimately owns for the same device, since the vendor's
