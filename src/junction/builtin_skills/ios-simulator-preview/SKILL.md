...
Mirror a running iOS Simulator into Junction's Browser side panel so the user
...
→ the `junction:preview` marker → Browser panel iframe.** Nothing leaves the
...
browser via a browser tool, while Junction's panel is driven by the hidden
...
`JUNCTION_HOME`, when set, must be an ABSOLUTE path (`~` is fine — the launcher
...
```
<!-- junction:preview url="http://127.0.0.1:PORT" -->
```
