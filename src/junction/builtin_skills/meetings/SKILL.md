...
Transcription uses Junction's own streaming endpoint (`/api/ws/stt`), driven from
the browser. Cloud transcription is an optional extra
(`pip install junction[voice]`); when it is not installed the app says so and the
