...
| `'…' is a blocked target for computer use (…)` | Junction's own dashboard is permanently refused — driving it would let you change your own security settings | do the task another way; tell the user why |
...
- **Electron apps are slow on the first snapshot.** Slack, VS Code, Obsidian and
  Junction's own desktop app need an accessibility opt-in that takes ~2 seconds to
