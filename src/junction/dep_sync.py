...
rewrite ``Scripts\\junction.exe`` while the gateway is served by the very venv it
...
gateway's auto-update, ``junction update``, and the console-script bootstrap. Four
...
rather than a stylistic preference. ``junction._bootstrap`` reaches for this
...
def locked_console_scripts(target_py: Path) -> list[str]:
    ...
    ordinary single-checkout setup -- pip cannot replace ``Scripts\\junction.exe``,
...
def installed_console_script_target(target_py: Path, script: str) -> str | None:
    ...
    Reads the entry points off the ``junction`` distribution rather than the
