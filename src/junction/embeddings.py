...
# junction-core MCP server — the GGUF weights themselves are mmap'd and
...
def _install_diskcache_stub() -> None:
    ...
    Junction never constructs a disk-backed LLM state cache (we only use
    ...
    class Cache:  # pragma: no cover - never constructed by Junction
        def __init__(self, *args: object, **kwargs: object) -> None:
            raise RuntimeError(
                "diskcache is stubbed out by junction.embeddings — disk-backed "
...
def models_dir() -> Path:
    """Directory where downloaded embedding models live (respects JUNCTION_HOME)."""
...
def _emit_embed_timing(
    ...
    *,
    ...
    try:
        ...
        recorder.histogram("junction.embed.queue_wait", wait_ms, unit="ms")
        recorder.histogram(
            "junction.embed.inference",
            infer_ms,
            unit="ms",
...
def _is_sensitive_model_path(path: Path) -> bool:
    ...
    try:
        ...
        # shared. The exact path stays available on demand: `junction doctor`
...
def resolve_custom_model() -> "CustomModelSpec | None":
    ...
    Resolution order for the path: ``JUNCTION_EMBED_MODEL_PATH`` env, then the
...
def default_embedding_space_signature() -> str:
    ...
    :meth:`~junction.vector_memory.VectorMemoryStore.reconcile_embedding_space`
...
def reconcile_store_embedding_space(store: "_ReconcilableStore") -> int:
    ...
    non-mutating ``store_embedding_space_is_stale`` probe instead — ``junction
...
class EmbeddingBackend(abc.ABC):
    ...
    - ``model_id`` + ``dim`` identify the vector space. Vectors produced under
      ...
      keys off :func:`junction.knowledge.embedder.embed_signature`, which
...
def reset_shared_embedder() -> None:
    """Drop the singleton (tests, disable-embeddings, JUNCTION_HOME changes)."""
    global _shared_embedder
    with _shared_embedder_lock:
...
class ModelDownloadManager:
    ...
    async def ensure_model(self, attempts: int = 1) -> bool:
        ...
        ``JUNCTION_SKIP_MODEL_DOWNLOAD=1`` escape hatch is set — test runs
...
def reset_download_manager() -> None:
    """Drop the singleton (tests, JUNCTION_HOME changes)."""
