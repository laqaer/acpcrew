"""``junction seed`` — copy a hand-authored fixture into ``$JUNCTION_HOME``.

Fixtures ship as package data at ``src/junction/tests_fixtures/<name>/``.
Each fixture is a valid ``JUNCTION_HOME`` tree with a ``fixture.yaml``
declaring ``schema-version``. Shipping inside the package is what lets
``junction seed`` work after ``pip`` installs the wheel into
``site-packages/junction/`` (a source-tree-relative path walk would
...
except ImportError:  # pragma: no cover — Junction targets py3.10.
    _resource_files = None  # type: ignore[assignment]

# ``junction.sel`` is a first-party module shipped in the same package, so
...
def _fixtures_root() -> Path:
    ...
    workspaces (``src/junction/tests_fixtures/``) and in the pip-installed
    layout where the package lives under
    ``site-packages/junction/``. ``files()`` returns a ``Traversable``
    which, for the regular filesystem loader Junction uses, is a real
    ...
    return Path(str(_resource_files("junction") / _FIXTURES_PKG))
...
def _resolve_fixture(name: str) -> Path:
    ...
    - Empty or ``.``-only names resolve to the fixtures root itself — would
      copy the whole ``tests_fixtures/`` tree into ``$JUNCTION_HOME``.
    ...
    if not candidate.is_dir():
        ...
        # ``src/junction/tests_fixtures/`` or the PRD. Sorted for stable
...
def _resolve_target(*, for_main_home_check: bool = False) -> Path:
    """Return ``$JUNCTION_HOME`` as a ``Path``, or raise ``SeedError``.
    ...
    if for_main_home_check:
        ...
        try:
            ...
            raise SeedError(
                f"cannot resolve $JUNCTION_HOME ({raw!r}) for main-home "
...
def seed(fixture_name: str, *, replace: bool = False) -> None:
    """Copy fixture ``fixture_name`` into ``$JUNCTION_HOME``.
    ...
    if dst.is_symlink() and not dst.exists():
        raise SeedError(
            f"$JUNCTION_HOME is a dangling symlink: {dst}. "
    ...
    # ``$JUNCTION_HOME`` pointing at a regular file (or a symlink to one) would
    ...
    if dst.exists() and not dst.is_dir():
        raise SeedError(
            f"$JUNCTION_HOME is not a directory: {dst}. "
    ...
    if dst.exists() and any(dst.iterdir()):
        ...
        if dst.is_symlink():
            raise SeedError(
                f"refusing to seed into a symlinked $JUNCTION_HOME: {dst}. "
                "Point it at a real directory.",
                guardrail=SeedError.GUARDRAIL_SYMLINK_REPLACE,
            )
        if not replace:
            raise SeedError(
                f"$JUNCTION_HOME is not empty: {dst}. "
...
def seed_cmd(args) -> int:  # noqa: ANN001 — argparse.Namespace at call site
    ...
    directory tree into ``$JUNCTION_HOME`` and 1.B's ``--seed-replace`` will
    ...
    # this flag is always ``True`` on the success path (if ``JUNCTION_HOME``
    ...
    target_set = "JUNCTION_HOME" in os.environ
    try:
