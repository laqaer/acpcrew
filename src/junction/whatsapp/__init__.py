...
the shared ``junction.messaging`` pipeline; the protocol layer is `neonize
<https://github.com/krypton-byte/neonize>`_ (Apache-2.0 Python bindings to
whatsmeow), an optional dependency installed with ``junction[whatsapp]``.

Because the agent sends *as the operator*, two properties are load-bearing:

- **Echo discipline** (:mod:`junction.whatsapp.echo`): the account's own
...
- **Deny-by-default reach** (:mod:`junction.whatsapp.transport`): only the
  ...
  tool approval or session control (:mod:`junction.whatsapp.group_gate`).
