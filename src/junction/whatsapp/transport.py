...
Wraps :class:`junction.whatsapp.client.WhatsAppClient` in the channel-neutral
...
1. **shape**: the message is unwrapped (ephemeral, view-once and edited
   ...
   :mod:`junction.whatsapp.media`. Anything that yields neither text, nor
...
4. **group gate**: group chats are dropped unless configured, then gated by
   mode/mention/cooldown (:mod:`junction.whatsapp.group_gate`);
...
from junction.messaging.attachments import append_attachment_context, cleanup
from junction.messaging.dispatch import inbound_permitted
...
from junction.whatsapp.attachments import ingest_media
...
from junction.whatsapp.group_gate import GroupGate, GroupVerdict
from junction.whatsapp.jids import (
    LID_SERVER,
    is_group_jid,
    jid_to_str,
    jid_user,
...
from junction.whatsapp.media import describe, unsupported_note, unwrap_message
from junction.whatsapp.renderer import WHATSAPP_CHUNK_LIMIT
