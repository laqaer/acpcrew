...
The channel-neutral half is :mod:`junction.messaging.outbound_files`: it finds
...
memoized Pillow handle in :mod:`junction.imaging`, and the refusal is spoken by
...
from junction.imaging import image_dimensions, pil_available
...
def _decode_refusal(file: OutboundFile) -> Rejection | None:
    ...
    Header-only inspection: :func:`junction.imaging.image_dimensions` reads the
...
def rejection_note(rejections: list[Rejection]) -> str:
    ...
    Written so it survives :func:`junction.whatsapp.renderer.to_whatsapp_text`
