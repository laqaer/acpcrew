...
never load the bundled Go core, so a missing ``junction[whatsapp]`` extra
...
from junction.platform_compat import make_owner_only_dir, restrict_to_owner
from junction.whatsapp.jids import OwnIdentity, jid_to_str

logger = logging.getLogger(__name__)

MISSING_EXTRA_HINT = (
    "The WhatsApp channel needs the optional dependency extra: " "pip install 'junction[whatsapp]'"
)
...
class WhatsAppClient:
    ...
    async def send_text(
        ...
        from junction.whatsapp.renderer import render_chunks_off_loop
    ...
    def _parse_jid(self, jid_str: str) -> Any:
        ...
        from junction.whatsapp.jids import USER_SERVER, normalize_jid
