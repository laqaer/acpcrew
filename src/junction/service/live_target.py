...
* a systemd **system** unit (what ``junction service install`` writes on Linux)
...
checkout the process ``execve``s into that checkout's own ``junction``, moving
...
EXEC_MARKER = "JUNCTION_LIVE_EXECED"
...
def pointer_path() -> Path:
    ...
    # ``junction.config.loader.config_dir`` patch seam still governs where the
...
def target_bin(checkout: Path) -> Path:
    """The ``junction`` a live target is executed through.
    ...
    if sys.platform == "win32":
        return checkout / ".venv" / "Scripts" / "junction.exe"
    return checkout / ".venv" / "bin" / "junction"
...
def maybe_reexec(argv: list[str], *, log: object = None) -> None:
    ...
    env = {
        **os.environ,
        ...
        "JUNCTION_PROJECT_DIR": str(target),
        # The target's venv leads PATH. Without this, a bare ``junction`` in a
...
def _warn(log: object, message: str) -> None:
    ...
    if callable(warn):
        ...
        return
    print(f"junction: live-target: {message}", file=sys.stderr)
