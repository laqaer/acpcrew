...
SERVICE_NAME = "junction"  # systemd unit name (without .service)
LAUNCHD_LABEL = "dev.junction.gateway"  # launchd Label
...
def launchd_live_program() -> "os.PathLike[str]":
    ...
    return (
        ...
        / "Library"
        / "Application Support"
        / "Junction"  # brand-ok: frozen Electron Application Support directory
        / "live-gateway"
    )
...
def junction_bin() -> str:
    ...
    1. ``JUNCTION_SERVICE_BIN`` if set — an explicit operator override. This
...
def service_environment(home: str) -> "dict[str, str]":
    ...
    * ``JUNCTION_KIRO_BIN`` — propagated only when the installer already has it
    ...
    kiro_bin = os.environ.get("JUNCTION_KIRO_BIN", "").strip()
    if kiro_bin:
        env["JUNCTION_KIRO_BIN"] = os.path.abspath(kiro_bin)
    # JUNCTION_PORT is the ONLY input DASHBOARD_PORT reads, so a service that
    ...
    # same way JUNCTION_KIRO_BIN is:
    # captured from the installer's environment, so
    # `JUNCTION_PORT=5477 junction service install` bakes 5477 into the unit.
    #
    # No validation here on purpose. `cli.py`'s main() already rejects a
    # JUNCTION_PORT that is not an integer in 1-65535 before any subcommand
    ...
    port = os.environ.get("JUNCTION_PORT", "").strip()
    if port:
        env["JUNCTION_PORT"] = port
    return env
...
def headless_auth_warning(environ: "Mapping[str, str] | None" = None) -> str:
    ...
    ``junction service install`` is not there when the service starts: the probe
    ...
    if _home_override_is_set(environ):
        lines.append("")
        lines.append("   JUNCTION_HOME is set here but is also not inherited, so confirm")
...
def service_path(home: str) -> str:
    ...
    interactive shell that ran ``junction service install``. Common
...
def restart_command_hint() -> str:
    ...
    * ``SYSTEMD`` — the unit is **system-level** at
      ``/etc/systemd/system/junction.service`` (see
      :mod:`junction.service.linux`). ``systemctl --user`` fails on AL2
    ...
    * ``LAUNCHD`` / ``UNSUPPORTED`` — defer to the service-aware
      ``junction restart`` CLI, which resolves the right mechanism itself.
