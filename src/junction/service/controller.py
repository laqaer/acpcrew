...
importing :mod:`junction.service.linux` or :mod:`junction.service.macos`
...
from junction.service import linux, macos
from junction.service.common import Platform, current_platform, headless_auth_warning
...
def installed_unit_path() -> "Path | None":
    ...
    environment: a host running ``junction gateway`` in the foreground has none,
...
def install_service() -> int:
    ...
    ``/etc/systemd/system/junction.service`` and to run
    ``systemctl daemon-reload / enable / restart``. The gateway itself
    runs as ``User=$USER`` once started — junction code is never
