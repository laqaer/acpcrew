...
The plist lives at ``~/Library/LaunchAgents/dev.junction.gateway.plist``
...
LOG_DIR = Path.home() / "Library" / "Logs" / "Junction"
STDOUT_LOG = LOG_DIR / "gateway.log"
STDERR_LOG = LOG_DIR / "gateway.err"
#: See :func:`junction.service.common.launchd_live_program` for why the agent
...
def render_live_program(target_bin: str, *, working_dir: str | None = None,
    ...
    that re-invokes ``junction`` by name, resolves to the checkout that is now
    ...
    lines = [
        "#!/bin/sh",
        "# Generated by Junction (service install / Dev Fleet make-live).",
...
def _repairer_bin() -> str:
    ...
    Resolution is deliberately narrow: an explicit ``JUNCTION_SERVICE_BIN``, else
    the ``junction`` console script sitting beside the running interpreter. There
    is no ``PATH`` fallback, because ``PATH`` cannot answer the question being
    asked — "which install is running" — and an unrelated or older ``junction``
    ...
    if override:
        resolved = junction_bin()
        if not _is_executable_file(Path(resolved)):
            raise OSError(
                f"refusing to restore the launchd launcher: JUNCTION_SERVICE_BIN "
                f"resolves to {resolved!r}, which is not an executable file."
            )
        return resolved
    sibling = Path(sys.executable).parent / (
        "junction.exe" if os.name == "nt" else "junction"
    )
    if _is_executable_file(sibling):
        return str(sibling)
    raise OSError(
        "refusing to restore the launchd launcher: no junction console script "
        f"beside the running interpreter ({sys.executable}). PATH is not "
        "consulted, because an unrelated junction ahead of this install on PATH "
        "would be persisted into the agent. Set JUNCTION_SERVICE_BIN to the "
...
def ensure_live_program() -> bool:
    ...
    ``EnvironmentVariables`` (a non-default ``JUNCTION_PORT``, for instance), so
...
def install() -> None:
    ...
    write_live_program(render_live_program(junction_bin()))
