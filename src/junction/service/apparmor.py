...
profile carrying no ``CAP_SYS_ADMIN``. Junction's Linux sandbox needs a user
...
``ch-run``, ``QtWebEngineProcess``, ``1password``, ``Discord``). Junction was the
outlier.
...
# fake `apparmor_parser` executed as root by `junction service install` — a local
...
def render_profile(abi: str | None) -> str:
    ...
    return f"""# Managed by Junction — regenerated by `junction service install`.
#
# This profile grants ONE permission: creating an unprivileged user namespace.
# Junction's Linux sandbox isolates the agent by entering a user namespace and
...
# blocks that second step, so Junction fail-closes and refuses to run the agent
# rather than run it unisolated.
#
# It is a NAMED profile with no attachment path: systemd applies it to the
# junction service via AppArmorProfile=. It therefore cannot apply to any other
...
#
# Removing this file re-breaks Junction's sandbox on this host: the gateway will
# refuse to spawn the agent instead of running it without isolation.
# `junction service uninstall` removes it for you.
...
def validate(parser: str, text: str) -> tuple[bool, str]:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", suffix=".aa", prefix="junction_profile_", delete=False, encoding="utf-8"
        ) as handle:
            handle.write(text)
            tmp = handle.name
        proc = subprocess.run(
...
# import junction: this runs under sudo, and importing our own package would
...
def verify_enforcement(
    ...
    2. **It must NOT run anything user-writable as root.** So the interpreter is a
       ...
       import junction — otherwise ``sudo`` would execute user-controlled
       site-packages.
...
LAUNCHER_PROFILE_NAME = "junction-launcher"
...
def default_exec_path() -> str | None:
    ...
    Returning None is the correct answer for a foreground ``junction gateway`` in
    ...
    the machine. That case is served by ``junction service install`` instead.
    """
...
def render_launcher_profile(abi: str | None, exec_path: Path) -> str:
    """Render the attached profile for a directly launched Junction.
    ...
    """
    abi_line = f"abi <abi/{abi}>,\n\n" if abi else ""
    return f"""# Managed by Junction — regenerated by `junction sandbox install-profile`.
#
# This profile grants ONE permission: creating an unprivileged user namespace.
# Junction's Linux sandbox isolates the agent by entering a user namespace and
# then a mount namespace, over-mounting credential paths such as ~/.aws and
...
#
# The attachment below is a grant keyed on a path, so `junction sandbox
...
# profile from matching; `junction sandbox status` reports that, and re-running
# `junction sandbox install-profile` re-points it.
#
# flags=(unconfined) means this profile restricts nothing else; it is the same
# shape /etc/apparmor.d/chrome and /etc/apparmor.d/brave use.
#
# Removing this file re-breaks Junction's sandbox for this app: the gateway will
# refuse to spawn the agent instead of running it without isolation.
# `junction sandbox remove-profile` removes it for you.
{abi_line}include <tunables/global>
...
def install_launcher(
    ...
    """Attach the userns profile to a directly launched Junction. NEVER raises.
