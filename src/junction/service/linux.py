...
The unit lives at ``/etc/systemd/system/junction.service`` and is
...
from junction.service.common import systemd_quote as _sd_quote
...
# ``Environment=JUNCTION_PORT`` was frozen at `service install` time, so before
# this file there was no supported way to change it on a running unit.
ENV_DIR = Path("/etc/junction")
ENV_FILE_PATH = ENV_DIR / "junction.env"
...
def _current_user() -> str:
    ...
    invoking human, never root, so ``sudo junction service install`` must target
...
def _home_for_user(user: str) -> str:
    ...
    ``sudo -H junction service install`` the process's ``HOME`` is ``/root``
...
def render_unit(apparmor_profile: str = "") -> str:
    ...
    return (
        ...
        # JUNCTION_PORT) without a re-install. The leading "-" makes a missing
        # file non-fatal, so the unit still starts where install could not write
        # /etc/junction (the baked Environment= values then apply).
...
def _write_unit_via_sudo(contents: str) -> subprocess.CompletedProcess[str]:
    ...
    ``daemon-reload``.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="junction-unit-", suffix=".service")
    try:
...
def _install_file_via_sudo(contents: str, dest: Path, mode: str = "0644") -> None:
    ...
    Same escalation path as the unit file — no second mechanism, and no junction
    or LLM-influenced code runs under sudo; only ``install`` is invoked.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="junction-", suffix=".tmp")
    try:
        with os.fdopen(fd, "w") as fh:
            fh.write(contents)
...
def _seed_env_file() -> None:
    ...
    ``Path.exists()``: a pre-existing root-owned ``/etc/junction`` with a
...
def install() -> apparmor.ProfileOutcome:
    ...
    No junction / LLM / agent code runs under sudo: only ``tee`` and
    ...
    if user == "root":
        raise ServiceInstallError(
            ...
            "as that user (e.g. via their login, or `sudo -u <user> junction "
    ...
    # `JUNCTION_PORT=...` edit + restart works without re-installing.
    _seed_env_file()
...
def install_launcher_profile(exec_path: str | None = None) -> apparmor.ProfileOutcome:
    ...
    ``aa-exec`` running under sudo. No junction or LLM-influenced code does.
...
def uninstall() -> None:
    ...
    # operator-authored or -edited /etc/junction/junction.env (including one
    ...
    # dir, so it never removes anything else parked under /etc/junction.
...
def status() -> str:
    ...
    ``junction service status`` doesn't prompt for a password just to
