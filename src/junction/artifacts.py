...
from junction.artifact_source import is_verifiable_root
from junction.config.loader import JunctionConfig, config_dir
from junction.deploy.webapp_types import (  # noqa: F401 — re-export for API compatibility
    WebAppArchitecture,
    WebAppCost,
    WebAppDeployTarget,
    WebAppLifecycle,
...
#: :mod:`junction.widget_artifacts`). Every chat-emitted ``<mcwidget>`` becomes
...
class ArtifactPublication:
    ...
    *stable* across every version — pushing a new version reuses the same id
    ...
    auto_sync: bool = True  # push a new remote version on every Junction bump
    #: Sync authority for the bound provider: ``"mirror"`` (Junction is the sole
    #: writer; remote is a guarded/blind mirror — a mirror provider) or
    #: ``"live"`` (the remote owns a live CRDT; Junction is a participant —
    ...
    last_synced_junction_version: int = 0
    ...
    # Maps str(junction_version) -> remote_version_number.
...
class ArtifactComment:
    ...
    Comments can be local-only (never leaves Junction) or synced to/from a
...
def _strip_session_scope(key: str) -> str:
    ...
    * an event's ``session_id`` comes from the caller's FULL session key —
      ...
      ``JUNCTION_SESSION_KEY`` / the signed pid sidecar, which are
      scope-qualified.
    ...
    * a channel-born slot's name is its session key run through
    ...
    ``MAX_CONTENT_BYTES`` at import time, so a top-level ``junction.messaging``
...
class ArtifactStore:
    ...
    def create(
        ...
        *,
        ...
        :func:`junction.artifact_source.classify_source`. Required for a link
        ...
        :mod:`junction.widget_artifacts` should set it.
        """
    ...
    def create_image(
        ...
        *,
        ...
        auto-registered widgets. Only :mod:`junction.image_artifacts` sets it.
        """
    ...
    def allowed_source_roots(self, source_root: str = "") -> list[Path]:
        ...
        * when supplied, the artifact's own ``source_root`` — the project root
        ...
        try:
            ...
            for extra in JunctionConfig.load().publish.relocate_roots:
    ...
    def _try_read_source_path(self, source_path: str, source_root: str = "") -> str | None:
        ...
        try:
            ...
            # user's home, the JUNCTION_HOME tree, a configured relocate root,
    ...
    def prune_auto_widgets(self, *, keep: int = MAX_AUTO_WIDGET_ARTIFACTS) -> int:
        ...
        :mod:`junction.widget_artifacts`), so this sweep is what keeps that from
    ...
    def _parse_publication(raw_pub: Any) -> "ArtifactPublication | None":
        ...
        try:
            last_synced = int(raw_pub.get("last_synced_junction_version", 0) or 0)
        except (TypeError, ValueError):
            last_synced = 0
        try:
        ...
        return ArtifactPublication(
            ...
            last_synced_junction_version=last_synced,
