...
from junction import __version__, dep_sync, platform_compat
from junction.beacon import distribution, is_default_home
...
from junction.constants import CLI_BIN, CLI_CONSOLE_STEMS, DATA_WARNING, PRODUCT_NAME
...
from junction.dashboard.tailnet import is_governance_pinned_off, tailnet_origin
...
from junction.env import activate_mise
from junction.frontend import build_frontend_sync, ensure_dev_dist_symlink
...
from junction.hooks import HookManager, hooks_config_from_config_dict
...
# Client-side port resolution lives in junction.port_resolution, a light leaf
...
# import and patch these names as ``junction.cli_server.<name>``, and the
...
# intercepting.
from junction.port_resolution import (  # noqa: F401
    _JUNCTION_SERVER_SUBCOMMANDS,
    _args_look_like_junction,
    _basename_stem,
    _config_url_port,
    _gateway_owns_port,
    _is_junction_process,
    _marker_port,
    resolve_client_port,
...
def _token(args: argparse.Namespace) -> None:
    ...
    remote-mint path (:func:`junction.instances.token_mint.mint_remote_token`)
    ...
    # default = no checks; see junction.preflight.
...
def _stop(cli_port: int | None = None) -> None:
    """Stop a running Junction gateway.
    ...
    # Only kill processes that are actually Junction gateways.
    ...
    pids = [p for p in pids if _is_junction_process(p)]
    if not pids:
        sel().log_api_access(
            caller="cli",
            operation="gateway_stop",
            outcome="no_target",
            source="cli",
            resources=f"port={port} reason=no_junction_process",
        )
        print(f"No {PRODUCT_NAME} gateway currently running on port {port}.")
        sys.exit(1)
...
def _wait_for_pids_exit(pids: list[int], timeout: float) -> list[int]:
    ...
    The incumbent holds an exclusive ``flock`` on ``<JUNCTION_HOME>/gateway.lock``
    (see :mod:`junction.gateway_lock`) for its whole lifetime, and the release
    ...
    Junction gateways once, before the stop, and a pid recycled onto an unrelated
...
def _own_console_script() -> str | None:
    ...
    script path (``python -m junction restart``, a frozen bundle, a launcher
    that rewrites argv).
    """
    ...
    if not path.is_absolute():
        # argv[0] may be a bare name found on PATH ("junction") or a relative
        # path; resolve it the way the shell did.
        resolved = shutil.which(argv0)
        if not resolved:
            return None
        # MUST be absolutized. ``shutil.which`` returns an argument that already
        # has a directory component *unchanged*, so ``.venv/bin/junction``
        # (a `cd ~/checkout && .venv/bin/junction restart` invocation) comes back
...
def _spawn_detached_gateway(port: int | None = None) -> subprocess.Popen[bytes]:
    ...
    replacement refused by the ``JUNCTION_HOME`` ownership guard exits 1 within
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "junction", "gateway"]
    if port is not None:
...
def _probe_gateway_ready(port: int, timeout: int = 3) -> int:
    ...
    Same contract as :func:`junction.pod.runtime.health` (the status code, or
...
def _wait_gateway_ready(
    ...
    * **Early death short-circuits the wait.** A replacement refused by the
      ``JUNCTION_HOME`` ownership guard exits within milliseconds; polling the
    ...
    while time.monotonic() < deadline:
        try:
            ...
            with loopback_urlopen(req, timeout=3) as resp:
                ...
                token = data.get("token", "")
            if token:
                # Print the canonical loopback host (junction.localhost when it
                ...
                origin = dashboard_origin(JunctionConfig.load().dashboard.url)
                if origin and "localhost" not in origin:
                    print(f"   {origin}/?token={token}")
                return
...
def _restart(cli_port: int | None = None) -> None:
    """Restart a running Junction gateway.
    ...
    # the JUNCTION_HOME lock (see _wait_for_pids_exit). Filter with _stop()'s own
    # junction-process predicate so an unrelated listener on this port never
    ...
    incumbents = [p for p in listeners if _is_junction_process(p)]
...
def _update(force: bool = False) -> None:
    ...
    * **wheel / cli.sh** — fetch the release feed, compare versions, and
      ...
      caused the ``JUNCTION_PROJECT_DIR not set`` error for cli.sh installs.
    * **externally managed** (desktop app, Docker) — print guidance on how
    ...
    from junction.platform.update_governance import resolve_remote_url, update_blocked_reason
    ...
    from junction.cli import _ensure_node  # circular import: cli -> cli_server -> cli
    ...
    # script — that is the one case still needing a terminal without junction
    # running).
...
def _refresh_agent_config(proj: str) -> None:
    ...
    * ``TimeoutExpired`` is caught. It is raised, not returned, so without a
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "junction", "setup", "--agent-only"],
            cwd=proj,
            stdin=subprocess.DEVNULL,
            capture_output=True,
            timeout=30,
...
def _update_wheel(layout) -> None:
    ...
    # Shell safety: cdn_bases() reads JUNCTION_CDN_BASE which is operator-set.
    ...
    if not feed_url.startswith("https://"):
        ...
        sys.exit(1)
    try:
        req = urllib.request.Request(feed_url, headers={"User-Agent": "junction-update/1"})
    ...
    if manifest.get("schema") != "junction-cli-artifact-manifest-v1":
    ...
    from junction.dashboard.handlers.updates import _is_newer
...
def _should_reconcile_launchd_launcher() -> bool:
    ...
    ``JUNCTION_HOME`` does not scope, so a dev, pod, or worktree gateway
...
async def _gateway(
    *,
    ...
    from junction.cli import _ensure_node, _node_ok  # circular import: cli -> cli_server -> cli
...
def _service_cmd(args: argparse.Namespace) -> int:
    ...
    Wraps :mod:`junction.service.controller` so that platform detection
...
def _sandbox_cmd(args: argparse.Namespace) -> int:
    ...
    live in :mod:`junction.service.controller`, and this layer only parses
