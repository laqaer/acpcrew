...
the same in-session model the rest of Junction uses (no new model plumbing). It
...
_AUTHOR_SYSTEM = """\
You are authoring a Junction DYNAMIC WORKFLOW: one Python module that orchestrates
...
class WorkflowService:
    ...
    async def author(
        ...
        *,
        ...
        # uses the tool-less ``junction-lite`` agent. That is the lever that makes a
        ...
        provider, *_ = await self._sessions.get_or_create(key, agent="junction-lite")
        try:
            errors: list[str] = []
            source = ""
