...
human approval; see :mod:`junction.connections.l0_record`.
...
CONSECUTIVE failures (:mod:`junction.connections.l0_drift`) so one flaky night
...
from junction.connections import l0_drift
from junction.connections.l0_record import ObservedBaseline, RecordError, record_baselines
...
def _staleness(*, fail_on_stale: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "overdue": overdue,
        "runbook": (
            "run `python -m junction.connections.l0_probe --record` on a "
            "networked machine and commit src/junction/connections/registry.json"
        ),
    }
