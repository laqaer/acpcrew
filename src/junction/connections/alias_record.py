...
5. **The TRANSITION belongs to the writer of the spec, not to the alias pass.**
   :func:`~junction.agent.rebuild_agent_config` opens and commits it around its own
...
6. **Both record writes are LOUD, and the pending write FAILS CLOSED.**
   :func:`~junction.atomic_write.atomic_write` publishes through a temp file and
...
:func:`~junction.agent.rebuild_agent_config` writes one canonical spec, and that
