...
what the last pass emitted (:mod:`junction.connections.alias_record`), so every
...
inertness); see :func:`junction.agent._apply_connection_tool_aliases`.
...
7. **Ownership of an already-written pair is PERSISTED, never inferred from its
   ...
   :mod:`junction.connections.alias_record`, which now owns the decision. This
...
from junction.connections.registry import get_all_registry_providers
...
def derived_alias(slug: str, tool: str) -> str:
    ...
    record in :mod:`junction.connections.alias_record`.
    """
