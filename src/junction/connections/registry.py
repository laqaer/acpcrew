...
class Provider(_RequiredProviderFields, total=False):
    ...
    :mod:`junction.connections.tool_aliases`), so declaring one does not rename
...
# Public so the baseline recorder (junction.connections.l0_record) rewrites the
...
# refresh is `python -m junction.connections.l0_probe --record` run by a human
