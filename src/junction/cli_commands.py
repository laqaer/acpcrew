...
from junction import __version__, beacon, platform_compat
from junction.agent import reset_agent_model
...
from junction.apps.scaffold import scaffold_app
from junction.cli_server import _marker_port, resolve_client_port
...
from junction.cron import CronSchedule, CronService, format_schedule
...
from junction.eval.runner import EvalRunner, format_results, score_by_dimension
from junction.eval.scenario import AssertionType, load_scenario, load_scenarios
...
from junction.vector_memory import LessonWriteOutcome, VectorMemoryStore, _lesson_display_text
...
def _internal_secret(port: int) -> str:
    ...
    browser cookie. Mirrors `junction.mcp_core._internal_secret`.
...
def _run_app_mcp_server(app_name: str) -> None:
    ...
    """
    module_name = f"junction.apps.builtins.{app_name.replace('-', '_')}.mcp_server"
    try:
    ...
    if action == "list":
        ...
        for name, agent in cfg.agents.items():
            ...
            print(
                ...
                f"{getattr(agent, 'source', 'junction'):<12}"
            )
        ...
        if args.name in cfg.agents:
            ...
            sys.exit(1)
        cfg.agents[args.name] = JunctionAgentConfig(
...
def _agent_reset_model(args: argparse.Namespace) -> None:
    ...
    """
    name = getattr(args, "agent", None) or "junction"
    try:
...
def _cron(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        # JUNCTION_PORT only, so on a --port auto gateway it names a sibling, and the
...
def _policy(args: argparse.Namespace) -> None:
    ...
    if action == "show":
        ...
        from junction.platform.governance_profiles import _profiles_dir
...
def _pod(args: argparse.Namespace) -> None:
    ...
    test instances)."""
    from junction.pod.cli import dispatch
...
def _container_valued_sections() -> dict[str, type]:
    ...
    ``JunctionConfig`` later is covered without anyone remembering to edit this.
    ...
    """
    out: dict[str, type] = {}
    for field in dataclasses.fields(JunctionConfig):
        ann = field.type
...
def _assert_config_sections_are_objects(raw: dict) -> None:
    ...
    ``JunctionConfig.load()``, and ``load()`` is itself destructive on a file it
...
def _tailnet(args: argparse.Namespace) -> None:
    ...
    # Validate the raw file BEFORE anything calls ``JunctionConfig.load()``, for
    ...
    # ``resolve_client_port`` → ``JunctionConfig.load()``, so there is no version of
    ...
    #
    # An explicit ``--port``/``JUNCTION_PORT`` still wins over both, because that is
    ...
    if not port and os.environ.get("JUNCTION_PORT") is not None:
        port_source = "JUNCTION_PORT"
...
def _telemetry(args: argparse.Namespace) -> None:
    ...
    try:
        effective = JunctionConfig.load().telemetry.beacon_enabled
