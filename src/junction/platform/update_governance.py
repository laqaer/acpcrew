...
``governance.UpdatePins``), applied at the three places Junction replaces its own
code: ``POST /api/update``, ``junction update``, and the unattended gateway-boot
...
def is_primary_branch(branch: str) -> bool:
    ...
    * Letting it merely narrow (accept only when it agrees) turns the same
    ...
    unattended update, only the badge. ``junction update`` and the dashboard
...
def tracks_upstream(proj: str, branch: str, *, remote: str = "origin") -> bool:
    ...
    * the branch — ``branch.main.remote=origin`` with
    ...
    through ``junction update`` or the dashboard, both of which have a human
    deciding.
    """
