...
:func:`~junction.platform.governance.active_update_pins`), which a
...
async def _kill_and_reap(proc: asyncio.subprocess.Process) -> None:
    """Delegate to :func:`junction.platform_compat.kill_and_reap`.
    ...
    # ``junction.platform_compat.kill_process_tree_async`` (inside the shared
...
def _trusted_path_env() -> dict[str, str] | None:
    ...
    path in the policy (``/opt/acme/bin/acme-pkg update junction``). An
...
async def apply_policy_update() -> bool | None:
    ...
    boot check. Without it, `POST /api/update` and `junction update` would run
...
def resolve_provider() -> CommandProvider | None:
    ...
    **Presence is the selection.** When the active pins define a ``check_command``
    ...
    """
    try:
        from junction.platform.governance import active_update_pins
