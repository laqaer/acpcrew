"""Install-layout detection shared between ``junction update`` and the dashboard.
...
def detect_install_layout() -> InstallLayout:
    ...
    ``is_git_worktree`` FIRST classified a container whose ``JUNCTION_PROJECT_DIR``
...
def release_channel() -> str:
    """The release channel this install follows, from ``$JUNCTION_HOME/channel``.
