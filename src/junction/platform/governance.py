"""The Junction Governance Model — archetypes, ceiling, evaluator, loader.
...
_POLICY_ENV = "JUNCTION_SECURITY_POLICY"
...
# document claims.  Reported verbatim by ``junction policy show`` so an operator
...
_CU_ACTION_CLASSES: Dict[str, Tuple[str, ...]] = {
    ...
    # Control plane: drops Junction's OWN cached snapshots and touches no other
...
CU_MCP_SERVER = "junction-computer"
...
def computer_use_action_from_title(tool_name: str) -> str:
    ...
    ``mcp__junction-computer__computer_click`` -> ``computer_click``.  Returns
...
# them: a policy cannot deny a capability by leaving it out.  ``junction policy
...
SCOPE_CATALOG: Dict[str, ScopeSpec] = {
    ...
    # by junction.safety_override at the activation seam, against the HOST
    ...
    #   * ``junction telemetry enable`` — exits 1 without writing, for the same
    ...
    #   * ``junction config set dashboard.tailscale.enabled true`` — exits 1
...
def _policy_trust_settings() -> Tuple[bool, Dict[str, str]]:
    ...
    Sourced from the **admission policy** (``JUNCTION_ADMISSION_POLICY`` env path,
...
def _policy_signature_required() -> bool:
    ...
    ``read_policy_trust_root`` already does.  ``junction doctor`` surfaces it.
...
def strip_ungoverned_auto_approve(servers: Mapping[str, object]) -> Dict[str, object]:
    ...
    ``agent.py``'s managed-server block states the rule for the servers Junction
...
def may_skip_gate_now(ref: str) -> bool:
    ...
    try:
        from junction.platform.governance_profiles import any_configured_profile_governs
...
def sanitize_agent_config_governance(config: MutableMapping[str, object]) -> None:
    ...
    config about to be written to ``junction.json``.
