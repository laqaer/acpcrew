...
if TYPE_CHECKING:
    from junction.platform.interfaces import ImportSource, McpScope

from junction import security, sso_status
from junction.platform.interfaces import CapabilityResult, InterceptDecision

# ``agent``, ``sandbox``, ``embeddings``, ``apps.registry`` and ``slack.enterprise``
# import ``junction.platform`` at module-load time, so importing them at the top
...
# ``junction.platform.context`` (the ``sel.py`` deferred pattern).  Because that
...
class DefaultAgentRuntime:
    ...
    def managed_mcp_servers(self) -> Dict[str, dict]:
        ...
        from junction import agent  # circular import: agent imports platform
...
class DefaultSandboxPolicy:
    ...
    def strict_dirs(self) -> List[str]:
        from junction import sandbox  # circular import: sandbox imports platform
...
class DefaultEmbeddingSource:
    ...
    def registry_model(self) -> str:
        from junction import embeddings  # circular import: embeddings imports platform
...
class DefaultAppRegistryPolicy:
    ...
    def public_git_hosts(self) -> "frozenset[str]":
        from junction.apps import registry  # circular import: apps.registry imports platform
