...
Precedence (first match wins):
  1. ``JUNCTION_PROFILE`` env var (explicit operator/dev override).
  2. A non-empty ``junction.plugins`` entry-point group (companion installed) —
     the cheap, authoritative signal: capability comes from the installed
     companion, so its presence is what actually matters.
  3. Identity signal: a present SSO-marker directory (``JUNCTION_SSO_MARKER_PATH``,
     default ``~/.midway``), but ONLY when the opt-in ``JUNCTION_SSO_PROFILE_PROBE``
     env var (or the legacy ``JUNCTION_MIDWAY_PROFILE_PROBE``, still set by
...
from junction.platform.context import PROFILE_ENTERPRISE, PROFILE_STANDALONE
...
# Back-compat aliases for the ``JUNCTION_PROFILE`` env value: an operator or a
...
#
# Two names are honored: the current ``JUNCTION_SSO_PROFILE_PROBE`` and the
# legacy ``JUNCTION_MIDWAY_PROFILE_PROBE`` still set by already-deployed managed
...
_SSO_PROBE_ENV = "JUNCTION_SSO_PROFILE_PROBE"
_LEGACY_SSO_PROBE_ENV = "JUNCTION_MIDWAY_PROFILE_PROBE"
...
_SSO_MARKER_ENV = "JUNCTION_SSO_MARKER_PATH"
_DEFAULT_SSO_MARKER_NAME = ".midway"


def resolve_profile(cfg: "JunctionConfig", *, entry_points: "Sequence[object]") -> str:
    """Resolve the active profile.  See module docstring for precedence."""
    # 1. Explicit env override.
    env = os.environ.get("JUNCTION_PROFILE", "").strip().lower()
    if env in _VALID_PROFILES:
        return env
    if env in _PROFILE_ALIASES:
        return _PROFILE_ALIASES[env]
    if env:
        logger.warning("Unknown JUNCTION_PROFILE=%r; falling back to standalone", env)
