...
def is_git_worktree(root: str) -> bool:
    ...
    **When git cannot answer, this degrades to the on-disk markers of a
    ...
    unrelated repository that ``JUNCTION_PROJECT_DIR`` happens to name — that
...
def running_from_checkout(root: str) -> bool:
    """Is *root* the checkout the ``junction`` package THIS process runs from?
    ...
    release install (a wheel in its own venv) whose ``JUNCTION_PROJECT_DIR``
    ...
    Delegates to :func:`junction._bootstrap._source_checkout_root`, which
...
class UpdateCapability:
    ...
    def for_channel(self, channel: str) -> "UpdateCapability":
        ...
        """
        if self.managed_by != MANAGED_BY_JUNCTION or self.remediation is None:
            return self
...
def derive_capability(
    ...
    *install_root* defaults to ``JUNCTION_PROJECT_DIR`` and *dist* to the value
    ...
    if install_root is None:
        install_root = os.environ.get("JUNCTION_PROJECT_DIR", "")
