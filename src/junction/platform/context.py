...
Junction uses the Composed Platform Providers (CPP) model to share one core
...
``junction.platform.interfaces`` — and ships a ``Default*`` adapter for each
...
if TYPE_CHECKING:  # avoid import cycles — config.loader imports heavy modules
    ...
    from junction.platform.governance import GovernanceCeiling
    from junction.platform.interfaces import (
    ...
    from junction.platform.security_authority import PolicyAuthority
...
def _reserved_slot_is_default(field_name: str, value: Any) -> bool:
    ...
    try:
        ...
        # ``junction`` import — every one above is inside ``TYPE_CHECKING`` —
        # because it is the leaf the rest of the platform imports.  ``defaults``
        # pulls in ``junction.security`` and ``sso_status``, which reach back
        ...
        from junction.platform import defaults as _defaults
...
class PlatformContext:
    ...
    Built once at boot by :func:`junction.platform.bootstrap.bootstrap_context`
...
def current_context() -> PlatformContext:
    ...
    probe or ``JUNCTION_PROFILE=enterprise``) but no context was installed — meaning
    ...
    if _ACTIVE is None:
        ...
        # importing junction.platform stays cheap; only the lazy-default path needs it.
        from junction.config.loader import JunctionConfig
        from junction.platform.bootstrap import (  # circular import: bootstrap imports context
            build_default_context,
        )
        from junction.platform.discovery import (  # circular import: discovery imports context
            plugin_entry_points,
        )
        from junction.platform.profile import (
...
def redact_via_context(text: str) -> str:
    ...
    try:
        ...
        from junction.security import redact as _security_redact
