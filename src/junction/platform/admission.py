...
source** (`JUNCTION_ADMISSION_POLICY` env path, or
`~/.junction/admission_policy.json`), never from the plugin being admitted, so a
...
_POLICY_ENV = "JUNCTION_ADMISSION_POLICY"
...
_MANIFEST_FILENAME = "junction_plugin.json"
...
def canonical_signing_bytes(body: Mapping[str, object]) -> bytes:
    """The ONE canonicalization every Junction trust-root signature is taken over.
...
_DEFAULT_POLICY_BODY: Dict[str, object] = {
    ...
    "capability_ceiling": {},
    "_comment": (
        "Default permissive admission policy seeded by Junction at first run. "
...
def seed_default_policy() -> bool:
    ...
    is skipped entirely when the ``JUNCTION_ADMISSION_POLICY`` env override is
...
def _record_admission_posture(posture: str) -> None:
    ...
    try:
        from junction.platform.governance_health import set_admission_posture
...
def load_admission_policy() -> AdmissionPolicy:
    ...
    A MISSING policy (no ``JUNCTION_ADMISSION_POLICY`` env override and no file at
...
def read_policy_trust_root() -> "AdmissionPolicy":
    ...
    here is a reliability event: log it, stay predictable, let ``junction doctor``
...
def evaluate_admission(
    ...
    #    package spoof an approved identity via junction_plugin.json; checking
