...
``junction.plugins`` entry point pointing at its composition root
...
def plugin_entry_points() -> List[importlib.metadata.EntryPoint]:
    """Return the registered ``junction.plugins`` entry points (may be empty).
...
def discover_companion_context(
    profile: str, cfg: "JunctionConfig", *, policy: "AdmissionPolicy | None" = None
    ...
    if not eps:
        raise PlatformCompositionError(
            ...
            "companion package, or set JUNCTION_PROFILE=standalone to run the "
