...
class ProviderRegistry(Protocol):
    ...
    def create_factory(self, cfg: "JunctionConfig") -> Callable[..., Any]:
...
class AgentExecutableResolver(Protocol):
    ...
    ``sandbox.py`` calls this before applying Junction's OS-level sandbox so an
    ...
    ``kiro-cli`` paths and the explicit ``JUNCTION_KIRO_BIN`` override are used
...
class IdentityProvider(Protocol):
    ...
    def preflight_checks(self) -> List[Callable[[], None]]:
        ...
        WIRED: ``junction.preflight.run_preflight_checks`` (called from the
...
class ExternalAccessPolicy(Protocol):
    ...
    * **Cloud deployment.** ``junction/deploy/`` provisions real infrastructure
...
class AppsLoader(Protocol):
    ...
    def default_registries(self) -> List[Dict[str, Any]]:
        ...
        inside ``JunctionConfig`` is deliberate: an edition default must never be
...
class KnowledgeProvider(Protocol):
    ...
    def extra_connectors(self, cfg: "JunctionConfig") -> Dict[str, Any]:
        """Return ``{source_type: BaseConnector}`` to merge into the sync map.

        The values are ``junction.knowledge.connectors.base.BaseConnector``
...
class TunnelProvider(Protocol):
    ...
    def status_snapshot(self) -> Optional[Dict[str, Any]]:
        ...
        Edition-neutral primitives only (no import of ``junction.tunnel``): keys
    ...
    async def ensure_available(self, *, install: bool = True) -> str:
        ...
        demand (the ``/junction dashboard`` command, the setup wizard).
...
class JailProvider(Protocol):
    ...
    **Re-entry contract.** Before a successful re-exec the core gate sets the
    ``JUNCTION_JAILED`` env marker; the jailed CHILD re-runs the gate, sees the
    ...
    def maybe_reexec_into_jail(self, argv: List[str], mode: str) -> Optional[int]:
        ...
        **mode contract:** ``"off"`` / already-jailed / disabled → ``None``.
        ...
        gate's ``JUNCTION_JAILED`` re-entry guard short-circuits before this is
