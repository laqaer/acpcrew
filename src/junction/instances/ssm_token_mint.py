...
:mod:`junction.cloud.ssm`, the launcher's existing send-command chokepoint) so
...
* The *remote* command is the same ``junction token``/``restart`` subcommand
...
def _redacted_tail(text: str, limit: int = _OUTPUT_TAIL_CHARS) -> str:
    ...
    Mirrors :func:`junction.instances.token_mint._redacted_output_tail`'s
...
async def mint_remote_token_ssm(
    ...
    *,
    ...
    """Run ``junction token`` on *ssm_target* over SSM and return the parsed JWT.

    Mirrors :func:`junction.instances.token_mint.mint_remote_token`'s contract
...
async def run_remote_junction_ssm(
    ssm_target: str,
    subcommand: str,
    *,
    aws_profile: str = "",
    ...
    """Run ``junction <subcommand>`` on *ssm_target* over SSM.

    The SSM-transport sibling of
    :func:`junction.instances.token_mint.run_remote_junction`. Returns
    ...
    logger.info("Running 'junction %s' on %s over SSM", subcommand, target)
    try:
        result = await asyncio.wait_for(
            asyncio.to_thread(
                cloud_ssm.run_command,
