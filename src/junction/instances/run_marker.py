...
The problem this solves:
    Token mint SSHes to the remote desktop and runs ``junction token`` resolved
    from a fixed PATH candidate list (:data:`token_mint.REMOTE_BIN_CANDIDATES`,
    first entry ``$HOME/.local/bin/junction``). When that launcher symlinks into
    an *uninstalled* git worktree (no ``.venv``), every mint fails with
    "Junction venv not found" — even though the gateway itself is up and healthy,
...
The fix:
    At startup the gateway records the absolute path to *its own* ``junction``
...
def read_launcher(port: int) -> str | None:
    ...
    ``junction`` launcher (see :func:`write_marker`), or ``None`` when the
    marker is absent, unreadable, or empty (a source-tree ``python -m
    junction`` launch records no launcher). Same trust argument as
...
def gateway_launcher_path() -> str | None:
    """Absolute path to the running gateway's own ``junction`` launcher.

    In an OSS venv install ``sys.executable`` is ``<venv>/bin/python`` and the
    console script ``pip install -e .`` creates is its sibling
    ``<venv>/bin/junction`` (``junction.exe`` on Windows). Returns ``None`` when
    ...
    if not exe:
        return None
    base = Path(exe).with_name("junction.exe" if os.name == "nt" else "junction")
    try:
...
def write_marker(port: int) -> None:
    ...
    * ``gateway-<port>.bin`` — the launcher path for the SSH token mint, or
      **empty** when no console script sits beside ``sys.executable`` (a
      source-tree ``python -m junction`` launch). Mint's shell clause requires
    ...
    * ``gateway-<port>.pid`` — this process's pid, the identity a client checks
    ...
    Both go through :func:`junction.atomic_write.atomic_write`, which uses a
    ...
    if not launcher:
        logger.debug(
            "No venv junction launcher next to %s — writing port-only run-marker", sys.executable
        )
    try:
...
def prune_markers(*, keep_port: int) -> None:
    ...
    from junction import port_resolution
