"""Mint a remote Junction dashboard token over SSH.

Build the shell snippet that runs ``junction token`` on the remote desktop and
...
DEFAULT_REMOTE_BIN = "$HOME/.local/bin/junction"
...
# Extract the JWT from a `junction token` URL: http://localhost:7777?token=eyJ...
# (also matches https://.../?token=...&foo=bar).
_TOKEN_RE = re.compile(r"[?&]token=([^\s&]+)")

# Bare Junction-token / JWT shape, used to scrub a token that reached stdout
...
# How long to wait for the remote `junction token` to return before giving up.
...
def parse_token_from_stdout(stdout: str) -> str:
    """Extract the JWT from a ``junction token`` URL, or ``""`` if absent."""
...
def _token_subcommand(
    ...
    """Return the ``junction token`` subcommand (ttl/port pre-validated).

    ``--port`` is essential when the remote gateway isn't on the default 7777:
    ``junction token`` calls its own dashboard to mint, so the port must match
...
def build_candidate_command(
    ...
    *,
    ...
    Generic over the junction *subcommand* (e.g. ``token``, ``restart``).
    ...
    *running* gateway wrote for that port (``<data-home>/run/gateway-<port>.bin``,
    ...
    See :mod:`junction.instances.run_marker`.
    """
    ...
    return prefix + " ".join(
        [
            ...
            "  fi;",
            "done;",
            f'echo "junction binary not found in any of: {", ".join(candidates)}" >&2;',
            "exit 127",
        ]
    )
...
def _run_marker_clause(subcommand: str, port: int) -> str:
    ...
    :func:`junction.instances.run_marker.write_marker`) under each candidate
...
def _redacted_output_tail(stdout: str, limit: int = _OUTPUT_TAIL_CHARS) -> str:
    ...
    Why this exists: ``junction token`` on a remote that predates the
...
async def mint_remote_token(
    ...
    *,
    ...
    """SSH to *ssh_host*, run ``junction token``, and return the parsed JWT.

    *remote_port* is passed to ``junction token --port`` so the remote mint
    targets the gateway the instance actually runs on (not the default 7777).

    *embed_parent_port* is passed to ``junction token --embed-parent-port`` so the
    ...
    if proc.returncode != 0:
        ...
        # Belt-and-suspenders for older remotes: pre-fix `junction token` printed
...
async def run_remote_junction(
    ...
    *,
    ...
    """Run ``junction <subcommand>`` on *ssh_host* over SSH.
    ...
    *marker_port* — when the caller knows the remote dashboard port, pass it so
    ...
    ``~/.local/bin/junction`` points at an uninstalled worktree.
    ...
    *connect_timeout_secs* — this is the same one-shot ssh-exec shape as
    ...
    logger.info("Running 'junction %s' on %s", subcommand, ssh_host)
    try:
        proc = await asyncio.create_subprocess_exec(
            *argv,
            stdout=asyncio.subprocess.PIPE,
