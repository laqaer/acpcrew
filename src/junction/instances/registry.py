...
* Writes go through :func:`junction.atomic_write.atomic_write` (temp file +
...
in-memory copy, so a live gateway and an out-of-band ``junction`` CLI edit don't
...
from junction.config.loader import _DEFAULT_PORT, config_dir
from junction.instances.constants import TTL_PATTERN
from junction.instances.validation import _AWS_PROFILE_RE as _validation_aws_profile_re

logger = logging.getLogger(__name__)

# Monkeypatchable in tests via ``monkeypatch.setattr`` alongside JUNCTION_HOME,
# per the shared-state test-isolation lesson. ``None`` means "derive from
# config_dir() at call time" so JUNCTION_HOME overrides are always honoured.
