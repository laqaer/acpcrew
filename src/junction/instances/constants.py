...
in ``junction.config.loader``; a user can override them via
``junction config set instances.<key> <value>``. Keeping the canonical default
...
# ``junction config set instances.ssh_compression false``. See §5.2.
...
# ``junction config set instances.connect_timeout_secs <value>`` so operators on
...
# How long (secs) to wait for the remote `junction token` to return before
...
# user-tunable via ``junction config set instances.mint_timeout_secs <value>``.
...
# `junction restart` that invalidates tokens); an iframe loaded with a stale
