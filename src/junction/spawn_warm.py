...
endpoint and Multitask Mode dispatch. Keeping it in the handlers package forced
``multitask_chat`` to choose between a function-local import (against the repo's
``top-level-imports`` rule) and pulling the ENTIRE handler tree into every
process that imports it — including the Slack gateway, which imports it at
module scope. This module depends on config, cwd validation and agent
...
from junction.subagent import validate_cwd
