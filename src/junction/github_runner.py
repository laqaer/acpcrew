...
This module MUST NOT import ``junction.dashboard.*`` (at module scope or
...
STRICT_PROVIDER_BIN_ENV = "JUNCTION_PROVIDER_BIN_STRICT"
...
PROVIDER_EXECUTABLE_DIRS = (
    "/usr/local/libexec/junction",
    "/usr/libexec/junction",
    "/opt/homebrew/bin",
    "/usr/local/bin",
    "/usr/bin",
...
# its own caller-specific override (JUNCTION_ISSUE_RADAR_GH / JUNCTION_SAGE_GH).
GH_BIN_ENV = "JUNCTION_GH_BIN"
...
GH_PREVALIDATED_ENV = "_JUNCTION_GH_PREVALIDATED"
...
def agent_writable_roots() -> tuple[Path, ...]:
    ...
    """
    raw_roots = [os.environ.get("JUNCTION_PROJECT_DIR")]
    try:
...
def check_provider_path_component_windows(
    ...
    information on Windows (see :mod:`junction.windows_acl`).
...
def validate_provider_executable(candidate: str) -> str:
    ...
    * anything inside the agent's project checkout or workspace root, the one
    ...
    Set ``JUNCTION_PROVIDER_BIN_STRICT=1`` on shared or multi-tenant hosts to
    ...
    there (see :mod:`junction.windows_acl`). An **elevated** gateway is refused
...
# JUNCTION_GH_BIN value) so a changed override never serves a stale binary
...
def resolve_gh(*, override_env: str = "", cache: bool = True) -> str:
    ...
    generic ``JUNCTION_GH_BIN``, then :func:`provider_executable_candidates`
...
def _audit_run(
    ...
    try:
        from junction.sel import sel  # lazy: heavy runtime dep, see module docstring
