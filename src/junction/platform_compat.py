...
#
# Nothing Junction does at startup needs these folders: they are only ever
...
def ensure_utf8_console() -> None:
    ...
    JunctionHub client) Python encodes prints as cp1252 — so the FIRST non-ASCII
    ...
    for name in ("stdout", "stderr"):
        ...
        try:
            ...
            # observed in the 3-layer Windows gateway spawn: junction.exe launcher
...
def _log_tool_outside_trusted_dirs(name: str, directories: tuple[str, ...]) -> None:
    ...
    tool they already have, and ``junction stop`` would quietly no-op. The
...
def find_port_listeners(port: int) -> list[PortListener]:
    ...
    # silently drops IPv6 listeners entirely, so `junction stop` /
    # `junction restart` no-op on a dual-stack or `[::]`-bound gateway
...
def process_command_line(pid: int) -> str:
    ...
    Used to confirm a listener PID is actually a Junction gateway when the image
    name alone is ambiguous (the venv ``junction.exe`` re-execs ``python.exe``).
    """
    try:
        if sys.platform == "linux":
            raw = Path(f"/proc/{pid}/cmdline").read_bytes()
...
async def kill_pid_async(pid: int, sig: int = SIGTERM) -> bool:
    ...
    :func:`junction.executors.subprocess_executor` (the same bounded pool the
...
async def kill_process_tree_async(pid: int, sig: int = SIGTERM) -> bool:
    ...
    off the loop via :func:`junction.executors.subprocess_executor`. Raises
...
async def kill_and_reap(proc: asyncio.subprocess.Process, *, timeout: float | None = None) -> None:
    ...
    async def _cleanup() -> None:
        ...
        if not same_group:
            ...
            # call time, so tests patching ``junction.platform_compat.
...
# POSIX execute bit). A hook is a small script Junction shells out to; on
...
def is_executable_file(
    ...
    *,
    ...
    determined when Junction actually invokes it, not by a meaningless bit.
...
def find_python_interpreter(reject: Optional[Callable[[str], bool]] = None) -> str | None:
    ...
    need *an* interpreter to re-exec Junction itself should prefer
...
def count_open_fds() -> int | None:
    ...
    The one shared probe behind both the ``junction.process.open_fds`` gauge
