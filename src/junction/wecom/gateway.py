...
``JUNCTION_READY``. ``warn_if_wecom_uncredentialed`` remains as the
...
def warn_if_channel_uncredentialed(
    ...
    ``JUNCTION_READY``, off the boot-path window), once per collapsed-flag
    ...
    channel_logger = logging.getLogger(f"junction.{channel_type}.gateway")
