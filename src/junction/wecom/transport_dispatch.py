...
if TYPE_CHECKING:
    ...
    from junction.wecom.client import WeComClient, WeComInbound

logger = logging.getLogger(__name__)

# Canonical kiro-cli agent fallback so WeCom sessions load junction-core
...
# Telegram paths' _DEFAULT_JUNCTION_AGENT.
...
class WeComDispatcher:
    ...
    async def handle_message(self, inbound: "WeComInbound") -> None:
        ...
        # configured default, else the canonical "junction" agent -- so the
        # session loads junction-core (spawn_run) instead of kiro-cli's bare
