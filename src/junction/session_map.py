...
from junction.config.paths import config_dir, kiro_sessions_dir
...
from junction.sel import _infer_source, sel

logger = logging.getLogger(__name__)

# Public because another instance's map is read by file path, not through this
# class: :func:`junction.session_storage.cotenant_sids` opens the map belonging
...
def _normalize_unbind_reason(reason: str) -> str:
    ...
    The runtime guard behind :data:`~junction.messaging.link.UNBIND_REASONS`. An
...
class SessionMap:
    ...
    def _mirror_key(self, key: str) -> str:
        ...
        (:func:`~junction.messaging.link.legacy_dashboard_mirror_key`); when
    ...
    def channel_key_for_stem(self, stem: str) -> str:
        ...
        ``discord_junction_direct_123`` there is no way to tell which
