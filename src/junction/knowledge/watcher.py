...
from junction.config.loader import JunctionConfig, default_project_dir
...
class KnowledgeWatcher:
    ...
    async def _discover_drop_folder(self) -> None:
        ...
        toggling the flag takes effect without a restart, matching Junction's
    ...
    def _chunk_budget() -> int:
        ...
        """
        try:
            return max(0, int(JunctionConfig.load().knowledge.auto_ingest_chunk_budget))
        except Exception:
    ...
    def _sweep_chunk_budget() -> int:
        ...
        """
        try:
            return max(0, int(JunctionConfig.load().knowledge.sweep_chunk_budget))
        except Exception:
    ...
    async def _maybe_dedup_sweep(self) -> None:
        ...
        """
        try:
            every = max(0, int(JunctionConfig.load().knowledge.dedup_every_n_sweeps))
        except Exception:
