...
from junction.effort import EFFORT_LEVELS, is_valid_effort
...
# because this pool does NOT ride the shared junction.acp.worker_pool engine
# (which shields its workers by construction). Imported defensively so llm_pool
# stays importable standalone / in hermetic tests.
try:
    from junction.session_pid import register_protected_pid, unregister_protected_pid
...
# Sandbox modes accepted by ``junction.sandbox.wrap_argv`` (see its docstring).
...
def _get_sandbox_mode(config: Optional[dict] = None) -> str:
    ...
    - ``sandbox`` **present but malformed/unrecognised** (typo, wrong type) ->
      ...
      intent to run unsandboxed, so we re-enable Junction's OS-level confinement
...
class AcpWorker(Worker):
    """Long-lived AcpClient session with junction-knowledge agent."""
    ...
    async def start(self) -> None:
        if AcpClient is None:
            raise RuntimeError("AcpClient not available (junction.acp.client not installed)")
...
class CCWorker(Worker):
    ...
    async def _spawn(self) -> None:
        ...
        # vanilla machine). Users can opt in by setting JUNCTION_KNOWLEDGE_FETCH_TOOLS
        # to a comma-separated list of MCP tool names their backend exposes.
        fetch_tools = os.environ.get("JUNCTION_KNOWLEDGE_FETCH_TOOLS", "").strip()
        if fetch_tools:
