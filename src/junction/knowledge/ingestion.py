...
def get_embed_rate_limiter() -> EmbedRateLimiter:
    ...
    try:
        ...
        rate = max(0, int(JunctionConfig.load().knowledge.embed_rate_limit))
    except Exception:
        rate = 0
...
def _max_ingest_file_mb() -> float:
    ...
    from junction.config.loader import JunctionConfig  # circular import
    try:
        return JunctionConfig.load().knowledge.max_ingest_file_mb
    except Exception:
