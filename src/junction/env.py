...
# called from ``JunctionConfig.load``) rather than read here. That is not
...
def publish_config_path_dirs(values: object) -> None:
    ...
    Called by ``JunctionConfig.load`` on every load, which is what keeps the
...
_NODE_BIN_DIR_ENV = "JUNCTION_NODE_BIN_DIR"
...
def _validated_bin_dir(val: str) -> str | None:
    ...
    ``JUNCTION_NODE_BIN_DIR`` override and the ``ensure-node.sh`` marker file --
...
def node_bin_dirs() -> tuple[str, ...]:
    ...
    1. ``JUNCTION_NODE_BIN_DIR`` -- explicit operator override.
    ...
    # Under a JUNCTION_HOME override data_home() mkdirs, so it can raise on an
...
def _ensure_node_script() -> Path | None:
    ...
    Search order mirrors :func:`junction.cli._ensure_node`: the explicit
    ``JUNCTION_PROJECT_DIR`` first, then the source-tree root two levels above
...
def is_toolbox_install() -> bool:
    """Return True if the running junction binary was installed via Toolbox."""
...
def git_build_info() -> tuple[str, str]:
    ...
    Reads ``JUNCTION_PROJECT_DIR`` (the git tree the gateway runs from) and
    ...
    pip-wheel installs (no ``JUNCTION_PROJECT_DIR`` or no ``.git``) — so callers
...
def augmented_path(base_path: str = "", *, home: str | None = None) -> str:
    ...
    When Junction runs under systemd or another non-login shell the
    ...
    to resolve the ``junction`` / ``junction-core`` console-script wrappers
...
def activate_mise(env: MutableMapping[str, str] | None = None) -> list[str]:
    ...
    installed, when disabled via ``JUNCTION_NO_MISE``, or when invoking /
    ...
    if target.get("JUNCTION_NO_MISE"):
        logger.debug("mise activation skipped: JUNCTION_NO_MISE set")
        return []
    mise = _mise_bin()
    if not mise:
        return []
