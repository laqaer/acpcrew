...
Lessons are saved via the ``junction learn`` CLI (called by the LLM via bash)
...
from junction.project_scope import canonical_scope, project_scope_satisfied
...
class Lesson:
    ...
    # ever withheld. See ``junction.project_scope``.
...
class LessonStore:
    ...
    def save(self, lesson: Lesson) -> None:
        ...
        explicit refinement (the /api/lessons route, ``junction learn add``)
