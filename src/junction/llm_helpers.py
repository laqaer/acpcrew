...
from junction.acp.client import AcpError, AcpPromptBusy, advertised_model_ids
from junction.acp.types import EVENT_STEER_CONSUMED, TurnUsage
from junction.config.loader import JunctionConfig
from junction.hooks import fire_tool_hooks, get_global_hook_store
...
from junction.security import is_denied, is_sensitive_bash_command, is_sensitive_path
from junction.sel import sel as _sel
...
def configured_fallback_chain() -> tuple[str, ...]:
    ...
    ``JunctionConfig.load()`` is fingerprint-cached (mtime/size/mode of both
    ...
    """
    try:
        fm = JunctionConfig.load().agent.fallback_model
    except Exception:
        return ()
    if not fm:
        return ()
    ...
    try:
        ...
        try:
            try:
                ...
                from junction.dashboard.handlers.usage import persist_token_record_async
...
def _provider_label(provider: Any) -> str:
    ...
    """
    try:
        from junction.acp.types import PROVIDER_LABEL_DEFAULT
...
async def background_turn(
    ...
    *,
    ...
    """
    from junction.session import BACKGROUND_AGENT, BACKGROUND_KEY  # circular import
...
def save_conversation_turn(
    ...
    ``/junction sessions`` displays the correct agent name.
    """
    log.append(
        key,
        "user",
...
async def save_conversation_turn_off_loop(
    ...
    raises :class:`~junction.history.HistoryLockTimeout` on any concurrent
    ...
    Unlike :func:`~junction.history.append_off_loop`, this **awaits** the write
    ...
    The whole turn is written under one :meth:`~junction.history.ConversationLog.atomic_appends`
