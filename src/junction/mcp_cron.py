...
Runs as ``junction mcp-cron`` — kiro-cli spawns it as a child process
...
from junction.cron_trigger import _JOB_ID_RE, trigger_cron_job
...
from junction.mcp_shared import call_tool_with_logging, run_mcp_stdio_loop
...
from junction.sandbox import _AGENT_DENIED_ENV_KEYS
...
from junction.validation import MCP_CRON_SCHEMAS, ValidationError, validate_tool_args
...
def _vet_cron_capability_governance(session_key: str | None = None) -> str | None:
    ...
    *body* under the ``commands`` scope): this is the on/off *capability* gate.
    ...
    try:
        ...
        # log_warning=False: this runs inside the junction-cron stdio MCP server,
        ...
        # and hard-fail the stdio junction-cron tool call.
        try:
...
def _vet_command_governance(command: str) -> str | None:
    ...
    try:
        ...
        # log_warning=False: stdio junction-cron server (see _vet_cron_capability_
...
def _vet_shell_command(command: str) -> str | None:
    ...
    from junction.hooks import effective_denied_regexes_from_config
...
def _call_tool(name: str, raw_args: dict[str, Any]) -> str:
    ...
    return call_tool_with_logging(
        ...
        session_key=_resolve_session_key() or "mcp_cron",
        downstream_service="junction-cron",
    )
...
def _caller_channel_id() -> str:
    ...
    ``JUNCTION_CHANNEL_ID`` has the same defect the session key had: process
...
def _caller_is_cli() -> bool:
    ...
    return os.environ.get("JUNCTION_CLI", "") == "1"
...
def _authz_session_key() -> str:
    ...
    ``JUNCTION_SESSION_KEY``, or ``JUNCTION_HOST_PID`` plus its HMAC sidecar --
...
#: session to name: ``junction cron add`` from the CLI (``cli_commands``, which
...
#: ``junction cron add`` does not appear in ``cron_list`` from chat. The CLI
...
def _unidentified_caller_refusal(tool_name: str) -> str:
    ...
    return (
        ...
        "refused. Manage jobs from the CLI (`junction cron ...`), which carries "
        "admin authority." + strict_identity_diagnosis("junction-cron")
    )
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "cron_add":
        ...
        if channel is None:
            channel = _caller_channel_id() or os.environ.get("JUNCTION_CHANNEL_ID") or None
    ...
    if name == "cron_trigger":
        ...
        # not the client resolver: DASHBOARD_PORT reads JUNCTION_PORT only, and the
