...
from junction import __version__, release_channel
...
#: :data:`junction.release_channel._PEP440_PRERELEASE`, which owns the rule.
...
def _channel() -> str:
    ...
    Thin alias for :func:`junction.release_channel.channel`, which owns the
...
def _versions_text(note: str) -> str:
    lines = [
        f"junction_version: {__version__}",
        f"channel: {_channel()}",
...
def _issue_url(result: BundleResult, note: str, *, prefill: bool = True) -> str:
    ...
    * **The form's own fields arrive filled.** A free-form body skipped the
    ...
    if not prefill:
        ...
        # Nothing is lost: `junction doctor` already prints the bundle path and
...
def _prune_old_bundles(out_dir: Path, keep: int) -> None:
    ...
    try:
        bundles = sorted(
            out_dir.glob("junction-diagnostics-*.zip"),
...
def collect_bundle(
    *,
    ...
    filename = f"junction-diagnostics-{stamp}-{uuid.uuid4().hex[:8]}.zip"
    ...
    with os.fdopen(_fd, "wb") as _raw, zipfile.ZipFile(_raw, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        manifest = {
            "tool": "junction-diagnostics",
            "junction_version": __version__,
            "channel": _channel(),
