...
:mod:`junction.dashboard.channel_slots`.
...
def clean_session_folder(raw: object) -> str:
    ...
    rules :func:`junction.config.loader._coerce_session_folder` applies when
...
def stored_folder_name(raw: object) -> str:
    ...
    :func:`~junction.config.loader._coerce_session_folder`'s sanitising. A
...
def configured_folder_name(namespace: str) -> str:
    ...
    Reads live config (``JunctionConfig.load()`` is fingerprint-cached), so
...
async def lookup_channel_folder(state: "DashboardState", namespace: str) -> str:
    ...
    **Read-only by contract, and read under the store lock.** This is the
    ...
    :meth:`~junction.dashboard.state.DashboardState.read_folders` so it sees
...
async def ensure_channel_folder(
    ...
    *relabel* must be true ONLY on a save that actually carried a
    ...
    :meth:`~junction.dashboard.state.DashboardState.mutate_folders`
