...
from junction.embeddings import get_shared_embedder, model_file_present
...
from junction.safety_override import safety_override, until_shutdown_permitted
...
def _yolo_duration_fields() -> tuple[str, bool]:
    ...
    """
    try:
        label = str(JunctionConfig.load().agent.yolo_duration)
    except Exception:
...
async def api_status(request: web.Request) -> web.Response:
    ...
    from junction.dashboard.handlers import updates as _updates_mod
...
def _scan_mcp_processes() -> dict[str, object]:
    ...
    matches both "junction_sandbox" and "kiro-cli"); per-category counts can
    ...
    Sandbox counting platform differences:
      ...
      macOS:  sandbox-exec execs the target command, replacing the process
              ...
              "junction_sandbox" string (only in the -f path arg) is lost.
...
def _collect_system_metrics() -> dict[str, object]:
    ...
    try:
        from junction.resource_status import probe as _resource_probe
...
def _channel_members() -> tuple[str, ...]:
    ...
    from junction.messaging.registry import governed_members
