...
``$JUNCTION_HOME``. The CLI imports token_auth (and thus this module)
transitively for every ``junction`` subcommand, so an import-time write
...
def _get_secret() -> bytes:
    ...
    does not write ``token_signing.key`` into ``$JUNCTION_HOME`` — see the
