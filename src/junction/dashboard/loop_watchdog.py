...
Junction's gateway runs the dashboard HTTP server, every agent turn, and all
...
from junction.dashboard.stall_enrichment import collect_stall_enrichment

logger = logging.getLogger("junction.dashboard.loop_watchdog")
...
def _default_dump(file: "typing.IO[str] | typing.Any | None" = None) -> None:
    ...
    *file* can be any object with a ``fileno()`` method (including
    :class:`~junction.dashboard.crash_dump_store.DumpFile`).
    """
...
def _default_arm_later(timeout: float, file: "typing.IO[str] | typing.Any | None" = None) -> None:
    ...
    *file* can be any object with a ``fileno()`` method (including
    :class:`~junction.dashboard.crash_dump_store.DumpFile`).  faulthandler's C
    ...
    * the daemon-thread :meth:`check` that dumps **without** exiting at
    ...
    Args:
        ...
        enrich: Enrichment collector ``silence_secs -> lines``, injectable for
            tests.  Defaults to
            :func:`junction.dashboard.stall_enrichment.collect_stall_enrichment`.
    ...
    def __init__(
        ...
        *,
        ...
        # armed timer so a dashboard-only process (e.g. `junction chat`, where
