...
from junction import mcp_apps_render, model_registry, session_directive
...
from junction.agent_discovery import warm_project_agent_names
from junction.autonudge import get_instance
...
from junction.connections import get_visible_providers
...
from junction.dashboard.chat_persistence import _build_history_prefix, save_slot_off_loop
from junction.dashboard.chat_summary import generate_session_summary
...
from junction.dashboard.turn_dispatch import (
    format_approval_no_budget_card,
    format_approval_timeout_card,
    spawn_guarded_turn,
    tool_approval_timeout_secs,
)
from junction.executors import run_in_embed_pool, subprocess_executor
...
from junction.image_artifacts import register_images_off_loop
...
from junction.mcp_discovery import junction_managed_names
from junction.members import record_activity
...
from junction.quick_prompts import QUICK_PROMPTS
...
from junction.session import SessionClosingError, SpeculativeResumeRefused
from junction.slack.handler import post_linked_approval, resolve_linked_approval
from junction.slack.outbound import PostedOptions
from junction.trust_patterns import (  # noqa: F401 -- compatibility re-export
    _mask_quoted_separators,
    approval_command,
    approval_display_command,
)
from junction.trust_patterns import extract_base_command as _extract_base_command
from junction.trust_patterns import extract_bash_command as _extract_bash_command
from junction.trust_patterns import extract_full_command as _extract_full_command
from junction.trust_patterns import matches_trusted_pattern as _matches_trusted_pattern
from junction.trust_patterns import (  # noqa: F401 -- compatibility re-export
    split_command_segments as _split_command_segments,
)
from junction.validation import ValidationError, infer_use_case, validate_ask_user_question
from junction.widget_artifacts import register_widgets_off_loop
...
from junction.dashboard.chat_utils import (  # noqa: E402
...
def _empty_auto_continue_enabled() -> bool:
    ...
    try:
        return bool(JunctionConfig.load().session.empty_response_auto_continue)
...
def _turn_outcome(stop_reason: str | None, *, exhausted: bool = False) -> str:
    ...
    Single source of truth shared by the ``junction.turn.duration`` emit in
    ...
    by ``junction.watchdog.recovery.outcome``), so folding it into ``error``
...
def _emit_turn_metric(
    ...
    *,
    ...
    """Emit junction.turn.duration (best-effort).
    ...
    try:
        ...
        pass
    try:
        get_recorder().histogram("junction.turn.duration", value, unit="ms", attrs=attrs)
    except Exception:
        logger.debug("turn metric emit failed", exc_info=True)


def _emit_recovery_outcome(mechanism: str, outcome: str, attempts: int) -> None:
    """Emit junction.watchdog.recovery.outcome (best-effort).
    ...
    try:
        get_recorder().counter(
            "junction.watchdog.recovery.outcome",
            attrs={
                "mechanism": mechanism,
                "outcome": outcome,
...
def _connections_managed_mcp_names() -> frozenset[str]:
    ...
    * :func:`junction_managed_names` -- our own MCP store wrote the entry. This is
    ...
    * :func:`get_visible_providers` -- the name is a Connections provider with a
    ...
    """
    try:
        managed = junction_managed_names()
...
def _resolve_channel_target(
    ...
    *principal* lets a caller that has ALREADY established the recipient
    ...
    try:
        ...
        from junction.platform.governance_profiles import vet_and_audit
...
async def _recover_app_agent_binding(
    cfg: "JunctionConfig", slot: "_ChatSlot", *, project: str | None
) -> Any:
    ...
    from junction.apps.bridges import register_app
    from junction.apps.manager import app_lifecycle_lock, is_app_enabled
...
async def _handle_goal_command(state: "DashboardState", slot: "_ChatSlot", message: str) -> None:
    ...
    if _goal_svc is None:
        body = (
            ...
            "Set `JUNCTION_AUTONUDGE=1` and restart the gateway."
        )
...
def _requeue_unconsumed_steers(state: "DashboardState", slot: "_ChatSlot") -> None:
    ...
    from junction.dashboard.session_control import containment_meta
...
def _arm_queued_delivery_settlement(
    ...
    sub-agent completion (``JunctionGateway._defer_queued_delivery``), keyed on the
...
def _drop_stale_admissions(state: DashboardState, slot: _ChatSlot) -> None:
    ...
    if not slot._queue:
        return
    # circular import: session_control imports this package's modules at module level.
    from junction.dashboard import session_control as _sc
...
async def _start_next_queued_turn(state: DashboardState, slot: _ChatSlot) -> bool:
    ...
    try:
        merge = JunctionConfig.load().dashboard.merge_queued_messages
    except Exception:
        logger.warning(
    ...
    # circular import: session_control imports this package's modules at module level.
    from junction.dashboard.session_control import (
...
def _emit_ttft_metric(t0: float, session_key: str, *, is_new: bool, resumed: bool) -> None:
    ...
    try:
        ...
        # ``junction.metrics.provider.get_recorder`` reach this emit.
        from junction.metrics.provider import get_recorder

        get_recorder().histogram(
            "junction.chat.first_token.duration",
...
async def _run_chat(
    ...
    *,
    ...
    _is_cc_provider = JunctionConfig.load().agent.provider == "claude_code"
    ...
    try:
        ...
        # agent name (e.g. "junction") instead of the Junction slot name
        # (e.g. "default") which has no matching ~/.kiro/agents/ config.
        kiro_agent: str | None = None
        memory_store: str | None = None
        # The Junction agent's own default model ("" = inherit). Ranks below the
        ...
        if is_slash:
            ...
            # Provider-agnostic session replay: Junction's conversation_log
            ...
            if is_new and not _provider_has_history and state.context_builder.conversation_log:
                ...
                if state.sessions.consume_replay_suppression(session_key):
                    ...
                    from junction.context import (  # circular: context -> chat
            ...
            # Use resolved kiro agent name (e.g. "junction"), not the slot
            # name (e.g. "default"), so build_message's is_custom check
            # correctly identifies junction sessions and enables skills.
            ...
            if is_new and isinstance(_color_theme, str) and _color_theme.startswith("custom-"):
                from junction.platform.governance_profiles import governance_permits
            ...
            # default behavior).
            from junction.context import window_for_provider_client  # circular: context -> chat
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                # this turn, then record the applier's real outcome on Junction's
                ...
                if (slot_trusted or yolo_active) and not _child_low_fidelity:
                    ...
                    if not _pre_tool_hooks_fired:
                        ...
                        if _pre_tool_hooks_should_block(pre_hook_results):
                            ...
                            continue
                    # always=False — Junction owns trust scope; per-call request_permission
                ...
                if _u.input_tokens or _u.output_tokens or _u.credits:
                    ...
                    await persist_token_record_async(
                        ...
                        # maps e.g. "default" to "junction" before dispatch, so the
                ...
                # junction.turn.duration → turn latency p50/p90 + fault rate.
        ...
        # ``assistant_text``; run_script_hook caps ONLY the JUNCTION_HOOK_CONTEXT
        ...
        if should_queue_hook_continuation(slot._stopping, needs_session_reset, _stop_reason) and (
            ...
            if _hook_reasons:
                _nudge_cap = (
                    await asyncio.to_thread(JunctionConfig.load)
        ...
        if assistant_text and state.slack_client and _mirror_thread and _mirror_chan:
            try:
                from junction.slack.format import (  # circular: slack.format -> dashboard.state -> chat
        ...
        # junction.turn.timeout.cause for the ceiling path.
