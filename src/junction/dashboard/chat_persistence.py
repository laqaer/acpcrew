...
from junction.dashboard.channel_slots import slot_closed_since
...
from junction.effort import EFFORT_LEVELS, EFFORT_VALUES
...
from junction.validation import ARTIFACT_SLUG_RE
...
def _rehydrate_slot_from_history(
    ...
    *kiro_model_map* lets a bulk caller build the agent→model map once and share
    ...
    if meta.get("closed") and not adopt_closed:
        return None
    try:
        _restore_cfg = JunctionConfig.load()
    except Exception:
        _restore_cfg = None
...
async def save_slot_off_loop(
    ...
    *,
    ...
    non-blocking acquire and raises :class:`~junction.history.HistoryLockTimeout`
    under any concurrent holder (e.g. a workflow/cron result appending via
    :func:`~junction.history.append_off_loop`) — so calling the save inline on
    ...
    This helper mirrors :func:`~junction.history.append_off_loop`: on a running
