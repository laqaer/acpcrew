...
def register(app: web.Application) -> None:
    ...
    _is_dev_env = os.environ.get("JUNCTION_HOME", "").endswith("-dev")
    if _is_dev_env or env_flag_enabled("JUNCTION_DEV_MODE"):
    ...
    # Session revocation (called by `junction logout` CLI)
    ...
    for _builtin_name in BUILTIN_NAMES:
        try:
            _mod = importlib.import_module(f"junction.apps.builtins.{_builtin_name}")
            if hasattr(_mod, "register_routes"):
                _mod.register_routes(app)
        except ModuleNotFoundError as exc:
            if exc.name != f"junction.apps.builtins.{_builtin_name}":
                raise
