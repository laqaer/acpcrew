...
from junction.config.paths import data_home, kiro_sessions_dir
from junction.context_blocks import USER_LABEL
...
# in the same unit as Junction's own exactly-counted blocks. Never applied to
...
def context_trace(slot: str, days: int = 14) -> dict[str, Any]:
    ...
    Junction did NOT inject — kiro-cli's own base prompt, its tool catalogue and
    its steering files. It is an ESTIMATE and labelled as one everywhere it is
    surfaced: the provider reports occupancy in tokens while every Junction
...
def read_effective_agent(source: object) -> str:
    ...
    can be served by ``junction``. Recording the alias would attribute the turn
...
def _build_token_record(
    ...
    *,
    ...
    return {
        ...
        # junction.context_blocks.split_blocks. Sizes are characters (exact and
...
def get_usage_cache() -> dict:
    ...
    try:
        from junction.dashboard.handlers.sessions import _usage_cache
