...
from junction.dashboard.handlers.source_providers import is_owner_dashboard_request
...
from junction.skill_trust import ReviewedProjectChanged as _ReviewedProjectChanged
from junction.skill_trust import (
    TrustStoreFull,
...
def _list_aim_prompts():
    ...
    import junction.dashboard.handlers as _pkg
...
async def api_skills(request: web.Request) -> web.Response:
    ...
    Sources:
    - ``junction``: ``~/.kiro/crew/skills/`` (managed by SkillsLoader; editable)
    ...
    still be loaded via Junction text-injection or an external MCP server).
    ...
    # blocking filesystem work — junction list_skills() (os.walk + per-file
    ...
    # (see junction.executors). No result cache: the endpoint always reflects
...
async def api_skills_pending(request: web.Request) -> web.Response:
    ...
    def _prune_and_list() -> list:
        ...
        try:
            ttl = JunctionConfig.load().skills.pending_ttl_days
...
async def api_skill_pending_approve(request: web.Request) -> web.Response:
    ...
    def _approve_and_bound() -> str | None:
        ...
        if nm:
            ...
            try:
                cfg = JunctionConfig.load().skills
