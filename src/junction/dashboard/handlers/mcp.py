...
from junction.agent import _atomic_json_write, kiro_agents_dir_path, rebuild_agent_config
...
from junction.config.paths import data_home, kiro_agents_dir
from junction.dashboard.state import DashboardState
from junction.env import emit_env
...
from junction.mcp_gateway import hazards, is_gateway_supported
from junction.mcp_gateway.hashing import hash_command
from junction.mcp_gateway.rewriter import records_dir
from junction.mcp_gateway.shareability import ShareEvidence, ShareVerdict, assess
from junction.mcp_gateway.verdict_cache import load_cache
from junction.mcp_provenance import ABSENT, resolve_write, stamp
...
from junction.platform.governance import may_skip_gate_now
...
def _sync_mcp_to_agent(name: str, enabled: bool, *, remove: bool = False) -> None:
    """Serialize the junction.json read-modify-write with bridges' app-MCP
    ...
    """
    from junction.apps.bridges import _mcp_lock
    from junction.dashboard.handlers.agents import _installed_agent_config
...
def _sync_mcp_to_agent_unlocked(name: str, enabled: bool, *, remove: bool = False) -> None:
    """Sync MCP server state to junction.json mcpServers (not tools/allowedTools)."""
    from junction.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
    ...
    if enabled and not remove:
        # Ensure server exists in junction.json mcpServers when enabled
...
def _sync_mcp_to_agent_batch_unlocked(names: list[str], enabled: bool) -> None:
    """Batch sync multiple MCP servers to junction.json in a single read-modify-write."""
    from junction.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
        _installed_agent_config,
    )
    ...
    changed = False
    if enabled:
        # Ensure all servers exist in junction.json mcpServers
        mcp_servers = cfg.setdefault("mcpServers", {})
        try:
            gdata = json.loads(_GLOBAL_MCP_JSON.read_text(encoding="utf-8"))
        except (FileNotFoundError, json.JSONDecodeError):
...
async def _run_mcp_probe() -> None:
    ...
    try:
        # circular import: mcp_discovery defers imports of junction.agent
        ...
        from junction.mcp_discovery import probe_all  # noqa: F811
...
async def api_mcp_servers(request: web.Request) -> web.Response:
    ...
    try:
        ...
        pass
    # Junction-scope entries: disabled state (consent-disabled installs and
    # custom adds live only here) + which rows the JSON editor can manage.
    junction_mcps = _load_json_or_empty(_junction_mcp_json()).get("mcpServers", {})
    result: list[dict] = []
    for s in servers:
        d = s.to_dict()
...
async def api_mcp_active(request: web.Request) -> web.Response:
    ...
    For non-junction agents, reads ``mcpServers`` from the agent's config
    in ``~/.kiro/agents/`` — these are the only servers kiro-cli loads
    when ``--agent <name>`` is passed.  For junction (or no agent),
    reads from global ``~/.kiro/settings/mcp.json`` as before.
    """
    agent = request.query.get("agent", "")

    # Resolve Junction agent name → kiro agent name so "default" → "junction"
    if agent:
        try:
            from junction.config.loader import JunctionConfig, resolve_agent_bindings  # noqa: F811
    ...
    # Non-junction agent: read from agent config
    ...
    # Junction / default: read from global mcp.json
    from junction.mcp_discovery import list_servers  # noqa: F811
    ...
    # Also include the managed Junction servers (always enabled). NOTE for
    # ``junction-computer``: "enabled" here means the SERVER is registered, not
...
async def _evaluate_shareability(servers: list[Any]) -> None:
    ...
    from junction.mcp_gateway.evaluate import evaluate_new_servers
    ...
    await evaluate_new_servers(
        list(servers), records_dir(JunctionConfig.load().mcp_gateway.socket_path)
    )
...
async def _bg_measure_all() -> None:
    ...
    try:
        ...
        await evaluate_new_servers(
            list(servers),
            records_dir(JunctionConfig.load().mcp_gateway.socket_path),
            budget=None,
            on_progress=report,
        )
...
async def api_mcp_sync(request: web.Request) -> web.Response:
    ...
    2. Adds them to both junction agent config AND global mcp.json
       (kiro-cli ACP only reads the global config).
    3. Resets all sessions so changes take effect.
    """
    from junction.mcp_discovery import (  # noqa: F811
        junction_managed_names,
    ...
    if to_sync:
        ...
        async with _get_mcp_lock():
            ...
            _managed = junction_managed_names()
            for s in to_sync:
                if s.is_remote:
    ...
    # toggled enable/disable which writes to junction.json but requires
    # a session restart for kiro-cli to pick up the change.
    from junction.dashboard.handlers.sessions import _reset_all_sessions  # noqa: F811
...
async def api_mcp_toggle(request: web.Request) -> web.Response:
    ...
    2. Syncs ``tools``/``allowedTools`` in ``junction.json`` (non-ACP mode).
    """
    try:
    ...
    async with _get_mcp_lock():
        ...
        if name not in servers:
            ...
            from junction.mcp_discovery import (
                list_servers as _ls,  # circular import: mcp_discovery defers imports of junction.agent which shares state with this module
            )
        ...
        # 2. Sync to junction.json tools/allowedTools (lock prevents lost updates vs agents.py)
        from junction.dashboard.handlers.agents import _get_config_lock  # noqa: F811
...
async def api_mcp_toggle_all(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        ...
        # Batch sync: single read-modify-write of junction.json
        from junction.dashboard.handlers.agents import _get_config_lock  # noqa: F811
...
async def api_mcp_remove(request: web.Request) -> web.Response:
    ...
    junction.json.  If the optional ``aim`` package manager happens to be
    ...
    from junction.dashboard.handlers._shared import _capability_manager
...
async def api_mcp_server_detail(request: web.Request) -> web.Response:
    ...
    if request.method == "DELETE":
        ...
        # would otherwise both read junction.json and the last write would drop
    ...
    # Sync to junction.json (enable by default). Config lock across the offloaded
...
def _junction_mcp_json() -> Path:
    """Junction-scope ``mcp.json`` path, resolved against the live data home."""
    return _JUNCTION_MCP_JSON if _JUNCTION_MCP_JSON is not None else data_home() / "mcp.json"
...
def _atomic_write(path: Path, data: dict) -> None:
    ...
    :func:`junction.atomic_write.atomic_write` with ``restrict_to_owner=True``
...
def _set_junction_entry(name: str, *, enabled: bool, spec: dict | None = None) -> str:
    ...
    ``"disabled"``, or ``"noop"``.
    """
    data = _load_json_or_empty(_junction_mcp_json())
    servers = data.setdefault("mcpServers", {})
    ...
    _atomic_write(_junction_mcp_json(), data)
    return action


def _get_junction_entry(name: str) -> dict | None:
...
def _replace_junction_spec(name: str, spec: dict) -> bool:
    """Replace the full spec of an existing Junction-managed entry.
...
def _remove_junction_entry(name: str) -> bool:
...
def _remove_from_agent_file(path: Path, name: str) -> bool:
    ...
    ``~/.kiro/agents/junction.json`` / ``~/.claude/agents/junction.mcp.json``
...
def _purge_server_config(name: str) -> dict[str, str]:
    ...
    actions["junction"] = "removed" if _remove_junction_entry(name) else "noop"
    ...
    _remove_from_agent_file(kiro_agents_dir() / "junction.json", name)
    for scope in _extra_mcp_scopes():
...
async def _do_mcp_apply(request: web.Request) -> web.Response:
    ...
    Request body::

        {
          "changes": [
            {
              "name": "slack-mcp",
              "junction": true,     // desired MC visibility
    ...
    so the provider-native agent files (``~/.kiro/agents/junction.json`` and
    ``~/.claude/agents/junction.md`` + ``junction.mcp.json``) reflect the
    ...
    try:
        ...
        async with _get_mcp_lock():
            for change in changes:
                ...
                desired_mc = bool(change.get("junction", True))
                ...
                if desired_mc and not desired_kiro and not any(desired_extra.values()):
                    has_mc = _scope_has_entry(name, _junction_mcp_json())
                    if not has_mc:
                ...
                outcome["actions"]["junction"] = await _offload_config_write(
                    _set_junction_entry,
                    name,
                    enabled=desired_mc,
                    spec=preserved_spec,
                ...
                # reachable from Junction sessions — a permission-shaping
...
async def api_mcp_gateway_status(request: web.Request) -> web.Response:
    ...
    """
    from junction.config.loader import JunctionConfig  # noqa: F811
    ...
    cfg = JunctionConfig.load().mcp_gateway
    return web.json_response(
        {
            "enabled": cfg.enabled,
...
async def api_mcp_gateway_enable(request: web.Request) -> web.Response:
    ...
    """
    from junction.config.loader import config_path  # circular import
    from junction.dashboard.handlers.agents import _get_config_lock  # circular import
...
def _stub_eligibility(
    ...
    *,
    ...
    try:
        runtime = records_dir(JunctionConfig.load().mcp_gateway.socket_path)
...
async def api_mcp_gateway_servers(request: web.Request) -> web.Response:
    ...
    gw_cfg = JunctionConfig.load().mcp_gateway
...
def _load_shareability_state() -> tuple[
    ...
    """
    try:
        rt = records_dir(JunctionConfig.load().mcp_gateway.socket_path)
    ...
    *,
    ...
    return assess(
        ShareEvidence(
            ...
            # when it declines the caller-identity extension: junction-core
            # consumes the injected caller block and shares a backend safely,
            # while junction-cron reads process identity. Asking the server's own
...
async def api_mcp_gateway_set_stub(request: web.Request) -> web.Response:
    ...
    """
    from junction.config.loader import (  # noqa: F811
        ConfigReadError,
        config_path,
