...
import junction
from junction import beacon, dep_sync, platform_compat
from junction.computer_use.types import MAX_SCREENSHOT_MAX_PX as _CU_MAX_SCREENSHOT_MAX_PX
from junction.computer_use.types import MAX_TREE_NODES_LIMIT as _CU_MAX_TREE_NODES_LIMIT
...
from junction.dashboard.stt_stream import _STREAMING_PROVIDERS
from junction.dashboard.token_auth import MAX_SESSION_TTL_SECS, generate_token, parse_duration
from junction.effort import EFFORT_LEVELS
from junction.executors import discovery_executor
from junction.metrics import provider as _metrics_provider
from junction.model_router.catalog import MODEL_ID_MAX_LEN, MODEL_ID_PATTERN
from junction.security_posture import build_posture_snapshot_async, posture_counts_async
...
def _masked_config_dict(cfg: JunctionConfig) -> dict:
    """Return ``cfg.to_dict()`` with sensitive string values masked.

    Applied ONLY to the GET /api/config/junction response — never to the value
    ...
    from junction.config.validation import _is_sensitive_path

    masked = copy.deepcopy(cfg.to_dict())

    # Drop unknown/edition-contributed top-level sections (JunctionConfig.
...
# probes (e.g. `junction token`'s stale-dashboard warning) can detect that
...
def _sel():
    ...
    import junction.dashboard.handlers as _pkg  # noqa: F811 — circular import
...
async def logo(request: web.Request) -> web.StreamResponse:
    ...
    import junction.dashboard.handlers as _h  # noqa: F811
    from junction.hooks import validate_file_path  # noqa: F811

    cfg = _h.JunctionConfig.load()
    ...
    names = ["junction-logo.png"]
    if "-nightly." in __version__:
        names.insert(0, "junction-logo-nightly.png")
    for name in names:
...
async def api_ready(request: web.Request) -> web.Response:
    ...
    * **Shutdown requested** — when SIGTERM/SIGINT or ``POST /api/shutdown``
    ...
    shutting_down = junction.shutdown_event.is_set()
...
def _theme_payload(cfg: JunctionConfig) -> dict[str, object]:
...
async def pwa_file(request: web.Request) -> web.StreamResponse:
    ...
    # at JunctionWebsite/dist) still passes the traversal guard.
...
def _is_apple_silicon() -> bool:
    ...
    Rosetta 2 — which Junction's bundled Python does — so it cannot be used to
...
def _stt_providers() -> list[str]:
    ...
    if "apple" in providers:
        from junction import apple_speech
...
async def api_stt_config(request: web.Request) -> web.Response:
    ...
    if request.method == "PUT":
        ...
        path = config_path()
        from junction.agent import _atomic_json_write  # noqa: F811
...
def _voice_extra_importable() -> bool:
    ...
    Reads ``junction.transcribe``'s own import outcome (its module-level
...
def _stt_prereq_commands(provider: str = "whisper") -> list[str]:
    ...
    if provider == "transcribe":
        ...
        # by THIS gateway process" (see junction.transcribe.is_available); the
        ...
        if not _voice_extra_importable():
            ...
            if os.name == "nt":
                ...
                cmds.append(f"& '{exe}' -m pip install junction[voice]")
            else:
                cmds.append(f"{shlex.quote(sys.executable)} -m pip install 'junction[voice]'")
...
async def api_stt_install(request: web.Request) -> web.Response:
    ...
    provider = JunctionConfig.load().stt.provider
    if provider == "transcribe":
...
def _build_stt_install_script(provider: str = "whisper") -> str:
    ...
    - ``faster``: installs faster-whisper via pip (CTranslate2, no system ffmpeg)
      ...
      library is imported in-process by ``junction.transcribe``, so a system
    ...
    :func:`junction.transcribe._find_whisper` probes via its
...
async def api_stt_transcribe(request: web.Request) -> web.Response:
    ...
    from junction.transcribe import is_available, transcribe_audio  # noqa: F811
    ...
    try:
        ...
        if text:
            from junction.security import (  # noqa: F811
                redact_credentials,
...
async def api_security_stats(_request: web.Request) -> web.Response:
    ...
    denied = 0
    try:
        from junction.dashboard.handlers.security import build_denied_commands_snapshot_async
...
async def api_junction_config(request: web.Request) -> web.Response:
    """GET/PUT /api/config/junction — read or update Junction config."""
    # Re-imported at call time (not reused from the module-level binding) so a
    # test that redirects ``junction.config.loader.config_path`` at a temp path
    # is observed by this handler.
    from junction.config.loader import config_path  # noqa: F811
    ...
    if request.method == "PUT":
        ...
        from junction.config.loader import ConfigReadError, update_config_locked  # noqa: F811
        from junction.dashboard.handlers.agents import _get_config_lock  # noqa: F811
            ...
            async with _get_config_lock():
                ...
                if "conductor_skill" in applied:
                    if agent.get("conductor_skill"):
                        from junction.dashboard.handlers.agents import (  # noqa: F811
                            _regen_conductor,
                        )
                        ...
                        try:
                            from junction.skills import SkillsLoader  # noqa: F811
...
def _agent_values() -> set[str]:
    ...
    return {"", *JunctionConfig.load().agents}
...
def _validate_role_model(
    ...
    *provider* is forwarded to :func:`_model_rejected_reason` so a caller holding
    ...
    from junction.model_router.catalog import is_catalog_slug
...
_EDITABLE_CONFIG: dict[str, dict] = {
    ...
    # toggle), the GUI twin of `junction telemetry disable`. Only the boolean
...
def _tailnet_governance_pinned_off() -> bool:
    ...
    """
    from junction.dashboard import tailnet  # noqa: F811 - local: keeps the import edge lazy

    return tailnet.is_governance_pinned_off(audit_tool="config_patch_dashboard_tailnet")


async def api_junction_config_patch(request: web.Request) -> web.Response:
    """PATCH /api/config/junction — update a single config field."""
    from junction.config.loader import ConfigReadError, config_path, update_config_locked
    ...
    if path_key == "agent.provider":
        ...
        # For claude_code this (re)writes ~/.claude/agents/junction.mcp.json —
        ...
        try:
            from junction.agent import rebuild_agent_config  # noqa: F811  circular import
    ...
    # junction processes pick the value up when they next start.
...
async def api_session_agents_list(request: web.Request) -> web.Response:
    ...
    from junction.session_workspace import list_results  # noqa: F811
...
async def api_session_agent_result(request: web.Request) -> web.Response:
    ...
    from junction.session_workspace import read_result  # noqa: F811

    content = read_result(session_id, agent_id)
    if not content:
        return web.json_response({"error": "not found"}, status=404)
    from junction.security import redact_credentials, redact_exfiltration_urls  # noqa: F811
...
async def api_session_agent_stream(request: web.Request) -> web.StreamResponse:
    ...
    from junction.session_workspace import result_path  # noqa: F811
...
async def api_logout(request: web.Request) -> web.Response:
    ...
    Called by ``junction logout`` CLI. Requires loopback + local secret
    (same auth as /api/token/local) to prevent unauthorized revocation.
    """
    import junction.dashboard.handlers as _h  # noqa: F811
    from junction.dashboard.token_auth import revoke_all_sessions  # noqa: F811
...
async def api_shutdown(request: web.Request) -> web.Response:
    ...
    """
    import junction.dashboard.handlers as _h  # noqa: F811
    from junction import shutdown_event  # noqa: F811
...
async def api_app_token(request: web.Request) -> web.Response:
    ...
    """
    from junction.dashboard.token_auth import generate_token, validate_app_secret
