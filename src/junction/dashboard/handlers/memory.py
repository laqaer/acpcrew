...
from junction.executors import embed_executor, run_in_embed_pool
...
def _get_vector_store(state: DashboardState):
    ...
    if not hasattr(state, "_standalone_vector"):
        ...
        # observe it. ``JunctionConfig`` deliberately shadows this module's own
        ...
        from junction.vector_memory import VectorMemoryStore  # noqa: F811
...
async def api_memory_embedding_model(request: web.Request) -> web.Response:
    ...
    # JUNCTION_EMBED_MODEL_PATH wins over memory.embed_model_path for the PATH,
    ...
    if os.environ.get("JUNCTION_EMBED_MODEL_PATH", "").strip():
        return web.json_response(
            {"ok": False,
             "error": "JUNCTION_EMBED_MODEL_PATH is set, so it overrides the configured "
    ...
    async with _faiss_install_lock:
        try:
            ...
            try:
                ...
                # `junction doctor` points the user at a manual
...
async def api_memory_stats(request: web.Request) -> web.Response:
    ...
    # ``JunctionConfig`` at call time is what lets a test substitute it on the
    # source module.
    from junction.config.loader import JunctionConfig  # noqa: F811
    ...
    from junction.memory import legacy_memory_present  # noqa: F811
...
def _build_memory_graph(mem: Any, lessons: list) -> tuple[list[dict], list[dict]]:
    ...
    # "Junction (Public)"): the full header almost never occurs verbatim inside
    ...
    # parenthetical qualifier stripped: "Junction (Public)" -> "junction",
