...
from junction.executors import discovery_executor, subprocess_executor
...
def _sel():
    import junction.dashboard.handlers as _pkg  # circular import: __init__ imports terminal

    return _pkg.sel()


class _ConptyBackend(Protocol):
    """Structural type for the Windows ConPTY backend (:class:`junction.conpty.WindowsPty`).
...
def _bash_init_script(token: str) -> bytes:
    ...
    return (
        ...
        '    . "$HOME/.profile"\n'
        "fi\n"
        f"builtin printf '\\033]697;JunctionReady;{token}\\007'\n"
    ).encode("utf-8")
...
async def api_terminal_ws(request: web.Request) -> web.WebSocketResponse | web.Response:
    ...
    if existing:
        ...
        # no POSIX pty/fork; junction.conpty drives the Win32 pseudo-console via
        # ctypes (stdlib, no extra dependency).
        from junction.conpty import WindowsPty
        ...
        env = {**os.environ, "JUNCTION_TERMINAL": "1"}
        ...
        try:
            ...
            if _is_bash_shell(shell):
                ...
                ready_marker = (
                    f"\x1b]697;JunctionReady;{token}\x07".encode("ascii")
                )
...
def _list_vetted_completions(
    ...
    ``Junction-terminal-completion``. Each entry reports ``at``, the offset the
