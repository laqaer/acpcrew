...
lock + ``_set_junction_entry`` / ``_replace_junction_spec``) so there is
...
from junction.dashboard.handlers import mcp as _mcp
from junction.dashboard.handlers.mcp import (
    _find_server_spec_anywhere,
    _get_junction_entry,
    _get_mcp_lock,
    _is_valid_mcp_name,
    _replace_junction_spec,
)
from junction.mcp_discovery import MCP_REDACTED_HEADER_VALUE, redact_mcp_headers
...
def _load_junction_config_strict() -> dict | None:
    ...
    """
    try:
        text = _mcp._junction_mcp_json().read_text(encoding="utf-8")
...
async def api_mcp_custom_add(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        ...
        data = _load_junction_config_strict()
        if data is None:
            sel().log_api_access(
                caller="dashboard",
            ...
            return web.json_response(
                {
                    ...
                    f" remove {_mcp._junction_mcp_json()} and retry"
                },
                status=500,
            )
        ...
        await _mcp._offload_config_write(_mcp._atomic_write, _mcp._junction_mcp_json(), data)
...
async def api_mcp_custom_get(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        entry = _get_junction_entry(name)
    if entry is None:
...
async def api_mcp_custom_update(request: web.Request) -> web.Response:
    ...
    404 when the name isn't a Junction-managed entry.  The server's
    ...
    async with _get_mcp_lock():
        existing = _get_junction_entry(name)
        if existing is None:
        ...
        replaced = await _mcp._offload_config_write(_replace_junction_spec, name, spec)
    if not replaced:
        return web.json_response({"error": f"server '{name}' not found"}, status=404)
