...
the ``junction-computer`` stdio MCP process does no accessibility work and no
...
``junction computer doctor --json`` rather than calling ctypes in-process (a
...
from junction.computer_use import overlay as cursor_overlay
...
# The permission probe is a short-lived child running ``junction computer doctor
...
async def _probe_permissions(platform_id: str) -> dict[str, str]:
    ...
    Shells ``junction computer doctor --json`` instead of calling the frameworks
    ...
    for, so ``junction computer doctor --json`` reported ``accessibility=granted``
    ...
    # install as the gateway (and falls back to ``<interpreter> -m junction``
    # when no console script is on PATH — the systemd-user-service case).
    from junction.agent import _junction_mcp_invocation

    exe, prefix = _junction_mcp_invocation(DOCTOR_SUBCOMMAND)
...
def _snapshot() -> dict[str, Any]:
    ...
    """
    from junction.computer_use.backend import get_shared_backend, platform_id_for_current_os
    ...
    limits = JunctionConfig.load().computer_use
...
def _write_limits(patch: dict[str, Any]) -> None:
    ...
    Read-modify-write on the raw JSON (not ``JunctionConfig.to_dict()``) so an
...
async def api_computer_use_config_save(request: web.Request) -> web.Response:
    ...
    **There is no governance ceiling on this route, and therefore no 409.** Computer
    ...
    if STATE_KEY_ENABLED in state_patch and state_patch[STATE_KEY_ENABLED] != enabled_before:
        ...
        # ``junction.agent`` is the single place a test can substitute the
        ...
        from junction.dashboard.handlers.sessions import _reset_all_sessions
...
async def api_computer_use_invoke(request: web.Request) -> web.Response:
    ...
    MACHINE endpoint: the ``junction-computer`` stdio MCP shim is the only caller,
    ...
    ``session_key`` is resolved STRICTLY on the shim side (``JUNCTION_SESSION_KEY``,
    else ``JUNCTION_HOST_PID`` + the HMAC sidecar), which refuses an unresolvable
    ...
    from junction.computer_use.tools import dispatch_tool
...
async def api_computer_use_frame(request: web.Request) -> web.Response:
    ...
    * a capture with no published surface scope emits no frame, because a frame
    ...
    """
    from junction.dashboard.origin import is_loopback
