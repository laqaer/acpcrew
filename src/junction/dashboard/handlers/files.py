...
from junction.config.loader import JunctionConfig, WorkspaceConfig, config_dir, data_home
from junction.dashboard import part_stream
from junction.dashboard.chat_utils import dashboard_slot_key
from junction.dashboard.file_index import _SKIP_DIRS as _WALK_SKIP_DIRS
...
from junction.hooks import FileTooLargeError, safe_read_file_bytes, safe_read_prefix
from junction.messaging import upload_gate
from junction.messaging.display_safety import redact_for_display
from junction.messaging.link import is_channel_session_key
from junction.messaging.outbound_files import OutboundFile
from junction.messaging.raster import SNIFF_BYTES, sniff_raster_mime
from junction.platform import redact_via_context as redact
from junction.sandbox import popen_limited, sandboxed_spawn_argv
...
from junction.slack.handler import is_tracked_channel
...
async def api_outbox_notify(request: web.Request) -> web.Response:
    ...
    from junction.config.loader import outbox_dir  # noqa: F811
    from junction.hooks import FileTooLargeError, safe_read_file_bytes  # noqa: F811
...
async def api_slack_upload_file(request: web.Request) -> web.Response:
    ...
    if target_channel:
        ...
        try:
            creds = JunctionConfig.load().load_credentials()
            owner_id = creds.get("JUNCTION_OWNER_ID", "")
            if owner_id:
...
async def api_channel_upload_file(request: web.Request) -> web.Response:
    ...
    from junction.dashboard.chat_runner import _resolve_mirror_target
...
# verified by the shared sniffer (:mod:`junction.messaging.raster`), so all
...
async def _stream_video_part(
    ...
    All the file handling lives in :func:`~junction.dashboard.part_stream.
...
async def api_workspaces_create(request: web.Request) -> web.Response:
    ...
    from junction.validation import WORKSPACE_NAME_RE  # noqa: F811
...
async def api_file_read(request: web.Request) -> web.Response:
    ...
    from junction.validation import (  # noqa: F811
        FILE_READ_SCHEMA,
        ValidationError,
        validate_tool_args,
    )
...
def _open_checked_file(
    ...
    *log_open_failure* keeps log volume a per-endpoint decision: a
    ...
    """
    import junction.dashboard.handlers as _h  # noqa: F811  # circular import
...
async def api_file_raw(request: web.Request) -> web.Response:
    ...
    # (junction.messaging.raster), which requires the full PNG signature and
...
def _resolve_project_relative(raw: str) -> tuple[str, str | None]:
    """Resolve a relative path against JUNCTION_PROJECT_DIR (resolve=1).
...
async def api_file_search(request: web.Request) -> web.Response:
    ...
    # test that stubs ``junction.security.is_sensitive_path`` is observed by the
    # project-root rejection below.
    from junction.security import is_sensitive_path  # noqa: F811
    ...
    if project:
        ...
        from junction.config.loader import workspace_dir_for  # noqa: F811
    ...
    if not search_roots:
        # Fallback: project dir, then the junction workspace.
        #
...
def _project_git_branch(base: str) -> dict:
    ...
    for _ in range(_GIT_ROOT_WALK_LIMIT):
        ...
        # probe for existence rather than is_dir() — otherwise every Junction
...
async def api_dashboard_config(request: web.Request) -> web.Response:
    ...
    # Offloaded: JunctionConfig.load() stats, reads, parses, and validates config
    ...
    if request.method == "PUT":
        ...
        #    `junction config set` (which takes that same file lock) cannot land
        ...
        from junction.config.loader import update_config_locked  # noqa: F811
        from junction.dashboard.handlers.agents import (  # lazy: import cycle
            _get_config_lock,
        )
