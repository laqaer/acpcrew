...
Both share the collector engine in :mod:`junction.diagnostics`, the exact same
code path the ``junction doctor --bundle`` CLI uses. Auth + host validation are
...
def _sel():
    ...
    # circular import: the parent package `junction.dashboard.handlers` imports
