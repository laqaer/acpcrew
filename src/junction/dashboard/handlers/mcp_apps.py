...
auditing — happen gateway-side in :mod:`junction.mcp_gateway.app_call`, so a
...
from junction.dashboard.chat_utils import _history_key_for, effective_session_key
from junction.dashboard.handlers._shared import _is_restricted_session, _read_session_key
...
from junction.mcp_gateway.rewriter import default_socket_path
...
def _socket_path() -> str:
    ...
    override (empty -> the standard ``$JUNCTION_HOME`` location)."""
    try:
        # circular import: config.loader pulls dashboard modules; keep lazy.
        from junction.config.loader import JunctionConfig

        configured = (JunctionConfig.load().mcp_gateway.socket_path or "").strip()
        if configured:
            return configured
