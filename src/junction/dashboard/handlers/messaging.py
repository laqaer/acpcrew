...
from junction.dashboard.channel_folders import (
    LIVE_RELOAD_FIELDS,
...
from junction.dashboard.handlers._shared import read_bounded_json
...
from junction.dashboard.token_auth import caller_names_a_missing_slot
from junction.messaging.display_safety import redact_for_display
from junction.messaging.link import CHANNEL_SESSION_NAMESPACES, SLACK_NAMESPACE, ChannelLink
...
from junction.messaging.transport import delivery_confirmed
from junction.notifications.bus import (
    NotificationPayload,
    NotificationValidationError,
)
from junction.platform.governance_profiles import HOST_SESSION_KEY
from junction.platform_compat import IS_MACOS
from junction.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls
from junction.slack.format import build_options_blocks, extract_options
from junction.slack.outbound import OPTIONS_FALLBACK_TEXT, PostedOptions
from junction.spawn_warm import warm_project_agents_for_spawn
from junction.subagent_persistence import _agent_dir, read_state
...
def _sel():
    ...
    import junction.dashboard.handlers as _pkg  # noqa: F811
...
async def api_notification_channels(request: web.Request) -> web.Response:
    ...
    """
    from junction.notifications.settings import PROTECTED_CHANNELS
...
async def _deliver_channel_dm(
    ...
    # junction.dashboard.handlers, so a top-level import closes that cycle. Only
...
async def api_send_message(request: web.Request) -> web.Response:
    ...
    from junction.slack.handler import is_allowed_user, is_tracked_channel  # noqa: F811
    from junction.validation import USER_ID_RE  # noqa: F811
            ...
            if slot_key:
                ...
                if slot:
                    ...
                    if slot.running or slot._in_stage_execution:
                        ...
                        # junction.dashboard.handlers (for MAX_PROMPT_BYTES,
                        ...
                        from junction.dashboard.turn_dispatch import spawn_guarded_turn
...
async def api_slack_pins(request: web.Request) -> web.Response:
    ...
    from junction.slack.handler import is_tracked_channel  # noqa: F811
...
async def api_slack_profile(request: web.Request) -> web.Response:
    ...
    from junction.slack.handler import is_allowed_user  # noqa: F811
...
def _write_env_updates(updates: dict[str, str | None]) -> None:
    ...
    through :func:`junction.atomic_write.atomic_write` with
    ...
    # writers (the `junction secrets import` migrator and the WeChat/Weixin QR
...
async def api_slack_manifest(request: web.Request) -> web.Response:
    ...
    Mirrors ``junction manifest --url`` so the settings UI can offer one-click
    ...
    alias = request.query.get("alias", "").strip() or "junction"
...
async def _slack_config_save_locked(request: web.Request) -> web.Response:
    ...
    if "command" in body:
        ...
        new_cmd = cmd or "junction"
        if new_cmd != slack_cfg.get("command", "junction"):
...
def _threshold_pct_rejection(body: dict[str, Any], key: str) -> tuple[str, str] | None:
    ...
    if key not in body:
        return None
    from junction.config.loader import THRESHOLD_PCT_MAX, THRESHOLD_PCT_MIN
...
async def api_teams_activity(request: web.Request) -> web.Response:
    ...
    from junction import webhooks  # noqa: F811
    from junction.teams.client import (  # noqa: F811
...
async def api_teams_config_get(request: web.Request) -> web.Response:
    ...
    from junction.teams.client import HAS_JWT  # noqa: F811
    ...
    return web.json_response(
        {
            ...
            # PyJWT ships in the optional `junction[teams]` extra and the channel
...
async def api_teams_config_save(request: web.Request) -> web.Response:
    ...
    if credential_touched:
        ...
        # than via JunctionConfig.load(), which triggers _load_resolved() ->
    ...
    async with _get_config_lock():
        ...
        if verified_triple is not None:
            ...
            # avoid JunctionConfig.load() which would trigger migration write-back).
...
async def api_webex_config_save(request: web.Request) -> web.Response:
    ...
    # torn read and a side-effecting one: ``JunctionConfig.load()`` normalizes and
