...
if TYPE_CHECKING:
    from junction.providers.base import LLMProvider  # noqa: F811
...
from junction.history import SEARCH_MIN_CHARS, _archive_dir, is_incognito_transcript
from junction.llm_helpers import run_bg_oneliner
from junction.mcp_discovery import sync_discovered_servers
    ...
    if now - _health_cache_ts > _HEALTH_REFRESH_SECS:
        async with _health_lock:
            ...
            if time.monotonic() - _health_cache_ts > _HEALTH_REFRESH_SECS:
                try:
                    from junction.dashboard import session_health
...
def _text_scrape_enabled() -> bool:
    ...
    try:
        ...
        return bool(JunctionConfig.load().dashboard.usage_text_scrape_enabled)
    except Exception:
...
def _wrap_argv_usage_scrape(kiro_bin: str) -> tuple[list[str], str | None]:
    ...
    return _wrap_argv_at_configured_tier(
        [kiro_bin, "chat", "--no-interactive", "--agent", "junction-lite", "/usage"]
    )
...
def _open_slot_transcript_keys(state: DashboardState) -> set[str]:
    ...
    ``discord:junction:direct:123`` mapped to a stem that does not exist.
    ...
    """
    from junction.dashboard.chat_utils import slot_history_key, slot_transcript_key
...
def _service_wait_ping(
    ...
    # MCP gateway disabled (the default) JUNCTION_SESSION_KEY is unset and one MCP
...
async def api_session_tool_policy(request: web.Request) -> web.Response:
    ...
    Used by managed MCP servers (junction-core, junction-cron) to filter
...
async def api_sessions_restart(request: web.Request) -> web.Response:
    ...
    Also syncs MCP servers from mcp.json → junction.json so newly
...
async def api_session_archive_list(request: web.Request) -> web.Response:
    ...
    from junction.history import _archive_dir, _safe_key
