...
from junction.cron import CronStoreBusy, is_valid_timezone
...
async def api_cron_run(request: web.Request) -> web.Response:
    ...
    # ANOTHER process (`junction cron add`, the MCP cron_add tool), which writes
    ...
    *,
    ...
    if not (in_slots or in_restricted or is_channel_ns):
        ...
        if not exists:
            ...
            # original JUNCTION_SESSION_KEY. No session JSONL means
...
async def api_lessons_create(request: web.Request) -> web.Response:
    ...
    from junction.learn import Lesson  # noqa: F811
...
async def api_crons(request: web.Request) -> web.Response:
    from junction.cron import compute_next_run_ts, format_schedule, get_local_tz  # noqa: F811
