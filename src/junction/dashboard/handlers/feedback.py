...
(``beacon.telemetry_permitted``). When that gate says no (``JUNCTION_TELEMETRY_DISABLED``,
...
from junction.dashboard.handlers._shared import read_bounded_json, read_capped_response
from junction.dashboard.session_pulse_counter import (
...
_FORM_CATEGORY = "Junction"  # brand-ok: literal Aperture portal identifier
...
_SERVICE_ID = "Junction"  # brand-ok: literal Aperture portal identifier
...
# template (GET rendering.../form/template for category=Junction,  # brand-ok: registered category id
...
_RATING_QUESTION = "How would you rate your experience with Junction today?"  # brand-ok: verbatim registered template text, ingestion 400s on mismatch
...
def _telemetry_permitted(audit_tool: str = "") -> bool:
    ...
    same two config inputs the beacon and ``junction telemetry status`` use.
...
async def api_feedback_submit(request: web.Request) -> web.Response:
    ...
    Body: ``{rating, feedback?, email?, sessionId, junctionVersion}``.
    ...
    # `sessionId` and `junctionVersion` are client-supplied and land in
    ...
    junction_version = str(body.get("junctionVersion") or "")
    junction_version, _ = redact_exfiltration_urls(junction_version)
    junction_version, _ = redact_credentials(junction_version)
    ...
    payload = {
        ...
        # GET rendering.../form/template?category=Junction&name=SessionFeedback&version=1.0.1,  # brand-ok: registered category id
        # whose own metadataList lists userId, sessionId, junction_version in
        # that order.
        "metadataList": [
            {"key": "userId", "value": user_id, "pii": True},
            {"key": "sessionId", "value": session_id, "pii": False},
            {"key": "junction_version", "value": junction_version, "pii": False},
        ],
    }
