...
from junction.dashboard.state import VALID_MEMORY_MODES, DashboardState
from junction.messaging.link import is_channel_session_key
from junction.messaging.privacy_mode import hydrate as _hydrate_conv_flags
from junction.messaging.privacy_mode import is_incognito as is_thread_incognito
from junction.messaging.privacy_mode import is_temporary as is_thread_temporary
from junction.security import is_sensitive_path, redact_credentials, redact_exfiltration_urls
from junction.skill_trust import is_project_trusted as _is_project_trusted
from junction.skills import skills_dir

if TYPE_CHECKING:
    from junction.platform.interfaces import CapabilityManager
...
def _audit_admission(surface: str, resource: str, allowed: bool, error: str = "") -> None:
    ...
    """
    from junction.sel import sel as _sel  # circular import: sel imports config
...
def _capability_manager() -> "CapabilityManager":
    ...
    uniform.
    """
    from junction.platform.capability_bound import bind_capability_manager
...
def _canonical_skill_roots() -> list[Path]:
    ...
    try:
        ...
        out.extend(Path(p).expanduser().resolve() for p in JunctionConfig.load().skills.extra_paths)
    except Exception:
...
def _expand_resource_uri(uri: str, agent_path: Path) -> str | None:
    ...
    Thin alias for :func:`junction.agent_discovery.expand_skill_uri` — the
...
def _resolve_loaded_by_agents(
    ...
    ``skill://`` URIs (it may still be loaded via Junction text-injection or
...
def collect_skills_blocking(
    ...
    1. ``skills_loader.list_skills()`` — junction skills (default source).
...
def _resolve_skill_root(name: str, state: DashboardState, session_key: str = "") -> Path | None:
    ...
    *session_key* scopes ``kiro-workspace/`` to the requesting chat slot's
    ...
    if name.startswith("kiro-user/"):
        ...
        # (consistent with the junction/kiro branches below).
        if is_sensitive_path(str(resolved)):
            return None
        return resolved
    else:
        ...
        if not rel or ".." in rel or rel.startswith("/") or rel.startswith("~"):
            return None
        # Root precedence must match SkillsLoader.load_skill(): junction ->
        ...
        roots = [skills_dir()]
        try:
            roots.extend(Path(p).expanduser() for p in JunctionConfig.load().skills.extra_paths)
        except Exception:
            logger.debug("failed to load extra skill paths from config", exc_info=True)
...
def _persisted_session_paths(slot_name: str) -> list["Path"]:
    ...
    if (
        ...
        # Defence-in-depth against path traversal; ``JUNCTION_SESSION_KEY``
...
def _session_has_persisted_history(slot_name: str) -> bool:
    ...
    * A legitimate MCP subprocess whose in-memory slot was evicted by the
      ...
      ``JUNCTION_SESSION_KEY`` env var, so it keeps sending the same
