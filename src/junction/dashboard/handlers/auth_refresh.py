...
from junction.dashboard.origin import check_origin, is_https_request
...
from junction.dashboard.tailnet import TailnetTrust, peer_pin_key, resolve_forwarded_peer
...
async def api_auth_logout(request: web.Request) -> web.Response:
    ...
    2. Access cookie: ``revoke_access_cookie`` adds the access cookie's nonce
       ...
       ``revoke_all_sessions`` (junction logout) which kills every session.
