...
(``_set_junction_entry`` + the file lock) so there is exactly one code
...
from junction.mcp_providers.base import ProviderRegistry, ProviderUnavailableError
...
def _build_registry() -> ProviderRegistry:
    ...
    from junction.dashboard.handlers._shared import admits_registry
...
def _configured_server_names() -> set[str]:
    """Names of all MCP servers Junction currently knows about.

    Same source as the ``/api/mcp`` list endpoint so the ``installed``
    cross-ref matches what the config table shows.
    """
    # circular import: mcp_discovery defers imports of junction.agent which
...
async def api_mcp_discover(request: web.Request) -> web.Response:
    ...
    # Cross-ref against Junction's configured servers so the UI can badge
...
async def api_mcp_discover_install(request: web.Request) -> web.Response:
    ...
    write it into the Junction scope enabled=true (reusing the mcp.py write
...
async def _install_from_official(request: web.Request, server_id: str) -> web.Response:
    """Fetch the registry entry, translate, and write into Junction scope."""
    registry = await asyncio.to_thread(_get_registry)
    ...
    async with _get_mcp_lock():
        ...
        if existing is None:
            ...
            await _offload_config_write(_set_junction_entry, name, enabled=enable_now, spec=spec)
        else:
    ...
    try:
        # circular import: junction.agent imports dashboard handlers.
...
async def _install_via_capability(request: web.Request, server_id: str) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        # for a full junction.json RMW; a direct call would freeze the gateway if
