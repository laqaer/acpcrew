...
from junction import __version__ as _local_version
from junction import dep_sync, shutdown_event
from junction.changelog import Release, base_version, build_release_list
...
from junction.platform.update_layout import set_release_channel, wheel_update_command
from junction.platform.update_provider import CommandProvider, resolve_provider
...
_CLI_MANIFEST_SCHEMA = "junction-cli-artifact-manifest-v1"
...
def _invalidate_update_check(channel: str) -> None:
    ...
    ``$JUNCTION_HOME/channel`` would be both redundant and a synchronous read on a
...
async def _do_update_check() -> None:
    ...
    :func:`~junction.platform.update_capability.derive_capability`, the one
    ...
    * **git checkout** — fetch the remote, then report an update when the
      ...
      when the remote ``src/junction/__init__.py`` ``__version__`` outranks the
    ...
    try:
        ...
        try:
            if generation != _check_generation:
                ...
                # Offloaded: this reads $JUNCTION_HOME/channel, and the data home can
...
async def _check_git_checkout(proj: str, capability: UpdateCapability) -> None:
    ...
    show = await asyncio.create_subprocess_exec(
        "git",
        "show",
        f"{target_sha}:src/junction/__init__.py",
        cwd=proj,
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.DEVNULL,
    )
...
def _changelog_path() -> Path | None:
    ...
    1. ``JUNCTION_PROJECT_DIR/CHANGELOG.md`` — dev / git installs.
    2. Bundled ``junction/CHANGELOG.md`` — pip-wheel installs where
    ...
    if proj:
        ...
        if p.is_file():
            return p
    # updates.py lives at junction/dashboard/handlers/ — parents[2] == junction/
...
async def api_releases(request: web.Request) -> web.Response:
    ...
    belongs to; :mod:`junction.changelog` owns that rule and the reasoning.
...
async def _build_frontend(proj: str, state: DashboardState) -> None:
    ...
    Delegates to the shared :func:`junction.frontend.build_frontend_async`
    helper so the build/stage logic is not duplicated across the three update
    paths (CLI ``junction update``, this dashboard endpoint, and the gateway
    auto-update). That helper runs ``npm ci`` (fallback ``npm install``) then
    ``npm run build`` in ``<proj>/website`` and copies ``website/dist`` into
    the served ``src/junction/static/dist`` — without which the dashboard
...
async def _venv_pip_install(proj: str, state: DashboardState) -> bool:
    ...
    Delegates the choice of HOW to :func:`junction.dep_sync.sync_or_reinstall`:
...
async def _restart_gateway(state: DashboardState) -> bool:
    ...
    try:
        ...
        if not os.path.isfile(exe) or not os.access(exe, os.X_OK):
            ...
            return False
        # circular import: junction.dashboard.chat imports from
        # junction.dashboard.handlers (which re-exports this module), so this
        ...
        reexec_python_module("junction", sys.argv[1:])
        return True
    finally:
...
async def api_update_apply(request: web.Request) -> web.Response:
    ...
    proj = os.environ.get("JUNCTION_PROJECT_DIR", "")
    if not proj:
        return web.json_response({"error": "JUNCTION_PROJECT_DIR not set"}, status=400)
...
async def api_log_level(request: web.Request) -> web.Response:
    """POST /api/logs/level — change the junction logger level at runtime.
...
async def api_log_level_get(request: web.Request) -> web.Response:
    """GET /api/logs/level — current junction logger level."""
    root = logging.getLogger("junction")
...
def install_log_ring_handler() -> _RingLogHandler | None:
    ...
    logging.getLogger("junction").addHandler(handler)
