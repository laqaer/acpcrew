...
from junction.config.loader import CRED_WEIXIN_TOKEN, JunctionConfig, config_path, env_path
...
from junction.dashboard.handlers.messaging import is_direct_local_request
from junction.weixin.client import ILINK_BASE_URL, WeixinClient
...
def _render_qr_data_uri(scan_data: str) -> str:
    ...
    gateway event loop. Delegates to :mod:`junction.qr`, the single owner of QR
    encoding, so this surface and tailnet mobile access cannot drift into two
    different encoders.
    """
    from junction.qr import render_qr_data_uri  # noqa: PLC0415 - heavy import, QR path only
...
def _atomic_write(path: Path, text: str, *, secret: bool = False) -> None:
    ...
    Delegates to :func:`junction.atomic_write.atomic_write`, which every
