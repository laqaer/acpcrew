...
from junction import hooks, publish_sync
from junction import sel as _sel_mod
from junction.artifact_source import LINK, classify_source
...
from junction.hooks import FileTooLargeError, safe_read_file_bytes_with_identity, stat_identity
...
from junction.validation import infer_use_case


def sel():
    """Late-resolved sel() — calls the module function so test patching of
    ``junction.sel.sel`` (the canonical patch target) continues to work."""
    return _sel_mod.sel()
...
def _publish_governance_denied(request: web.Request, provider_name: str) -> str | None:
    ...
    Thin alias for :func:`junction.publish_governance.publish_denied_reason`,
