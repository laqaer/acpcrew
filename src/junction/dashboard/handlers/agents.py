...
from junction.config.schema import SCHEMA_REGISTRY, config_entry_to_dict
from junction.dashboard.chat_persistence import get_reasoning_effort_ordered
...
def _installed_agent_config() -> Path:
    """Return the installed agent config path (~/.kiro/agents/junction.json).
...
async def api_agent_config(request: web.Request) -> web.Response:
    ...
    Reads/writes ``~/.kiro/agents/junction.json`` — the live config that
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            # so they don't reappear on upgrade.  Stored in ~/.junction/config.json
            # (NOT junction.json — kiro-cli rejects unknown fields).
            ...
            from junction.platform.governance import sanitize_agent_config_governance
...
async def api_default_agent(request: web.Request) -> web.Response:
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            known = set((await asyncio.to_thread(JunctionConfig.load)).agents.keys())
        except Exception:
            known = set()
...
async def api_capability_mcp_install(request: web.Request) -> web.Response:
    ...
    try:
        ...
        from junction.dashboard.handlers.mcp import (  # noqa: E402 circular: mcp imports agents
            _sync_mcp_to_agent,
        )

        async with _get_config_lock():
            # Off the loop: _sync_mcp_to_agent acquires bridges' synchronous
            # _mcp_lock and does a full RMW of junction.json. If a concurrent app
...
async def api_agents_installed(request: web.Request) -> web.Response:
    ...
    junction is always first; junction-lite is excluded.
    ...
    def _collect() -> list[Any]:
        ...
        agents.sort(key=lambda a: (0 if a.name == "junction" else 1, a.name))
        return agents
...
def _cc_models(request: web.Request, configured_default: str = "") -> list[dict]:
    ...
    static catalog of everything Junction knows how to name, so a free-tier user
...
def _wrap_list_models_argv(argv: list[str]) -> tuple[list[str], str | None]:
    ...
    shim, or a ``JUNCTION_KIRO_BIN`` pointing at a nonstandard launch path all
...
async def api_models(request: web.Request) -> web.Response:
    ...
    try:
        from junction.acp.client import (  # noqa: F811
            _resolve_kiro_bin_for_spawn,
            _resolve_ssh_auth_sock,
        )
        from junction.env import augmented_path  # noqa: F811
        ...
        if proc.returncode != 0:
            from junction.platform import redact_via_context  # noqa: F811
                    ...
                    async with _get_config_lock():
                        ...
                        if "model" in patch_body:
                            ...
                            if data["model"] is None:
                                ...
                                # Shared with `junction agent reset-model` so the
...
async def api_junction_agents(request: web.Request) -> web.Response:
    ...
    if project_dir:
        ...
        base = dataclasses.asdict(JunctionAgentConfig())
        agents.extend(
...
async def api_junction_agents_sync(request: web.Request) -> web.Response:
    ...
    try:
        ...
        for disc in discovered_agents:
            if (
                ...
                cfg.agents[disc.name] = JunctionAgentConfig(
        ...
        # Only prune package-installed agents (never user-created or junction-owned).
...
async def api_junction_agent_resolved_model(request: web.Request) -> web.Response:
    ...
    Junction agent name; omitted falls back to the configured default agent.
...
def _model_pin_rejected(model: str, request: web.Request, provider: str) -> str | None:
    ...
    *provider* is passed in rather than resolved here so this whole path adds no
    ...
    ``JunctionConfig.load()`` deep-copies the validated dict even on a cache
...
async def api_junction_agents_create(request: web.Request) -> web.Response:
    """POST /api/agents — create a new Junction agent."""
    ...
    async with _get_config_lock():
        ...
        cfg.agents[name] = JunctionAgentConfig(
            ...
            source=body.get("source", "junction"),
        )
        cfg.save()
    _sel().log_api_access(
        caller=request.get("user", "dashboard"),
...
async def api_junction_agent_update(request: web.Request) -> web.Response:
    """PUT /api/agents/{name} — update a Junction agent."""
...
async def api_junction_agent_delete(request: web.Request) -> web.Response:
    """DELETE /api/agents/{name} — delete a Junction agent."""
...
def _regen_conductor() -> None:
    ...
    try:
        ...
        if not cfg.agent.conductor_skill:
            return
        from junction.conductor_skill import generate_conductor_skill  # noqa: F811
