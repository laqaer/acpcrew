...
from junction.agent import _VALID_HOOK_EVENTS, _shipped_defaults, kiro_agents_dir_path
from junction.agent_discovery import list_agents
from junction.config.loader import JunctionConfig, data_home
...
from junction.validation import sanitize_string
...
def _get_hook_store(state: DashboardState):
    ...
    if state._hook_store is None:
        from junction.hooks import (  # noqa: F811  # circular import
            ScriptHookStore,
...
async def api_kiro_hooks(request: web.Request) -> web.Response:
    """GET /api/kiro-hooks — read-only view of kiro-cli agent hooks from junction.json."""
    from junction.platform import redact_via_context as redact

    agent_cfg = kiro_agents_dir_path() / "junction.json"
    try:
...
async def api_hook_test(request: web.Request) -> web.Response:
    ...
    # circular import: junction.hooks pulls dashboard state at module load, so
    # this handler defers the import to call time (matches _get_hook_store above).
    from junction.hooks import HOOK_EVENT_STOP, run_script_hook  # noqa: F811
...
def _load_hook_context(hook_id: str) -> str:
    ...
    lives in :func:`junction.webhooks.resolve_context` so the dashboard's
...
def _verify_hook_token(request: web.Request) -> str | None:
    ...
    if auth.startswith("Bearer "):
        ...
        candidate = request.headers.get("x-junction-token", "")
    if not candidate:
        return None
...
async def api_hooks_agent(request: web.Request) -> web.Response:
    ...
    Payload:
        ...
        agent (str): agent name for routing (default: junction)
        deliver (bool): send result to Slack DM + dashboard notification
        timeoutSeconds (int): max agent run duration

    Headers:
        Authorization: Bearer <token>  (or X-Junction-Token)
        X-Junction-Timestamp: unix seconds — required when the token requires
            signatures
        X-Junction-Signature: sha256=<hex hmac of "<timestamp>.<raw body>">
    """
...
async def _run_hook_inner(
    ...
    from junction.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK  # noqa: F811
    ...
    if is_new and state.context_builder:
        ...
        full_message, _ = await run_in_embed_pool(
            ...
            provider_type=JunctionConfig.load().agent.provider,
        )
    result_text = ""
