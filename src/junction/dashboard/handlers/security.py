...
from junction.apps.manager import disable_app, get_app, list_apps
from junction.apps.official_catalog import CatalogUnavailable
...
from junction.apps.routes import app_lifecycle_lock
...
from junction.security import DENY_REASON_MATCH_PREFIX
...
def _denied_state(data: dict) -> dict:
    ...
    *data* is the file root (the opt-out object itself), not a config wrapper.
    """
    from junction.hooks import _coerce_bool
...
async def _pinned_ids_for_snapshot_async() -> set:
    ...
    """
    from junction.security import pinned_builtin_command_ids_for_snapshot
...
def _reload_live_hooks(request: web.Request, denied_state: dict) -> None:
    ...
    from junction.hooks import HooksConfig
...
def build_trusted_apps_snapshot() -> dict:
    ...
    """
    agent = JunctionConfig.load().agent
...
class TrustSettingOverlayOwned(Exception):
    ...
    ``JunctionConfig`` deep-merges the user-owned ``config.local.json`` overlay OVER
...
async def _mutate_agent_config(mutate) -> None:
    ...
    **Why a targeted raw edit and not ``JunctionConfig.load()`` → ``save()``.**
    ...
    parse as a JSON object. ``JunctionConfig.load()`` degrades a corrupt file to
...
async def api_trusted_app_grant(request: web.Request) -> web.Response:
    ...
    # against other in-process lifecycle work, but `junction app uninstall` runs in a
    ...
    async with app_lifecycle_lock(name):
        ...
        if was_granted and not is_builtin:
            ...
            if record:
                ...
                # (``junction app disable``) calls it from a DIFFERENT PROCESS
