...
The dashboard already had every *piece* of tailnet access — :mod:`junction.
dashboard.tailnet` resolves the MagicDNS name, :mod:`junction.dashboard.
tailnet_serve` publishes and withdraws ``tailscale serve``, and the CLI wires
them together as ``junction tailnet up`` — and no way to get there from the UI.
...
:func:`junction.dashboard.handlers.tailnet._derive_state` gives: one owner for
...
from junction.qr import render_qr_data_uri
...
def _derive_step(
    *,
    ...
    6. ``occupied`` — serve holds this port/mount for something that is not this
       ...
       (``junction tailnet up``) for the operator to run deliberately.
...
def _dashboard_port(request: web.Request) -> int:
    ...
    ``tailscale serve`` whatever else holds it — the hazard ``junction tailnet
...
async def api_tailnet_mobile_qr(request: web.Request) -> web.Response:
    ...
    # `junction logout` all still apply unchanged.
    #
    ...
    try:
        _cfg = await asyncio.to_thread(JunctionConfig.load)
