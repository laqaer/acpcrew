...
and generically surfaces every other ``junction.*`` metric so newly-added emit
...
from junction import __version__, beacon
...
from junction.hooks import validate_file_path
from junction.metrics.provider import TELEMETRY_ENV_VAR, env_pin
from junction.metrics.schema import RESOURCE_ATTR_PROCESS_START_TIME
from junction.security import redact_credentials, redact_exfiltration_urls

logger = logging.getLogger(__name__)

_STARTUP_METRIC = "junction.session.startup.duration"
_TURN_METRIC = "junction.turn.duration"
...
# every attribute present: ``junction.gateway.request.duration`` carries
...
# junction.watchdog.recovery.outcome. Counting them as faults would inflate the
...
def _telemetry_cfg() -> _TelemetryState:
    ...
    ``enabled`` is the EFFECTIVE state, not the stored flag: ``JUNCTION_TELEMETRY``
...
def _iter_metric_points(
    ...
    Every ``junction.*`` data point is yielded; the name filter is load-bearing
    rather than defensive. One meter carries all three namespaces the recorder
    accepts — ``junction.*``, ``gen_ai.*`` and ``app.<app_id>.*``
...
def _gauge_series(
    ...
    Shards are per-PID and several junction processes (gateway, MCP daemons)
...
def _aggregate(shard_paths: list[Path]) -> dict[str, Any]:
    ...
    # generic surface for every other junction.* metric
    other_hist: dict[str, _Hist] = {}
    ...
    # wins WITHIN one process. Shards are per-PID and several junction
...
async def api_telemetry_startup(request: web.Request) -> web.Response:
    """GET /api/telemetry/startup — session-startup latency + all junction.* metrics.
    ...
    list surfacing every other emitted junction.* metric.
...
async def api_context_trace(request: web.Request) -> web.Response:
    ...
    Returns what Junction added to each turn of one session, block by block, so
    ...
    :func:`junction.dashboard.handlers.usage.context_trace`.
...
async def api_beacon_status(request: web.Request) -> web.Response:
    ...
    ``JUNCTION_TELEMETRY_DISABLED``, a CI host, a non-default data home, or a
    ...
    try:
        # to_thread, not a bare load(): JunctionConfig.load() stats and reads
...
async def api_collection_status(request: web.Request) -> web.Response:
    ...
    ``JUNCTION_TELEMETRY`` overrides ``telemetry.enabled`` inside the collector: a
