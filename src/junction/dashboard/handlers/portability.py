"""Portability API handlers — export/import Junction state as zip."""
...
from junction.portability import apply_import_zip, create_export_zip, validate_import_zip
from junction.sel import sel as _sel_fn  # circular import — sel imports lazily
...
async def api_portability_export(request: web.Request) -> web.Response:
    """GET /api/portability/export — download Junction state as zip."""
    ...
    filename = f"junction-export-{ts}.zip"
...
async def api_portability_import(request: web.Request) -> web.Response:
    """POST /api/portability/import — upload and apply a Junction export zip."""
    if "user" not in request or not request["user"]:
        return web.json_response({"error": "authentication required"}, status=401)
    caller = request["user"]
