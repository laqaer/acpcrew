...
from junction.dashboard.state import MAX_LIVE_SLOTS, MAX_SLOTS_PER_CREATOR, SlotOrigin
from junction.history import metadata_now_iso, transcript_stem
from junction.security import redact, redact_and_truncate
from junction.sel import sel
from junction.validation import MAX_LONG_STRING
...
def session_control_enabled() -> bool:
    ...
    """
    try:
        return bool(JunctionConfig.load().agent.session_control)
    except Exception:
        logger.warning(
...
async def prewarm_enabled_check() -> None:
    ...
    ``JunctionConfig.load()`` re-reads and validates the file on the FIRST call
...
async def create_session(
    ...
    *,
    ...
    # `JunctionConfig.load()`, which is filesystem work that must not run on the
...
async def stop_target(
    ...
    *,
    ...
    from junction.dashboard.chat_handlers import stop_slot_turn
