...
from junction.platform_compat import pid_exists
...
def open_dump_file(dumps_dir: Path | None = None) -> DumpFile:
    ...
    f.write(f"# Junction loop-stall crash dump — opened {ts}\n")
...
def dump_first_stack_lines(dump_path: Path, max_lines: int = 5) -> list[str]:
    ...
    ``junction doctor`` does) actively misleads whoever is diagnosing the
