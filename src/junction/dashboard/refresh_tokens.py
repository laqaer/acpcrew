...
from junction.dashboard.revocation_gen import (
    current_revocation_gen,
    current_revocation_gen_or_none,
)
from junction.dashboard.token_secret import _get_secret
...
# will be forced through the regular ``junction token`` URL flow again
...
class RefreshStateManager:
    ...
    window (the user simply re-mints via the ``junction token`` URL) and keeping
...
def generate_refresh_token(
    ...
    *,
    ...
    Omit it to start a fresh chain (initial mint after ``junction token``
    URL is consumed).
    ...
    payload_dict = {
        ...
        # (junction logout) ends refresh chains, not just access cookies.
...
def validate_refresh_token(token: str) -> tuple[bool, str, str, str, str, float]:
    ...
    # cookie types — revoke_all_sessions() (junction logout) bumps it once and
