...
from junction.context_management import extract_plan_metadata, rephrase_plan
from junction.dashboard.chat_folder_suggest import maybe_suggest_folder
from junction.dashboard.chat_utils import (
    slot_history_key,
)
from junction.dashboard.state import NEW_SESSION_TITLE, DashboardState, _ChatSlot
from junction.llm_helpers import background_turn, run_bg_oneliner
...
def _ui_language() -> str:
    ...
    **Call this OFF the event loop.** ``JunctionConfig.load()`` stats, reads and
...
async def _rephrase_plan_lite(
    ...
    *,
    ...
    """Rephrase a plan using the cheap background session (junction-lite)."""
