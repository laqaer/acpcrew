...
from junction.acp.client import resolve_prompt_timeout
...
_TURN_DEADLINE: ContextVar[float | None] = ContextVar("junction_turn_deadline", default=None)
...
def _acp_prompt_ceiling() -> float:
    ...
    Resolves through :func:`~junction.acp.client.resolve_prompt_timeout`, which
...
def tool_approval_timeout_secs() -> float:
    ...
    * against the budget REMAINING in the turn that is already running. A prompt
    ...
    """
    try:
        configured = float(JunctionConfig.load().agent.tool_approval_timeout_secs)
    except Exception:
...
async def bounded_chat_turn(coro: "Coroutine[Any, Any, Any]") -> Any:
    ...
    the :func:`chat_turn_timeout_secs` resolution — a ``JunctionConfig.load()``
