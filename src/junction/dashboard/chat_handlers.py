...
from junction.agent_discovery import cached_project_agent_names, warm_project_agent_names
...
from junction.dashboard.channel_slots import channel_slot_name, note_slot_closed
from junction.dashboard.chat_auto_tag import maybe_auto_tag
from junction.dashboard.chat_delivery import (
    STEER_REQUEUED,
    STEER_STEERED,
    queue_for_next_turn,
...
from junction.dashboard.chat_title import _maybe_auto_title
...
from junction.dashboard.system_notices import SESSION_RELOAD_KIND, is_system_notice
from junction.dashboard.turn_dispatch import spawn_guarded_turn
from junction.history import carry_provenance, is_incognito_transcript
...
async def api_chat(request: web.Request) -> web.StreamResponse:
    ...
    # (the system prompt and the junction-commands / web-browse skills tell it
...
def _generate_state(cfg: JunctionConfig, slot: Any) -> str:
    ...
    for aid, fut in list(slot._approval_futures.items()):
        if not fut.done():
            ...
            sel().log_tool_invocation(
                ...
                agent=getattr(slot, "agent", "") or "junction",
                source="dashboard",
...
async def _restore_slot_nudge_loop(
    ...
    if loop is None or not loop.active:
        return
    try:
        from junction import autonudge  # circular: autonudge -> dashboard.chat -> chat_handlers
...
async def api_chat_slot_delete(request: web.Request) -> web.Response:
    ...
    if slot._app:
        from junction.apps.teardown import (
...
async def api_chat_slot_agent(request: web.Request) -> web.Response:
    ...
    async with slot._lock:
        ...
        logger.info(
            "Slot %s agent switched to %r, resetting session", name, agent_name or "junction"
        )
...
def _model_rejected_reason(model_name: str, provider: str | None = None) -> str | None:
    ...
    *provider* lets a caller that has already loaded the config supply it, so
    this adds no read of its own: ``JunctionConfig.load()`` deep-copies the
    ...
    if provider is None:
        try:
            provider = JunctionConfig.load().agent.provider
