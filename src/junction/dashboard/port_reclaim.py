"""Reclaim the dashboard bind port from a *stale* Junction gateway.

Background
----------
The gateway binds its dashboard port once at startup (:func:`_start_site` in
:mod:`junction.dashboard.server`). If the port is already taken it retries for
...
dump-then-exit from :mod:`junction.dashboard.loop_watchdog`, or a ``kill -9``)
...
* the holder is a *Junction gateway* process — reusing the same structural
  command-line check that gates ``junction stop`` (:func:`_is_junction_process`)
...
* the holder does **not** answer an HTTP liveness probe — a *responsive*
  ...
  ``JUNCTION_HOME`` sharing the port) and is never killed.

Complements — does not replace — :class:`junction.gateway_lock.GatewayLock`,
...
:class:`junction.dashboard.loop_watchdog.LoopStallWatchdog`.
"""
...
logger = logging.getLogger("junction.dashboard.port_reclaim")
...
FOREIGN_HOLDER = "foreign_holder"  # held by a non-Junction process — never touched
...
async def _terminate_pids(
    ...
    *,
    ...
    silently looping).
    """
    from junction.cli_server import _pid_exited
    ...
    def _signal(pid: int, sig: int) -> bool:
        if platform_compat.IS_WINDOWS:
            ...
            # Windows branch of `junction stop`.
            try:
...
async def reclaim_stale_gateway_port(
    ...
    *,
    ...
    is_junction: Callable[[int], bool] | None = None,
    ...
    """Reclaim *port* iff it is held by a stale/unresponsive Junction gateway.
    ...
    # as `junction stop`). Bound to an explicit non-Optional local so the call
    ...
    # ``identity_timeout``: the shared ``_is_junction_process`` shells out to
