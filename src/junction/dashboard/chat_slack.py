...
from junction.dashboard import state as dashboard_state
from junction.dashboard.chat_backfill import (
    backfill_content,
    gap_summary,
...
from junction.dashboard.state import DashboardState, _log_task_exception
from junction.messaging.link import SLACK_NAMESPACE
from junction.platform.context import redact_via_context
from junction.platform.governance_profiles import vet_and_audit
from junction.security import redact_and_truncate
from junction.sel import sel
from junction.slack.channel_resolver import _CACHE_FILENAME, ChannelNameResolver
...
from junction.sync_bridge import handoff_to_slack
...
def _format_backfill_parts(content: str, icon: str) -> list[str]:
    ...
    Thin delegate to :func:`junction.slack.format.render_for_slack`, which owns
...
async def drain_slack_backfill(
    ...
    if selection.skipped_turns and selection.recent:
        ...
        link = ""
        try:
            # Offloaded: JunctionConfig.load() reads and validates the config
