...
:mod:`junction.dashboard.urls` leaf and is re-exported below, so ``from
junction.dashboard.origin import ...`` keeps working for every existing caller
...
from junction.dashboard.urls import (  # noqa: F401
    _BIND_ALL,
    _BIND_LOCAL,
...
def is_proxied_request(request: web.Request) -> bool:
    ...
    * **upstream** — a proxy on *another* host (an nginx box, a container
      ...
      (``JUNCTION_BIND``), which presents a NON-loopback peer.
...
def check_origin(
    ...
    *,
    ...
    # ``JUNCTION_ALLOWED_LOOPBACK_PORTS`` (folded into the allowed set above).
    return False
