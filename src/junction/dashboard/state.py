...
from junction.config.loader import DASHBOARD_PORT, _raw_config, config_dir
from junction.constants import (
    OPTIONS_RE_LINE,
    SUBAGENT_BATCH_COMPLETION_PREFIX,
    SUBAGENT_COMPLETION_PREFIX,
)
from junction.dashboard.chat_compaction_notice import deliver_channel_compaction_notice
...
from junction.messaging.renderer import display_safe
...
from junction.notifications.rate_limit import AppRateLimiter
from junction.notifications.resource_pressure import ResourcePressureNotifier
from junction.notifications.settings import ChannelSettings
from junction.preview_text import strip_markdown_preview
from junction.release_channel import channel as _release_channel_of_build
...
if TYPE_CHECKING:
    ...
    from junction.dashboard.loop_watchdog import LoopStallWatchdog  # noqa: F401
    from junction.messaging.transport import MessagingTransport  # noqa: F401
    from junction.power import SleepInhibitor  # noqa: F401
    from junction.slack.outbound import PostedOptions  # noqa: F401
...
# entrypoint via set_build_info() — AFTER JUNCTION_PROJECT_DIR is detected and
...
def _ascii_slot_key(name: str) -> str:
    """Return *name* with any character outside printable ASCII replaced by ``-``.
     ...
     junction-core sends as the ``X-Session-Key`` HTTP header on every gateway
...
class _ChatSlot:
    ...
    def __init__(
        ...
        # conversation (see junction.dashboard.channel_folders). Persisted,
...
class DashboardState:
    ...
    def __init__(
        ...
        from junction.dashboard.file_index import FileIndexRegistry
    ...
    async def request_question(
        ...
        :func:`junction.validation.validate_ask_user_question` — this method
    ...
    def get_or_create_slot(
        ...
        *linked_session_key* binds a new slot to the session its conversation
        ...
        if name:
            ...
            # that junction-core sends as the ``X-Session-Key`` HTTP header
    ...
    def _serialize_subagent_batch(
        ...
        try:
            ...
            from junction.dashboard.ws_event_scope import effective_allowed_events
...
def _governance_status() -> str:
    ...
    try:
        from junction.platform.governance_health import governance_status
...
def _cached_check_status(url: str) -> dict | None:
    ...
    from junction.dashboard.handlers.source_providers import get_cached_check_status
