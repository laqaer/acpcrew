...
Translates OpenAI API format into Junction's slot-based chat system,
allowing any OpenAI SDK client to talk to Junction agents by setting
`model` to the agent name (e.g. "router", "lite").

Limitations:
- ``usage`` fields are hardcoded to zero; Junction does not track token
...
async def _blocking_response(
    ...
    Note: ``usage`` is hardcoded to zero — Junction does not expose token
