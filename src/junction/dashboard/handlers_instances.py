...
from junction.history import SEARCH_MIN_CHARS
    ...
    return web.json_response(
        {
            ...
            "instances": items,
            "warm_set_cap": JunctionConfig.load().instances.warm_set_cap,
        }
    )
...
async def api_instances_connect(request: web.Request) -> web.Response:
    ...
    if status.state.value == "connected":
        ...
        # (a failed self-heal re-mint, or a remote `junction restart` that
