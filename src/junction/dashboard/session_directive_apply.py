...
tool's OWN return over the MCP pipe; this string is recorded on Junction's
...
from junction.autonudge import APPROVAL_STALL_REASON, AUTONUDGE_STOP_REASON
...
def _audit(session_key: str, kind: str, outcome: str) -> None:
    ...
    """
    try:
        # Local import: junction.sel transitively pulls config -> apps ->
...
async def _monitor_update(session_key: str, args: dict[str, Any]) -> str:
    from junction.autonudge import get_instance
    from junction.autonudge_authz import authorize_and_update_nudge
...
async def _set_project(state: Any, slot: Any, args: dict[str, Any]) -> str:
    ...
    if rp != old_project:
        slot._pending_reset_history_key = effective_session_key(slot)
        try:
            from junction.dashboard.chat_handlers import _save_recent_project
...
async def _suggest_followup(state: Any, slot: Any, args: dict[str, Any]) -> str:
    from junction.dashboard.chat_handlers import _redact_followup_item
