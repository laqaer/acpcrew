...
* **Recency-bounded.** Only sessions modified within the dashboard's configured
  ...
  window, matching :func:`~junction.dashboard.chat_persistence.restore_recent_sessions`.
...
from junction.dashboard.channel_folders import lookup_channel_folder
...
from junction.messaging.link import channel_namespace_of, is_channel_session_key
...
def channel_slot_name(session_key: str) -> str:
    ...
    :meth:`~junction.session.SessionManager.channel_key_for_stem`.
    """
...
def surface_channel_session(
    ...
    *folder_id* is the channel's configured session folder (see
    :mod:`junction.dashboard.channel_folders`), applied only when the session
