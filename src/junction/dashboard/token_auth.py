...
from junction.dashboard.revocation_gen import (  # noqa: F401  # re-exports
    _REVOCATION_FILE,
...
# token_signing.key into $JUNCTION_HOME (the CLI imports token_auth for every
# junction subcommand; an import-time write would break gateway --seed and
# pollute the home for read-only commands).
from junction.dashboard.token_secret import (  # noqa: F401  # re-exports
    _SECRET_KEY_FILE,
    _get_secret,
    _load_or_create_secret,
)
from junction.executors import subprocess_executor
from junction.mcp_gateway.socketsec import PeerCredResult, check_peer_is_self, get_peer_pid
...
class TokenStateManager:
    ...
    and sync contexts (CLI commands like `junction token`). The lock hold time
...
_BYPASS_EXACT = {
    ...
    "/api/token/local",
    "/api/shutdown",
    # `junction logout` (CLI) authenticates with loopback + the local secret via
    ...
    # without this entry every `junction logout` is denied 403 by the middleware
...
# (handle_app_api_proxy in junction/apps/routes.py) and continues to
...
def validate_token(token: str, *, use_session_exp: bool = False) -> tuple[bool, str, str]:
    ...
    # Revocation generation: an explicit revoke_all_sessions() (e.g. junction
...
def token_embed_parent_port(token: str) -> int | None:
    ...
    parent dashboard's port (the embedding desktop app's ``JUNCTION_PORT``) is
...
def _unix_request_socket(request: web.Request) -> Any:
    ...
    :func:`~junction.dashboard.origin.request_is_unix_socket` discriminator
...
def derive_caller_app(
    ...
    **Why this exists.** App-ownership checks gate on ``request["app"]``, which
    ...
    that transport, and an app agent granted ``@junction-dashboard`` arrived
