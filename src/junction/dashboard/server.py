...
from junction import platform_compat, port_resolution
from junction.apps.backend import start_enabled_app_backends
from junction.apps.hooks_integration import (
    init_hooks_system,
    on_gateway_shutdown,
    on_gateway_startup,
)
from junction.apps.manager import cleanup_migrated_builtin, register_builtin_apps
from junction.autonudge import get_instance as _autonudge_get
from junction.autonudge_authz import authorize_and_add_nudge
from junction.browser_cli import launch as browser_cli_launch
from junction.browser_cli import snapshots as browser_cli_snapshots
from junction.browser_cli import token as browser_cli_token
from junction.browser_cli import view as browser_cli_view
from junction.channel_transcript_migration import migrate_channel_transcripts
from junction.config import data_home
...
from junction.dashboard import (
    cautious_boot,
    channel_slots,
    chat,
    handlers,
...
from junction.dashboard.handlers.feedback import setup_feedback_routes
from junction.dashboard.handlers.knowledge import setup_knowledge_routes
from junction.dashboard.handlers.link_meta import setup_link_meta_routes
from junction.dashboard.handlers.secrets import setup_secrets_routes
from junction.dashboard.handlers.source_providers import (
    register_status_delta_sink,
    unregister_status_delta_sink,
)
from junction.dashboard.handlers.weixin_qr import setup_weixin_routes
from junction.dashboard.handlers.whatsapp_setup import setup_whatsapp_routes
...
from junction.dashboard.port_reclaim import (
    FOREIGN_HOLDER,
    HEALTHY_PEER,
    RECLAIMED,
    reclaim_stale_gateway_port,
)
from junction.dashboard.routes import register_all
from junction.dashboard.slowloris import build_hardened_runner
from junction.dashboard.state import _DEFAULT_PORT, DashboardState
...
from junction.deploy import _register_core_skills as _register_deploy_skills
from junction.deploy.handlers import register_routes as _register_deploy_routes
from junction.executors import subprocess_executor
from junction.hooks import ScriptHookStore, set_global_hook_store
...
from junction.mcp_gateway.socketsec import chmod_socket_0600
...
from junction.skill_usage import register_skill_read_observer
from junction.skills import SkillsLoader, set_pending_consumed_hook, set_pending_staged_hook
from junction.tunnel.setup import setup_tunnel

if TYPE_CHECKING:
    from junction.dashboard._types import (  # noqa: F401
        ContextBuilder,
        ConversationLog,
        CronService,
        HistoryConsolidator,
...
async def _should_prevent_sleep(state: DashboardState, port: int) -> bool:
    ...
    * **The dashboard is published on this machine's tailnet**, and
    ...
    """
    try:
        # JunctionConfig.load() does a stat and, on a cache miss, a JSON read +
...
_STRICT_INTERNAL_API_PATHS = frozenset(
    {
        ...
        # Computer use: the ``junction-computer`` stdio shim's forwarding leg.
...
_MIXED_INTERNAL_API_PATHS = frozenset(
    {
        ...
        # httpOnly, ``JUNCTION_INTERNAL_SECRET`` is stripped from agent env by
...
def _extra_frame_ancestors(
    ...
    (embedding) dashboard's port — its ``JUNCTION_PORT`` — so the embedded remote
    ...
    return [
        f"http://{host}:{port}"
        for host in ("127.0.0.1", "localhost", "[::1]", "junction.localhost")
    ]
...
def _register_dist_static_routes(app: web.Application, dist_dir: Path) -> None:
    ...
    # (generic lucide icon / "JUNCTION" hero). Stable, un-hashed filenames, so
...
def _precompute_telemetry(state: "DashboardState") -> None:
    ...
    from junction.dashboard.handlers_system import _get_owner_hash, _get_static_system_info
...
def _deferred_session_control(handler_name: str) -> Callable:
    ...
    async def _route(request: web.Request) -> web.StreamResponse:
        from junction.dashboard.handlers import session_control
...
def _register_mcp_routes(app: web.Application) -> None:
    ...
    # Computer use: the thin ``junction-computer`` stdio shim's only call. Lives
    ...
    # Auto-nudge (feature-flagged — returns 503 when JUNCTION_AUTONUDGE unset)
    from junction.dashboard.handlers.autonudge import (
    ...
    from junction.dashboard.handlers.workflows import (
        api_workflow_author,
        api_workflow_run,
    ...
    from junction.dashboard.handlers.webapp_preview import register_webapp_preview_routes
    ...
    from junction.dashboard.handlers.sandbox_doc import register_sandbox_doc_routes
...
def _export_bound_port(runner: web.AppRunner, port: int) -> None:
    ...
    Sets ``JUNCTION_BOUND_PORT`` in this process's environment once the TCP
    ...
    Deliberately a DISTINCT variable from ``JUNCTION_PORT``: that one means
    ...
    ``JUNCTION_BOUND_PORT`` carries ephemeral truth only: consumed by
    ...
    *port* is ``0`` for an OS-assigned ephemeral bind (``--port auto``); the
    ...
    if bound:
        os.environ["JUNCTION_BOUND_PORT"] = str(bound)
        logger.debug("Exported JUNCTION_BOUND_PORT=%d for child processes", bound)
    else:
        logger.warning(
...
async def _start_site(
    ...
    *,
    ...
    non-Junction process is never touched; those (and any case where the holder
    ...
    logger.error(
        ...
        PRODUCT_NAME,
        CLI_BIN,
        "junction",  # systemd unit name; not the product CLI
    )
...
def _dispatch_override_expiry_notification(state: DashboardState, notify_coro_factory: Any) -> bool:
    ...
    """
    if not JunctionConfig.load().agent.notify_override_expiry:
        return False
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
...
def _register_instances_hooks(app: web.Application, state: DashboardState, port: int) -> None:
    ...
    async def _instances_startup(app_: web.Application) -> None:
        ...
        # http://127.0.0.1:<port> (NOT localhost / junction.localhost) — mixing
...
def _arm_prevent_sleep_poll(state: DashboardState, port: int) -> None:
    ...
    *port* is the port this server actually bound, needed because one of the two
    ...
    the configured one for the same reason ``junction tailnet up`` insists on
...
async def start_dashboard(
    ...
    if consolidator is None and conversation_log is not None:
        try:
            from junction import history as _hist_mod
            ...
            _scfg = JunctionConfig.load().skills
    ...
    # --- Dynamic Workflows ---
    try:
        from junction.dashboard.handlers import workflows as wf_handlers
        from junction.dashboard.workflow_inject import inject_workflow_result
        from junction.security import redact_credentials, redact_exfiltration_urls
        ...
        try:
            _wf_timeout_secs = int(JunctionConfig.load().agent.workflow_run_timeout_secs)
        except Exception:
    ...
    async def _reconcile_app_resources() -> None:
        from junction.apps.bridges import reconcile_enabled_app_resources
    ...
    from junction.apps.spawn_sdk import build_spawn_impl
    ...
    async def _hooks_startup(app_: web.Application) -> None:
        ...
        from junction.apps.dev_mode import init_dev_mode_watcher
    ...
    async def _hooks_shutdown(app_: web.Application) -> None:
        ...
        from junction.apps.dev_mode import stop_dev_mode_watcher
    ...
    # validated.
    _ts_cfg = JunctionConfig.load().dashboard.tailscale
    ...
    # junction.localhost) onto a single origin so the SPA's per-origin
    ...
    # dashboard on "localhost" one time and "junction.localhost" the next (e.g.
    # `junction token` historically printed localhost while the gateway
    # auto-opens junction.localhost) lands the browser in a different, empty
    ...
    try:
        _exit_after = float(JunctionConfig.load().dashboard.loop_stall_exit_after_secs)
    except Exception:
    ...
    async def _notify_slack_override_expired() -> None:
        ...
        await _dm_owner(
            state,
            "\U0001f512 Safety override expired. Tools now require approval. Reply `/junction yolo` to re-authorize.",
        )
    ...
    def _on_override_expired(source: str) -> None:
        ...
        try:
            ...
            from junction.messaging.session_trust import clear_trusted_sessions
    ...
    from junction.channel import ChannelManager, run_channel_agent
    from junction.dashboard.handlers_channel import _spawn_agent_task
...
async def start_api_server(
    ...
    # widened it (dashboard.url opt-out of local_only, or the JUNCTION_BIND
