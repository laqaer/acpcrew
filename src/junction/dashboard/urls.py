...
Split out of :mod:`junction.dashboard.origin` so callers that only need URL
...
Why it matters: ``junction`` CLI invocations and every MCP stdio subprocess
...
_CANONICALIZABLE_LOOPBACK_HOSTS = frozenset({"127.0.0.1", "::1", "localhost", "junction.localhost"})
...
def is_loopback(host: str) -> bool:
    ...
    if host in ("localhost", "127.0.0.1", "::1", "junction.localhost"):
        return True
    try:
...
def parse_dashboard_url(url: str) -> tuple[str, int]:
    ...
    ``JUNCTION_PORT`` env var always overrides the port (dev mode).
    """
    if not url:
    ...
    if env_port:
        try:
            ...
            logger.warning(
                "JUNCTION_PORT=%r is not a valid integer; using port %d from config",
                env_port,
                port,
            )
    return host, port
...
def devspaces_proxy_url(port: int) -> str | None:
    ...
    its origin via ``dashboard.url`` or ``JUNCTION_CORS_ORIGINS``.
    """
    return None
...
def bind_address_for(local_only: bool) -> str:
    ...
    the port to be exposed. The official image sets ``JUNCTION_BIND=0.0.0.0``;
...
def resolve_dashboard_host(local_only: bool, configured_host: str = "") -> str:
    ...
    if local_only:
        ...
        # ``junction.localhost`` (and other ``*.localhost`` names) are NOT
        ...
        # honor — so canonicalizing onto ``junction.localhost`` would 302 those
...
def should_canonicalize_host(
    ...
    *,
    ...
    Converges loopback aliases (127.0.0.1 / localhost / junction.localhost) onto
...
def build_allowed_origins(
    ...
    *tailnet_host* is this machine's MagicDNS name when tailnet access is
    ...
    origins: set[str] = {
        ...
        f"http://junction.localhost:{port}",
    }
    ...
    for _co in os.environ.get("JUNCTION_CORS_ORIGINS", "").split(","):
        if _co.strip():
    ...
    for _p in os.environ.get("JUNCTION_ALLOWED_LOOPBACK_PORTS", "").split(","):
        _p = _p.strip()
        if _p.isdigit():
            origins.add(f"http://127.0.0.1:{_p}")
            origins.add(f"http://localhost:{_p}")
            origins.add(f"http://[::1]:{_p}")
            origins.add(f"http://junction.localhost:{_p}")
    return origins
...
def build_allowed_hosts(allowed_origins: set[str]) -> set[str]:
    ...
    rejected.
    """
    hosts: set[str] = {"localhost", "127.0.0.1", "::1", "junction.localhost"}
