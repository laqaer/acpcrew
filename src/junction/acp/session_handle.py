...
existing ``from junction.acp.runtime import AcpSessionHandle`` call sites keep
working.
"""
...
class WatchdogSettings:
    ...
    # snapshot. Telemetry-only (the junction.watchdog.action attr): a BOOLEAN,
...
def _clamp_to_prompt_ceiling(key: str, value: float, chat_ceiling: float) -> float:
    ...
    Resolved via :func:`~junction.acp.client.prompt_timeout_for_ceiling` on the
...
class AcpSessionHandle:
    ...
    def _emit_watchdog_metric(
        ...
        """Emit junction.watchdog.action + junction.watchdog.idle.duration (best-effort).
        ...
        try:
            ...
            rec = get_recorder()
            rec.counter("junction.watchdog.action", attrs=attrs)
            # ms, like every other junction duration histogram: the dashboard's
            # generic aggregation reports all histograms under *_ms keys, so a
            # seconds-unit instrument would render 1000x off there.
            rec.histogram(
                "junction.watchdog.idle.duration",
                float(idle) * 1000.0,
                unit="ms",
    ...
    def _emit_tool_interrupted_sel(self, site: str) -> None:
        ...
        Junction observes but does not control (kiro-cli denied tool execution).
        Best-effort — a failed audit must not break the turn. This handle has no
        Junction session_key, so the ACP sessionId is recorded in metadata for
        correlation.
        """
        logger.warning(
