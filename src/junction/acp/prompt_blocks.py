...
* :meth:`junction.acp.session_handle.AcpSessionHandle.prompt` -- the live path
...
* :meth:`junction.acp.client.AcpClient._send_prompt` -- the direct-client path.
...
from junction.hooks import is_unc_shape, safe_read_file_bytes, unc_probe_allowed

# The budget constants and Pillow machinery live in the LEAF module
# junction.imaging (shared with the gateway's tool-result rewrite, which must
...
from junction.imaging import (  # noqa: F401 -- constants re-exported, see comment
    MAX_IMAGE_B64_BYTES,
    MAX_IMAGE_EDGE_PX,
    ...
    *,
    ...
    if allow_image:
        ...
        for match in _PATH_RE.finditer(message):
            ...
            # is platform-gated anyway. See junction.hooks.unc_probe_allowed.
