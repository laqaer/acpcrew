...
from junction.acp.kas_permissions import allowed_tools_to_permissions
...
#: :mod:`junction.acp.kas_permissions` translates it into ``permissions``, so
...
def load_agent_spec(agents_dir: Path, agent_id: str) -> dict[str, Any]:
    ...
    stays free of :mod:`junction.agent`, which imports the config loader and
...
def build_kas_custom_agents(agents_dir: Path, agent_id: str) -> list[dict[str, Any]]:
    ...
    A prompt-less spec (e.g. ``junction-lite``) is projected with the small
