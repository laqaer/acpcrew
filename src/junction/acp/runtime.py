...
here so ``from junction.acp.runtime import AcpSessionHandle`` (and the
...
from junction import agent_scratch, platform_compat
...
from junction.acp._dispatch import reject_option_id as _reject_option_id
from junction.acp._dispatch import (
    set_mode_params,
)
...
from junction.env import augmented_path, resolve_krb5_ccname
...
from junction.metrics.events import (
...
from junction.validation import MODEL_ID_RE
...
def _format_runtime_rpc_error(error: object) -> str:
    ...
    error', 'data': "Mode 'junction' not found"}`` — which names an internal ACP
...
def _get_rss_tree_mb(pid: int) -> float | None:
    ...
    On Linux the junction-lite background runtime is spawned through the
...
class AcpRuntime:
    ...
    def __init__(
        ...
        # runtimes (e.g. the junction-lite background runtime) have no
        ...
        # (never in __init__: JunctionConfig.load() is a synchronous disk
    ...
    async def _is_stale(self) -> str | None:
        ...
        junction-lite background runtime observed growing unbounded (multi-GB
        ...
        try:
            ...
            init_resp = await self._send_and_await(
                "initialize",
                {
                    ...
                    # telemetry — bucketed as "(none)" instead of "junction". Nest it to
    ...
    def _audit_denied_off_loop(
        ...
        *,
        ...
        import: a module-level import of ``junction.sel`` would be circular
    ...
    async def _session_start_budget(self) -> float:
        ...
        ``_resolve_session_start_timeout`` calls ``JunctionConfig.load()``,
