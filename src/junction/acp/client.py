...
from junction import agent_scratch, model_registry, platform_compat
...
from junction.acp.prompt_blocks import build_prompt_blocks
...
from junction.constants import (
    ...
    JUNCTION_SPAWNED_ENV,
    JUNCTION_SPAWNED_VALUE,
)
from junction.env import augmented_path, mise_data_dir, resolve_krb5_ccname
...
from junction.skill_usage import get_global_skill_read_observer

logger = logging.getLogger(__name__)

CLIENT_NAME = "junction"
...
def _resolve_kiro_bin() -> str | None:
    ...
    Returns the installed binary's own path. Junction never copies the CLI and
...
def _mise_node_installs_dir() -> Path:
    ...
    The data root comes from :func:`junction.env.mise_data_dir` so that
...
def _vendored_claude_acp_roots(pkg_dir: Path | None = None) -> list[Path]:
    ...
    *pkg_dir* (the installed ``junction`` package directory) defaults to this
    ...
    if pkg_dir is None:
        pkg_dir = Path(__file__).resolve().parent.parent  # .../junction
    roots.append(pkg_dir / "_vendor" / "node_modules")

    # 2. Explicit project dir (JUNCTION_PROJECT_DIR points at the repo root):
...
def _resolve_spawn_env(env: dict[str, str], *, kiro_api_key: bool = False) -> dict[str, str]:
    ...
    from junction.config.loader import inject_kiro_cli_api_key, strip_kiro_cli_api_key
...
# ``session/prompt`` — so without special handling Junction waits the full 2h timeout.
...
def prompt_timeout_for_ceiling(configured: float) -> float:
    ...
    against the ceiling without a second synchronous ``JunctionConfig.load()``.
    """
    if configured <= 0:
...
def resolve_prompt_timeout() -> float:
    ...
    try:
        ...
        configured = float(JunctionConfig.load().agent.chat_turn_timeout_secs)
    except Exception:
...
async def _effective_prompt_timeout_async(timeout: float | None) -> float:
    ...
    → ``JunctionConfig.load()``: stat, read, validate), so resolving it inline
...
def _make_unified_diff(old: str, new: str, path: str, max_len: int = 65536) -> str:
    ...
    Thin delegate to :func:`junction.acp._dispatch.make_unified_diff`, kept as
...
def _select_tool_title(
    ...
    *,
    ...
    (e.g. "List Junction ACP module files" rather than `ls /workplace/...`).
...
class AcpClient:
    ...
    def _sync_effort_levels(self) -> None:
        ...
        if levels:
            ...
            from junction.dashboard.chat_persistence import update_reasoning_effort_values
    ...
    async def _spawn(self) -> None:
        ...
        Junction's public core only ever drives the kiro-cli backend. The
        ...
        if self._session_key:
            env["JUNCTION_SESSION_KEY"] = self._session_key
        else:
            env.pop("JUNCTION_SESSION_KEY", None)
        if self._channel_id:
            env["JUNCTION_CHANNEL_ID"] = self._channel_id
        else:
            env.pop("JUNCTION_CHANNEL_ID", None)
        ...
        env[JUNCTION_SPAWNED_ENV] = JUNCTION_SPAWNED_VALUE
        ...
        try:
            ...
            from junction.session import (  # circular: session -> config.loader -> providers.acp -> acp.client
                _track_child_pids,
                _track_pid,
                _track_session_pid,
            )
    ...
    async def _snapshot_process_tree(self) -> None:
        ...
        if self._child_pids:
            from junction.session import _track_child_pids
    ...
    def _reset_state(self) -> None:
        ...
        # These untrack helpers live in junction.session, which imports this
        # module transitively, so they must be imported inline.
        from junction.session import _untrack_child_pids, _untrack_pid, _untrack_session_pid
        ...
        if resume_sid and self._can_load_session:
            ...
            if file_ok:
                try:
                    ...
                    from junction.acp.runtimes import session_load_meta
    ...
    async def ensure_ready(self) -> None:
        ...
        # Telemetry (junction.session.startup.duration): time the cold-start work
        ...
        try:
            ...
            try:
                ...
                # -> config.loader (provider reads JunctionConfig). Keep it lazy so
    ...
    async def _wait_for_response(
        ...
        *,
        ...
        to ``session/new``.
        """
        from junction import shutdown_event
    ...
    async def _reject_unknown_server_request(self, msg: JsonRpcMessage) -> None:
        ...
        Junction implements only ``session/request_permission`` as an inbound
    ...
    def _emit_tool_interrupted_sel(self, site: str) -> None:
        ...
        that Junction observes but does not control.  Logged so the audit trail reflects
        that tools were blocked even though the decision was made outside Junction.
        ...
        try:
            ...
            # ``junction.sel.sel`` and lets a substituted emitter be observed.
            from junction.sel import sel
    ...
    def _extract_tool_event(self, msg: JsonRpcMessage) -> AcpEvent | None:
        ...
        if update.get("sessionUpdate") == UPDATE_TOOL_CALL:
            ...
            # some backends' Bash tool emits "List Junction ACP module files"
    ...
    def _extract_tool_call_refinement(self, msg: JsonRpcMessage) -> AcpEvent | None:
        ...
        # Bash's "List Junction ACP module files" rather than `ls /workplace/...`).
