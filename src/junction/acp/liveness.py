...
def is_wait_tool(title: str) -> bool:
    """True when a tool title names the junction-core ``wait`` tool.

    Titles vary by transport ("wait", "junction-core___wait", "wait (mcp)");
...
class LivenessOracle:
    ...
    One instance per :class:`~junction.acp.session_handle.AcpSessionHandle`.
    ...
    def _check_tool(self, runtime_pid: int | None, tool: ToolCallState) -> tuple[str, str]:
        ...
        # Declared-duration contract for the junction-core wait tool: the
