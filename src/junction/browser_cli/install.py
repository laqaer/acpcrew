...
Node is located through :func:`junction.env.find_node_tool` rather than bare
...
from junction.browser_cli import os_deps
from junction.config.paths import config_dir
from junction.constants import GITHUB_SLUG
from junction.env import augmented_path, find_node_tool, node_augmented_path
...
_STANDALONE_PREFIX_ENV = "JUNCTION_PLAYWRIGHT_CLI_HOME"
...
def _step(
    ...
    and the log is the longer-lived of the two surfaces: `junction logs` output
...
def _download_browser(path: str, engine: str | None = None) -> list[dict[str, Any]]:
    ...
    :mod:`junction.browser_cli.os_deps`), and even there a refusal is not fatal.
