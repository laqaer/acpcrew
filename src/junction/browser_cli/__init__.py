...
- :mod:`junction.browser_cli.install` — detection, installation, and the
  capability gate. Presence of ``playwright-cli`` is the gate; there is no
  toggle, flag, or consent file anywhere in this package.
- :mod:`junction.browser_cli.os_deps` — which Linux hosts the browser download's
...
- :mod:`junction.browser_cli.view` — supervises ``playwright-cli show``, the
  CLI's own dashboard, as a long-lived loopback-only child process.
- :mod:`junction.browser_cli.snapshots` — retention for the timestamped YAML
