...
from junction.messaging.driver import DirectiveConsumer, TurnDriver
...
class ChannelTurn:
    ...
    constructed channel-side via :func:`junction.messaging.link.build_dm_session_key`
    ...
    user_text: str
    renderer: Any
    """A :class:`junction.messaging.renderer.Renderer`."""
    ...
    """A :class:`~junction.messaging.link.ChannelLink` naming THIS conversation.
    ...
    :func:`junction.messaging.link.bind_origin_mirror` for why the bind must be
...
def build_tool_gate(ctx_builder: Any, *, session_key: str, agent: str) -> Callable[[Any], str]:
    ...
    no channel package needs to import ``junction.slack``.
    """
