...
(:mod:`junction.messaging.attachments`) sniffs a downloaded temp file, and
outbound extraction (:mod:`junction.messaging.outbound_files`) sniffs a local
...
Deliberately dependency-free: it imports nothing from ``junction``, so either
