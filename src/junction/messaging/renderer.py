...
from junction.constants import OPTIONS_RE_TRAILER
from junction.messaging.display_safety import redact_for_display
from junction.messaging.tables import render_tables, render_tables_with_metadata
...
def chunk_for_transport(text: str, capabilities: TransportCapabilities) -> list[str]:
    ...
    :func:`~junction.messaging.split.split_markdown_bytes`, the char path via
    :func:`~junction.messaging.split.split_markdown_safe`. A blind fixed-width
    ...
    if max_bytes > 0:
        from junction.messaging.split import split_markdown_bytes
