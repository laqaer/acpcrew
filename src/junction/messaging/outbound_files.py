...
limit downstream (:mod:`junction.messaging.split`). Extraction has to run
...
which offsets are fenced comes from :func:`junction.messaging.split.iter_fence_spans`
...
:mod:`junction.messaging.attachments` on the inbound side: silently dropping a
...
* a denylist check (:func:`junction.security.is_sensitive_path`), so
...
on its own. :mod:`junction.discord.renderer` is the first consumer: it seals a
...
from junction.widget_parse import mask_inline_code
...
class ExtractLimits:
    ...
    :mod:`junction.image_artifacts`, which happen to carry the same numbers:
...
async def extract_local_refs_off_loop(
    ...
    :func:`junction.image_artifacts.register_images_off_loop`: that pool is
