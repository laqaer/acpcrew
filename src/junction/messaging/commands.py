...
* HERE -- the decision plus the reply TEXT, the same shape
  :func:`~junction.messaging.link.release_conversation_location` already uses for
...
* NOT here -- anything address-shaped. Nothing below accepts a chat id, a thread
  ...
  already-bound :class:`~junction.messaging.queue_receipt.ReceiptSurface`, which
...
**Services are duck-typed on purpose.** ``junction.subagent`` and
``junction.taskrunner`` both reach ``junction.slack`` transitively, so importing
...
module additionally imports nothing from ``junction.dashboard`` -- see
...
async def run_yolo_command(
    ...
    *,
    ...
    and Slack's ``/junction yolo`` drive, so a grant taken in one channel shows up
...
def parse_dashboard_ttl(arg: str, *, parse_duration: DurationParser) -> int:
    ...
    *arg* is everything after the channel's own command tokens -- ``"2h"``,
    ...
    is ``/junction dashboard <ttl>`` (the TTL is the third word) and Teams' is
    ...
    *parse_duration* supplies the duration vocabulary and is injected rather than
    imported: it lives in ``dashboard/token_auth.py``, and ``junction.messaging``
    imports nothing from ``junction.dashboard`` -- that one-way direction is what
