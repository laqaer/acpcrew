...
``junction`` is often run from a git checkout (``pip install -e .``). A plain
...
``install_requires`` at install time) and ``junction update`` already re-runs
...
:func:`junction.dep_sync.sync_or_reinstall`'s decision: an editable
...
launched through the very ``junction.exe`` a reinstall would have to rewrite.
...
package's dependencies are known to exist. ``junction.dep_sync`` is the one
...
# exists to heal. `junction.__init__` itself imports nothing but asyncio.
...
def _import_cli() -> Callable[[], None]:
    ...
    from junction.cli import main as cli_main
...
def _source_checkout_root() -> Path | None:
    ...
    An editable install resolves ``junction`` inside ``<repo>/src/``; a wheel
    install resolves it inside ``site-packages``. Only the former has our
    ``setup.cfg`` two levels up.
    """
    root = Path(__file__).resolve().parents[2]
    if (root / "setup.cfg").is_file() and (root / "src" / "junction").is_dir():
        return root
    return None
...
def _self_heal(missing: str) -> bool:
    ...
    agent input -- and the argv is built by :mod:`junction.dep_sync`, which picks
    ...
    ``junction.exe`` this very process was launched through, and a reinstall that
