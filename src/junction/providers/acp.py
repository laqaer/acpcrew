...
from junction.acp.runtime import AcpRuntime, AcpRuntimeError
...
from junction.providers.cleanup import _is_safe_path
...
# to a fresh session + Junction history replay (see _start_kiro_runtime_impl).
...
class AcpProvider(LLMProvider):
    ...
    def __init__(
        ...
        # runtime so per-agent watchdog windows key off the canonical agent,
        # never off a cross-namespace name match.
        self._canonical_agent: str = canonical_agent or ""
        ...
        # cleared). Signals SessionManager.get_or_create to replay Junction's
        ...
        # Junction toggle is authoritative over any global kiro setting.
    ...
    async def _start_kiro_runtime(self) -> None:
        ...
        already emits ``junction.session.startup.duration`` for the *claude* path
    ...
    def _emit_kiro_startup_metric(
        ...
        try:
            ...
            if resume_outcome:
                rec.counter(
                    "junction.session.resume.outcome",
                    1,
                    attrs={
    ...
    async def _load_session_with_retry(
        ...
        * runtime dies mid-retry   → return ``None`` (caller's respawn handles it).
        ...
        # the caller migrates to a fresh session with Junction history replay.
        logger.warning(
    ...
    async def _start_kiro_runtime_impl(
        ...
        runtime = AcpRuntime(
            ...
            canonical_agent=self._canonical_agent,
        )
        ...
        try:
            ...
            if resume_sid:
                ...
                if should_load:
                    ...
                    # to a fresh session/new below WITH Junction history replay
