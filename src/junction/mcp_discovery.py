...
from junction.mcp_grant import grant_observed
from junction.mcp_provenance import ABSENT, resolve_write
from junction.mcp_utils import kiro_entry_client_id, kiro_entry_scopes, mcp_server_alias
...
def _get_probe_timeout() -> int:
    try:
        ...
        return JunctionConfig.load().dashboard.mcp_probe_timeout_secs
    except Exception:
...
# the dashboard badges (junction / kiroGlobal / ccGlobal) and are the
# source of truth for the ``presence`` field on each server.
SCOPE_JUNCTION = "junction"
...
def _mcp_sources() -> tuple[tuple[Path, str], ...]:
    ...
    The tuple is in merge/priority order, highest first: the junction-specific
    ...
    if _MCP_SOURCES is not None:
        return _MCP_SOURCES
    return (
        (data_home() / "mcp.json", SCOPE_JUNCTION),
...
def _extra_scope_sources() -> list[tuple[Path, str]]:
    ...
    Each returned :class:`~junction.platform.interfaces.McpScope` becomes a
...
# junction-specific file, then the Kiro global. ``ccGlobal`` (and every other
...
# (junction > kiro-global > seam provider globals) — discovery, apply, and
# rebuild all agree, so the dashboard never shows a spec the agent won't run.
_CORE_SCOPE_ORDER: tuple[str, ...] = (SCOPE_JUNCTION, SCOPE_KIRO_GLOBAL)
...
def _load_agent_config(*, user_home: Path | None = None) -> dict[str, Any]:
    ...
    AND the installed junction.json — because defaults.json may not have
    mcpServers (they're added dynamically at install time by ``junction setup``).
    """
    ...
    from junction.agent import AGENT_FILENAME  # circular import: agent imports mcp_discovery
...
def configured_mcp_aliases(*, data_home: Path, user_home: Path) -> set[str]:
    """Return canonical names reserved by every effective Junction MCP source."""
    names: set[str] = set()
...
def _load_mcp_json_by_source() -> dict[str, dict[str, Any]]:
    ...
    unknown path falls back to :data:`SCOPE_JUNCTION`.
    """
    ...
    for p in scan_paths:
        scope = path_to_scope.get(p, SCOPE_JUNCTION)
        if not p.is_file():
            continue
        try:
...
def _load_mcp_json() -> dict[str, Any]:
    ...
    # junction-specific file > kiro global > any seam provider globals.
...
# Managed server name -> the ``junction`` CLI subcommand that serves it.
_MANAGED_SERVER_SUBCOMMANDS = {
    "junction-core": "mcp-core",
    "junction-cron": "mcp-cron",
    "junction-computer": "mcp-computer",
    "junction-dashboard": "mcp-dashboard",
}
...
_MANAGED_SERVER_TOOL_MODULES = {
    "junction-core": "junction.mcp_core",
    "junction-cron": "junction.mcp_cron",
    "junction-computer": "junction.mcp_computer",
    "junction-dashboard": "junction.mcp_dashboard",
}


#: Managed servers that advertise ``junction.caller-identity`` AND are safe to
...
_MANAGED_SERVERS_CALLER_AWARE: frozenset[str] = frozenset(
    {"junction-core", "junction-cron", "junction-dashboard"}
)
...
#: not-session-bound classification but not sufficient. ``junction-computer``
...
#: collision. Contrast ``junction-dashboard``, which refuses an unidentified
...
def _fix_stale_managed_command(name: str, spec: dict) -> None:
    ...
    Delegates to :func:`junction.agent._junction_mcp_invocation`, the single
    source of truth for the managed invocation. That handles every layout:
    a standalone ``bin/junction`` (POSIX) / ``Scripts\\junction.exe`` (Windows
    pip install) console script when one resolves, the Windows bundle's
    ``bin\\junction.cmd`` shim (unwrapped to ``<root>\\python.exe -P -s -m
    junction <sub>``), and otherwise the ``<interpreter> -m junction <sub>``
    fallback. Both ``command`` AND ``args``
    are rewritten — the fallback needs ``["-m", "junction", <sub>]``, so
    re-resolving the command alone (the old behavior) silently dropped the args
    and spawned a bare ``junction`` that isn't on PATH (Windows: ``command not
    found: junction``; the built-in cron/core tools then never load).
    """
    ...
    if invocation is None:
        try:
            from junction.agent import _junction_mcp_invocation  # circular import

            invocation = _junction_mcp_invocation(subcommand)
        except Exception:
...
def _is_first_party_managed_argv(
    ...
    invocation (:func:`junction.agent._junction_mcp_invocation`, the single
    ...
    *env* must equal the package-derived managed env too
    (:func:`junction.agent._managed_mcp_env` — ``{}`` on a default install, the
    ``JUNCTION_HOME`` pin under an override home). ``probe_server`` merges the
    ...
    try:
        ...
        from junction.agent import _junction_mcp_invocation, _managed_mcp_env
    ...
    # Refuse the interpreter fallback (`<python> -m junction <sub>`): `python
    ...
    # gateway's cwd — a planted `junction/` tree there would shadow the
...
def list_servers() -> list[McpServerInfo]:
    ...
    #    don't overwrite it.  Order = junction-specific > Kiro global >
    ...
    for scope in _scope_priority(by_source):
        for name, spec in by_source.get(scope, {}).items():
            ...
            elif scope == SCOPE_JUNCTION and spec.get("disabled") and name not in servers:
                # Consent-disabled entries (registry installs and custom adds
                # land with ``disabled: true`` until the user enables them)
                # live ONLY in the Junction scope. They must still get a row:
    ...
    #
    #    MC presence = "will this load in Junction sessions after the next
    ...
    #    Junction has an explicit ``disabled: true`` override.
    #    Kiro/CC presence = raw membership in that provider's global config.
    agent_names = set(agent_cfg.get("mcpServers", {}).keys())
    junction_own = by_source.get(SCOPE_JUNCTION, {})
    # Every scope other than junction is a raw-membership global scope
    ...
    global_scopes = [s for s in _scope_priority(by_source) if s != SCOPE_JUNCTION]
    for name, server in servers.items():
        mc_disabled = (
            isinstance(junction_own.get(name), dict) and junction_own[name].get("disabled") is True
        )
        in_any_source = name in agent_names or any(
            name in by_source.get(scope, {}) for scope in by_source
        )
        presence: dict[str, bool] = {SCOPE_JUNCTION: in_any_source and not mc_disabled}
    ...
    try:
        init_body = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "junction-probe", "version": "1.0.0"},
            },
        }
        hdrs = {
            **server.headers,
...
async def probe_server(
    ...
    *client_info* overrides the ``clientInfo`` sent in the handshake. The
    ...
    try:
        ...
        # above). ``strip_python_env`` keeps Junction's PYTHONPATH/PYTHONHOME out
        ...
        # (``agent._junction_mcp_invocation`` + ``agent._managed_mcp_env`` via
        ...
        try:
            ...
            if not _declared_temp_upper:
                from junction.mcp_gateway.backend_tmp import allocate_probe_tmp, tmp_env
        ...
        try:
            ...
            if probe_tmp is not None:
                from junction.mcp_gateway.backend_tmp import sweep_backend_tmp
        ...
        if probe_tmp is not None:
            ...
            from junction.mcp_gateway.backend_tmp import record_owner
            ...
            json.dumps(
                {
                    ...
                    "params": {
                        ...
                        if client_info
                        else {"name": "junction-probe", "version": "1.0.0"},
                    },
                }
            )
            + "\n"
        ...
        #     does go stale ("command not found: junction; the built-in cron/core
        ...
        if probe_tmp is not None:
            if platform_compat.IS_POSIX:
                ...
                try:
                    from junction.mcp_gateway.backend_tmp import sweep_all_backend_tmp
...
def _envs_agree(agent_env: dict, source_env: dict) -> bool:
    ...
    compared through :func:`junction.env.spec_env_path` — the agent config
...
def sync_to_agent_config(servers: list[McpServerInfo]) -> bool:
    ...
    """
    from junction.agent import install_agent  # circular import

    install_agent()

    # Audit: log which servers triggered the config rebuild
    try:
        from junction.sel import sel  # circular import
...
def junction_managed_names() -> set[str]:
    ...
    :func:`junction.mcp_provenance.resolve_write`, which reads the marker on the
    ...
    return {
        name
        for name, spec in by_source.get(SCOPE_JUNCTION, {}).items()
        if isinstance(spec, dict)
    }
...
def register_servers_for_cc(
    ...
    # name we own -- see junction_managed_names.
