...
measurement lives in :mod:`junction.eval.bench`.
"""
...
def _bench_dispatch(args: argparse.Namespace) -> int:
    ...
    # scope, so every `junction` invocation of every subcommand loads it. Hoisting
    ...
    # `junction.vector_memory` plus `sqlite3` into the boot path of commands that
...
def _compare(args: argparse.Namespace) -> int:
    from junction.eval.bench import compare_reports
...
def _load_corpus(key: str):  # noqa: ANN202 - Corpus, but imported lazily
    ...
    from junction.eval.bench import datasets
    from junction.eval.bench.adapters import load_locomo, load_longmemeval
...
def _retrieval(args: argparse.Namespace) -> int:
    from junction.eval.bench import IngestConfig, RetrievalConfig, run_retrieval, write_report
    ...
    from junction.eval.bench import format_report
