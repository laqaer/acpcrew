...
Runs as ``junction mcp-computer``: kiro-cli spawns it as a child process and calls
...
``junction.caller-identity``, so gatewayd injects one whenever it can name the
caller — the only identity source that works on a pooled backend serving many
sessions), then the env var, then ``JUNCTION_HOST_PID`` plus the HMAC sidecar
...
``JUNCTION_SESSION_KEY`` is injected only by the ACP spawn path
...
ERR_GATEWAY_UNREACHABLE = (
    "the Junction gateway is not reachable, so computer use cannot run "
...
# macOS, the only platform with a driver (``JUNCTION_SESSION_KEY`` is injected only
# by the ACP spawn path and ``JUNCTION_HOST_PID`` only by the Linux sandbox
# launcher).
#
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    # only to an element Junction can inspect: with no index there
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    # GUI-launched kiro-cli (``JUNCTION_SESSION_KEY`` is injected only by the ACP
    # spawn path, and ``JUNCTION_HOST_PID`` only by the Linux sandbox launcher), so
