...
Runs as ``junction mcp-core`` — kiro-cli spawns it as a child process
...
from junction.agent_discovery import list_agents
...
from junction.dashboard.origin import dashboard_socket_path
from junction.history import _SEARCH_SCAN_WINDOW as SEARCH_SCAN_WINDOW
from junction.history import ConversationLog, is_incognito_transcript, snippet_needles
from junction.knowledge.dedup import dedup_sweep
from junction.knowledge.embedder import create_embedder_from_config
...
from junction.mcp_caller import current_caller
from junction.mcp_shared import (
    call_tool_with_logging,
    internal_caller,
    run_mcp_stdio_loop,
)
from junction.mcp_tools import build_tool_list, dispatch
from junction.members import record_activity
from junction.messaging.link import is_legacy_slack_key, legacy_key
from junction.platform import redact_via_context as redact
from junction.port_resolution import resolve_client_port_src
...
# ``patch("junction.mcp_core.sel")``, ``patch("junction.mcp_core.time")`` --
...
def _resolve_api_port() -> tuple[int, str]:
    ...
    Delegates to :func:`junction.port_resolution.resolve_client_port_src`, the
    same precedence every client CLI command applies: ``JUNCTION_PORT``, then a
...
def _list_tools() -> list[dict[str, Any]]:
    ...
    Declared per domain under :mod:`junction.mcp_tools`; this stays the
...
def _get_ppid(pid: int) -> int:
    ...
    - macOS: ``proc_pidinfo`` via libproc (see ``_ppid_via_libproc``). The old
      ...
      their parent session key (empty ``JUNCTION_SESSION_KEY``) and surfacing
...
def _resolve_session_key() -> str:
    ...
    topology): gatewayd strips client-forged ``junction.caller`` blocks and
    ...
    Warm-pool kiro-cli processes have no JUNCTION_SESSION_KEY env var (the pool
    ...
    sk = os.environ.get("JUNCTION_SESSION_KEY", "")
    if sk:
        return sk
    try:
        from junction.session_pid_sig import read_session_pid_txt
        ...
        host_pid = os.environ.get("JUNCTION_HOST_PID", "")
        if host_pid.isdigit():
...
def _resolve_session_key_strict() -> str:
    ...
    1. The gateway-injected ``JUNCTION_SESSION_KEY`` env var.
    2. The direct ``JUNCTION_HOST_PID`` -> ``session_pid_<pid>.txt``
       lookup, but ONLY when the HMAC sidecar written by the gateway
       verifies (:func:`junction.session_pid_sig.verify_session_pid`).
       PID-namespace sandboxing strips ``JUNCTION_SESSION_KEY`` from the
    ...
    client-forged ``junction.caller`` block on every inbound frame and
    ...
    try:
        host_pid = os.environ.get("JUNCTION_HOST_PID", "")
        if host_pid.isdigit():
            from junction.session_pid_sig import verify_session_pid
...
def strict_identity_diagnosis(server: str = "junction-core") -> str:
    ...
    if _resolve_session_key_strict():
        return ""
    if os.environ.get("JUNCTION_HOST_PID", "").isdigit():
        ...
        return (
            ...
            f"verify. Check `junction doctor` (trust root) — {server} does not need "
    ...
    return (
        ...
        f"a verifiable identity. `junction doctor` reports the same check."
    )
...
def _deny_channel_agent_messaging(caller_session: str, tool_name: str) -> str | None:
    ...
    an AUTO-APPROVED call (junction-core is in the default ``allowedTools``)
    ...
    if not caller_session.startswith("channel:"):
        return None
    try:
        # Resolved from ``junction.sel`` at call time, not through the
...
def _vet_messaging_governance(
    ...
    ``junction-core`` stdio subprocess, which DOES boot the platform via
    ...
    try:
        ...
        # No logger here: this runs inside the junction-core stdio MCP server,
        ...
        try:
            from junction.platform.governance_profiles import audit_governance_degraded
...
def _vet_browse_governance(caller_session: str) -> str | None:
    ...
    on a default install). Runs inside the ``junction-core`` stdio subprocess,
...
def _governance_app() -> str:
    ...
    An app backend process carries ``JUNCTION_APP_NAME`` (set in
    ...
    (``bind:{type:"app"}``) resolve.  NOTE: the managed ``junction-core`` MCP
    ...
    """
    return os.environ.get("JUNCTION_APP_NAME", "")
...
def _http_error_body(exc: urllib.error.HTTPError) -> dict:
    ...
    An HTTP response body is content originating outside Junction, so the
...
def _current_session_thread_ts() -> str | None:
    ...
    ``JUNCTION_HOST_PID`` lookup. It deliberately drops the ``/proc`` ancestor
...
def _history_is_incognito(meta: dict) -> bool:
    ...
    Dict-shaped convenience over :func:`junction.history.is_incognito_transcript`,
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    Handlers live beside their descriptors in :mod:`junction.mcp_tools`; this
...
#: Whether this server advertises ``junction.caller-identity`` — i.e. whether it
...
def run_mcp_core_server() -> None:
    ...
    run_mcp_stdio_loop(
        ...
        _list_tools,
        _call_tool,
        # Pooled-operation opt-in: junction-core consumes the per-call
        # ``junction.caller`` identity (see _resolve_session_key*), so it is
        # safe to share one backend across sessions. junction-cron advertises
        # too, for the same reason. The two managed servers that do not --
        # junction-computer and junction-dashboard -- are pooled all the same
