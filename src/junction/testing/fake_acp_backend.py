...
newline-delimited stdio) that ``junction.acp.client.AcpClient`` drives:
...
the ``JUNCTION_KIRO_BIN`` override (the provider stays ``acp``); it is never
selectable from a real gateway. Run standalone as
``python -m junction.testing.fake_acp_backend`` (the pytest harness and the
live-test harness both point ``JUNCTION_KIRO_BIN`` at a launcher that runs it).
