"""Test helpers for consumers that need a seeded ``$JUNCTION_HOME``.
...
1. ``seeded_home(fixture_name)`` — plain context manager. Use from any
   ...
   the previous ``JUNCTION_HOME`` env var on exit.
2. ``seeded_home_fixture`` — pytest fixture wrapping (1). Parameterised
   ...
   ``junction.testing.fixtures`` to their ``conftest.py`` via
...
# ``pytest`` is an OPTIONAL runtime dependency. ``junction.testing`` ships in
# the runtime wheel (not ``dev_requirements``) so downstream consumers can do
# ``pip install junction`` and use ``seeded_home`` from any test harness
...
from junction.seed import seed
...
def seeded_home(fixture_name: str) -> Iterator[Path]:
    """Context manager yielding a populated ``$JUNCTION_HOME`` tempdir.

    On enter: creates a fresh temp directory, points ``JUNCTION_HOME`` at
    it, and runs ``seed(fixture_name)`` to populate the tree. On exit:
    restores the previous ``JUNCTION_HOME`` (or unsets if it wasn't set)
    ...
    Example:
        with seeded_home("minimal") as home:
            ...
            # agent code under test reads $JUNCTION_HOME here
    """
    previous = os.environ.get("JUNCTION_HOME")
    tmpdir = Path(tempfile.mkdtemp(prefix="junction-seeded-home-"))
    try:
        ...
        os.environ["JUNCTION_HOME"] = str(tmpdir)
        seed(fixture_name)
        yield tmpdir
    finally:
        ...
        if previous is None:
            ...
            os.environ["JUNCTION_HOME"] = previous
        if tmpdir.exists():
...
if pytest is not None:
    ...
    def seeded_home_fixture(request: "pytest.FixtureRequest") -> Iterator[Path]:
        ...
        Defined only when pytest is importable — ``junction.testing`` is in
