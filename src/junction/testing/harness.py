"""Test harness for spawning isolated Junction gateways.

Companion to the ``--test-mode`` / ``--json-ready`` / ``--port`` / ``--approval``
CLI flags on ``junction gateway``. Provides a context manager that spins up
an isolated, headless gateway from the current workspace's source tree (not
the system-installed ``junction``), reads the ``JUNCTION_READY:{...}`` line
...
Usage:
    ...
    from junction.testing.harness import spawn_feature_gateway

    with spawn_feature_gateway() as handle:
        # drive Playwright / urllib / etc against handle.url
        ...
    # subprocess and tmp JUNCTION_HOME are gone by here
...
# Override via ``JUNCTION_HARNESS_READY_TIMEOUT``.
...
class GatewayHandle:
    ...
    Attributes:
        ...
        home: Path to the throwaway ``JUNCTION_HOME`` directory the gateway
...
def _resolve_workspace_src() -> Path:
    ...
    (``<pkg>/src/junction/testing/harness.py``) to find the package's
    ...
    # <pkg>/src/junction/testing/harness.py -> <pkg>/src
    src = here.parent.parent.parent
    if not (src / "junction" / "__init__.py").exists():
        raise GatewaySpawnError(
            f"Could not locate junction package at {src}. "
            f"Harness expects harness.py at <pkg>/src/junction/testing/, "
            f"with the source tree at <pkg>/src/junction/."
        )
    return src


def parse_ready_line(line: str) -> dict[str, Any]:
    """Parse and validate a single ``JUNCTION_READY:{...}`` line.
...
def _wait_for_ready_line(
    ...
    *,
    ...
    """Read stdout until we see ``JUNCTION_READY:{...}`` or hit the timeout.
        ...
        while True:
            ...
            if remaining <= 0:
                ...
                raise GatewaySpawnError(
                    ...
                    f"{timeout:.1f}s. Override with JUNCTION_HARNESS_READY_TIMEOUT.\n"
                    f"--- stderr (last) ---\n{stderr_text[-4000:]}"
                )
    ...
    *,
    ...
    Args:
        fixture: Named fixture (``empty`` / ``minimal`` / ``rich``) passed
            through to ``junction gateway --seed`` so the tmp JUNCTION_HOME
        ...
        approval: Approval mode to pass through ``--approval``. ``"reads"``
            ...
            ``JUNCTION_HOME`` (the harness always provides one); pass
        ...
        timeout: Override the ready-line timeout in seconds. Falls back to
            ``JUNCTION_HARNESS_READY_TIMEOUT`` env var, then to
            ``DEFAULT_READY_TIMEOUT``.

    Yields:
        ``GatewayHandle`` once the gateway has bound its dashboard port
        and emitted the ``JUNCTION_READY:{...}`` line. The handle is only
    ...
    home = Path(tempfile.mkdtemp(prefix="junction-harness-"))
    ...
    try:
        if timeout is None:
            env_timeout = os.environ.get("JUNCTION_HARNESS_READY_TIMEOUT")
        ...
        env = {
            **os.environ,
            ...
            # ``junction gateway`` boot runs ``rebuild_agent_config``, which writes the
            ...
            "JUNCTION_SKIP_MODEL_DOWNLOAD": "1",
        }
        ...
        cmd = [
            ...
            "gateway",
            "--test-mode",
            # ``--seed`` populates the (empty) tmp JUNCTION_HOME from the named
