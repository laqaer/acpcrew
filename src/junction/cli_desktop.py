...
running with ``JUNCTION_DEBUG`` set, there is no artifact at all.

Debug-only, and gated by the same ``JUNCTION_DEBUG`` check as ``junction perf``.
"""
...
from junction.perf_sampler import gate_refusal_message, profiling_enabled
...
def artifact_missing_message() -> str:
    ...
    return (
        ...
        f"JUNCTION_DEBUG set, so the usual cause is that the running app was "
        "launched without it -- restart the app with JUNCTION_DEBUG=1 and let it "
