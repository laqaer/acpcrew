...
the ``TMPDIR``/``TMP``/``TEMP`` triple plus ``JUNCTION_SCRATCH`` pointing at
...
Sweep hygiene follows the house rules of :mod:`junction.agents_janitor`:
...
def scratch_env(path: Path) -> dict[str, str]:
    ...
    both platforms; ``JUNCTION_SCRATCH`` is the prompt-visible name for
    deliberate work products (clones, logs, screenshots).
    """
    value = str(path)
    return {"TMPDIR": value, "TMP": value, "TEMP": value, "JUNCTION_SCRATCH": value}
