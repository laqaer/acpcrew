# Junction Eval Harness

Multi-session evaluation harness for benchmarking Junction's cross-session memory, lesson application, and context accumulation.
...
junction eval memory_recall_basic
junction eval memory_recall_basic lesson_application
...
Scenarios are JSON files in `src/junction/eval/scenarios/`. Each defines sessions with turns and assertions (YAML is also supported for backward compatibility):
...
```
junction eval            CLI entry point — scenario selection, output
...
Key design: each session creates a **new provider instance**, simulating a user closing and reopening Junction. Cross-session context must come from persisted memory, not conversation history.
