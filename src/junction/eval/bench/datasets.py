...
def cache_dir() -> Path:
    ...
    Kept off ``JUNCTION_HOME`` deliberately: these are hundreds of megabytes of
    reproducible third-party data, and sweeping them into the directory that
    ``junction snapshot`` backs up would bloat every snapshot with bytes that a
    URL already describes.
    """
    override = os.environ.get("JUNCTION_BENCH_CACHE")
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "junction" / "bench-data"
