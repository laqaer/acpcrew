...
``--out-dir``'s ``mkdir``, and the corpus cache root, which ``JUNCTION_BENCH_CACHE``
...
from junction import pinned_fs
...
def guard_output_dir(path: str | Path, *, what: str) -> Path:
    ...
    """
    from junction.security import is_sensitive_path, path_contains_sensitive
...
def _supports_pinned_walk() -> bool:
    ...
    Delegates to :func:`junction.pinned_fs.supports_pinned_walk`. Kept as a
...
def _open_in_pinned_parent(
    ...
    Thin wrapper over :func:`junction.pinned_fs.open_in_pinned_parent` that keeps
...
def _pin_parent(resolved_parent: str, *, what: str) -> int:
    ...
    Thin wrapper over :func:`junction.pinned_fs.pin_parent`, which owns the
...
def _refuse_hardlink_alias(fd: int, *, what: str, name: str) -> None:
    ...
    Thin wrapper over :func:`junction.pinned_fs.refuse_hardlink_alias`, which owns
...
def _is_reparse_point(path: Path) -> bool:
    ...
    Thin wrapper over :func:`junction.pinned_fs.is_reparse_point`, which explains
