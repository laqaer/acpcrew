"""Config-driven hook system for Junction's message pipeline.
...
from junction.validation import _bounded_pattern_search
...
class HookManager:
    ...
    def on_tool_call(
        ...
        *,
        ...
        # Junction-side read-only auto-approve — the LAST branch before allow(),
        ...
        if is_shell:
            ...
            from junction.dashboard.state import is_read_only_bash
            ...
            # leading read-ish verb and rejects EVERY `mcp__junction-computer__*` title
...
def _governance_denial(
    ...
    *tool_name* because the title grammar cannot encode every identity; both are
    ...
    try:
        from junction.platform.governance import gate_decision
        from junction.platform.governance_profiles import resolve_active_scope
...
def _app_owns_mcp_server(mcp_server_name: str, app: str) -> bool:
    ...
    ``<app>:``-prefixed (host/managed servers such as ``junction-cron`` never
    match).
    """
...
def _builtin_app_for_agent(resolved_agent: str) -> str:
    ...
    ``junction`` — so an alias NAMED after a builtin's agent would otherwise lend
...
_INTERNAL_READ_ALLOWLIST: dict[str, str] = {
    ...
    # trusted Junction with the session by running ``kiro-cli login`` outside
...
def register_internal_read_path(read_id: str, rel_path: str) -> None:
    ...
    * the resolved ``~/<rel_path>`` must already be classified sensitive by
      :func:`junction.security.is_sensitive_path` — the carve-out is only valid
...
_AUDIT_ONLY_READ_IDS: dict[str, str] = {
    ...
    # status module (``junction.connections.status``) STATS the identical
...
async def run_script_hook(
    ...
    try:
        ...
        env = {
            **os.environ,
            "JUNCTION_HOOK_EVENT": hook.event,
            "JUNCTION_HOOK_CONTEXT": env_context,
        }
...
class ScriptHookStore:
    ...
    async def fire(
        ...
        only the ``JUNCTION_HOOK_CONTEXT`` env var is length-capped downstream in
