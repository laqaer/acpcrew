...
- Rendering a Junction artifact to ``(bytes-on-disk, content_type)`` — including
  wrapping ``widget`` artifacts in a standalone HTML document.
- The version-bump → push chokepoint and the 1:1 Junction↔destination version
  invariant. Reverts mirror as a new version (never a pointer-rollback).
- Best-effort sync: a ``push_version`` failure NEVER fails the Junction update;
...
from junction.artifacts import Artifact, ArtifactPublication, ForkMetadata, get_default_store
...
# dashboard theme). Mirrors JunctionWebsite/src/lib/widgetSrcdoc.ts.
...
def unwrap_widget_html(html: str) -> str | None:
    ...
    Used on pull: a sentinel-bearing document was a Junction widget, so we strip
...
def _publication_summary(pub: ArtifactPublication) -> dict[str, object]:
    ...
    return {
        ...
        "last_synced_junction_version": pub.last_synced_junction_version,
...
async def push_version(art: Artifact, *, force: bool = False) -> None:
    ...
    exactly once per Junction version (idempotent + the 1:1 invariant); a force
    ...
    # Push exactly once per Junction version. If this version was already synced
    ...
    if not force and fresh.version == pub.last_synced_junction_version and not wrapper_stale:
        return
    ...
    await asyncio.to_thread(
        lambda: store.update_publication(
            ...
            last_synced_junction_version=fresh.version,
    ...
    logger.info(
        "artifact version synced: slug=%s provider=%s junction_v=%s dest_v=%s",
        art.slug,
        pub.provider,
        fresh.version,
        res.version_number,
...
_DRIFT_PREFIXES = (_DRIFT_PREFIX, "The remote copy changed outside Junction")
...
async def refresh_publication(slug: str) -> Artifact:
    ...
    # mirror it into Junction's append-only history; instead surface a re-sync
    # prompt (the conflict banner's "Force re-sync" re-asserts Junction's
    # current content as a new top version). Junction stays authoritative.
    expected_dest_v = pub.version_map.get(str(pub.last_synced_junction_version))
...
def _kind_for_content_type(content_type: str) -> str:
    """Map an upstream content-type back to a Junction artifact kind.
...
def _source_target(art: Artifact, source: str):
    ...
    if source == "publication" and art.publication is not None:
        pub = art.publication
        expected = pub.version_map.get(str(pub.last_synced_junction_version))
...
async def upstream_status(slug: str) -> dict[str, object]:
    ...
    if is_pub and art.publication is not None and art.publication.collab_mode == "live":
        ...
        local_ahead_live = (
            art.version > pub.last_synced_junction_version
    ...
    local_ahead = (
        ...
        and (
            art.version > art.publication.last_synced_junction_version
            # Wrapper envelope changed since last push — widget needs re-render (#3373).
...
async def pull_upstream(
    ...
    if is_pub and updated.publication is not None:
        ...
        await asyncio.to_thread(
            lambda: store.update_publication(
                slug,
                last_synced_junction_version=updated.version,
