"""Sidecar store for Junction's per-agent bookkeeping.
...
"no agent with name X found" line). Junction therefore keeps its private
...
State file (``~/.kiro/crew/agent_model_state.json``, honoring ``JUNCTION_HOME``)::

    {
      ...
      "junction-heartbeat": {"cc_model": "auto"}
    }
...
def _state_path() -> Path:
    """Return the sidecar path (resolved fresh so JUNCTION_HOME is honored)."""
