...
class TrustedAcpExecutableSnapshot:
    ...
    Junction never copies the CLI and runs the copy, because a multi-call Kiro
...
def _acp_executable_is_runnable(
    ...
    *,
    ...
    (Junction is not the authority on where Kiro CLI is installed).
...
def snapshot_trusted_acp_executable(
    ...
    *,
    ...
    Junction always executes the Kiro CLI the user installed, at the path it was
    ...
    # ``JUNCTION_KIRO_BIN=./kiro-cli`` resolves against the GATEWAY's cwd, but ACP
...
def register_process_start_override_attestation(
    *,
    ...
    override = active_environ.get("JUNCTION_KIRO_BIN", "")
...
def _default_spec_lister() -> list[tuple[str, Path]]:
    ...
    """
    from junction.agent import present_required_agent_specs  # circular import
...
class KiroPrerequisiteService:
    ...
    def __init__(
        ...
        *,
        ...
        if data_home is not None:
            ...
            configured_home = self._environ.get("JUNCTION_HOME", "")
            self._data_home = (
    ...
    async def _run_auth_command(
        ...
        *,
        ...
        do not gate sign-in); Junction is not the authority on where Kiro CLI is
