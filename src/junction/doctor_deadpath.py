...
def kiro_agents_dir_path() -> Path:
    ...
    Resolved live via :func:`junction.config.paths.kiro_agents_dir`, which
...
class DeadPath:
    ...
    ``env[JUNCTION_HOME]``) so a report points at exactly what to fix.
    """

    spec: str  # spec filename (e.g. "junction.json")
...
def _sanitize_for_terminal(value: str) -> str:
    ...
    ``unreadable`` reason — are printed to the operator's terminal by ``junction
...
def _default_repair() -> None:
    ...
    (:func:`junction.agent.rebuild_agent_config`), which re-resolves every
...
def check_dead_paths(*, agents_dir: Path | None = None, repair=_default_repair) -> DeadPathReport:
    ...
    Args:
        agents_dir: The agents directory to scan. Defaults to
            ...
            themselves (notably ``junction doctor``, which has its OWN override
...
def doctor_dead_paths(issues: list[str], *, agents_dir: Path | None = None) -> None:
    """Render the ``Agent Spec Paths`` section of ``junction doctor``.
    ...
    appended to *issues* so ``junction doctor`` exits nonzero and the failure
