...
from junction.agent import kiro_agents_dir_path
from junction.agent_discovery import spec_model
...
from junction.constants import COMPACT_WAIT_TIMEOUT_SECS
from junction.executors import maintenance_executor, subprocess_executor
from junction.mcp_gateway.abort import schedule_abort
...
from junction.metrics.events import SESSION_IDLE_EXPIRED, emit_counter
from junction.metrics.provider import get_recorder
from junction.providers.base import CancelOutcome, LLMProvider
from junction.pycache_gc import PYCACHE_GC_INTERVAL_SECS, prune_pycache
from junction.sandbox import cleanup_stale_sandbox_profiles
from junction.sel import sel
from junction.session_map import _kiro_sessions_dir  # noqa: F401
from junction.session_map import MIRROR_OPT_OUT_FLAG
from junction.session_map import SessionMap as SessionMap  # noqa: F401
from junction.session_map import UnbindListener, set_unbind_listener
...
from junction.session_pid import _track_child_pids as _track_child_pids  # noqa: F401
from junction.session_pid import _track_pid as _track_pid  # noqa: F401
from junction.session_pid import _track_session_pid as _track_session_pid  # noqa: F401
from junction.session_pid import _untrack_child_pids as _untrack_child_pids  # noqa: F401
from junction.session_pid import _untrack_pid as _untrack_pid  # noqa: F401
from junction.session_pid import _untrack_session_pid as _untrack_session_pid  # noqa: F401
from junction.session_pid import (
    cleanup_orphaned_session_roots,
)
from junction.session_pid import (  # noqa: F401
...
def _is_claude_backend(provider: Any) -> bool:
    ...
    """
    from junction.providers.acp import AcpProvider  # circular import: providers -> session
...
def _provider_label(provider: Any) -> str:
    ...
    """
    from junction.providers.acp import provider_label  # circular: providers -> session
...
def detect_provider_switch(session_map: "SessionMap", session_key: str, new_provider: str) -> bool:
    ...
    is achieved via Junction's own history replay (build_session_replay), never
...
#: Every value the ``junction.session.pool.decision`` counter can report. A
...
BACKGROUND_AGENT = "junction-lite"

# Heartbeat session key — used by HeartbeatService.  Spawned with the full
# ``junction`` agent so polled tasks can call read-only MCP tools (CR/ticket
...
def _session_model(cfg: "JunctionConfig", agent: str | None) -> "str | None":
...
class _Session:
    ...
    # from Junction's conversation_log. Ensures replay fires exactly once
...
class SessionManager:
    ...
    async def get_bg_session(self) -> "AcpSessionHandle | _ProviderBgSession":
        ...
        from junction.acp.runtime import AcpRuntime, AcpRuntimeDead
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                return await self._bg_runtime.create_session(agent="junction-lite")
    ...
    async def get_subagent_runtime(
        ...
        while True:
            ...
            async with lock:
                ...
                agent = agent or self._get_session_agent(parent_session_key) or "junction"
    ...
    async def _get_or_bootstrap_run_runtime(
        ...
        factory bakes in from ``JunctionConfig``. Instead, cold-start ONE
    ...
    def _companion_runtime_pids(self) -> set[int]:
        ...
        - ``self._bg_runtime`` — the background runtime backing ``get_bg_session``
          (junction-lite title-gen / memory consolidation).
    ...
    def context_info(self) -> list[dict[str, object]]:
        ...
        for key, sess in self._sessions.items():
            ...
            if ClaudeCodeProvider is not None and isinstance(sess.provider, ClaudeCodeProvider):
                ...
                if model == "auto" and agent and agent != "junction":
    ...
    async def get_or_create(
        ...
        **extra_factory_kwargs: Any,
        ...
        if pooled is not None:
            ...
            # Junction's conversation_log will inject history via
            ...
            async with self._lock:
                ...
                if _existing is not None and not _is_recycling:
                    ...
                    if _provider_switched or _replay_needed:
                        ...
                        # replay Junction's conversation_log into the new session
