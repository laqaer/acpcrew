...
The parent Junction process is completely unaffected — isolation applies
...
# ``_SENSITIVE_ENV_PREFIXES`` because Junction's OWN sandboxed Python
# subprocesses (cron scripts, app backends, code-review workers) import
# ``junction`` via PYTHONPATH — and on the packaged app must keep writing
...
#:
#: Kept deliberately free of ``junction`` imports and run under ``-I -S``, like
...
def _probe_sandbox_exec() -> bool:
    ...
    # binary: the probe runs under (allow default) with Junction's credentials,
    ...
    if not os.path.exists(target):
        return False
    fd, profile_path = tempfile.mkstemp(suffix=".sb", prefix="junction_probe_")
    try:
...
def _resolve_agent_executable(executable: str) -> str:
    ...
    launcher with the direct executable it ultimately invokes so Junction can
...
def _build_launcher_script(
    ...
    *,
    ...
    if strip_python_env:
        ...
        # Junction's PYTHONPATH/PYTHONHOME leak in and shadow their own deps.
...
"""Namespace sandbox launcher — spawned by Junction."""
import sys
# Harden against stdlib shadowing. This launcher runs as
# ``python <config_dir>/run/junction_sandbox_*.py``, so CPython prepends the
...
def main():
    ...
    os.environ["JUNCTION_HOST_PID"] = str(os.getpid())
    ...
    if pid > 0:
        ...
        for _candidate in (f"/run/user/{{REAL_UID}}", "/dev/shm"):
            try:
                ...
                _probe = tempfile.mkdtemp(dir=_candidate, prefix="junction_sb_")
                os.rmdir(_probe)
        ...
        for src_path, filename in EXPOSE_FILES:
            if src_path in expose_data:
                ...
                # stdlib — sys/ctypes/os/tempfile — and never junction), so it
        ...
        os.environ["JUNCTION_SANDBOX_ACTIVE"] = "1"
        ...
        os.environ["JUNCTION_SANDBOX_LEVEL"] = SANDBOX_LEVEL
...
def namespace_argv(
    ...
    *,
    ...
    fd, path = tempfile.mkstemp(
        suffix=".py", prefix=f"junction_sandbox_{os.getpid()}_", dir=run_dir
    )
...
# cannot nest inside Junction's sandbox-exec wrap — the kernel returns EPERM
...
def kiro_internal_sandbox_enabled() -> bool:
    ...
    False, which keeps Junction's own sandbox engaged — failure resolves
    ...
    try:
        ...
        if not isinstance(data, dict):
            ...
            # resolve toward Junction's own sandbox, not raise.
            return False
        ...
        # Every failure resolves toward Junction's own sandbox.
        return False
...
def _spawns_kiro_cli(argv: list[str]) -> bool:
    ...
    cron script) has no internal sandbox of its own and MUST keep Junction's
...
def sandbox_exec_argv(
    ...
    *,
    ...
    run_dir = _ensure_run_dir()
    fd, path = tempfile.mkstemp(
        suffix=".sb", prefix=f"junction_sandbox_{os.getpid()}_", dir=run_dir
    )
    ...
    # its own env scrub (see the export beside ``JUNCTION_HOST_PID``). Without
    # this, an in-sandbox ``wrap_argv`` call cannot tell that Junction's own
...
def _sandbox_env_unset_args(sandbox_level: str, strip_python_env: bool) -> list[str]:
    ...
    env-scrub guarantee is identical whether or not Junction's own seatbelt is
...
def cleanup_stale_sandbox_profiles(*, legacy_dir: str | None = None) -> int:
    ...
    Also sweeps legacy /tmp/junction_sandbox_*.py files that predate the
    ...
    if os.path.isdir(run_dir):
        for entry in os.listdir(run_dir):
            if not entry.startswith("junction_sandbox_"):
                continue
            ...
            middle = entry[len("junction_sandbox_") : -len(suffix)]
    ...
    # ── Sweep legacy /tmp/junction_sandbox_*.py (age only, no PID segment) ──
    if os.path.isdir(legacy_dir):
        try:
            with os.scandir(legacy_dir) as it:
                for dentry in it:
                    if not dentry.name.startswith("junction_sandbox_"):
                        continue
...
def _cleanup_retired_acp_snapshot_dir() -> int:
    ...
    Junction used to copy the whole kiro-cli binary here per ACP spawn generation
    ...
    the sweep above only matches `junction_sandbox_*` FILES in this same dir, and
...
def _allow_no_isolation() -> bool:
    ...
    try:
        from junction.config.loader import (
            JunctionConfig,  # circular import: sandbox is a low-level dep of config.loader
        )

        return bool(getattr(JunctionConfig.load().agent, "sandbox_allow_no_isolation", False))
    except Exception:
        return False
...
def _allow_unsandboxed_exec() -> bool:
    ...
    try:
        ...
        return bool(getattr(JunctionConfig.load().agent, "sandbox_allow_unsandboxed_exec", False))
    except Exception:
        return False
...
def configured_sandbox_mode() -> str:
    ...
    try:
        ...
        return str(getattr(JunctionConfig.load().agent, "sandbox", _SANDBOX_MODE_FALLBACK))
    except Exception:
        logger.warning(
...
# Junction namespace sandbox. Deny-by-default: the gate keys ONLY on the
# explicit, single-purpose ``JUNCTION_SANDBOX_ACTIVE``, which is exported at
# exactly one site — the namespace launcher main() (see the export beside
# ``JUNCTION_HOST_PID``). We deliberately do NOT key on ``JUNCTION_HOST_PID``:
...
# launcher sets ``JUNCTION_SANDBOX_ACTIVE`` at the same site, no fallback marker
...
# environment Junction already sanitised, which is what makes the passthrough
# below safe for callers that use ``wrap_argv`` directly rather than
# ``sandboxed_spawn_argv``.
_IN_SANDBOX_MARKER = "JUNCTION_SANDBOX_ACTIVE"
...
# as ``unknown``.
_IN_SANDBOX_LEVEL_VAR = "JUNCTION_SANDBOX_LEVEL"
...
def _bundled_cli_invocation() -> str | None:
    ...
    ``junction`` on their PATH at all — the CLI is bundled INSIDE the AppImage.
    ...
    ``shutil.which("junction")`` is deliberately NOT trusted as evidence here:
    ...
    if not name.startswith("junction") or not os.path.isfile(resolved):
        return None
...
def _apparmor_userns_restricted() -> bool:
    ...
    :mod:`junction.service.apparmor`: ``sandbox`` is a low-level dependency of
...
def _no_backend_guidance() -> str:
    ...
    * anything else on such a host — the profile must be applied by systemd
      (``junction service install``), because the only executable in a foreground
    ...
    if sys.platform.startswith("linux") and _apparmor_userns_restricted():
        ...
        if appimage:
            # Name the CLI by ABSOLUTE PATH, not as `junction`. An AppImage user
            # has no junction on PATH (see _bundled_cli_invocation), so the bare
...
def _inside_junction_sandbox() -> bool:
    """True when this process already runs inside a Junction OS sandbox.
    ...
    ``JUNCTION_SANDBOX_ACTIVE`` marker (see ``_IN_SANDBOX_MARKER``).
    """
...
def _macos_sandbox_state() -> bool | None:
    ...
    ``JUNCTION_SANDBOX_ACTIVE`` marker proves the marker was forged or inherited
...
def _inside_macos_sandbox() -> bool:
    ...
    Junction did NOT create — notably kiro-cli >= 2.13's own internal seatbelt
    ...
    sandbox is active, not that Junction built it or scrubbed the environment.
...
def _command_log_label(argv: list[str]) -> str:
    ...
    if name in {"node", "npm", "npx"}:
        return "node"
    if name in {"kiro", "kiro-cli", "junction"}:
        return "kiro"
...
def _first_party_no_backend_passthrough(
    ...
    try:
        from junction.sel import sel  # circular import: sandbox is low-level
...
class SandboxUnavailableError(RuntimeError):
    ...
    ``"foreign_sandbox"`` (an outer Seatbelt sandbox Junction did not create
...
def _governance_sandbox_floor() -> str | None:
    ...
    try:
        from junction.platform.governance_profiles import governance_floor_ordinal
...
def _floor_mandates_sandbox(floor: str | None) -> bool:
    ...
    if not floor:
        return False
    from junction.platform.governance import _ORDINAL_SCALES
...
def _clamp_sandbox_mode_to_floor(mode: str, floor: str | None) -> str:
    ...
    """
    from junction.platform.governance import _ORDINAL_SCALES, OrdinalControl
...
def wrap_argv(
    ...
    *,
    ...
    # Already inside a Junction sandbox (script cron, sandboxed agent child, app
    ...
    # other's blind spot: the marker proves Junction built the outer sandbox and
    ...
    if _inside_junction_sandbox() and _macos_sandbox_state() is not False:
        if not getattr(wrap_argv, "_nested_passthrough_logged", False):
            ...
            logger.info(
                "wrap_argv: already inside a Junction sandbox — nested OS "
        ...
        # to Junction's own seatbelt, an equally-safe audited layer — a nested
    ...
    # initialize nested inside Junction's seatbelt (kernel EPERM even under an
    ...
    if delegate_to_kiro:
        if extra_hidden_dirs or extra_visible_dirs:
            # A delegated sandbox cannot enforce Junction-specific path hides.
    ...
    if backend == "none":
        ...
        # outer sandbox is NOT one Junction built: a Junction-built one carries
        # JUNCTION_SANDBOX_ACTIVE and was already passed through above. The
        ...
        if floor_mandates_sandbox or not opted_in:
            ...
            if transient:
                ...
                # config-level fix that hands isolation back to Junction's own
                ...
                guidance = (
                    ...
                    f"{GITHUB_SLUG}/main/docker/seccomp/junction-seccomp.json"
                    " -o junction-seccomp.json\n"
                    "        docker run --security-opt seccomp=junction-seccomp.json ...\n"
...
def sandboxed_spawn_argv(
    ...
    *,
    ...
    agent-influenced spawn in ``src/junction`` to route through.
    ...
    Args:
        ...
        strip_python_env: Strip ``PYTHONPATH``/``PYTHONHOME`` so a foreign
            Python child does not inherit Junction's interpreter paths. Applied
    ...
    # as Junction-spawned even when its cmdline carries no Junction fingerprint
    # (e.g. ``npx @playwright/mcp``).
    scrubbed[JUNCTION_SPAWNED_ENV] = JUNCTION_SPAWNED_VALUE
...
# junction.slice/junction-agents.slice inside the user manager). It is also
# the aggregate enforcement boundary — see ensure_agents_slice_limits().
_CGROUP_AGENTS_SLICE = "junction-agents.slice"
...
# ── aggregate agent-slice soft ceiling (memory.high on junction-agents.slice) ──
...
def _slice_memory_events_high() -> int | None:
    ...
    understands systemd's dash-hierarchy (``junction-agents.slice`` nests
    under ``junction.slice`` in the user manager's subtree). ``None`` when it
...
def _agents_slice_cgroup_dir() -> Path | None:
    ...
    systemd's dash-hierarchy places ``junction-agents.slice`` under
    ``junction.slice`` inside the user manager's subtree; the direct
    ...
    direct = base / "junction.slice" / _CGROUP_AGENTS_SLICE
    if direct.is_dir():
        return direct
    try:
...
def resource_limit_preexec() -> "Callable[[], None] | None":
    ...
    Returns the callable from :func:`junction.security.apply_resource_limits`,
    ...
    if _RESOURCE_PREEXEC is _UNSET:
        ...
        from junction.security import apply_resource_limits
...
def resource_limit_supervisor_argv() -> "tuple[str, ...]":
    ...
    if _RESOURCE_SUPERVISOR_ARGV is _UNSET:
        if os.name != "posix":
            ...
            return ()
        from junction.security import resource_limit_spec
