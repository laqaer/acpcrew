...
class SessionIndex:
    ...
    *stem_to_sid* maps a transcript filename stem to the kiro-cli session id it
    ...
    :func:`junction.history.transcript_stems`, never by re-deriving the naming
...
def _replay_store_cotenants() -> list[str]:
    ...
    ``JUNCTION_HOME`` cannot be, and stays a documented limitation.
    """
    try:
...
def reclaim_block_reason() -> str:
    ...
    ``KIRO_HOME=/etc`` alongside an isolated ``JUNCTION_HOME`` leaves this process
    ...
    return (
        ...
        "one cannot see. Point KIRO_HOME inside JUNCTION_HOME to reclaim from here."
    )
