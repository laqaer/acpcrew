"""Portable zip export/import for Junction state (dashboard endpoint).

Creates a zip archive of all Junction settings and memory for download
...
from junction.mcp_cron import _log_cron_denial, _vet_shell_command
from junction.security import is_sensitive_path
from junction.snapshot import (
...
def _mc_dir() -> Path:
    return Path(os.environ.get("JUNCTION_HOME", config_dir()))
...
def create_export_zip() -> tuple[bytes, dict]:
    """Create a zip archive of Junction state. Returns (zip_bytes, manifest_dict)."""
    mc = _mc_dir()
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    prefix = f"junction-export-{ts}"
