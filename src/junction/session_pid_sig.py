...
``JUNCTION_HOST_PID`` (PID-namespace sandboxing strips ``JUNCTION_SESSION_KEY``
...
* OUT OF SCOPE (unchanged from the env-only baseline): a same-uid agent
  ...
  (``JUNCTION_HOST_PID=<victim pid>``) to reuse a legitimate sidecar. This
  is not a capability the sidecar adds — the identical attack defeats
  env-only resolution (``JUNCTION_SESSION_KEY=<victim key>`` with the key
  ...
  ``JUNCTION_HOST_PID`` is launcher-declared, not attacker-chosen.
...
from junction.sel import _sel_hmac_key_bytes, sel_hmac_key_path
...
# from :func:`junction.sel.sel_hmac_key_path`, the single source of truth
...
# JUNCTION_HOME while SEL's default dir does not — deriving the path
...
# sidecar directly.
_SUBKEY_DOMAIN = b"junction.session_pid.sig.v1"
...
def _load_hmac_key() -> bytes | None:
    ...
    comes from :func:`junction.sel.sel_hmac_key_path` so both protocols
    ...
    (:func:`junction.sel.sel_hmac_key_bytes`). Without that fallback this
...
def signing_health() -> tuple[bool, Path]:
    ...
    For diagnostic surfaces (``junction doctor``), which is the only place that
    ...
    construct :class:`~junction.sel.SecurityEventLog` — creating the trust
...
def publish_session_pid(pid: int, session_key: str) -> None:
    ...
    Both files are written via :func:`junction.atomic_write.atomic_write`
