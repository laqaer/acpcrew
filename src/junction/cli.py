...
# The ``junction`` entry-point (console_scripts) bypasses ``__main__.py``,
# so we must run this here as well.
from junction._ssl_compat import _ensure_ssl_certs
...
from junction import __version__, cli_help, platform_compat
from junction.apps import builtins as _builtins_pkg
from junction.apps.builtins import BUILTIN_NAMES as _BUILTIN_NAMES
from junction.config import JunctionConfig, config_dir, ensure_data_home
...
from junction.seed import seed_cmd
...
# Markers that uniquely identify the Junction repo root for project-dir
# auto-detection. ``skills/`` + ``src/junction/`` is the stable signature:
# ``skills/`` is editable-at-root and ``src/junction/`` pins this to the
# Junction package repo (not just any directory that happens to contain a
# ``skills/`` folder).
_PROJECT_MARKERS = ("skills", "src/junction")
...
_JAILED_ENV_MARKER = "JUNCTION_JAILED"
...
def _child_argv() -> "list[str]":
    ...
    Reuses ``agent._resolve_junction_bin`` (the same self-invocation resolver
    junction-core/junction-cron use) so the jailed child inherits its venv
    ``bin/`` walk and ``os.access(X_OK)`` validation — rather than
    re-implementing a bare ``shutil.which`` that misses those cases.
    The resolver returns the bare ``"junction"`` sentinel when it finds no usable
    binary; in that case fall back to ``python -m junction`` so a non-PATH /
    editable run still re-execs correctly.
    """
    from junction.agent import _resolve_junction_bin

    exe = _resolve_junction_bin()
    if exe != "junction":  # a resolved, validated absolute path
        return [exe, *sys.argv[1:]]
    return [sys.executable, "-m", "junction", *sys.argv[1:]]
...
def _refuse_unjailed(command: str, reason: str) -> NoReturn:
    ...
    """
    logging.getLogger("junction").error(
...
def _jail_reexec_gate(command: str, no_jail_flag: bool) -> None:
    ...
    * Already inside the jail (``JUNCTION_JAILED`` marker set by a prior re-exec)
    ...
    * ``off`` (``--no-jail`` or ``JUNCTION_NO_JAIL`` truthy) → early return, no
    ...
    * With a backend present + ``mode != "off"``, a single fail-closed floor
    ...
    # ``off`` per-invocation: --no-jail OR the JUNCTION_NO_JAIL env hatch (the
    ...
    if no_jail_flag or env_flag_enabled("JUNCTION_NO_JAIL"):
...
def _project_dir_file() -> Path:
    """Return the path to the saved project_dir file, respecting JUNCTION_HOME."""
...
def _ensure_node(proj_dir: str = "") -> bool:
    ...
    script = None
    env_dir = os.environ.get("JUNCTION_PROJECT_DIR")
    for candidate in [
...
def _detect_project_dir() -> str | None:
    ...
    Search order:
    1. Walk up from CWD
    2. Read saved path from config_dir()/project_dir (respects JUNCTION_HOME)
    """
...
def _resolve_gateway_args(args: argparse.Namespace) -> dict:
    ...
    if approval == "yolo":
        home_env = os.environ.get("JUNCTION_HOME", "")
        if not home_env:
            print(
        ...
        try:
            ...
            print(
                f"--approval yolo refused: failed to resolve JUNCTION_HOME: {exc}",
                file=sys.stderr,
            )
            sys.exit(2)
        if home_resolved in protected_homes:
            print(
                "--approval yolo refused: JUNCTION_HOME resolves to a main "
                f"gateway home ({home_resolved}). Set JUNCTION_HOME to an isolated "
...
def _diagnostic_port(gw_kwargs: dict) -> int | None:
    ...
    ``dashboard.url``, with ``JUNCTION_PORT`` and then ``--port`` overriding it)
    ...
    try:
        ...
        return parse_dashboard_url(JunctionConfig.load().dashboard.url)[1]
    except Exception:
...
def _setup_cli_logging(command: str | None, verbose: int) -> None:
    ...
    of every ``junction`` record into the SAME file the file handler writes
    ...
    logging.getLogger("junction").setLevel(level)
    ...
    # at WARNING so third-party WARNINGs keep flowing even when junction's
    # own configured level is stricter (junction records below `level` are
    # already filtered at the junction logger, so this cannot over-log).
    ...
    if detached:
        # Root attach: junction records arrive once via propagation, and
        # third-party WARNINGs land formatted — replacing what the accidental
        # stderr echo used to provide unformatted.
        logging.getLogger().addHandler(fh)
    else:
        logging.getLogger("junction").addHandler(fh)

    # Install secret redaction filter — scrubs Bearer tokens from all junction
...
def _builtin_mcp_server_available(name: str) -> bool:
    """True when ``junction.apps.builtins.<name>.mcp_server`` resolves.
...
def main() -> None:
    ...
    # On Windows, force stdout/stderr to UTF-8 BEFORE anything prints — Junction's
    # non-ASCII output otherwise raises UnicodeEncodeError under the cp1252
    # default when stdout is a pipe (detached gateway, JunctionHub client). No-op
    # on POSIX. Must be the first statement: the JUNCTION_PORT error below prints
    # non-ASCII glyphs.
    platform_compat.ensure_utf8_console()

    # Clear any INHERITED sandbox-active marker before any argv-wrapping path
    # can run. JUNCTION_SANDBOX_ACTIVE tells the sandbox layer that OS isolation
    ...
    # JUNCTION_SANDBOX_LEVEL gets the same treatment: a stale inherited value
    # would be read as the ACTIVE tier by a descendant's passthrough and
    # corrupt its downgrade audit.
    os.environ.pop("JUNCTION_SANDBOX_ACTIVE", None)
    os.environ.pop("JUNCTION_SANDBOX_LEVEL", None)

    # Validate JUNCTION_PORT early — fail fast before anything else loads.
    # Range as well as type: an in-range check that lives only in the binder
    # would let `JUNCTION_PORT=70000 junction service install` bake an
    ...
    _raw_port = os.environ.get("JUNCTION_PORT")
    if _raw_port is not None:
        try:
            _port_val = int(_raw_port)
        except ValueError:
            print(
                f"❌ JUNCTION_PORT={_raw_port!r} is not a valid integer.\n"
                f"   Unset it or provide a numeric port (e.g. JUNCTION_PORT=6777).",
                file=sys.stderr,
            )
            sys.exit(1)
        if not 1 <= _port_val <= 65535:
            print(
                f"❌ JUNCTION_PORT={_raw_port!r} is outside the valid port range 1-65535.\n"
                f"   Unset it or provide a bindable port (e.g. JUNCTION_PORT=6777).",
                file=sys.stderr,
            )
            sys.exit(1)

    if not os.environ.get("JUNCTION_PROJECT_DIR"):
...
""",
    ...
    def _add_gateway_flags(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            metavar="FIXTURE",
            help=(
                "Seed $JUNCTION_HOME from the named fixture BEFORE starting the "
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Print a single line `JUNCTION_READY:{...}` to stdout once the "
                "dashboard is bound. Payload includes port, token, pid, and "
                "JUNCTION_HOME. Used by test harnesses to discover the bound "
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "auto-approves all tools (refused unless JUNCTION_HOME is "
...
""",
    ...
    pod_up.add_argument(
        "--approval",
        # Literal mirrors junction.pod.runtime.APPROVAL_MODES, which is the
    ...
    stop_parser.add_argument(
        ...
        default=None,
        help=(
            "Dashboard port (default: resolved from JUNCTION_PORT env or "
...
""",
    ...
    # cloud — provision + run Junction on the user's own AWS EC2 (bring-your-own
    # AWS; credentials resolved by the aws CLI, never stored by Junction).
...
""",
    ...
    logout_parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dashboard port (default: resolved from JUNCTION_PORT env or dashboard.url config)",
    )
    ...
    token_parser.add_argument(
        ...
        help=(
            ...
            "multi-instance embed (the embedding gateway's JUNCTION_PORT). Loopback "
    ...
    # them in the ``junction-computer`` MCP server instead.
...
""",
    ...
    agent_reset_model.add_argument(
        ...
        default="junction",
        help="Agent spec to reset (default: junction)",
    )
...
""",
    ...
    # ``gateway --seed <fixture>`` populates $JUNCTION_HOME from a hand-authored
    ...
    # ``JunctionConfig.load()`` and the file-log handler attach at line ~603):
    # both of those call ``config_dir()`` which ``mkdir``s $JUNCTION_HOME, which
    ...
    # the gateway against a half-seeded or wrong-state $JUNCTION_HOME would be
    ...
    # No subcommand given (`junction` with no args) — show banner + help and exit.
    ...
    # services.  Standalone (no companion, no JUNCTION_PROFILE) composes the
    ...
    try:
        boot_platform(JunctionConfig.load())
        ...
        if isinstance(exc, PlatformCompositionError):
            if args.command == "doctor":
                logging.getLogger("junction").debug(
            ...
            # config.
            logging.getLogger("junction").debug("platform boot deferred", exc_info=True)
    ...
    if args.command == "chat":
        ...
        from junction.cli_server import _gateway
        from junction.dashboard.state import set_build_info

        # Resolve the running build's git branch+commit ONCE here in the sync
        # entrypoint: provably AFTER JUNCTION_PROJECT_DIR detection (top of main())
        ...
        # JUNCTION_HOME so two ConversationLog writers can never clobber the same
        # session file. Held for the process lifetime; released by the kernel on
        # death (POSIX record lock — see junction.gateway_lock). The port is
        ...
        from junction.mcp_cron import run_mcp_server as run_mcp_cron_server

        run_mcp_cron_server()
    elif args.command == "mcp-core":
        from junction.mcp_core import run_mcp_core_server

        run_mcp_core_server()
    elif args.command == "mcp-computer":
        from junction.mcp_computer import run_mcp_server as run_mcp_computer_server
        ...
        importlib.import_module("junction.mcp_dashboard").run_mcp_server()
        ...
        from junction.cli_commands import _run_app_mcp_server
        ...
        from junction.computer_use.cli import run_computer

        run_computer(getattr(args, "computer_args", []))
    elif args.command == "eval":
        from junction.cli_commands import _run_eval

        asyncio.run(_run_eval(args))
    elif args.command == "security":
        from junction.cli_commands import _security

        _security(args)
    elif args.command == "tailnet":
        from junction.cli_commands import _tailnet
        ...
        from junction.cli_commands import _policy
        ...
        # function-local `from junction.cli_commands import …`: the latter trips
        ...
        importlib.import_module("junction.cli_commands")._handle_secrets(args)
    elif args.command == "pod":
        from junction.cli_commands import _pod
        ...
        from junction.cli_server import _service_cmd

        sys.exit(_service_cmd(args))
    elif args.command == "sandbox":
        from junction.cli_server import _sandbox_cmd

        sys.exit(_sandbox_cmd(args))
    elif args.command == "cloud":
        sys.exit(handle_cloud(args))
    elif args.command == "logs":
        from junction.cli_server import _logs_cmd
        ...
        from junction.cli_server import _logout, resolve_client_port
        ...
        from junction.planes import run_planes_command
        ...
        from junction.snapshot import snapshot_main

        rc = snapshot_main(parsed=args)
        if rc:
            raise SystemExit(rc)
    elif args.command == "restore":
        from junction.snapshot import restore_main

        rc = restore_main(parsed=args)
        if rc:
            raise SystemExit(rc)
    elif args.command == "agent":
        from junction.cli_commands import _handle_agent

        _handle_agent(args)
    elif args.command == "workspace":
        from junction.cli_commands import _handle_workspace
...
from junction.cli_bench import bench_cmd, register_bench_parser  # noqa: E402
from junction.cli_chat import _run_chat  # noqa: E402
from junction.cli_cloud import add_size_choices as _cloud_size_choices  # noqa: E402
from junction.cli_cloud import handle_cloud  # noqa: E402
from junction.cli_config import _config_cmd  # noqa: E402
from junction.cli_desktop import desktop_cmd, register_desktop_parser  # noqa: E402
from junction.cli_doctor import _doctor  # noqa: E402
from junction.cli_perf import perf_cmd, register_perf_parser  # noqa: E402
from junction.cli_setup import (  # noqa: E402, F401
    _fix_shell_profiles,
    _manifest,
    _setup,
)
