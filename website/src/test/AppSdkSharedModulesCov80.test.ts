...
/**
 * The federated-app import map resolves `react`, `@junction/app-sdk` and friends
 * to `/vendor/*.mjs` stubs that read `window.__junction_modules`. Nothing else
...
describe('app-sdk shared-modules registry', () => {
  it('registers the HOST module instances, not fresh copies', async () => {
    ...
    const registry = (window as unknown as {
      __junction_modules: Record<string, Record<string, unknown>>
    }).__junction_modules
  ...
  it('publishes every specifier the vendor stubs import', async () => {
    await import('../app-sdk/shared-modules')
    const registry = (window as unknown as {
      __junction_modules: Record<string, unknown>
    }).__junction_modules
