...
/** Open the instance dropdown and return the menu row for `name`. */
...
describe('InstanceTabBar', () => {
  ...
  it('renders Local + a tab per connected instance and switches to Local', async () => {
    ...
    // The trigger names the instance on screen; the destinations live in its menu.
  ...
  it('keeps a sticky tab for a was_connected instance whose tunnel is down', async () => {
    ...
    // scanning for the instance that failed would otherwise have to hover each row.
  ...
  it('carries unread counts for instances the dropdown is hiding', async () => {
    ...
    // Only the instances NOT on screen are counted, so the active pane's own stale
  ...
  it('does not print the instance name twice when its host alias IS its name', async () => {
  ...
  it('pins one instance out of the dropdown into an always-visible chip, and remembers it', async () => {
    // Nothing pinned by default: the instance lives behind the dropdown.
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
    ...
    // No chip row at all until something is pinned, so a user with one remote
    // instance pays no header width for the feature.
    ...
    // Pin it from the instance's own row: one lit/unlit pin per destination, no
    // second list of the same instances.
    await u.click(screen.getByRole('button', { name: /Switch instance/i }))
    const pinItem = await screen.findByTestId('instance-pin-cd-1')
    ...
    await waitFor(() =>
      expect(JSON.parse(localStorage.getItem('mc-instance-switcher-pinned')!)).toEqual(['cd-1']),
    )
    // ...and the chip is now on screen, outside the menu.
    const row = await screen.findByTestId('instance-chip-row')
    expect(row.textContent).toMatch(/Cloud One/)
  })

  it('toggles the pin without switching instances, and keeps the menu open', async () => {
    // The pin shares a row with the destination, so the two must stay separable:
    // pinning an instance the user is not on must not navigate there, and the menu has
    // to survive the click so a second instance can be pinned in the same visit.
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
      instances: { warm: { 'cd-1': { port: 7778, token: 't' } }, activeId: null, mru: ['cd-1'], unread: {} },
    })
    ...
    await u.click(await screen.findByRole('button', { name: /Switch instance/i }))
    await u.click(await screen.findByTestId('instance-pin-cd-1'))
    ...
    const again = await screen.findByTestId('instance-pin-cd-1')
    expect(again).toHaveAttribute('aria-checked', 'true')
    await u.click(again)
    await waitFor(() =>
      expect(JSON.parse(localStorage.getItem('mc-instance-switcher-pinned')!)).toEqual([]),
    )
  })
  ...
  it('pins from the keyboard, since Enter is the only way a keyboard user can', async () => {
    ...
    await u.click(await screen.findByRole('button', { name: /Switch instance/i }))
    const pin = await screen.findByTestId('instance-pin-cd-1')
    pin.focus()
  ...
  it('gives every row its own pin control instead of a separate pin section', async () => {
    ...
    // same instances below the destinations, so each instance appeared twice.
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const u = userEvent.setup()
    renderWithProviders(<InstanceTabBar />)

    await u.click(await screen.findByRole('button', { name: /Switch instance/i }))
    // One pin per destination (Local + the instance), each naming its own instance.
    expect(await screen.findByTestId('instance-pin-__local__')).toHaveAccessibleName(/Pin Local/i)
    expect(await screen.findByTestId('instance-pin-cd-1')).toHaveAccessibleName(/Pin Cloud One/i)
    // ...and each instance is listed once, as one switch target.
  ...
  it('keeps the resting pin at full strength, since it is the only affordance', async () => {
    // The labelled "Pin instances" list is gone, so this glyph is the whole feature's
    ...
    await u.click(await screen.findByRole('button', { name: /Switch instance/i }))
    const glyph = (await screen.findByTestId('instance-pin-cd-1')).querySelector('svg')
    expect(glyph?.getAttribute('class')).toMatch(/text-muted/)
    expect(glyph?.getAttribute('class')).not.toMatch(/opacity-/)
  })

  it('marks a pinned instance by FILL, so no state reads as the unpinned outline', async () => {
    // Fill is the only thing separating pinned from unpinned, and it must not be
    // conditional: a pinned instance rendered as an outline (however tinted) reads as
  ...
  it('switches by clicking a pinned instance chip, without re-minting a warm pane', async () => {
    setInstancePins(['cd-1'])
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
      instances: { warm: { 'cd-1': { port: 7778, token: 't' } }, activeId: null, mru: ['cd-1'], unread: {} },
  ...
  it('leads with the active instance and keeps it out of the pinned row', async () => {
    // Both destinations pinned, Local active: the chip row carries only the instance.
    setInstancePins(['__local__', 'cd-1'])
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
      instances: { warm: { 'cd-1': { port: 7778, token: 't' } }, activeId: null, mru: ['cd-1'], unread: {} },
    })
  ...
  it('stable order keeps a pinned active instance in place instead of leading with it', async () => {
    // Frequent switchers opt in: the instance on screen holds its configured slot and
    ...
    // No separate leading active chip — the active instance lives inside the row.
    const row = await screen.findByTestId('instance-chip-row')
    ...
    expect(row.textContent).toMatch(/Cloud One/)
    // ...and the active instance is highlighted where it sits, not moved.
    expect(within(row).getByRole('button', { name: /Cloud One/i })).toHaveAttribute('aria-current', 'true')
  })

  it('still leads with the active instance under stable order when it is NOT pinned', async () => {
    // Stable order fixes the pinned row; an unpinned active instance has no slot
    // there, so it still leads to stay reachable without opening the dropdown.
    setInstancePins(['__local__'])
    setStableOrder(true)
    vi.mocked(api.listInstances).mockResolvedValue(listResp([conn()]))
    const store = createTestStore({
      instances: { warm: { 'cd-1': { port: 7778, token: 't' } }, activeId: 'cd-1', mru: ['cd-1'], unread: {} },
    ...
    const lead = await waitFor(() => {
      const el = container.querySelector('.tb-instance-active-chip')
  ...
  it('toggles stable order from the dropdown, persists it, and keeps the menu open', async () => {
    ...
    await waitFor(() => expect(localStorage.getItem('mc-instance-switcher-stable-order')).toBe('1'))
    expect(await screen.findByTestId('instance-stable-order-toggle')).toHaveAttribute('aria-checked', 'true')
  })
...
describe('resolvePinnedPref', () => {
  it('reads a stored id list', () => {
    expect(resolvePinnedPref(JSON.stringify(['a', 'b']))).toEqual(['a', 'b'])
  })

  it('ignores a corrupted or non-array value instead of throwing', () => {
    expect(resolvePinnedPref('{oops')).toEqual([])
    expect(resolvePinnedPref('"a string"')).toEqual([])
    expect(resolvePinnedPref(JSON.stringify([1, 'a', null]))).toEqual(['a'])
  })

  it('defaults to nothing pinned', () => {
    expect(resolvePinnedPref(null)).toEqual([])
  })
})
