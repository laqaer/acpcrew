 ...
 * OR-ed value (agents matching workspace OR memory store) passes the common case
 * and fails the inverse: an agent whose memory store alone is shared gets "Shared"
...
function renderPane(over: Partial<React.ComponentProps<typeof AgentOverviewPane>> = {}) {
  render(
    <AgentOverviewPane
...
const isTagged = (key: string) =>
  (screen.getByTestId(`agent-wire-${key}`).textContent || '').includes('Shared')

describe('agent overview pane — the Shared pill follows its own resource', () => {
...
describe('agent overview pane — facts the three explanation boxes used to carry', () => {
  it('reports an unreadable schedule count as unknown, never as zero', () => {
    ...
    expect(screen.getByTestId('agent-wire-schedules').textContent).not.toContain('0')
  })

  it('shows the resolved model, which the agent itself only inherits', () => {
    renderPane()
    expect(screen.getByText('claude-opus-5')).toBeInTheDocument()
    expect(screen.getByTestId('agent-wire-model').textContent).toContain('Inherited')
  })
})

describe('agent overview pane — the webhook node reports the binding fact', () => {
  // The testid element itself carries the ghost treatment classes.
  const nodeBox = () => screen.getByTestId('agent-wire-webhook')

  it('stays a dashed ghost while nothing is bound', () => {
    renderPane({ webhookTokens: 0 })
    const node = screen.getByTestId('agent-wire-webhook')
    expect(node.textContent).toContain('No tokens bound')
    expect(nodeBox().className).toContain('border-dashed')
  })

  it('turns solid and counts once a token names this agent', () => {
    // The ghost treatment claims "a real input with no agent binding"; once a
...
describe('agent overview pane — diagram nodes open the pane they describe', () => {
  ...
  it.each(cases)('clicking the %s node navigates to the %s pane', (node, pane) => {
    ...
    fireEvent.click(screen.getByTestId(`agent-wire-${node}`))
  ...
  it('renders every node as a real button — an inert node is unrepresentable', () => {
    ...
    for (const [node] of cases) {
      expect(screen.getByTestId(`agent-wire-${node}`).tagName).toBe('BUTTON')
    }
  })
})
