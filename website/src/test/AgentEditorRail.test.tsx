/**
 * Guards on the agent editor's rail.
 *
...
import type { AgentEditorSection } from '../components/agent/agentEditorSections'

const SECTIONS: AgentEditorSection[] = [
  ...
  { key: 'schedules', group: 'When it works', icon: Clock, label: 'Schedules', count: '2/3' },
  {
    ...
    label: 'Webhook',
    disabled: true,
    reason: 'Tokens carry no agent binding yet.',
  },
  { key: 'danger', group: '', icon: Trash2, label: 'Danger zone', foot: true },
]

function renderRail(value: AgentEditorSection['key'] = 'overview') {
...
describe('agent editor rail — structure from the registry', () => {
  ...
  it('shows a count when the registry supplies one, and nothing when it does not', () => {
    renderRail()
    expect(screen.getByTestId('agent-rail-schedules')).toHaveTextContent('2/3')
    expect(screen.getByTestId('agent-rail-overview').textContent).toBe('Overview')
  })
})

describe('agent editor rail — selection', () => {
  it('names the tablist and marks only the active row selected', () => {
    renderRail('template')
    expect(screen.getByRole('tablist', { name: 'Agent settings' })).toBeInTheDocument()
    expect(screen.getByTestId('agent-rail-template')).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByTestId('agent-rail-overview')).toHaveAttribute('aria-selected', 'false')
  })

  it('keeps the whole rail to one Tab stop via a roving tabindex', () => {
    renderRail('schedules')
    expect(screen.getByTestId('agent-rail-schedules')).toHaveAttribute('tabindex', '0')
    for (const key of ['overview', 'template', 'webhook', 'danger']) {
      expect(screen.getByTestId(`agent-rail-${key}`)).toHaveAttribute('tabindex', '-1')
    }
  })

  it('points each row at the panel it controls', () => {
    renderRail()
    expect(screen.getByTestId('agent-rail-template')).toHaveAttribute('aria-controls', 'pane-template')
  })

  it('selects on click', () => {
    const { onChange } = renderRail()
    fireEvent.click(screen.getByTestId('agent-rail-schedules'))
    expect(onChange).toHaveBeenCalledWith('schedules')
  })
})

describe('agent editor rail — unsaved edits stay visible from any pane', () => {
  ...
  it('marks only the rows the caller reports as dirty', () => {
    ...
    expect(screen.getByTestId('agent-rail-dirty-template')).toBeInTheDocument()
    expect(screen.queryByTestId('agent-rail-dirty-schedules')).not.toBeInTheDocument()
  })
  ...
  it('gives the marker an accessible name, since a dot alone is invisible', () => {
    ...
    expect(screen.getByTestId('agent-rail-template'))
      .toHaveAttribute('title', expect.stringContaining('Unsaved changes'))
  })
})

describe('agent editor rail — the disabled row is visible, not selectable', () => {
  it('marks it aria-disabled and surfaces the reason rather than staying mute', () => {
    renderRail()
    const row = screen.getByTestId('agent-rail-webhook')
    ...
    expect(row).toHaveAttribute('title', expect.stringContaining('Tokens carry no agent binding yet.'))
  })
  ...
  it('renders the reason as text, not only as a hover title', () => {
    ...
    renderRail()
    const row = screen.getByTestId('agent-rail-webhook')
    expect(row).toHaveTextContent('Tokens carry no agent binding yet.')
  })
  ...
  it('does not point at a panel id that never exists', () => {
    ...
    renderRail()
    expect(screen.getByTestId('agent-rail-webhook')).not.toHaveAttribute('aria-controls')
    expect(screen.getByTestId('agent-rail-template')).toHaveAttribute('aria-controls', 'pane-template')
  })

  it('refuses a click on it', () => {
    const { onChange } = renderRail()
    fireEvent.click(screen.getByTestId('agent-rail-webhook'))
  ...
  it('arrow keys step over it instead of stranding the caret on it', () => {
    ...
    fireEvent.keyDown(screen.getByTestId('agent-rail-schedules'), { key: 'ArrowDown' })
    expect(onChange).toHaveBeenCalledWith('danger')
  })

  it('walks backwards too', () => {
    const { onChange } = renderRail('schedules')
    fireEvent.keyDown(screen.getByTestId('agent-rail-schedules'), { key: 'ArrowUp' })
  ...
  it('jumps to the first and last enabled row on Home and End', () => {
    ...
    fireEvent.keyDown(screen.getByTestId('agent-rail-template'), { key: 'Home' })
    expect(onChange).toHaveBeenCalledWith('overview')
    fireEvent.keyDown(screen.getByTestId('agent-rail-template'), { key: 'End' })
    expect(onChange).toHaveBeenCalledWith('danger')
  })
})
