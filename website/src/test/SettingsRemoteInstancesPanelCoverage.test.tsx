 ...
 *
 * Kept separate from RemoteInstancesPanel.test.tsx so the behavioural file stays a
...
describe('RemoteInstancesPanel — instance actions', () => {
  ...
  it('carries the AWS coordinates of the instance into a lifecycle call, and reports a failed Stop', async () => {
    ...
    await waitFor(() =>
      expect(api.cloudStop).toHaveBeenCalledWith('jn-3f9a', {
        profile: 'Admin',
        region: 'us-west-2',
        instanceId: 'i-0abc123456789def0',
      }),
    )
    expect(await screen.findByText(/stack jn-3f9a not found/, undefined, { timeout: 5_000 })).toBeInTheDocument()
  })
