...
describe('ChatPanel gateway start', () => {
  it('reports the start as under way and re-offers it once the window lapses', async () => {
    ...
    backendOnline = false
    await renderPanel()
    const start = await screen.findByRole('button', { name: 'Start Junction' }, { timeout: 5000 })
    ...
    expect(screen.queryByRole('button', { name: 'Start Junction' })).not.toBeInTheDocument()
    ...
    expect(await screen.findByRole('button', { name: 'Start Junction' })).toBeInTheDocument()
  })
})
