...
function WorkspaceForm({
  ...
  const submit = async () => {
    setWsError('')
    const n = wsName.trim()
    if (!n) { setWsError(i18nT('pages.junctionAgentsPage.workspace_name_is_required')); return }
    setSubmitting(true)
    try {
      ...
      setWsError(e instanceof Error ? e.message : i18nT('pages.junctionAgentsPage.failed_to_create_workspace'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{i18nT('pages.junctionAgentsPage.create_workspace')}</DialogTitle>
      </DialogHeader>
      <DialogBody>
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              ...
              <label htmlFor="ws-name" className="text-[11px] text-muted uppercase tracking-wider font-medium">{i18nT('pages.junctionAgentsPage.name')}</label>
              <InfoTip text={i18nT('pages.junctionAgentsPage.a_unique_identifier_for_this_workspace_agents_re')} />
            </div>
            <Input id="ws-name" placeholder={i18nT('pages.junctionAgentsPage.e_g_oncall')} value={wsName} onChange={e => handleNameChange(e.target.value)} autoFocus />
          </div>
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              {/* Native input associated via htmlFor+id; label-has-for's nesting requirement is a false positive. */}
              {/* eslint-disable-next-line jsx-a11y/label-has-for */}
              <label htmlFor="ws-dir" className="text-[11px] text-muted uppercase tracking-wider font-medium">{i18nT('pages.junctionAgentsPage.directory')}</label>
              <InfoTip text={i18nT('pages.junctionAgentsPage.subdirectory_inside_junction_where_this_workspa')} />
            </div>
            <Input id="ws-dir" placeholder={i18nT('pages.junctionAgentsPage.workspace')} value={wsDir} onChange={e => { setDirTouched(true); setWsDir(e.target.value) }} />
          </div>
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              <span className="text-[11px] text-muted uppercase tracking-wider font-medium">{i18nT('pages.junctionAgentsPage.copy_from_optional')}</span>
              <InfoTip text={i18nT('pages.junctionAgentsPage.copy_the_contents_of_an_existing_workspace_into')} />
            </div>
            <SimpleSelect
              options={workspaceOptions}
              value={copyFrom}
              onChange={setCopyFrom}
              clearLabel={i18nT('pages.junctionAgentsPage.none')}
              aria-label={i18nT('pages.junctionAgentsPage.copy_from_workspace')}
            />
          </div>
      ...
      <DialogFooter>
        <Btn onClick={onClose}>{i18nT('pages.junctionAgentsPage.cancel')}</Btn>
        <SendBtn onClick={submit} disabled={submitting}>{submitting ? i18nT('pages.junctionAgentsPage.creating') : i18nT('pages.junctionAgentsPage.create')}</SendBtn>
      </DialogFooter>
    </>
  )
}
...
function WorkspaceModal({
  ...
  return (
    <Dialog open={open} onOpenChange={next => { if (!next) onClose() }}>
      <DialogContent maxWidth={448} className="z-[110]" aria-label={i18nT('pages.junctionAgentsPage.create_workspace')}>
...
export function TemplateField({ label, options, value, onChange }: {
  ...
  return (
    <Field label={label} hint={i18nT('pages.junctionAgentsPage.the_agent_definition_it_boots_from_tools_mcp_ser')}>
      <SimpleSelect
        options={withCurrent(options, value)}
        value={value}
        onChange={onChange}
        triggerFallback={i18nT('pages.junctionAgentsPage.select_an_agent_template')}
        aria-label={label}
      />
    </Field>
  )
  ...
  return (
    ...
    >
      <SimpleSelect
        ...
        value={value}
        onChange={onChange}
        action={{ label: i18nT('pages.junctionAgentsPage.new_workspace_action'), onSelect: onNewWorkspace }}
        aria-label={i18nT('pages.junctionAgentsPage.workspace_2')}
      />
    </Field>
  )
}
...
export function MemoryStoreField({ options, value, onChange }: {
  ...
  return (
    <Field
      label={i18nT('pages.junctionAgentsPage.memory_store')}
      hint={i18nT('pages.junctionAgentsPage.which_store_its_lessons_and_history_are_written')}
      info={i18nT('pages.junctionAgentsPage.bindings_preview_info')}
    >
      <SimpleSelect options={withCurrent(options, value)} value={value} onChange={onChange} aria-label={i18nT('pages.junctionAgentsPage.memory_store')} />
      ...
      {options.length <= 1 && (
        <span className="flex items-start gap-1.5 text-[11.5px] leading-relaxed text-accent">
          ...
          {i18nT('pages.junctionAgentsPage.additional_stores_coming_with_memory_v2')}
        </span>
      )}
    </Field>
  )
...
export function ModelField({ options, value, onChange }: {
  ...
  return (
    <Field label={i18nT('pages.junctionAgentsPage.model')}>
      <SimpleSelect
        options={withCurrent(options, value)}
        ...
        optionLabels={withCurrent(options, value).map(m => (m === INHERIT_MODEL ? i18nT('pages.junctionAgentsPage.inherited') : m))}
        value={value}
        onChange={onChange}
        aria-label={i18nT('pages.junctionAgentsPage.edit_model')}
      />
    </Field>
  )
}
...
export function TriggersField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <Field label={i18nT('pages.junctionAgentsPage.triggers')} hint={i18nT('pages.junctionAgentsPage.triggers_hint')} info={i18nT('pages.junctionAgentsPage.triggers_info')}>
      <Input
        placeholder={i18nT('pages.junctionAgentsPage.triggers_placeholder')}
        value={value}
        onChange={e => onChange(e.target.value)}
        aria-label={i18nT('pages.junctionAgentsPage.triggers')}
      />
    </Field>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-2 border-t border-border pt-3">
        ...
        <Binding icon={<FolderOpen className="lucide-inline" aria-hidden="true" />} label={i18nT('pages.junctionAgentsPage.workspace_2')} value={agent.workspace} note={filesShared ? sharedNote : undefined} />
        <Binding icon={<Database className="lucide-inline" aria-hidden="true" />} label={i18nT('pages.junctionAgentsPage.memory_store')} value={agent.memory_store} note={memoryShared ? sharedNote : undefined} />
        <Binding
          icon={<Sparkles className="lucide-inline" aria-hidden="true" />}
          label={i18nT('pages.junctionAgentsPage.model')}
          value={agent.model || i18nT('pages.junctionAgentsPage.inherited')}
          muted={!agent.model}
        />
      </div>
    </Clickable>
  ...
  return (
    ...
    >
      ...
      <TableCell className={`font-mono ${agent.model ? 'text-muted' : 'italic text-muted'}`}>
        {agent.model || i18nT('pages.junctionAgentsPage.inherited')}
      </TableCell>
    </TableRow>
  )
}

export default function JunctionAgentsPage({ embedded }: { embedded?: boolean } = {}) {
  const provider = useProvider()
  ...
  const agents: JunctionAgent[] = agentsData?.agents || []
  ...
  const kiroAgentOptions = Array.isArray(installedAgents) ? installedAgents.map((x: { name: string }) => x.name).filter(Boolean) : ['junction']
  ...
  const { data: junctionCfg } = useQuery({
    queryKey: ['junctionConfig'],
    queryFn: () => api.junctionConfig(),
  })
  const memoryStoreOptions = junctionCfg?.memory_stores ? Object.keys(junctionCfg.memory_stores) : ['default']
  ...
  const openEdit = useCallback((a: JunctionAgent) => {
    sheetEpoch.current += 1
    setError('')
    setConfirmDelete(false)
  ...
  const createMut = useMutation({
    mutationFn: ({ epoch: _epoch, ...data }: CreatePayload & { epoch: number }) => api.createJunctionAgent(data),
    onSuccess: (r: AgentMutationResult, vars) => { settleFor(vars.epoch, r.error); refetchAgents() },
    onError: (e: Error, vars) => settleFor(vars.epoch, e.message || i18nT('pages.junctionAgentsPage.failed_to_create_agent')),
  })
  const updateMut = useMutation({
    mutationFn: ({ name, data }: { name: string; data: AgentUpdatePayload; epoch: number }) => api.updateJunctionAgent(name, data),
    onSuccess: (r: AgentMutationResult, vars) => { settleFor(vars.epoch, r.error); refetchAgents() },
  ...
  const defaultMut = useMutation({
    ...
    onError: (e: Error) => setError(e.message || i18nT('pages.junctionAgentsPage.failed_to_update_agent')),
  })
  const deleteMut = useMutation({
    mutationFn: ({ name }: { name: string; epoch: number }) => api.deleteJunctionAgent(name),
    onSuccess: (r: AgentMutationResult, vars) => { settleFor(vars.epoch, r.error); refetchAgents() },
    onError: (e: Error, vars) => settleFor(vars.epoch, e.message || i18nT('pages.junctionAgentsPage.failed_to_delete_agent')),
  })
  ...
  const chatWith = async (agent: string) => {
    ...
    try {
      ...
      settleFor(epoch, msg || i18nT('pages.junctionAgentsPage.failed_to_update_agent'))
      return
    }
  ...
  const sharedKind = (a: JunctionAgent): SharedKind => {
      ...
      <div className={`${embedded ? '' : 'px-4 md:px-6'} pb-8 overflow-y-auto flex-1 min-h-0`}>
        ...
        <div className="mb-3.5 flex items-start gap-2 rounded-lg border border-accent-subtle bg-bg-accent px-3 py-2.5">
          ...
          <span className="text-[12.5px] leading-relaxed text-muted">
            {i18nT('pages.junctionAgentsPage.bindings_preview_notice')}
          </span>
        </div>
        ...
        {agents.length > 1 && (
          <div className="mb-3.5 flex flex-wrap items-center gap-2.5 rounded-lg border border-border bg-bg-accent px-3 py-2.5">
            ...
            <span className="text-[13px]">{i18nT('pages.junctionAgentsPage.new_sessions_use')}</span>
            <SimpleSelect
              options={agents.map(a => a.name)}
              value={defaultAgent}
              onChange={n => { setError(''); defaultMut.mutate(n) }}
              aria-label={i18nT('pages.junctionAgentsPage.new_sessions_use')}
              style={{ width: 190 }}
            />
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          {agents.length > 0 && (
            <SearchInput
              className="w-[240px]"
              placeholder={i18nT('pages.junctionAgentsPage.filter_agents')}
              aria-label={i18nT('pages.junctionAgentsPage.filter_agents')}
              value={filter}
          ...
          {agents.length > 0 && (
            <SegmentedControl<AgentView>
              ...
              segments={[
                {
                  key: 'cards',
                  label: i18nT('pages.junctionAgentsPage.view_cards'),
                  icon: <LayoutGrid size={13} />,
                  tooltip: i18nT('pages.junctionAgentsPage.view_cards_tooltip'),
                },
                {
                  key: 'list',
                  label: i18nT('pages.junctionAgentsPage.view_list'),
                  icon: <Rows3 size={13} />,
                  tooltip: i18nT('pages.junctionAgentsPage.view_list_tooltip'),
                },
              ]}
            />
          )}
                ...
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead aria-label={i18nT('pages.junctionAgentsPage.workspace_2')}>
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      <InfoTip text={i18nT('pages.junctionAgentsPage.bindings_preview_info')} />
                    </span>
                  </TableHead>
                  <TableHead aria-label={i18nT('pages.junctionAgentsPage.memory_store')}>
                    <span className="inline-flex items-center gap-1.5">
                      {i18nT('pages.junctionAgentsPage.memory_store')}
                      <InfoTip text={i18nT('pages.junctionAgentsPage.bindings_preview_info')} />
                    </span>
                  </TableHead>
                  <TableHead>{i18nT('pages.junctionAgentsPage.model')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map(a => (
            ...
            {creating ? (
              <div className="flex flex-col gap-6">
                <section className="flex flex-col gap-3">
                  <h3 className="text-[12px] font-semibold uppercase tracking-wider text-muted">{i18nT('pages.junctionAgentsPage.identity')}</h3>
                  <Field label={i18nT('pages.junctionAgentsPage.name')}>
                    <Input placeholder={i18nT('pages.junctionAgentsPage.e_g_oncall')} value={name} onChange={e => setName(e.target.value)} autoFocus />
                  </Field>
                </section>
                <section className="flex flex-col gap-3">
                  <h3 className="text-[12px] font-semibold uppercase tracking-wider text-muted">{i18nT('pages.junctionAgentsPage.routing')}</h3>
                  <TriggersField value={triggers} onChange={setTriggers} />
                </section>
                <section className="flex flex-col gap-3">
                  <h3 className="text-[12px] font-semibold uppercase tracking-wider text-muted">{i18nT('pages.junctionAgentsPage.runtime_binding')}</h3>
                  <BindingFields
                    templateLabel={provider.labels.agentTemplateField}
                ...
                >
                  {pane === 'overview' && (
                    <AgentOverviewPane
                      ...
                      modelLabel={editModel === INHERIT_MODEL ? i18nT('pages.junctionAgentsPage.inherited') : editModel}
                      ...
                      {resolved && (
                        <div className="rounded-md border border-border bg-bg-accent px-3 py-2.5 text-[11.5px] leading-relaxed text-muted">
                          <span className="text-text">
                            {i18nT('pages.junctionAgentsPage.resolves_to', { model: resolved.model || i18nT('pages.junctionAgentsPage.inherited') })}
                          </span>
                          {' — '}
                          {resolved.pinned
          ...
          <DialogFooter>
            ...
            {creating ? (
              <SendBtn onClick={create} disabled={sheetBusy}>
                {createMut.isPending ? i18nT('pages.junctionAgentsPage.creating') : i18nT('pages.junctionAgentsPage.create')}
              </SendBtn>
            ) : (
              <SendBtn onClick={saveEdit} disabled={sheetBusy || dirtyPanes.size === 0}>{i18nT('pages.junctionAgentsPage.save_changes')}</SendBtn>
            )}
          </DialogFooter>
