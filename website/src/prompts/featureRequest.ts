...
export const FEATURE_REQUEST_PROMPT_FALLBACK = [
  ...
  '`gh label list --repo laqaer/acpcrew --limit 100`',
  ...
  "3. Direct creation via `gh issue create --repo laqaer/acpcrew --title \"$TITLE\" --body-file <file>` with one `--label '<name>'` flag per chosen label, single-quoted so a `$` or backtick in a label name stays literal (needs gh auth; fall back to option 2 on auth errors).",
  '',
