export const FEATURE_REQUEST_URL = 'https://github.com/laqaer/junction/issues/new'
...
export const FEATURE_REQUEST_PROMPT_FALLBACK = [
  ...
  "3. Direct creation via `gh issue create --repo laqaer/junction --title \"$TITLE\" --body-file <file>` with one `--label '<name>'` flag per chosen label, single-quoted so a `$` or backtick in a label name stays literal (needs gh auth; fall back to option 2 on auth errors).",
  '',
