export const FEATURE_REQUEST_URL = 'https://github.com/myrmitis/junction/issues/new'
...
export const FEATURE_REQUEST_PROMPT_FALLBACK = [
  ...
  '`gh label list --repo myrmitis/junction --limit 100`',
  ...
  "3. Direct creation via `gh issue create --repo myrmitis/junction --title \"$TITLE\" --body-file <file>` with one `--label '<name>'` flag per chosen label, single-quoted so a `$` or backtick in a label name stays literal (needs gh auth; fall back to option 2 on auth errors).",
  '',
