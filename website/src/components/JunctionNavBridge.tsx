 ...
 *
 * Also reports slot titles to the plugin via `window.__junctionReportSlotTitle`
 ...
 */
export default function JunctionNavBridge() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('junction-soft-navigate', handler)
    return () => window.removeEventListener('junction-soft-navigate', handler)
  }, [navigate])

  // Report slot titles to the plugin whenever slots change
  useEffect(() => {
    const fn = (window as unknown as { __junctionReportSlotTitle?: (slug: string, title: string) => void }).__junctionReportSlotTitle
