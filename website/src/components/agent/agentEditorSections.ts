/**
 * The agent editor's section registry — the single list the rail, its badges and
 * the overview diagram all render from.
 *
 * Why a registry rather than a hand-written rail: an agent accumulates surfaces
 ...
 *
 * The groups are phrased as questions about the agent rather than as
...
export interface AgentEditorSection {
  ...
  count?: string
  /** Another agent points at this row's workspace or memory store. Drawn as a dot,
...
export interface AgentEditorFacts {
  ...
  /** Schedules that currently wake this agent and are not paused. */
  activeSchedules: number
  /** Every schedule bound to this agent, paused included. */
  totalSchedules: number
  /** How many routing keywords the agent offers the orchestrator. */
  routingWords: number
  /** Another agent points at this agent's workspace or memory store. */
  sharesStorage: boolean
  /** False for the default agent, which cannot be removed. */
  canDelete: boolean
  /** Schedules could not be loaded, so counts are unknown rather than zero. */
  schedulesUnknown?: boolean
  /** Webhook tokens bound to this agent (unbound tokens are not counted: they
   *  can wake any agent, so charging them to every row would read as N bindings
  ...
  /** Panes whose controls differ from what is saved on the agent. */
  dirtyPanes: ReadonlySet<AgentPaneKey>
}

/**
 * Resolve the rail for one agent.
 *
 ...
 */
export function useAgentEditorSections(facts: AgentEditorFacts): AgentEditorSection[] {
  ...
  return useMemo(() => {
    const rows: AgentEditorSection[] = [
      {
        key: 'overview',
        group: t('components.agentEditor.group_who_it_is'),
        icon: LayoutDashboard,
        label: t('components.agentEditor.pane_overview'),
      },
      {
        key: 'template',
        group: t('components.agentEditor.group_what_it_can_do'),
        icon: Boxes,
        label: templateLabel,
      },
      {
      ...
      {
        key: 'place',
        group: t('components.agentEditor.group_where_it_works'),
        icon: FolderOpen,
        label: t('components.agentEditor.pane_workspace_memory'),
      ...
      {
        key: 'routing',
        group: t('components.agentEditor.group_how_work_arrives'),
        icon: Waypoints,
      ...
      },
      {
        ...
        // answer and an answer of none are different claims about the agent.
      ...
      },
      {
        ...
        // agent are counted; the pane itself discloses unbound ones. No badge
    ...
    if (canDelete) {
      rows.push({
        ...
        // already called "Delete agent", and two controls sharing one
