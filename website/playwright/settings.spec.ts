...
test.describe('Settings Page', () => {
  test('loads and renders the tab strip with known tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Remote gateways', exact: true })).toBeVisible()
  ...
  test('/instances redirects to /settings/instances', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Remote gateways', exact: true })).toBeVisible({ timeout: 5000 })
  })
})
