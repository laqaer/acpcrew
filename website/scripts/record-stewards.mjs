/**
 * Recording harness for Issue Radar → Stewards: navigating the roster.
 *
 * A still frame cannot prove a sequence, and two things on this surface are
 * sequences rather than states: selecting a steward in column 2 repaints column 3
 * with that steward's page, and the create control raises a dialog over it. So this
 * records both, end to end.
 *
 * Same fixtures and same stub as capture-stewards.mjs (see
 * lib/issue-radar-stewards-fixtures.mjs); only the driving differs.
 *
 * Produces webm, and mp4 + GIF when ffmpeg is present (it is, in the Playwright
 * image). Usage: node scripts/record-stewards.mjs [outDir]
 */
...
import { makeExtra, seedState } from './lib/issue-radar-stewards-fixtures.mjs'

const OUT = resolve(process.argv[2] || '../temp-screenshots/stewards')
const NAME = 'stewards-roster-flow'
...
const CREATE = '[data-testid="steward-create"]'
const ROW = (id) => `[data-testid="steward-row-${id}"]`
...
async function main() {
  ...
  }, seedState({ stewardView: { kind: 'steward', id: 'c_7f3a01' }, stewardFilter: 'all' }))
  ...
  await page.locator('[data-testid="steward-page"]').first().waitFor({ state: 'visible' })
  await page.waitForTimeout(1400)

  // 1. Select another steward, and wait on ITS row being current rather than on a
  ...
  // 2. Back to the steward with a populated work log, which is the page worth
  //    showing.
