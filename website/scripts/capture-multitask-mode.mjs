/**
 * Capture Multitask Mode UI evidence — gateway-free (stubbed dashboard API).
 *
 * Shots:
 *  1. new-dropdown.png       — the New split-button dropdown open, showing the
 *                              new Multitask Mode chat item under Autopilot.
 *  2. multitask-session.png  — a Multitask Mode session rendered through the
 *                              stubbed dashboard API. The transcript CONTENT is
 ...
 *                              plus attributed forwards) and the Multitask badge
 *                              in the sidebar row.
 *
 * Usage: node scripts/capture-multitask-mode.mjs [outDir]
 */
...
const OUT = process.argv[2] || '../temp-screenshots/multitask-mode'
mkdirSync(OUT, { recursive: true })

const now = Date.now() / 1000

const MULTITASK_MESSAGES = [
  ...
  { role: "assistant", content: "↩ re: “also write me a script that tails the gateway log with colors”\n\nDone — `scripts/tail-gateway.sh` colorizes level tokens (ERROR red, WARN yellow) and follows rotation. Tested against a live log; handles gaps on restart.", cls: "msg msg-a multitask-reply", ts: new Date((now - 600 + 8 * 30) * 1000).toISOString() },
  ...
  { role: "assistant", content: "Here's what's in flight:\n- **Stable feed 403** — running: just started\n- **Log tail script** — idle: Done — `scripts/tail-gateway.sh` colorizes level tokens (ERROR red, WARN yellow) and follows rotation. Tested against a live log; handles gaps on restart.\n- **TTL sweep explainer** — running: just started", cls: "msg msg-a multitask-reply", ts: new Date((now - 600 + 11 * 30) * 1000).toISOString() },
  { role: "assistant", content: "↩ re: “check why the stable feed returns 403”\n\nRoot cause: the CDN origin rejects the stale signing key. Retried with token=[REDACTED: credential] and it returned 200.", cls: "msg msg-a multitask-reply", ts: new Date((now - 600 + 12 * 30) * 1000).toISOString() },
  { role: "assistant", content: "↩ re: “and can you explain how our TTL sweep works?”\n\nThe TTL sweep runs every 6h: it scans expiry buckets, drops entries past their deadline, then compacts. Skips the compaction when nothing expired.", cls: "msg msg-a multitask-reply", ts: new Date((now - 600 + 13 * 30) * 1000).toISOString() },
  { role: "assistant", content: "↩ re: “also write me a script that tails the gateway log with colors”\n\nSwitched to plain grep as you asked — dropped the awk pass, same colors, one fewer dependency.", cls: "msg msg-a multitask-reply", ts: new Date((now - 600 + 14 * 30) * 1000).toISOString() },
]

const SLOTS = [
  { key: 'multitask-demo', title: 'Multi-topic afternoon', agent: 'junction', mode: 'multitask', surface: 'multitask', running: false, unread: 0, pinned: false, memory_mode: 'persistent', messages: 15, last_ts: new Date().toISOString(), created: new Date(Date.now() - 3600e3).toISOString() },
...
const extra = async (path, route) => {
  ...
  if (path === '/api/chat/slots/multitask-demo')
    return json(route, { ...SLOTS[0], messages: MULTITASK_MESSAGES }), true
...
// Shot 2: multitask session transcript + sidebar badge
...
await page.screenshot({ path: `${OUT}/multitask-session.png` })

console.log(`WROTE ${OUT}/new-dropdown.png`)
console.log(`WROTE ${OUT}/multitask-session.png`)
await browser.close()
srv.close()
