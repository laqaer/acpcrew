"""Tests that JunctionConfig.save() preserves all dataclass fields.
...
def test_to_dict_includes_all_dataclass_fields():
    """Every field on JunctionConfig must appear in to_dict() output."""
    cfg = JunctionConfig()
    d = cfg.to_dict()
...
def test_save_load_roundtrip_secretary_retention(cfg_file):
    ...
    JunctionConfig.load() never read them back into SecretaryConfig —
