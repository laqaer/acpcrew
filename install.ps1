...
Write-Host ""
Write-Host "  Junction — Windows setup (cloud mode)" -ForegroundColor Magenta
Write-Host "  Runs Junction on your own AWS EC2; this PC is the client." -ForegroundColor DarkGray
...
Write-Step 4 4 "Junction client"
$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
Push-Location $repoRoot
try {
    Write-Info "Installing the Junction client (pip)..."
    ...
    Write-Ok "Junction client installed"
} finally {
    Pop-Location
}

if (-not (Have "aws")) {
    Write-Warn "Restart your terminal so aws / session-manager-plugin are on PATH, then run: junction cloud launch"
    exit 0
}
if (-not (Have "session-manager-plugin")) {
    ...
    Write-Warn "session-manager-plugin is not on PATH yet. Restart your terminal, then run: junction cloud launch"
    exit 0
}
...
if (-not $identity) {
    ...
    Write-Info "Then run:  junction cloud launch"
    exit 0
}
Write-Ok "AWS credentials resolve."

if ($NonInteractive) {
    Write-Info "Prerequisites ready. Launch when you're set:  junction cloud launch"
    exit 0
}

Write-Host ""
Write-Host "  Launching Junction on AWS..." -ForegroundColor Cyan
if ($Size -ne "") {
    python -m junction cloud launch --size $Size
} else {
    python -m junction cloud launch
}
