...
| Nudge never fires | \`cat ~/.junction/autonudge.json\` — loop \`active:true\`? Right slot_key? |
...
Skill reference: \`~/.junction/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".
EOF
)
...
3. Read the skill: \`~/.junction/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".
