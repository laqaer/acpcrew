---
name: junction-app-dev
description: "Build, package, and publish Junction external apps. Covers app.json manifest, UI components, crons, skills, self-healing install, git publishing, and common pitfalls."
---

# Junction App Development

Guide for building external Junction apps that work reliably across Junction versions without depending on buggy lifecycle hooks or skill scanner behavior.

## When to Use

- User says "create a junction app", "build an app", "make an app for junction"
...
**Never depend on:**
- `onInstall` hook firing (unreliable across Junction versions)
...
All Junction apps MUST use the same visual language for consistency:
...
`src/junction/apps/builtins/` — they follow the same manifest, UI, and cron
...
```html
<!--
  This is what a Junction app header + card should look like.
...
import { useNavigate } from '@junction/app-sdk'
...
| Use `useNavigate()` from `@junction/app-sdk` for navigation | Use `window.location` (causes full reload) |
...
- `react-dom`
- `@junction/app-sdk` — hooks (`useAppApi`, `useAppEvents`, `useTheme`, `useAppInfo`,
...
Open a pull request to the Junction repo adding an entry to `app-registry.json`:
```json
{
  "name": "my-app",
...
1. `junction app install <path-or-git-url>`
2. Gateway restart: `junction gateway restart`
...
2. Install: `junction app install /path/to/my-app`
3. Restart gateway: `junction gateway restart`
...
- `register_routes(ctx)` receives an AppContext and MUST return `list[AppRoute]`
  (`from junction.apps.route_registry import AppRoute`) with paths RELATIVE to
...
- **Preferred: dev mode** — `junction app dev <name>` (off: `--off`). Serves that
...
- **Host components over hand-rolled ones**: the `@junction/ui` module-map
  ...
  (`window.__junction_modules?.['@junction/ui']`) and keep a small fallback for
...
- Access: `const sdk = window.__junction_modules?.['@junction/app-sdk']`, then
...
When an app proves out and should ship with Junction, port it into the repo —
the contracts CHANGE on both sides. Template: `src/junction/apps/builtins/issue_radar/`.

- Layout: `src/junction/apps/builtins/<snake_name>/` with `app.json`,
...
- Full worktree + build-gate discipline applies (see the junction-worktree-dev
  skill) — this is now Junction source.
