...
def test_dispatch_skipped_when_disabled() -> None:
    ...
    state = _make_state()
    factory = MagicMock()
    with patch("junction.dashboard.server.JunctionConfig.load", return_value=_cfg(False)):
...
def test_dispatch_schedules_when_enabled() -> None:
    ...
    async def _run() -> bool:
        ...
        with patch("junction.dashboard.server.JunctionConfig.load", return_value=_cfg(True)):
...
class TestDmOwner:
    ...
    def test_redacts_before_posting(self) -> None:
        ...
        with (
            patch(
                "junction.dashboard.server.redact_exfiltration_urls",
                return_value=("no-exfil", []),
            ) as m_exfil,
            patch(
                "junction.dashboard.server.redact_credentials",
