...
from junction.slack.events import SeenCache, _dispatch_queued, _route_message
...
class TestChannelActivationRouting:
    ...
    async def test_channel_off_does_not_record_history(self):
        ...
        with patch("junction.slack.events.sel") as mock_sel:
...
class TestRouteMessageStop:
    ...
    async def test_stop_cancels_active_task_and_cleans_up(self):
        ...
        with patch("junction.slack.events.is_owner", return_value=True), patch(
            "junction.slack.events.is_allowed_user", return_value=True
    ...
    async def test_stop_bypasses_semaphore(self):
        ...
        ) as mock_hm, patch("junction.slack.events.is_owner", return_value=True), patch(
            "junction.slack.events.is_allowed_user", return_value=True
        ), patch(
            "junction.slack.enterprise.check_message_origin", return_value=True
        ):
            await _route_message(orch, event, seen, is_mention=False)
            await asyncio.sleep(0)
