...
import junction.subagent_cost as sc
...
def test_append_writes_jsonl_line(cost_log):
    sc.append_cost_sample("junction", 0.34, 0.82)
...
def test_append_skips_zero_zero(cost_log):
    sc.append_cost_sample("junction", 0.0, 0.0)
...
def test_concurrent_appends_do_not_lose_samples(cost_log):
    for i in range(20):
        sc.append_cost_sample("junction", 0.3 + i * 0.001, 0.5)
    lines = cost_log.read_text(encoding="utf-8").strip().splitlines()
...
def test_p90_ignores_single_outlier(cost_log):
    ...
    recs = [{"agent": "junction", "mem_gb": 0.30, "cpu_cores": 0.1} for _ in range(20)]
    recs.append({"agent": "junction", "mem_gb": 9.9, "cpu_cores": 0.1})
    _seed(cost_log, recs)
...
def test_max_across_agents(cost_log):
    recs = (
        [{"agent": "junction-lite", "mem_gb": 0.30, "cpu_cores": 0.1} for _ in range(5)]
        + [{"agent": "junction", "mem_gb": 0.55, "cpu_cores": 0.1} for _ in range(5)]
    )
    _seed(cost_log, recs)
    val = sc.read_learned_cost("mem_gb")
...
def test_min_samples_fallback_returns_none(cost_log):
    _seed(cost_log, [{"agent": "junction", "mem_gb": 0.5, "cpu_cores": 0.1}])  # only 1
...
def test_corrupt_lines_skipped(cost_log):
    ...
    good = json.dumps({"agent": "junction", "mem_gb": 0.4, "cpu_cores": 0.1})
...
def test_window_limits_to_recent(cost_log):
    ...
    recs = [{"agent": "junction", "mem_gb": 0.1, "cpu_cores": 0.1} for _ in range(10)]
    recs += [{"agent": "junction", "mem_gb": 0.9, "cpu_cores": 0.1} for _ in range(3)]
    _seed(cost_log, recs)
...
def test_compaction_bounds_per_agent(cost_log):
    recs = [{"agent": "junction", "mem_gb": 0.3, "cpu_cores": 0.1, "ts": i} for i in range(100)]
    _seed(cost_log, recs)
...
def test_compaction_noop_when_within_bound(cost_log):
    recs = [{"agent": "junction", "mem_gb": 0.3, "cpu_cores": 0.1, "ts": i} for i in range(5)]
    _seed(cost_log, recs)
