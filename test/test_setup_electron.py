...
from junction.cli_setup import _setup, _setup_electron
...
class TestSetupElectronPlatformGuard:
    ...
    def test_skips_on_linux(self, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Linux")
        _setup_electron()
        out = capsys.readouterr().out
        assert "only available on macOS" in out

    def test_skips_on_windows(self, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Windows")
        _setup_electron()
        out = capsys.readouterr().out
        assert "only available on macOS" in out
...
class TestSetupElectronNodeGuard:
    ...
    def test_no_node(self, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Darwin")
        monkeypatch.setattr("junction.cli_setup.shutil.which", lambda _: None)
        _setup_electron()
        out = capsys.readouterr().out
...
class TestSetupElectronSourcesMissing:
    ...
    def test_sources_not_found_returns(self, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Darwin")
        monkeypatch.setattr("junction.cli_setup.shutil.which", lambda _: "/usr/local/bin/node")
        monkeypatch.setattr("junction.cli_setup._find_electron_dir", lambda: None)
        run_called = {"n": 0}
        monkeypatch.setattr(
            "junction.cli_setup.subprocess.run",
...
class TestSetupElectronBuild:
    ...
    def _electron_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_setup._find_electron_dir", lambda: electron_dir)
        return electron_dir

    def test_npm_install_failure_aborts(self, tmp_path, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.Path.home", lambda: tmp_path)
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Darwin")
        monkeypatch.setattr("junction.cli_setup.shutil.which", lambda _: "/usr/local/bin/node")
        self._electron_dir(tmp_path, monkeypatch)

        mock_fail = MagicMock(returncode=1, stdout="", stderr="npm ERR!")
        monkeypatch.setattr("junction.cli_setup.subprocess.run", lambda *a, **kw: mock_fail)
        _setup_electron()
        out = capsys.readouterr().out
    ...
    def test_electron_builder_failure_aborts(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_setup.subprocess.run", _side_effect)
        _setup_electron()
        out = capsys.readouterr().out
    ...
    def test_app_not_found_after_build(self, tmp_path, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.Path.home", lambda: tmp_path)
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Darwin")
        monkeypatch.setattr("junction.cli_setup.platform.machine", lambda: "arm64")
        monkeypatch.setattr("junction.cli_setup.shutil.which", lambda _: "/usr/local/bin/node")
        self._electron_dir(tmp_path, monkeypatch)

        mock_ok = MagicMock(returncode=0, stdout="", stderr="")
        monkeypatch.setattr("junction.cli_setup.subprocess.run", lambda *a, **kw: mock_ok)
        _setup_electron()
        out = capsys.readouterr().out
        assert "Junction.app not found" in out
    ...
    def test_successful_install(self, tmp_path, capsys, monkeypatch):
        ...
        app_dir = electron_dir / "dist" / "mac-arm64" / "Junction.app" / "Contents"
        ...
        assert (tmp_path / "Applications" / "Junction.app" / "Contents" / "Info.plist").exists()
    ...
    def test_replaces_existing_app(self, tmp_path, capsys, monkeypatch):
        ...
        old_app = tmp_path / "Applications" / "Junction.app" / "Contents"
        ...
        monkeypatch.setattr("junction.cli_setup.subprocess.run", lambda *a, **kw: mock_ok)
        _setup_electron()
        installed = tmp_path / "Applications" / "Junction.app" / "Contents" / "Info.plist"
        assert installed.read_text(encoding="utf-8") == "<new>"

    def test_x86_arch_fallback(self, tmp_path, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup.Path.home", lambda: tmp_path)
        monkeypatch.setattr("junction.cli_setup.platform.system", lambda: "Darwin")
        monkeypatch.setattr("junction.cli_setup.platform.machine", lambda: "x86_64")
        monkeypatch.setattr("junction.cli_setup.shutil.which", lambda _: "/usr/local/bin/node")
        electron_dir = self._electron_dir(tmp_path, monkeypatch)
        app_dir = electron_dir / "dist" / "mac" / "Junction.app" / "Contents"
        app_dir.mkdir(parents=True)
...
class TestSetupElectronOnly:
    """junction setup --electron-only dispatches correctly."""

    def test_electron_only_calls_setup_electron(self, capsys, monkeypatch):
        monkeypatch.setattr("junction.cli_setup._setup_electron", lambda: None)
    ...
    def test_electron_only_returns_early(self, monkeypatch):
        monkeypatch.setattr("junction.cli_setup._setup_electron", lambda: None)
        with patch("junction.agent.install_agent") as mock_agent:
            _setup(electron_only=True)
