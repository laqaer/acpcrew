...
from junction._ssl_compat import _CA_CANDIDATES, _ensure_ssl_certs
...
class TestEnsureSslCerts:
    ...
    def test_sets_env_from_first_existing_candidate(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch("junction._ssl_compat._CA_CANDIDATES", candidates),
        ):
            _ensure_ssl_certs()
    ...
    def test_cli_invokes_ensure_ssl_certs(self):
        ...
        mock_fn = MagicMock()
        with _patch("junction._ssl_compat._ensure_ssl_certs", mock_fn):
            import junction.cli

            importlib.reload(junction.cli)
