"""``junction-computer`` and ``junction-dashboard`` resolve the calling session
from the injected caller block.

The exact state #4622 described and fixed for ``junction-cron``: both servers
...
advertised ``junction.caller-identity``, and ``mcp_gateway/backend.py`` strips
...
from junction import mcp_computer, mcp_dashboard
