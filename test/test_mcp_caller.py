"""Unit tests for :mod:`junction.mcp_caller` — JUNCTION_HOST_PID resolution.
...
def test_from_env_uses_host_pid_env_before_walk(tmp_path, monkeypatch) -> None:
    """The sandbox launcher exports JUNCTION_HOST_PID (its own host pid — the
    ...
    with mock.patch.dict(
        os.environ,
        {"JUNCTION_SESSION_KEY": "", "JUNCTION_HOST_PID": "987654"},
        clear=False,
    ):
        with mock.patch("junction.config.loader.config_dir", return_value=tmp_path):
...
def test_from_env_host_pid_missing_file_falls_back_to_walk(tmp_path, monkeypatch) -> None:
    """A stale/dangling JUNCTION_HOST_PID (no matching file) must not break
    ...
    with mock.patch.dict(
        os.environ,
        {"JUNCTION_SESSION_KEY": "", "JUNCTION_HOST_PID": "999999"},
        clear=False,
    ):
        with mock.patch("junction.config.loader.config_dir", return_value=tmp_path):
            ctx = CallerContext.from_env()
