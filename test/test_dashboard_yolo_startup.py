...
from junction.dashboard.server import _apply_startup_yolo
from junction.dashboard.state import DashboardState
from junction.safety_override import SafetyOverride, reset_singleton, safety_override
...
def test_startup_seeds_the_adhoc_ttl_even_when_yolo_is_off() -> None:
    ...
    with patch("junction.safety_override.sel"), patch(
        "junction.config.loader.JunctionConfig.load", return_value=cfg
    ):
...
def test_apply_startup_yolo_handles_exception_gracefully() -> None:
    ...
    state = _make_state()
    with patch(
        "junction.dashboard.server.grant_declared_yolo",
