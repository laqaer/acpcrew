"""Tests for Issue Radar's steward HTTP surface (``backend/steward_routes.py``).
...
gate and the ``StewardStoreError -> 409`` guard. A route that is written but never
...
The coverage is weighted toward the conditions whose failure is otherwise silent:
  ...
  * **409, never 500.** A duplicate steward name and a second item entering an
    editing phase are ordinary user conditions; surfaced as 500 they read as "the
    backend broke" and a steward agent retries them forever.
  * **The ledger cannot lie.** ``PUT /steward/work`` writes the item and appends the
    event in one call, and a refused write must leave NO ledger line behind.
  * **A skip is always indexed.** ``phase: skipped`` through ``PUT /steward/work``
    ...
    other steward re-investigates the issue, which is the waste the index exists to
    remove. Asserted from ANOTHER steward's ``GET /steward``, because visible-to-me is
    not the property — visible-to-the-fleet is.
  * **``working`` is the NEWEST open item**, not any of them — the difference only
    shows up on a steward holding one parked and one active item.
  * **Nothing waits for a human.** There is no route through which a person answers
    a steward and no queue of items held for one, so the tests assert those endpoints
    are GONE rather than that they behave — a steward that needs a decision labels the
    issue, records the pass and releases its claim.

Data isolation is one patch: ``routes._scope`` is the only thing that decides
where steward records land, so pointing it at a ``TemporaryDirectory`` keeps every
...
STEWARD_ROUTES: tuple[tuple[str, str], ...] = (
    ("GET", "/stewards"),
    ("POST", "/stewards"),
    ("GET", "/stewards/names"),
    ("GET", "/stewards/settings"),
    ("PUT", "/stewards/settings"),
    ("GET", "/steward"),
    ("GET", "/steward/fabric"),
    ("PUT", "/steward"),
    ("DELETE", "/steward"),
...
#: registrar is checked against ``STEWARD_ROUTES`` by name, so a re-added handler under
#: a path nobody enumerates would pass every other test in this file silently.
RETIRED_ROUTES: tuple[tuple[str, str], ...] = (
    ("POST", "/steward/guidance"),
    ("GET", "/stewards/escalations"),
)
...
_MINIMAL: dict[tuple[str, str], dict] = {
    ("GET", "/stewards"): {"query": {"owner": OWNER, "repo": REPO}},
    ("POST", "/stewards"): {"body": {"owner": OWNER, "repo": REPO, "name": "Andromeda"}},
    ("GET", "/stewards/names"): {"query": {"owner": OWNER, "repo": REPO}},
    ("GET", "/stewards/settings"): {"query": {"owner": OWNER, "repo": REPO}},
    ("PUT", "/stewards/settings"): {
        "body": {"owner": OWNER, "repo": REPO, "settings": {"claim_ttl_hours": 12}}
    },
    ("GET", "/steward"): {"query": {"owner": OWNER, "repo": REPO, "id": "c_dead"}},
    ("GET", "/steward/fabric"): {"query": {"owner": OWNER, "repo": REPO}},
    ("PUT", "/steward"): {"body": {"owner": OWNER, "repo": REPO, "id": "c_dead"}},
    ("DELETE", "/steward"): {"body": {"owner": OWNER, "repo": REPO, "id": "c_dead"}},
    ("PUT", "/steward/work"): {
        "body": {
            "owner": OWNER, "repo": REPO, "steward_id": "c_dead", "number": 7,
...
def _registered() -> dict[tuple[str, str], object]:
    """Every steward route the registrar installs, keyed by (method, sub-path)."""
    app = web.Application()
...
class _Slot:
    """A steward's chat slot, stubbed at the one method the route uses.
    ...
    ``_trust`` is the in-memory grant that makes an unattended steward's tool calls
    auto-approve. It is a field on the real slot object, which is why stopping a steward
...
class _StewardRouteCase(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self) -> None:
        ...
        for patcher in (
            ...
            # Session -> steward resolution searches the connected repos, and the real
    ...
    def steward(self, name: str = "Andromeda", **spec) -> dict:
        return steward_store.create_steward(OWNER, REPO, {"name": name, **spec}, self.root)

    def work(self, steward_id: str, number: int, phase: str) -> dict:
        return steward_store.upsert_work_item(
            OWNER, REPO, steward_id, number, {"phase": phase}, self.root
        )

    def ledger(self, steward_id: str = "") -> list[dict]:
        return steward_store.read_events(OWNER, REPO, self.root, steward_id=steward_id)


# ── registration and the two gates ──────────────────────────────────────────


class TestRegistrationAndGates(_StewardRouteCase):
    def test_the_registrar_installs_exactly_the_documented_route_table(self):
        ...
        self.assertEqual(sorted(_registered()), sorted(STEWARD_ROUTES))
    ...
    async def test_every_route_is_denied_while_the_app_is_disabled(self):
        ...
        with mock.patch.object(routes, "is_app_enabled", return_value=False):
            for method, path in STEWARD_ROUTES:
    ...
    async def test_a_missing_repo_is_400_before_anything_else(self):
        self.assertEqual((await self.call("GET", "/stewards")).status, 400)
        res = await self.call("POST", "/stewards", body={"name": "Andromeda"})
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "missing_repo")

    async def test_local_steward_routes_do_not_require_write_access(self):
        # The write-permission decision, asserted rather than described: a
        # read-only repo can still hold stewards, because nothing on this path
...
class TestStewardsList(_StewardRouteCase):
    async def test_returns_stewards_settings_and_counts(self):
        self.steward("Andromeda")
        res = await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO})
        self.assertEqual(res.status, 200)
        page = _payload(res)
        self.assertEqual(page["owner"], OWNER)
        self.assertEqual(page["repo"], REPO)
        self.assertEqual([c["name"] for c in page["stewards"]], ["Andromeda"])
        self.assertEqual(page["settings"]["claim_ttl_hours"], 48)
        # A new steward is enabled and holds nothing: on duty, and nothing else.
        self.assertEqual(page["counts"], {"on_duty": 1, "working": 0, "paused": 0})
        self.assertEqual(page["stewards"][0]["status"], "idle")

    async def test_working_reads_the_newest_open_item_not_any_of_them(self):
        # The steward holds one item parked on CI and one being implemented. "Any
        ...
        steward = self.steward("Andromeda")
        self.work(steward["id"], 1, "awaiting-ci")
        self.work(steward["id"], 2, "implementing")
        counts = _payload(
            await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO})
        )["counts"]
        self.assertEqual(counts["working"], 1)

    async def test_a_steward_parked_on_its_newest_item_is_not_working(self):
        steward = self.steward("Andromeda")
        self.work(steward["id"], 1, "implementing")
        self.work(steward["id"], 2, "awaiting-ci")
        page = _payload(await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO}))
        self.assertEqual(page["counts"]["working"], 0)
        self.assertEqual(page["stewards"][0]["status"], "idle")

    async def test_a_retired_steward_is_neither_listed_nor_on_duty(self):
        steward = self.steward("Andromeda")
        steward_store.retire_steward(OWNER, REPO, steward["id"], self.root)
        page = _payload(await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO}))
        self.assertEqual(page["stewards"], [])
        # Retired is NOT paused: the steward is gone, not stopped.
    ...
    async def test_the_counts_are_independent_predicates_not_a_partition(self):
        working = self.steward("Andromeda")
        self.work(working["id"], 1, "implementing")
        parked = self.steward("Whirlpool")
        self.work(parked["id"], 2, "awaiting-reply")
        paused = self.steward("Sombrero", enabled=False)
        self.work(paused["id"], 3, "implementing")
        page = _payload(await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO}))
        # The paused steward still holds an in-flight item, so it is counted twice —
        # each chip is its own filter, so the numbers may sum past the steward count.
        self.assertEqual(page["counts"], {"on_duty": 3, "working": 2, "paused": 1})
        by_name = {c["name"]: c["status"] for c in page["stewards"]}
        self.assertEqual(by_name["Andromeda"], "working")
        # Parked on a human's reply, so not the actor — but it is not a status of
        # its own either: there is no per-steward "needs you", because a steward that
    ...
    async def test_no_steward_status_reports_waiting_on_a_human(self):
        ...
        the roster show a steward as blocked while the issue it is supposedly blocked on
        has already been labelled and released.
        """
        steward = self.steward("Andromeda")
        for number, phase in enumerate(sorted(steward_routes.PARKED_PHASES), start=1):
            self.work(steward["id"], number, phase)
        page = _payload(await self.call("GET", "/stewards", query={"owner": OWNER, "repo": REPO}))
        self.assertNotIn("needs_you", page["counts"])
        self.assertIn(page["stewards"][0]["status"], {"idle", "working", "paused"})


# ── POST /stewards, GET /stewards/names ─────────────────────────────────────


class TestCreateAndNames(_StewardRouteCase):
    async def test_create_returns_the_steward(self):
        res = await self.call(
            "POST", "/stewards",
            body={"owner": OWNER, "repo": REPO, "name": "Andromeda", "max_open": 5},
        )
        self.assertEqual(res.status, 200)
        steward = _payload(res)["steward"]
        self.assertTrue(steward["id"].startswith("c_"))
        self.assertEqual(steward["slot_key"], f"steward-{steward['id']}")
        self.assertEqual(steward["max_open"], 5)

    async def test_create_without_a_name_is_400(self):
        res = await self.call("POST", "/stewards", body={"owner": OWNER, "repo": REPO})
        self.assertEqual(res.status, 400)
    ...
    async def test_a_duplicate_name_is_409_with_the_stores_own_message(self):
        ...
        self.assertEqual(_payload(res)["code"], "steward_conflict")

    async def test_a_retired_stewards_name_is_still_taken(self):
        steward = self.steward("Andromeda")
        steward_store.retire_steward(OWNER, REPO, steward["id"], self.root)
        res = await self.call(
            "POST", "/stewards", body={"owner": OWNER, "repo": REPO, "name": "Andromeda"}
    ...
    async def test_names_suggests_unused_pool_names(self):
        self.steward("Andromeda")
        res = await self.call("GET", "/stewards/names", query={"owner": OWNER, "repo": REPO})
    ...
    async def test_the_suggestion_limit_is_clamped(self):
        res = await self.call(
            "GET", "/stewards/names", query={"owner": OWNER, "repo": REPO, "limit": "9999"}
        )
        self.assertLessEqual(len(_payload(res)["suggestions"]), steward_routes._MAX_SUGGESTIONS)


# ── GET/PUT/DELETE /steward ─────────────────────────────────────────────────


class TestStewardReadUpdateRetire(_StewardRouteCase):
    async def test_read_returns_the_steward_its_items_events_and_counts(self):
        steward = self.steward("Andromeda")
        self.work(steward["id"], 7, "implementing")
        self.work(steward["id"], 8, "skipped")
        steward_store.append_event(OWNER, REPO, steward["id"], 7, "claim", "took #7", self.root)
        res = await self.call(
            "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": steward["id"]}
        )
        self.assertEqual(res.status, 200)
        page = _payload(res)
        self.assertEqual(page["steward"]["name"], "Andromeda")
    ...
    async def test_read_of_an_unknown_steward_is_404_not_409(self):
        res = await self.call(
            "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": "c_deadbeef"}
        )
        self.assertEqual(res.status, 404)
        self.assertEqual(_payload(res)["code"], "steward_not_found")

    async def test_read_without_an_id_is_400(self):
        res = await self.call("GET", "/steward", query={"owner": OWNER, "repo": REPO})
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "missing_steward_id")

    async def test_a_retired_stewards_page_still_opens(self):
        steward = self.steward("Andromeda")
        steward_store.retire_steward(OWNER, REPO, steward["id"], self.root)
        res = await self.call(
            "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": steward["id"]}
        ...
        self.assertEqual(res.status, 200)
        self.assertTrue(_payload(res)["steward"]["retired_at"])

    async def test_update_merges_a_patch_and_keeps_the_face(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "PUT", "/steward",
            body={"owner": OWNER, "repo": REPO, "id": steward["id"], "name": "Whirlpool",
                  "auto_merge": False, "not_a_field": "x"},
        )
        self.assertEqual(res.status, 200)
        updated = _payload(res)["steward"]
    ...
    async def test_a_rename_onto_a_taken_name_is_409(self):
        first = self.steward("Andromeda")
        self.steward("Whirlpool")
        res = await self.call(
            "PUT", "/steward",
            body={"owner": OWNER, "repo": REPO, "id": first["id"], "name": "Whirlpool"},
        )
        self.assertEqual(res.status, 409)
    ...
    async def test_delete_retires_and_keeps_the_record(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "DELETE", "/steward", body={"owner": OWNER, "repo": REPO, "id": steward["id"]}
        )
        self.assertEqual(res.status, 200)
        retired = _payload(res)["steward"]
        ...
        self.assertIsNotNone(steward_store.read_steward(OWNER, REPO, steward["id"], self.root))


# ── identity: an agent is WHO ITS SESSION SAYS ──────────────────────────────


class TestAgentIdentityIsTheSession(_StewardRouteCase):
    """``GET /steward`` and ``PUT /steward/work`` as the STEWARD itself calls them.

    That call carries the internal secret and an ``X-Session-Key``, and NO owner,
    repo or steward id anywhere — ``issue_radar_steward_read`` takes no arguments at all,
    ...
    ``?owner&repo&id``, so both steward tools returned 400 in production while the
    stubbed tests stayed green.
    """

    def agent(self, steward: dict) -> str:
        """The session key a steward's own turn sends: ``dashboard:<slot key>``."""
        return f"dashboard:{steward['slot_key']}"

    async def test_a_parameterless_read_resolves_the_steward_from_the_session(self):
        steward = self.steward("Andromeda")
        self.work(steward["id"], 7, "implementing")
        res = await self.call("GET", "/steward", internal_auth=True, session=self.agent(steward))
        self.assertEqual(res.status, 200)
        page = _payload(res)
        self.assertEqual(page["steward"]["id"], steward["id"])
    ...
    async def test_the_response_satisfies_the_write_tools_identity_contract(self):
        # The bridge between the two files: ``issue_radar_steward_record`` reads
        ...
        # on a steward with NO work items, because the top-level echo is then the only
        # place owner/repo can come from.
        steward = self.steward("Andromeda")
        res = await self.call("GET", "/steward", internal_auth=True, session=self.agent(steward))
        self.assertEqual(res.status, 200)
        self.assertEqual(
            mcp_core._steward_identity(_payload(res)), (OWNER, REPO, steward["id"])
        )

    async def test_the_read_carries_the_repo_protocol_settings(self):
        # A resuming steward negotiates its claim against the TTL and applies the
        ...
        self.assertEqual(
            ...
            steward_store.DEFAULT_SETTINGS["needs_human_label"],
        )

    async def test_a_bare_slot_key_session_resolves_too(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "GET", "/steward", internal_auth=True, session=steward["slot_key"]
        )
        self.assertEqual(res.status, 200)
        self.assertEqual(_payload(res)["steward"]["id"], steward["id"])

    async def test_a_steward_holding_a_legacy_slot_key_still_reaches_its_ledger(self):
        """Earlier Junction builds minted a steward's slot key as ``crew-<id>``.

        That key is persisted twice — in the steward record and as the name of the
        live chat session — and neither is rewritten on upgrade. Identity is matched
        against the record's own ``slot_key``, never against a prefix, so a steward
        created by an earlier build keeps reading and writing its ledger from the
        session it already has. Asserted through both agent routes, because a lookup
        that only knew the new prefix would answer ``not_a_steward_session`` to a
        steward that is mid-run.
        """
        steward = self.steward("Andromeda")
        legacy_slot = "crew-" + steward["id"]
        path = steward_store.steward_path(OWNER, REPO, steward["id"], self.root)
        record = json.loads(path.read_text())
        record["slot_key"] = legacy_slot
        path.write_text(json.dumps(record))
        session = f"dashboard:{legacy_slot}"

        res = await self.call("GET", "/steward", internal_auth=True, session=session)
        self.assertEqual(res.status, 200)
        self.assertEqual(_payload(res)["steward"]["id"], steward["id"])
        ...
        self.assertEqual(res.status, 200)
        self.assertEqual([e["text"] for e in self.ledger(steward["id"])], ["took #7"])
    ...
    async def test_a_session_key_with_trailing_space_still_resolves(self):
        ...
        # steward as "not a steward" and strand it with no ledger.
        steward = self.steward("Andromeda")
        res = await self.call(
            "GET", "/steward", internal_auth=True, session=f" {self.agent(steward)} "
        )
        self.assertEqual(res.status, 200)

    async def test_a_supplied_id_cannot_redirect_the_read_to_another_steward(self):
        mine = self.steward("Andromeda")
        theirs = self.steward("Whirlpool")
        res = await self.call(
            "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": theirs["id"]},
            internal_auth=True, session=self.agent(mine),
        )
        self.assertEqual(res.status, 403)
        self.assertEqual(_payload(res)["code"], "steward_identity_mismatch")

    async def test_a_supplied_repo_cannot_redirect_the_read(self):
        mine = self.steward("Andromeda")
        res = await self.call(
            "GET", "/steward", query={"owner": "other", "repo": "elsewhere", "id": mine["id"]},
            internal_auth=True, session=self.agent(mine),
        )
        self.assertEqual(res.status, 403)
        self.assertEqual(_payload(res)["code"], "steward_identity_mismatch")
    ...
    async def test_its_own_identity_in_the_query_is_not_a_mismatch(self):
        mine = self.steward("Andromeda")
        res = await self.call(
            "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": mine["id"]},
            internal_auth=True, session=self.agent(mine),
        )
        self.assertEqual(res.status, 200)

    async def test_a_session_that_is_not_a_steward_is_refused(self):
        self.steward("Andromeda")
        res = await self.call(
            "GET", "/steward", internal_auth=True, session="dashboard:chat-3-1730000000"
        )
        self.assertEqual(res.status, 403)
        self.assertEqual(_payload(res)["code"], "not_a_steward_session")

    async def test_a_read_with_no_session_key_is_refused(self):
        self.steward("Andromeda")
        res = await self.call("GET", "/steward", internal_auth=True)
        self.assertEqual(res.status, 403)
    ...
    async def test_the_browser_path_is_unchanged(self):
        ...
        # chat session could read any steward's ledger with a cookie and a header.
        steward = self.steward("Andromeda")
        res = await self.call("GET", "/steward", session=self.agent(steward))
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "missing_repo")

    async def test_a_work_write_needs_no_owner_repo_or_steward_id(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "PUT", "/steward/work",
            body={"number": 7, "phase": "claimed", "event": "took #7", "event_kind": "claim"},
            internal_auth=True, session=self.agent(steward),
        )
        self.assertEqual(res.status, 200)
        item = steward_store.read_work_item(OWNER, REPO, steward["id"], 7, self.root)
        self.assertEqual(item["phase"], "claimed")
        self.assertEqual([e["text"] for e in self.ledger(steward["id"])], ["took #7"])

    async def test_a_work_write_naming_another_steward_is_refused_and_writes_nothing(self):
        mine = self.steward("Andromeda")
        theirs = self.steward("Whirlpool")
        res = await self.call(
            "PUT", "/steward/work",
            body={"owner": OWNER, "repo": REPO, "steward_id": theirs["id"], "number": 7,
                  "phase": "claimed", "event": "stealing #7", "event_kind": "claim"},
            internal_auth=True, session=self.agent(mine),
        )
        self.assertEqual(res.status, 403)
        self.assertEqual(_payload(res)["code"], "steward_identity_mismatch")
        # Refused, not redirected: neither steward's ledger records the attempt.
        self.assertEqual(self.ledger(theirs["id"]), [])
        self.assertEqual(self.ledger(mine["id"]), [])

    async def test_a_work_write_from_a_non_steward_session_is_refused(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "PUT", "/steward/work",
            body={"number": 7, "phase": "claimed", "event": "took #7", "event_kind": "claim"},
        ...
        self.assertEqual(res.status, 403)
        self.assertEqual(self.ledger(steward["id"]), [])

    async def test_a_retired_stewards_session_is_told_it_is_retired(self):
        # Identified, then refused for the real reason. "Not a steward" to a steward that
        ...
        self.assertEqual(res.status, 409)
        self.assertEqual(_payload(res)["code"], "steward_retired")

    async def test_a_malformed_agent_body_is_still_400_not_403(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "PUT", "/steward/work", body=None, internal_auth=True, session=self.agent(steward)
        )
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "invalid_json")
...
class TestAgentReachableRoutes(_StewardRouteCase):
    ...
    ``/steward`` entry admits the whole segment and cannot tell ``GET /steward`` from
    ``PUT``/``DELETE /steward``. Deny-by-default at the handler is therefore the only
    ...
    async def test_only_the_two_ledger_routes_are_reachable_by_an_agent(self):
        ...
        for method, path in STEWARD_ROUTES:
            with self.subTest(route=f"{method} {path}"):
                res = await self.call(
                    method, path, internal_auth=True, session="dashboard:steward-c_absent",
                    **_MINIMAL[(method, path)],
                )
                code = _payload(res).get("code")
                if (method, path) in steward_routes._AGENT_REACHABLE:
                    # Reached the handler. It still refuses this particular caller —
                    # the session names no steward — but not with the route gate.
                    self.assertNotEqual(code, "agent_route_denied")
                else:
                    self.assertEqual(res.status, 403)
                    self.assertEqual(code, "agent_route_denied")

    async def test_pause_and_the_steward_mutations_are_refused(self):
        # These are the three that matter: with the internal secret and the prefix
        # match, an agent could otherwise pause, rename or RETIRE a steward — its own
        # or any other.
        steward = self.steward("Andromeda")
        session = f"dashboard:{steward['slot_key']}"
        for method, path, body in (
            ("POST", "/steward/pause", {"id": steward["id"], "paused": True}),
            ("PUT", "/steward", {"id": steward["id"], "name": "Whirlpool"}),
            ("DELETE", "/steward", {"id": steward["id"]}),
        ):
            with self.subTest(route=f"{method} {path}"):
                res = await self.call(
        ...
        after = steward_store.read_steward(OWNER, REPO, steward["id"], self.root)
    ...
    async def test_the_browser_still_reaches_every_route(self):
        ...
        res = await self.call(
            "POST", "/steward/pause",
            body={"owner": OWNER, "repo": REPO, "id": steward["id"], "paused": True},
        )
        self.assertEqual(res.status, 200)
        self.assertIs(_payload(res)["steward"]["enabled"], False)


# ── PUT /steward/work ───────────────────────────────────────────────────────


class TestWorkItems(_StewardRouteCase):
    async def test_it_upserts_the_item_and_logs_one_event_in_one_call(self):
        steward = self.steward("Andromeda")
        res = await self.call(
            "PUT", "/steward/work",
            body={
                "owner": OWNER, "repo": REPO, "steward_id": steward["id"], "number": 7,
        ...
        self.assertEqual(len(self.ledger(steward["id"])), 1)
    ...
    async def test_a_write_without_an_event_is_refused(self):
        ...
        res = await self.call(
            "PUT", "/steward/work",
            body={"owner": OWNER, "repo": REPO, "steward_id": steward["id"], "number": 7,
                  "phase": "implementing", "event_kind": "implement"},
        )
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "event_required")
        self.assertIsNone(steward_store.read_work_item(OWNER, REPO, steward["id"], 7, self.root))
    ...
    async def test_a_bad_number_is_400(self):
        ...
        for number in (0, -1, True, "7"):
            with self.subTest(number=number):
                res = await self.call(
                    "PUT", "/steward/work",
                    body={"owner": OWNER, "repo": REPO, "steward_id": steward["id"],
    ...
    async def test_a_second_editing_item_is_409(self):
        ...
        res = await self.call(
            "PUT", "/steward/work",
            body={"owner": OWNER, "repo": REPO, "steward_id": steward["id"], "number": 8,
    ...
    async def test_a_refused_write_leaves_no_ledger_line(self):
        ...
        texts = [e["text"] for e in self.ledger(steward["id"])]
    ...
    async def test_a_traversal_id_is_400_not_404_and_never_reaches_a_path(self):
        ...
        404 would be a lie (nothing was looked up) and 409 would tell a steward agent
        ...
        for bad in ("/etc/policy", "../../../../etc/policy", "c_1234abcd/../escape"):
            res = await self.call(
                "PUT", "/steward/work",
                body={"owner": OWNER, "repo": REPO, "steward_id": bad, "number": 7,
                      "event": "x", "event_kind": "claim"},
            )
            self.assertEqual(res.status, 400, f"{bad!r} should be a bad request")

    async def test_an_unknown_steward_is_404(self):
        res = await self.call(
            "PUT", "/steward/work",
            body={"owner": OWNER, "repo": REPO, "steward_id": "c_deadbeef", "number": 7,
                  "event": "x", "event_kind": "claim"},
        )
        self.assertEqual(res.status, 404)

    async def test_a_retired_steward_cannot_take_new_work(self):
        steward = self.steward("Andromeda")
        steward_store.retire_steward(OWNER, REPO, steward["id"], self.root)
        res = await self.call(
            "PUT", "/steward/work",
...
class TestSharedSkipIndex(_StewardRouteCase):
    """The index's whole value is that it crosses stewards, so every assertion here
    writes as one steward and reads as another."""

    async def _skip(self, steward_id: str, number: int, **extra) -> web.Response:
        return await self.call(
            "PUT", "/steward/work",
            body={
                "owner": OWNER, "repo": REPO, "steward_id": steward_id, "number": number,
                "phase": "skipped", "event": f"passing on #{number}", "event_kind": "skip",
                **extra,
            },
        )

    async def _page(self, steward_id: str) -> dict:
        return _payload(
            await self.call(
                "GET", "/steward", query={"owner": OWNER, "repo": REPO, "id": steward_id}
            )
        )

    async def test_a_skip_by_one_steward_is_visible_to_another_through_get_steward(self):
        author = self.steward("Andromeda")
        reader = self.steward("Whirlpool")
        ...
        # The membership list the OTHER steward tests an issue against before it
        ...
        self.assertIsNone(steward_store.read_work_item(OWNER, REPO, reader["id"], 42, self.root))
    ...
    async def test_a_needs_human_pass_is_accepted_and_indexed_for_the_fleet(self):
        ...
        This is the whole handover: the steward records ``skipped`` with a
        ``needs-decision`` scope, its claim is released, and the reason and scope are
        visible to every OTHER steward in the repo — so the next one does not
        ...
        for number, scope in ((42, "needs-decision"), (43, "needs-investigation")):
            with self.subTest(scope=scope):
                ...
                self.assertIn(_payload(res)["item"]["phase"], steward_store.TERMINAL_PHASES)
                page = await self._page(reader["id"])
        ...
        # Two passes, no slot held by either steward.
        self.assertEqual(steward_store.open_slot_count(OWNER, REPO, author["id"], self.root), 0)
    ...
    async def test_an_unknown_skip_scope_is_indexed_as_other(self):
        steward = self.steward("Andromeda")
        await self._skip(steward["id"], 42, skip_scope="vibes")
        self.assertEqual((await self._page(steward["id"]))["recent_skips"][0]["scope"], "other")

    async def test_the_reason_prefers_the_prose_over_the_progress_line(self):
        steward = self.steward("Andromeda")
        await self._skip(steward["id"], 42, why="the fix needs a schema migration")
        page = await self._page(steward["id"])
        self.assertEqual(
            page["recent_skips"][0]["reason"], "the fix needs a schema migration"
        )

    async def test_re_skipping_through_the_route_keeps_the_first_stewards_reason(self):
        first = self.steward("Andromeda")
        second = self.steward("Whirlpool")
        ...
        # The second steward is told what STANDS, not what it sent.
        self.assertEqual(_payload(res)["skip"]["reason"], "first reason")
        self.assertEqual(_payload(res)["skip"]["steward_id"], first["id"])
    ...
    async def test_a_phase_that_is_not_skipped_indexes_nothing(self):
        ...
        res = await self.call(
            "PUT", "/steward/work",
            body={"owner": OWNER, "repo": REPO, "steward_id": steward["id"], "number": 42,
    ...
    async def test_recent_skips_is_bounded_while_the_number_list_is_not(self):
        ...
        # prose is capped for the steward's context, membership must stay complete or
        # it answers "not skipped" for an issue that is.
        steward = self.steward("Andromeda")
        total = steward_routes._MAX_RECENT_SKIPS + 5
        for number in range(1, total + 1):
            await self._skip(steward["id"], number)
        page = await self._page(steward["id"])
        self.assertEqual(len(page["skipped_numbers"]), total)
        self.assertEqual(len(page["recent_skips"]), steward_routes._MAX_RECENT_SKIPS)


# ── one work write is all-or-nothing ────────────────────────────────────────


class TestWorkWriteRollback(_StewardRouteCase):
    """``PUT /steward/work`` commits three files or none of them.
    ...
    shared index that every other steward filters against, recorded as passed with
    nothing in the log saying who passed on it or why.

    The rollback is only correct because ONE lock — the steward's — is held from before
    ...
    ``append_event`` is failed with ``OSError`` rather than ``StewardStoreError``: the
    ...
    ``StewardStoreError -> 409`` decorator.
    """

    def _body(self, steward_id: str, number: int, **extra) -> dict:
        return {
            "owner": OWNER, "repo": REPO, "steward_id": steward_id, "number": number,
    ...
    async def test_a_failed_event_append_leaves_no_newly_created_item(self):
        steward = self.steward("Andromeda")
        path = steward_store.work_item_path(OWNER, REPO, steward["id"], 7, self.root)
        with mock.patch.object(
            steward_store, "append_event", side_effect=OSError("no space left on device")
        ):
            with self.assertRaises(OSError):
                await self.call(
                    "PUT", "/steward/work",
                    body=self._body(steward["id"], 7, phase="claimed"),
                )
        ...
        # would also occupy one of the steward's open slots for an issue it never took.
        self.assertIsNone(steward_store.read_work_item(OWNER, REPO, steward["id"], 7, self.root))
        self.assertFalse(path.exists())
        self.assertEqual(self.ledger(steward["id"]), [])
        self.assertEqual((await self._page(steward["id"]))["items"], [])
    ...
    async def test_a_rollback_restores_the_value_a_committed_write_left(self):
        ...
        await self.call(
            "PUT", "/steward/work",
            body=self._body(
                steward["id"], 7, phase="claimed", next="read the traceback",
                event="took it",
            ),
        )
        with self._a_competing_write_commits_before_the_lock(steward["id"], 7):
            with self.assertRaises(OSError):
                await self.call(
                    "PUT", "/steward/work",
                    body=self._body(
                        steward["id"], 7, phase="implementing", event="starting",
        ...
        self.assertEqual(item["phase"], "claimed")
        self.assertEqual([e["text"] for e in self.ledger(steward["id"])], ["took it"])
    ...
    async def test_a_rollback_does_not_delete_an_item_committed_before_it_locked(self):
        ...
        removes a work item that committed — and with it that steward's claim on the
        issue, which is the state this whole route exists to keep honest.
        """
        steward = self.steward("Whirlpool")
        with self._a_competing_write_commits_before_the_lock(steward["id"], 8):
            with self.assertRaises(OSError):
                await self.call(
                    "PUT", "/steward/work",
                    body=self._body(
                        steward["id"], 8, phase="investigating", event="starting",
                        event_kind="investigate",
                    ),
                )

        item = steward_store.read_work_item(OWNER, REPO, steward["id"], 8, self.root)
        self.assertIsNotNone(item, "the rollback deleted an item another request created")
        assert item is not None
        self.assertEqual(item["next"], "written by the other request")

    @contextlib.contextmanager
    def _a_competing_write_commits_before_the_lock(self, steward_id: str, number: int):
        """One competing work write, committed at the last moment before this
        transaction can take the steward lock, with this request's ledger append failing.

        The seam is ``_steward_lock_path``: whoever is about to lock has to resolve the
        ...
        with mock.patch.object(
            steward_store, "_steward_lock_path", side_effect=_commit_the_other_request
        ), mock.patch.object(
            steward_store, "append_event", side_effect=OSError("no space left on device")
        ):
            yield
    ...
    async def test_a_competing_write_waits_for_the_whole_transaction(self):
        ...
        await self.call(
            "PUT", "/steward/work",
            body=self._body(steward["id"], 7, phase="claimed", event="took it"),
        )
        blocked = threading.Thread(
            target=steward_store.upsert_work_item,
            args=(OWNER, REPO, steward["id"], 7, {"next": "the later write"}, self.root),
            daemon=True,
        )
        ...
        with mock.patch.object(
            steward_store, "append_event", side_effect=_start_a_competing_write_then_fail
        ):
            with self.assertRaises(OSError):
        ...
        self.assertEqual(
            held_while_running, [""],
            "a competing write landed while the transaction held the steward lock",
        )
    ...
    async def test_a_work_write_does_not_deadlock_on_another_stewards_skip_write(self):
        """The lock held across the transaction must be the STEWARD's, not the shared one.

        A steward's progress write holds its steward lock for all three writes and takes the
        repo-wide index lock from inside it, for one read-modify-write. So while one
        steward is mid-transaction, another steward's own pass — its steward lock, then the
        shared index — must still go through.

        Driven from OUTSIDE the request: this parks the request at its last write and
        runs the other steward's pass from the test's own thread, so the assertion does
        ...
        one — would park every other steward in the repo behind it, and the join below
        would time out.
        """
        author = self.steward("Andromeda")
        other = self.steward("Whirlpool")
        real_append = steward_store.append_event
        ...
        def _the_other_stewards_pass() -> None:
            # Its own steward lock, then the shared index — the same order the parked
            # request is in the middle of, from a different steward.
            steward_store.upsert_work_item(
                OWNER, REPO, other["id"], 99, {"phase": "skipped"}, self.root
            )
            steward_store.record_skip(
                OWNER, REPO, 99, "already fixed upstream", "already-fixed",
                other["id"], self.root,
            )
            finished.append(True)

        with mock.patch.object(steward_store, "append_event", side_effect=_park_at_the_last_write):
            ...
            try:
                ...
                self.assertTrue(
                    finished,
                    "another steward's pass could not complete while this request was "
        ...
        self.assertEqual(res.status, 200)
        # Both passes are indexed: neither steward's write was dropped or rolled back.
        self.assertEqual(
            sorted(steward_store.read_skips(OWNER, REPO, self.root)), ["42", "99"]
        )
        item = steward_store.read_work_item(OWNER, REPO, other["id"], 99, self.root)
        assert item is not None
    ...
    async def test_a_rollback_never_removes_a_skip_another_request_committed_first(self):
        ...
        afterwards names the same steward, reason and scope for either of them — so
        ...
        author = self.steward("Andromeda")
        reader = self.steward("Whirlpool")
        real_record_skip = steward_store.record_skip
        ...
        with mock.patch.object(
            steward_store, "record_skip", side_effect=_land_the_other_pass_first
        ), mock.patch.object(
            steward_store, "append_event", side_effect=OSError("no space left on device")
        ):
            with self.assertRaises(OSError):
        ...
        # un-indexing it would put #42 back in front of every steward with nothing in
        # the log saying it had been decided.
        standing = steward_store.read_skips(OWNER, REPO, self.root)
        ...
        self.assertIsNone(steward_store.read_work_item(OWNER, REPO, author["id"], 42, self.root))
    ...
    async def test_a_failed_event_append_leaves_an_existing_item_byte_identical(self):
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(OSError):
                await self.call(
                    "PUT", "/steward/work",
                    body=self._body(
                        steward["id"], 7, phase="implementing", next="write the failing test",
        ...
        # progress the steward never made.
        self.assertEqual(
            steward_store.read_work_item(OWNER, REPO, steward["id"], 7, self.root),
            json.loads(before),
        )
    ...
    async def test_a_failed_event_append_leaves_the_shared_skip_index_unchanged(self):
        ...
        self.assertEqual(steward_store.read_skips(OWNER, REPO, self.root), {})
        # Asserted from the OTHER steward's page, because visible-to-the-fleet is the
        # property: an entry left here is one every other steward filters #42 out on.
        page = await self._page(reader["id"])
        self.assertEqual(page["skipped_numbers"], [])
        self.assertEqual(page["recent_skips"], [])
        self.assertIsNone(steward_store.read_work_item(OWNER, REPO, author["id"], 42, self.root))
    ...
    async def test_a_rollback_never_removes_a_skip_this_request_did_not_add(self):
        ...
        ``record_skip`` keeps the FIRST steward's reason, so a re-skip writes nothing
        and has nothing to undo. Removing by key would erase the standing decision —
        the earlier steward's audit trail — and put the issue back in front of every
        steward in the fleet. Both owners are covered: another steward's entry, and this
        steward's own from an earlier request, which a ``steward_id``-only check would get
        wrong.
        """
        first = self.steward("Andromeda")
        second = self.steward("Whirlpool")
        for author, retrier in ((first, second), (first, first)):
            with self.subTest(same_steward=author["id"] == retrier["id"]):
                await self.call(
                    "PUT", "/steward/work",
                    body=self._body(
                ...
                with mock.patch.object(
                    steward_store, "append_event", side_effect=OSError("no space left")
                ):
                    with self.assertRaises(OSError):
                        await self.call(
                            "PUT", "/steward/work",
                ...
                self.assertEqual(steward_store.read_skips(OWNER, REPO, self.root), standing)
    ...
    async def test_the_happy_path_still_returns_the_item_the_event_and_the_skip(self):
        ...
        res = await self.call(
            "PUT", "/steward/work",
            body=self._body(
                steward["id"], 42, phase="skipped", why="needs an owner decision",
        ...
        self.assertEqual(
            steward_store.read_work_item(OWNER, REPO, steward["id"], 42, self.root)["phase"],
            "skipped",
        )
        self.assertEqual([e["text"] for e in self.ledger(steward["id"])], ["passing on #42"])
        self.assertEqual((await self._page(reader["id"]))["skipped_numbers"], [42])
...
class TestPause(_StewardRouteCase):
    ...
    async def test_resuming_clears_the_reason(self):
        ...
        resumed = _payload(
            await self.call(
                "POST", "/steward/pause",
                body={"owner": OWNER, "repo": REPO, "id": steward["id"], "paused": False},
            )
        )["steward"]
        self.assertIs(resumed["enabled"], True)
        # A stale reason on a running steward would explain why it is stopped.
    ...
    async def test_paused_must_be_a_boolean(self):
        ...
        res = await self.call(
            "POST", "/steward/pause",
            body={"owner": OWNER, "repo": REPO, "id": steward["id"], "paused": "yes"},
        )
        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "invalid_paused")


# ── stopping a steward revokes its execution BEFORE the route answers ───────


class TestStopRevokesExecution(_StewardRouteCase):
    """Pause and retire must un-arm the steward by the time they respond.

    Writing the record is not stopping the steward. What gives a steward a turn is its
    ...
    one more unattended, auto-approved turn on a steward a human had just stopped.
    ...
    def _armed(self, steward: dict) -> tuple[_Slot, web.Application, _Nudge]:
        """A steward that is genuinely running: trusted slot, active loop."""
        slot = _Slot(trust=True)
        app = web.Application()
        app["state"] = _State(slot)
        return slot, app, _Nudge(steward["slot_key"])

    async def _stop(self, app: web.Application, svc: _Nudge, **body) -> web.Response:
        with mock.patch.object(steward_runtime, "_autonudge_instance", lambda: svc):
            if "paused" in body:
                method, path = "POST", "/steward/pause"
            else:
                method, path = "DELETE", "/steward"
            return await self.call(method, path, body=body, app=app)

    async def test_pausing_clears_trust_and_deactivates_the_loop(self):
        steward = self.steward("Andromeda")
        slot, app, svc = self._armed(steward)
        res = await self._stop(
            app, svc, owner=OWNER, repo=REPO, id=steward["id"], paused=True, reason="hold"
        )
        self.assertEqual(res.status, 200)
    ...
    async def test_retiring_clears_trust_and_deactivates_the_loop(self):
        steward = self.steward("Andromeda")
        slot, app, svc = self._armed(steward)
        res = await self._stop(app, svc, owner=OWNER, repo=REPO, id=steward["id"])
        self.assertEqual(res.status, 200)
        self.assertTrue(_payload(res)["steward"]["retired_at"])
        self.assertFalse(slot._trust)
        self.assertFalse(svc.loop.active)

    async def test_resuming_leaves_the_steward_armed(self):
        """The inverse must not fire: resuming is not stopping, and the watchdog is
        what brings a live steward back — re-arming here would give one slot two loops."""
        steward = self.steward("Andromeda")
        slot, app, svc = self._armed(steward)
        res = await self._stop(app, svc, owner=OWNER, repo=REPO, id=steward["id"], paused=False)
        self.assertEqual(res.status, 200)
    ...
    async def test_a_revocation_failure_still_answers_200(self):
        ...
        steward = self.steward("Andromeda")
        _slot, app, svc = self._armed(steward)
        with mock.patch.object(
            steward_runtime, "revoke_steward_execution",
...
class TestUpdateRevokesWhenAutoApprovalIsWithdrawn(_StewardRouteCase):
    """``PUT /steward`` is the OTHER way a steward stops being auto-approved.
    ...
    The transition is read off the steward RECORD either side of the write, not off the
    ...
    async def _patch(self, app: web.Application, svc: _Nudge, **body) -> web.Response:
        with mock.patch.object(steward_runtime, "_autonudge_instance", lambda: svc):
            return await self.call("PUT", "/steward", body=body, app=app)

    async def test_turning_unattended_off_revokes_before_the_route_answers(self):
        steward = self.steward("Andromeda")  # unattended defaults to True
        slot, app, svc = self._armed(steward)
        res = await self._patch(
            app, svc, owner=OWNER, repo=REPO, id=steward["id"], unattended=False
        )
        self.assertEqual(res.status, 200)
        self.assertIs(_payload(res)["steward"]["unattended"], False)
    ...
    async def test_turning_unattended_on_does_not_revoke(self):
        ...
        would disarm the steward the same request just switched on."""
        steward = self.steward("Andromeda", unattended=False)
        slot, app, svc = self._armed(steward)
        res = await self._patch(
            app, svc, owner=OWNER, repo=REPO, id=steward["id"], unattended=True
        )
        self.assertEqual(res.status, 200)
        self.assertIs(_payload(res)["steward"]["unattended"], True)
        self.assertTrue(slot._trust)
        self.assertTrue(svc.loop.active)

    async def test_a_patch_without_unattended_leaves_an_armed_steward_armed(self):
        """A rename is not a stop. Revoking on every patch would make editing a live
        steward's model or name cost it its next turn."""
        steward = self.steward("Andromeda")
        slot, app, svc = self._armed(steward)
        res = await self._patch(
            app, svc, owner=OWNER, repo=REPO, id=steward["id"], name="Whirlpool"
        )
        self.assertEqual(res.status, 200)
        self.assertEqual(_payload(res)["steward"]["name"], "Whirlpool")
        self.assertTrue(slot._trust)
        self.assertTrue(svc.loop.active)

    async def test_repeating_unattended_true_is_not_a_transition(self):
        """``unattended: true`` on an already-unattended steward changes nothing, so it
    ...
    async def test_a_patch_to_an_attended_steward_does_not_take_a_humans_trust(self):
        """``unattended: false`` on a steward that was ALREADY attended is not a
        withdrawal. It matters because ``revoke_steward_execution`` also clears
    ...
    async def test_disabling_through_the_patch_route_revokes_too(self):
        """``enabled`` is patchable here as well, and an unattended steward that stops
        ...
        slot, app, svc = self._armed(steward)
        res = await self._patch(
            app, svc, owner=OWNER, repo=REPO, id=steward["id"], enabled=False
        )
        self.assertEqual(res.status, 200)
        self.assertIs(_payload(res)["steward"]["unattended"], True)
        self.assertFalse(slot._trust)
    ...
    async def test_a_withdrawal_without_a_dashboard_is_not_a_500(self):
        ...
        res = await self.call(
            "PUT", "/steward",
            body={"owner": OWNER, "repo": REPO, "id": steward["id"], "unattended": False},
        )
        self.assertEqual(res.status, 200)
        after = steward_store.read_steward(OWNER, REPO, steward["id"], self.root)
        assert after is not None
...
class TestSettings(_StewardRouteCase):
    async def test_get_returns_defaults_for_a_never_configured_repo(self):
        res = await self.call("GET", "/stewards/settings", query={"owner": OWNER, "repo": REPO})
        self.assertEqual(res.status, 200)
        settings = _payload(res)["settings"]
    ...
    async def test_put_merges_and_leaves_untouched_fields_alone(self):
        ...
        second = _payload(
            await self.call(
                ...
                body={
                    ...
                    "settings": {"commit_trailer": "Steward: {name}"},
                },
            )
        )["settings"]
        ...
        self.assertEqual(second["commit_trailer"], "Steward: {name}")
        self.assertEqual(
        ...
        self.assertEqual(
            _payload(
                await self.call("GET", "/stewards/settings", query={"owner": OWNER, "repo": REPO})
            )["settings"],
            second,
        )
...
class TestNothingWaitsForAHuman(_StewardRouteCase):
    """A steward never parks an issue on a person, so neither endpoint that used to
    express that exists: no queue of held items, and no channel for a human to
    answer a steward mid-turn.
    ...
    def test_no_handler_for_either_survives_in_the_module(self):
        ...
        for name in ("_handle_steward_guidance", "_handle_steward_escalations", "_escalations"):
            with self.subTest(symbol=name):
                self.assertFalse(hasattr(steward_routes, name))
        ...
        for method, path in RETIRED_ROUTES:
            with self.subTest(route=f"{method} {path}"):
                request = _request(
                    ...
                    body={"owner": OWNER, "repo": REPO, "id": steward["id"],
                          "number": 7, "text": "do X"},
                    app=app,
                )
...
class TestIssueComment(_StewardRouteCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    async def test_the_cached_timeline_is_dropped_so_the_claim_reads_back(self):
        # Without this the steward's next timeline read is served the pre-comment
...
#: Every body-taking steward route, with the field a non-finite number is smuggled in
...
_NON_FINITE_BODIES: tuple[tuple[str, str, str], ...] = (
    ("PUT", "/stewards/settings", '{"owner": "%s", "repo": "%s", '
                                  '"settings": {"claim_ttl_hours": 1e309}}'),
    ("POST", "/stewards", '{"owner": "%s", "repo": "%s", "name": "Bode", "max_open": 1e309}'),
    ("PUT", "/steward", '{"owner": "%s", "repo": "%s", "id": "c_dead", "max_open": NaN}'),
    ("DELETE", "/steward", '{"owner": "%s", "repo": "%s", "id": "c_dead", "junk": -Infinity}'),
    ("POST", "/steward/pause", '{"owner": "%s", "repo": "%s", "id": "c_dead", '
                               '"paused": true, "junk": Infinity}'),
    ("PUT", "/steward/work", '{"owner": "%s", "repo": "%s", "steward_id": "c_dead", '
                             '"number": 7, "phase": "claimed", "event": "took it", '
                             '"event_kind": "claim", "pr_number": 1e309}'),
)


class TestNonFiniteNumbersAreRefused(_StewardRouteCase):
    ...
    async def test_the_reviewed_settings_field_is_the_400_and_nothing_is_written(self):
        ...
        res = await self.call("PUT", "/stewards/settings", body=body)

        self.assertEqual(res.status, 400)
        self.assertEqual(_payload(res)["code"], "non_finite_number")
        # The refusal is BEFORE the store, so the repo keeps its default and no
        # settings file was created.
        self.assertFalse(steward_store.settings_path(OWNER, REPO, self.root).is_file())
        self.assertEqual(
            steward_store.read_settings(OWNER, REPO, self.root)["claim_ttl_hours"],
            steward_store.DEFAULT_SETTINGS["claim_ttl_hours"],
        )
    ...
    async def test_a_nested_non_finite_number_is_found(self):
        ...
        steward = self.steward("Andromeda")
        body = json.loads(
            '{"owner": "%s", "repo": "%s", "steward_id": "%s", "number": 7, '
            '"phase": "claimed", "event": "took it", "event_kind": "claim", '
            '"ci_state": {"runs": [{"attempts": 1e309}]}}' % (OWNER, REPO, steward["id"])
        )

        res = await self.call("PUT", "/steward/work", body=body)
    ...
    async def test_the_agent_leg_is_refused_too(self):
        ...
        no owner/repo/steward id — identity comes from the session — so the refusal has
    ...
    async def test_a_legitimate_number_is_unaffected(self):
        ...
        real caller sends."""
        steward = self.steward("Andromeda", max_open=2)
        settings = _payload(
            await self.call(
                "PUT", "/stewards/settings",
                body={"owner": OWNER, "repo": REPO, "settings": {"claim_ttl_hours": 12}},
        ...
        updated = _payload(
            await self.call(
                "PUT", "/steward",
                body={"owner": OWNER, "repo": REPO, "id": steward["id"], "max_open": 5},
            )
        )["steward"]
