...
class TestApiServerSpawn:
    ...
    async def test_spawn_with_agent_warms_project_cache_first(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.spawn_warm.warm_project_agent_names", warm)
        # The warm only ever touches an ALLOWLISTED cwd; stand in for a config
        # whose allowed_roots admit tmp_path.
        monkeypatch.setattr(
            "junction.spawn_warm.validate_cwd",
...
class TestApiServerAuth:
    ...
    async def _start(self, tmp_path, monkeypatch, **kwargs):
        ...
        #  - server.py binds ``from junction.config import data_home`` at module
        #    scope (its own name) — used for the .local_secret write.
        #  - state.py binds ``from junction.config.loader import config_dir``.
        #  - token_auth.py imports ``config_dir`` LAZILY inside functions from
        #    junction.config.loader, so patch the real ``loader.config_dir``.
        # All three are REAL module attributes (loader has no __getattr__), so
        # monkeypatch restores them cleanly. Do NOT patch
        # ``junction.config.config_dir`` — that name is served by a PEP-562 lazy
...
class TestApiJunctionConfig:
    """Tests for PUT /api/config/junction inline validation."""
    ...
    def _make_app(tmp_path):
        ...
        app = web.Application()
        app.router.add_get("/api/config/junction", handlers.api_junction_config)
        app.router.add_put("/api/config/junction", handlers.api_junction_config)
        return app
    ...
    async def test_put_happy_path(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_max_turns": 50}})
            assert resp.status == 200
            import json
    ...
    async def test_put_rejects_bool(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_max_turns": True}})
    ...
    async def test_put_rejects_out_of_range(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"max_subagents": 999}})
            assert resp.status == 400
    ...
    async def test_put_persists_subagent_auto_max(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_auto_max": 32}})
            assert resp.status == 200
            import json
    ...
    async def test_put_flags_restart_required_for_startup_read_keys(
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": settings})
            assert resp.status == 200
    ...
    async def test_put_no_restart_when_startup_key_resent_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.dashboard.handlers.sel", lambda: MagicMock())
        monkeypatch.setattr(
            "junction.dashboard.handlers.agents._regen_conductor", lambda: None, raising=False
        )
    ...
    async def test_put_flags_restart_when_startup_key_set_for_the_first_time(
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_max_turns": 40}})
            assert resp.status == 200
            assert (await resp.json())["restart_required"] is True
    ...
    async def test_put_does_not_flag_restart_for_live_keys(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"conductor_skill": True}})
            assert resp.status == 200
            assert (await resp.json())["restart_required"] is False
    ...
    async def test_put_rejects_subagent_auto_max_above_ceiling(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_auto_max": 9999}})
            assert resp.status == 400
    ...
    async def test_put_rejects_subagent_auto_max_below_floor(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"subagent_auto_max": 1}})
            assert resp.status == 400
    ...
    async def test_put_rejects_max_subagents_below_fixed_floor(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            for bad in (1, 2):
                resp = await c.put("/api/config/junction", json={"agent": {"max_subagents": bad}})
                assert resp.status == 400
            # 0 (auto) and a valid pin (>= 3) are accepted.
            for ok in (0, 3):
                resp = await c.put("/api/config/junction", json={"agent": {"max_subagents": ok}})
                assert resp.status == 200
    ...
    async def test_put_corrupt_persisted_auto_max_clamped_to_ceiling(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"max_subagents": 9999}})
            assert resp.status == 400
    ...
    async def test_put_rejects_non_dict_agent(self, tmp_path, monkeypatch):
        monkeypatch.setattr("junction.dashboard.handlers.sel", lambda: MagicMock())
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": "not a dict"})
            assert resp.status == 400
    ...
    async def test_put_rejects_unrecognized_keys(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.json").write_text('{"agent": {}}')
        async with TestClient(TestServer(self._make_app(tmp_path))) as c:
            resp = await c.put("/api/config/junction", json={"agent": {"unknown_key": 42}})
            assert resp.status == 400
...
class TestSlackProfileMissingScope:
    ...
    async def test_missing_scope_names_the_needed_scope(self, tmp_path, monkeypatch):
        ...
        import junction.slack.handler as _handler
