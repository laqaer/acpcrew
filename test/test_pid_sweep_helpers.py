...
class TestKillPidTree:
    def test_rejects_non_positive_pid(self) -> None:
        ...
        from junction.session_pid import _kill_pid_tree
    ...
    def test_returns_root_killed_false_when_not_kiro(self) -> None:
        ...
        with (
            patch("junction.session_pid._is_managed_agent_process", return_value=False),
            patch("junction.acp.client._get_child_pids", return_value=[]),
        ):
            total, root_killed = _kill_pid_tree(99999)
    ...
    def test_kills_children_bottom_up(self) -> None:
        ...
        with (
            patch("junction.session_pid._is_managed_agent_process", return_value=True),
            patch("junction.acp.client._get_child_pids", return_value=[100, 200]),
            patch("os.kill", side_effect=fake_kill),
        ):
...
class TestSweepPidEntries:
    def test_prunes_dead_pids(self) -> None:
        from junction.session_pid import _sweep_pid_entries
    ...
    def test_reprobe_prunes_when_root_not_killed_but_dead(self) -> None:
        ...
        with (
            ...
            patch("junction.session_pid._is_managed_agent_process", return_value=True),
            patch("junction.session_pid._pid_in_spawn_grace", return_value=False),
            patch("junction.session_pid._kill_pid_tree", return_value=(1, False)),
        ):
            killed, dead, _ = _sweep_pid_entries(
                ["1:99999"],
                should_skip_tagged=lambda gw, p: False,
...
class TestPeriodicPidSweep:
    def test_only_sweeps_own_gateway_entries(self, session_pid_file: Path) -> None:
        from junction.session_pid import _periodic_pid_sweep
...
class TestKillConfirmedAndWriteback:
    def test_kills_confirmed_and_prunes_entries(self, session_pid_file: Path) -> None:
        from junction.session_pid import _kill_confirmed_and_writeback

        session_pid_file.write_text("1:99999\n1:88888\n")

        with (
            patch("junction.session_pid._kill_pid_tree", return_value=(1, True)),
            patch("junction.session_pid._write_back_pid_file") as mock_wb,
        ):
    ...
    def test_keeps_entry_on_kill_failure(self, session_pid_file: Path) -> None:
        ...
        with (
            patch("junction.session_pid._kill_pid_tree", return_value=(0, False)),
    ...
    def test_no_writeback_when_nothing_to_prune(self) -> None:
        ...
        with patch("junction.session_pid._write_back_pid_file") as mock_wb:
...
class TestPeriodicSweepIntegration:
    ...
    async def test_skip_sweep_when_pid_not_int(self) -> None:
        ...
        from junction.session_pid import _collect_active_pids
    ...
    async def test_catch_all_exception_does_not_crash(self, session_pid_file: Path) -> None:
        ...
        with patch(
            "junction.session_pid._periodic_pid_sweep",
...
class TestPidAgeSeconds:
    ...
    def test_young_pid_age_below_grace(self, tmp_path: Path) -> None:
        ...
        from junction.session_pid import _pid_age_seconds
    ...
    def test_non_linux_returns_none(self) -> None:
        ...
        with patch("junction.session_pid.sys.platform", "darwin"):
...
class TestPidInSpawnGrace:
    ...
    def test_macos_young_pid_returns_true(self) -> None:
        ...
        """
        from junction.session_pid import _pid_in_spawn_grace

        with (
            patch("junction.session_pid.sys.platform", "darwin"),
            patch("junction.session_pid._pid_age_seconds", return_value=30.0),
        ):
    ...
    def test_windows_returns_false(self) -> None:
        ...
        with patch("junction.session_pid.platform_compat.IS_WINDOWS", True):
    ...
    def test_linux_young_pid_returns_true(self) -> None:
        ...
        with patch("junction.session_pid._pid_age_seconds", return_value=30.0):
            assert _pid_in_spawn_grace(12345) is True
    ...
    def test_linux_old_pid_returns_false(self) -> None:
        ...
        with patch("junction.session_pid._pid_age_seconds", return_value=200.0):
            assert _pid_in_spawn_grace(12345) is False
    ...
    def test_linux_parse_failure_returns_true(self) -> None:
        ...
        with patch("junction.session_pid._pid_age_seconds", return_value=None):
            assert _pid_in_spawn_grace(12345) is True
...
class TestSweepGraceIntegration:
    ...
    def test_young_pid_not_selected_as_candidate(self, session_pid_file: Path) -> None:
        ...
        with (
            patch("os.kill"),  # alive
            patch("junction.session_pid._is_managed_agent_process", return_value=True),
            patch("junction.session_pid._pid_in_spawn_grace", return_value=True),
        ):
            killed_or_dead, candidates = _periodic_pid_sweep(my_gw, set())
    ...
    def test_non_linux_old_orphan_still_killed(self, session_pid_file: Path) -> None:
        ...
        with (
            ...
            patch("junction.session_pid._pid_age_seconds", return_value=9999.0),
            patch("junction.acp.client._get_child_pids", return_value=[]),
        ):
            killed, dead, _ = _sweep_pid_entries(
                ["1:99999"],
    ...
    def test_non_linux_young_orphan_spared(self, session_pid_file: Path) -> None:
        ...
        with (
            ...
            patch("junction.session_pid._is_managed_agent_process", return_value=True),
            patch("junction.session_pid.sys.platform", "darwin"),
            patch("junction.session_pid._pid_age_seconds", return_value=5.0),
            patch("junction.acp.client._get_child_pids", return_value=[]),
        ):
            killed, dead, _ = _sweep_pid_entries(
                ["1:99999"],
