"""Tests for the chat (sidebar) folder tools on the junction-dashboard server.
...
from junction.mcp_dashboard import _call_tool_inner, _list_tools
...
_FOLDERS = [
    {"id": "aaaaaaaaaaaa", "name": "junction", "parent_id": "", "history_count": 3},
...
def _verified_caller() -> Any:
    ...
    """
    with patch(
        "junction.mcp_dashboard._resolve_session_key_strict",
...
class TestFolderTree:
    def test_renders_paths_sessions_and_unfiled(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=_rows):
            out = _call_tool_inner("chat_folder_tree", {})
        # Derived human path, not just the leaf name.
        assert "junction/0811" in out
    ...
    def test_slot_pointing_at_unknown_folder_falls_back_to_unfiled(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_get):
            out = _call_tool_inner("chat_folder_tree", {})
    ...
    def test_folder_endpoint_error_is_not_reported_as_empty(self) -> None:
        with patch("junction.mcp_dashboard._get", return_value={"error": "Token required"}):
            out = _call_tool_inner("chat_folder_tree", {})
        assert out.startswith("Error:") and "Token required" in out

    def test_unexpected_body_shape_is_an_error(self) -> None:
        with patch("junction.mcp_dashboard._get", return_value={"folders": []}):
            out = _call_tool_inner("chat_folder_tree", {})
...
class TestFolderCreate:
    def test_creates_subfolder_under_existing_parent_path(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._post", return_value=made
        ) as mock_post:
            out = _call_tool_inner("chat_folder_create", {"name": "0812", "parent": "junction"})
        path, body = mock_post.call_args.args
        assert path == "/api/chat/folders"
        assert body == {"name": "0812", "parent_id": "aaaaaaaaaaaa"}
        assert "junction/0812" in out and "dddddddddddd" in out
    ...
    def test_a_redacted_segment_is_not_recreated_on_every_call(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_get), patch(
            "junction.mcp_dashboard._post", side_effect=_post
        ):
    ...
    def test_the_length_limit_is_measured_on_what_gets_stored(self) -> None:
        ...
        seg = "k" * 95
        with patch("junction.mcp_dashboard.redact", side_effect=lambda s: s + "x" * 20), patch(
            "junction.mcp_dashboard._get", side_effect=_rows
        ), patch("junction.mcp_dashboard._post") as mock_post:
    ...
    def test_a_name_that_only_overruns_after_redaction_is_refused(self) -> None:
        ...
        """
        with patch(
            "junction.mcp_dashboard.redact", side_effect=lambda s: s + "x" * 30
        ), patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._post"
        ) as mock_post:
    ...
    def test_mkdir_p_creates_missing_parent_segments(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            ...
            out = _call_tool_inner(
                "chat_folder_create", {"name": "week1", "parent": "junction/2026/august"}
            )
        # "junction" exists; "2026" and "august" are created, then the leaf.
    ...
    def test_name_is_required(self) -> None:
        ...
        with pytest.raises(ValidationError):
            _call_tool_inner("chat_folder_create", {"parent": "junction"})
...
class TestAmbiguousFolderPaths:
    ...
    DUPES = [
        {"id": "aaaaaaaaaaaa", "name": "junction", "parent_id": ""},
    ...
    def test_create_refuses_an_ambiguous_parent_path(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._get), patch(
            "junction.mcp_dashboard._post"
        ) as mock_post:
            out = _call_tool_inner(
                "chat_folder_create", {"name": "x", "parent": "junction/0811"}
            )
        assert out.startswith("Error:")
    ...
    def test_move_refuses_an_ambiguous_destination_path(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._get), patch(
            "junction.mcp_dashboard._patch"
        ) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move", {"folder": "junction", "new_parent": "junction/0811"}
            )
    ...
    def test_session_move_refuses_an_ambiguous_destination_path(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._get), patch(
            ...
            out = _call_tool_inner(
                ...
                {"session": "chat-1-100", "folder": "junction/0811"},
            )
        assert out.startswith("Error:")
        mock_patch.assert_not_called()
    ...
    def test_an_id_still_addresses_one_of_the_duplicates(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=self._get), patch(
            "junction.mcp_dashboard._patch", return_value={"ok": True}
        ) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session",
    ...
    def test_mkdir_p_does_not_add_a_third_duplicate_sibling(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._get), patch(
            ...
            out = _call_tool_inner(
                "chat_folder_create", {"name": "leaf", "parent": "junction/0811/deeper"}
            )
...
class TestSlashBearingFolderNames:
    ...
    def test_the_literal_folder_wins_when_no_nested_pair_exists(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._getter(self.LITERAL)), patch(
            "junction.mcp_dashboard._patch", return_value={"ok": True}
        ) as mock_patch:
            out = _call_tool_inner(
    ...
    def test_collision_between_the_two_readings_is_refused(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._getter(self.BOTH)), patch(
            "junction.mcp_dashboard._patch"
        ) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session", {"session": "chat-1-100", "folder": "A/B"}
    ...
    def test_a_reading_divergence_that_survives_the_path_render_is_refused(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=self._getter(padded)), patch(
            "junction.mcp_dashboard._patch"
        ) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session", {"session": "chat-1-100", "folder": "A/B"}
...
class TestFolderNameRedaction:
    ...
    def test_created_parent_segments_are_redacted_before_the_write(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            ...
            out = _call_tool_inner(
                "chat_folder_create", {"name": "leaf", "parent": f"junction/{self.LEAKY}"}
            )
...
class TestFolderMove:
    ...
    def test_move_to_root(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            ...
            out = _call_tool_inner(
                "chat_folder_move", {"folder": "junction/0811", "new_parent": "root"}
            )
    ...
    def test_unknown_folder_errors(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._post"
        ) as mock_post, patch("junction.mcp_dashboard._patch") as mock_patch:
    ...
    def test_resolve_only_walk_refuses_a_mid_path_duplicate(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_get), patch(
            ...
            out = _call_tool_inner("chat_folder_move", {"folder": "junction/0811/deeper"})
        assert out.startswith("Error:") and "share the same parent" in out
        assert "bbbbbbbbbbbb" in out and "cccccccccccc" in out
        mock_post.assert_not_called()
        mock_patch.assert_not_called()
...
class TestFolderMoveSession:
    def test_an_unverifiable_caller_cannot_file_another_session(self) -> None:
        ...
        """
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._resolve_session_key_strict", return_value=""
        ), patch("junction.mcp_dashboard._patch") as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session",
                {"session": "chat-3-300", "folder": "junction/0811"},
            )
        assert out.startswith("Error:")
    ...
    def test_moves_by_slot_key_into_a_path(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._patch", return_value={"ok": True, "folder_id": "bbbbbbbbbbbb"}
        ) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session",
                {"session": "chat-3-300", "folder": "junction/0811"},
    ...
    def test_unfile_to_top_level(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=_rows), patch(
            "junction.mcp_dashboard._patch", return_value={"ok": True, "folder_id": ""}
        ) as mock_patch:
...
class TestASubagentCannotOutrankItsParent:
    ...
    def test_a_subagent_is_shown_no_sessions(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed), patch(
            "junction.mcp_dashboard._resolve_session_key_strict",
    ...
    def test_a_subagent_WITH_a_slot_uses_that_slot(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows_with_sub), patch(
            "junction.mcp_dashboard._resolve_session_key_strict",
            return_value="subagent:abc123",
        ):
            out = _call_tool_inner("chat_folder_tree", {})
...
class TestTheFolderPolicyIsTheEndpointsNotThisServers:
    ...
    def test_an_apps_create_reaches_the_endpoint(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed), self._as(
            "chat-1-100"
        ), patch("junction.mcp_dashboard._post", return_value=made) as mock_post:
    ...
    def test_an_apps_move_reaches_the_endpoint(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed), self._as(
            "chat-1-100"
        ), patch("junction.mcp_dashboard._patch", return_value=moved) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move", {"folder": "junction/0811", "new_parent": "Travel"}
            )
        assert not out.startswith("Error:")
    ...
    def test_the_endpoints_ownership_refusal_is_surfaced_not_reinvented(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed), self._as(
            "chat-1-100"
        ), patch("junction.mcp_dashboard._patch", return_value=denied):
            out = _call_tool_inner(
                "chat_folder_move", {"folder": "junction/0811", "new_parent": "Travel"}
            )
        assert out.startswith("Error:")
    ...
    def test_an_app_can_still_file_its_own_session(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed), self._as(
            "chat-1-100"
        ), patch("junction.mcp_dashboard._patch", return_value={"ok": True}) as mock_patch:
            out = _call_tool_inner(
                "chat_folder_move_session",
                {"session": "chat-1-100", "folder": "junction/0811"},
            )
    ...
    def test_an_app_owned_linked_session_is_refused(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=rows), patch(
            "junction.mcp_dashboard._resolve_session_key_strict",
...
class TestPrivateSessionsAreInvisible:
    ...
    def test_no_archived_count_is_rendered(self) -> None:
        ...
        with patch("junction.mcp_dashboard._get", side_effect=_rows_with_history):
            out = _call_tool_inner("chat_folder_tree", {})
        assert "Work" in out
        assert "42" not in out and "archived" not in out

    def test_the_tree_omits_them(self) -> None:
        with patch("junction.mcp_dashboard._get", side_effect=self._mixed):
            out = _call_tool_inner("chat_folder_tree", {})
...
class TestTheVerifiedCallerKeyReachesTheRequest:
    ...
    def test_create_carries_the_verified_key(self):
        with patch(
            "junction.mcp_dashboard._resolve_session_key_strict", return_value=self.VERIFIED
        ), patch(
            "junction.mcp_dashboard._post", return_value={"target": "chat-2", "title": "w"}
        ) as post:
    ...
    def test_stop_carries_the_verified_key(self):
        with patch(
            "junction.mcp_dashboard._resolve_session_key_strict", return_value=self.VERIFIED
        ), patch("junction.mcp_dashboard._post", return_value={"ok": True}) as post:
    ...
    def test_read_carries_the_verified_key(self):
        with patch(
            ...
            "junction.mcp_dashboard._get", return_value={"messages": [], "total": 0}
        ) as get:
    ...
    def test_a_trimmed_transcript_invents_no_cursor_on_an_empty_window(self):
        ...
        with patch(
            ...
            "junction.mcp_dashboard._get", return_value={"messages": [], "total": 7}
        ):
            out = _call_tool_inner("session_read_message", {"target": "peer"})
    ...
    def test_an_unverifiable_caller_never_reaches_the_request(self):
        ...
        with patch(
            "junction.mcp_dashboard._resolve_session_key_strict", return_value=""
        ), patch("junction.mcp_dashboard._post") as post, patch(
            "junction.mcp_dashboard._get"
        ) as get:
            for tool, args in (
