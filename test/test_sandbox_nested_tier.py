"""Tests for nested-sandbox passthrough tier detection (JUNCTION_SANDBOX_LEVEL).
...
def clean_state(monkeypatch):
    ...
    monkeypatch.delenv("JUNCTION_SANDBOX_LEVEL", raising=False)
    monkeypatch.setattr(
...
class TestLauncherExportsLevel:
    ...
    def test_linux_launcher_templates_level_constant(self, level):
        ...
        assert 'os.environ["JUNCTION_SANDBOX_LEVEL"] = SANDBOX_LEVEL' in script
    ...
    def test_linux_launcher_level_export_after_scrub(self):
        ...
        level_pos = script.index('os.environ["JUNCTION_SANDBOX_LEVEL"]')
    ...
    def test_macos_env_prefix_carries_level(self, level, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert f"JUNCTION_SANDBOX_LEVEL={level}" in wrapped
            ...
            assert wrapped.index(f"JUNCTION_SANDBOX_LEVEL={level}") > last_u
            assert wrapped.index("JUNCTION_SANDBOX_ACTIVE=1") > last_u
        finally:
            if profile:
                os.unlink(profile)
...
class TestPassthroughTierComparison:
    ...
    def test_downgrade_detected_audited_and_warned(self, mock_detect, monkeypatch, caplog):
        ...
        monkeypatch.setenv("JUNCTION_SANDBOX_LEVEL", "standard")
    ...
    def test_unrecognized_level_value_treated_as_unknown(self, mock_detect, monkeypatch):
        ...
        monkeypatch.setenv("JUNCTION_SANDBOX_ACTIVE", "1")
        monkeypatch.setenv("JUNCTION_SANDBOX_LEVEL", "bogus-tier")
        monkeypatch.setattr(sandbox_mod, "_macos_sandbox_state", lambda: None)
        with patch("junction.sel.sel") as mock_sel:
    ...
    def test_downgrade_survives_sel_failure(self, mock_detect, monkeypatch, caplog):
        ...
        monkeypatch.setattr(sandbox_mod, "_sandbox_env_unset_args", lambda *a, **k: ["-u", "X"])
        argv = ["kiro-cli", "acp"]
        with patch("junction.sel.sel", side_effect=OSError("SEL down")):
            with caplog.at_level(logging.WARNING, logger="junction.sandbox"):
                result, cleanup = wrap_argv(argv, mode="strict")
        assert result[0] in sandbox_mod._ENV_BINARY_CANDIDATES
