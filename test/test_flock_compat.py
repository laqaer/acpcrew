...
class TestFlockCompat:
    def test_posix_delegates_to_real_fcntl(self):
        ...
        from junction import flock_compat
    ...
    def test_windows_fallback_is_a_noop(self, monkeypatch):
        ...
        # CLI (and `junction cloud` on Windows) can import the lock-using modules.
        sys.modules.pop("junction.flock_compat", None)
        ...
        try:
            fc = importlib.import_module("junction.flock_compat")
            ...
            monkeypatch.undo()
            sys.modules.pop("junction.flock_compat", None)
            importlib.import_module("junction.flock_compat")
    ...
    def test_cli_import_graph_has_no_bare_fcntl_import(self):
        ...
        # `junction.cli` at import time may do a bare `import fcntl` (POSIX-only)
        # — they must go through flock_compat. A future bare import would crash
        # `python -m junction cloud launch` on Windows before the handler runs.
        #
