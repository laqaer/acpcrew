...
class TestConfiguredPort:
    def test_returns_default_without_config(self, monkeypatch):
        ...
        with patch("junction.cli_server.JunctionConfig") as mock_cfg:
...
class TestCustomDomainUsesConfiguredPort:
    def test_prints_configured_port(self, monkeypatch, capsys, tmp_path):
        monkeypatch.setenv("JUNCTION_PORT", "4321")
        monkeypatch.setattr("junction.cli_setup.Path", lambda *a, **kw: tmp_path / "hosts")
        _maybe_setup_custom_domain()
...
class TestDashboardUrlUsesConfiguredPort:
    def test_input_prompt_contains_configured_port(self, monkeypatch, capsys):
        monkeypatch.setenv("JUNCTION_PORT", "5678")
        ...
        monkeypatch.setattr("junction.cli_setup.JunctionConfig.load", lambda: mock_cfg)
        monkeypatch.setattr("junction.cli_setup.config_path", lambda: MagicMock())
        monkeypatch.setattr("junction.cli_setup.socket.gethostbyname", lambda _: "10.0.0.1")
        monkeypatch.setattr("junction.cli_setup.socket.gethostname", lambda: "myhost")
