...
class TestSpawnRunSessionKeyRouting:
    def test_uses_env_var_when_set(self):
        """JUNCTION_SESSION_KEY env var is used as parent_session."""
        with patch("junction.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"JUNCTION_SESSION_KEY": "sess-from-env"}
        ):
...
class TestSendMessageCronSession:
    ...
    def test_default_notification_only(self):
        ...
        """
        with patch("junction.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"JUNCTION_SESSION_KEY": "dashboard:chat-1"}
        ):
            mock_post.return_value = {"ok": True}
    ...
    def test_cron_bare_send_attaches_caller_session(self):
        ...
        with patch("junction.mcp_core._post") as mock_post, patch.dict(
            "os.environ", {"JUNCTION_SESSION_KEY": "cron:abc123"}
        ):
...
class TestKnowledgeSearchCache:
    ...
    def _reset(self):
        import junction.mcp_core as mc

        mc._KNOWLEDGE_CACHE = None

    def test_reuses_store_when_db_unchanged(self, tmp_path):
        from junction.mcp_core import _get_knowledge_search
...
class TestSessionKeyHeaderError:
    ...
    def test_ascii_key_is_header_safe(self):
        from junction.mcp_core import _session_key_header_error
...
class TestDeployArtifactMCPTool:
    ...
    def test_deploy_artifact_blocked_by_scan(self):
        ...
        with patch("junction.mcp_core._post") as mock_post, \
                patch("junction.deploy.pending.add_pending") as mock_pending:
