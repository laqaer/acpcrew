...
from junction.cron import CronJob, CronSchedule, CronService, format_schedule
...
class TestMcpCronListTimezone:
    ...
    def _calls_as_the_fixture_owner(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("JUNCTION_SESSION_KEY", _TZ_OWNER)
    ...
    def test_cron_list_uses_job_timezone(self, tmp_path) -> None:
        ...
        with patch("junction.mcp_cron.CronService") as mock_cls, \
                patch("junction.mcp_cron.get_local_tz", return_value=mock_tz):
...
class TestSlackCronListTimezone:
    ...
    def test_uses_job_timezone(self, tmp_path) -> None:
        ...
        with patch(
            "junction.messaging.commands.get_local_tz",
            return_value=("America/New_York", ZoneInfo("America/New_York")),
        ), patch("junction.messaging.commands.compute_next_run_ts", return_value=None):
