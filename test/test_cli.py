...
from junction.cli_doctor import _doctor
from junction.cli_server import _update
...
def _write_agent_config(path: Path, *, tools: list[str], allowed: list[str], servers: dict) -> None:
    """Write a ``junction.json`` agent config with the given managed
...
def _healthy_agent_file(path: Path) -> None:
    """Write a ``junction.json`` whose managed MCP servers are all present
    ...
    (``mcp_cleanup.JUNCTION_BIN_MCP_SERVERS``) rather than hardcoded: doctor
    ...
    added — which is exactly how these fixtures broke when ``junction-computer``
    landed.
    """
    from junction.mcp_cleanup import JUNCTION_BIN_MCP_SERVERS

    refs = [f"@{name}" for name in JUNCTION_BIN_MCP_SERVERS]
    _write_agent_config(
        path,
        tools=refs,
        allowed=refs,
        servers={
            name: {
                # sys.executable, not a literal like /usr/local/bin/junction:
                ...
                # The subcommand is the server name minus the "junction-" prefix
                # ("junction-core" -> "mcp-core"), matching the real invocation.
                "args": [f"mcp-{name.split('-', 1)[1]}"],
            }
            for name in JUNCTION_BIN_MCP_SERVERS
        },
    )
...
def _pin_default_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr(JunctionConfig, "load_credentials", lambda self: {})
...
class TestDoctor:
    ...
    def _hermetic_embeddings_runtime(self, monkeypatch):
        ...
        import junction.cli_doctor as _doc
    ...
    def test_doctor_with_kiro(self, tmp_path):
        agent_file = tmp_path / "junction.json"
        ...
        with (
            patch(
                "junction.cli_doctor.shutil.which",
                side_effect=lambda b, **_kw: f"/usr/local/bin/{b}",
            ),
            patch("junction.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("junction.cli_doctor.subprocess.run", return_value=mock_run),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("junction.cli_doctor.is_local_only", return_value=True),
            patch("junction.cli_doctor.config_dir", return_value=tmp_path),
            patch("junction.cli_doctor.probe_server", side_effect=_noop_probe_server),
        ):
            _doctor()
    ...
    def test_doctor_windows_missing_whisper_ffmpeg_is_non_fatal(self, tmp_path, monkeypatch):
        ...
        def _cfg_with_stt() -> JunctionConfig:
            cfg = JunctionConfig()
            cfg.stt.enabled = True
            cfg.stt.provider = "whisper"
            return cfg

        monkeypatch.setattr(JunctionConfig, "load", classmethod(lambda cls: _cfg_with_stt()))
        ...
        with (
            patch("junction.cli_doctor.shutil.which", side_effect=_which),
            patch("junction.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("junction.cli_doctor.subprocess.run", return_value=mock_run),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("junction.cli_doctor.is_local_only", return_value=True),
    ...
    def test_doctor_faster_windows_arm_is_fatal_even_on_windows(
        ...
        def _cfg_with_faster() -> JunctionConfig:
            cfg = JunctionConfig()
            cfg.stt.enabled = True
            cfg.stt.provider = "faster"
            return cfg

        monkeypatch.setattr(JunctionConfig, "load", classmethod(lambda cls: _cfg_with_faster()))
        monkeypatch.setattr(_doc, "_find_whisper", lambda path=None: None)
        monkeypatch.setattr(_doc, "ensure_ffmpeg_in_path", lambda: None)
        ...
        with (
            patch("junction.cli_doctor.shutil.which", side_effect=lambda b, **_k: f"/bin/{b}"),
            patch("junction.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("junction.cli_doctor.subprocess.run", return_value=mock_run),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("junction.cli_doctor.is_local_only", return_value=True),
    ...
    def test_doctor_reports_platform_boot_error_without_crashing(self, tmp_path, capsys):
        ...
        boot_err = PlatformCompositionError(
            "profile=amazon resolved no companion; set JUNCTION_PROFILE=standalone"
        )
        with (
            patch(
                "junction.cli_doctor.shutil.which",
        ...
        assert "JUNCTION_PROFILE=standalone" in out

    def test_doctor_without_kiro(self, tmp_path):
        with (
            patch("junction.cli_doctor.shutil.which", return_value=None),
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("junction.cli_doctor.is_local_only", return_value=True),
            patch("junction.cli_doctor.config_dir", return_value=tmp_path),
        ):
    ...
    def test_doctor_remote_shows_ssh_tunnel_hint(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("no gateway")),
            patch("junction.cli_doctor.is_local_only", return_value=False),
            patch("junction.cli_doctor.config_dir", return_value=tmp_path),
            patch("junction.cli_doctor.machine_hostname", return_value="myhost"),
        ):
            try:
                _doctor()
            except SystemExit:
    ...
    def test_doctor_slack_workspace_allowed_ok(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("junction.cli_doctor.config_dir", return_value=tmp_path),
            patch("junction.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("junction.cli_doctor.JunctionConfig.load_credentials", return_value=slack_creds),
            patch("junction.slack.enterprise.validate_enterprise", return_value=True) as mock_ve,
        ):
            _doctor()
        out = capsys.readouterr().out
    ...
    def test_doctor_slack_workspace_not_allowed_flags_issue(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("junction.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("junction.cli_doctor.JunctionConfig.load_credentials", return_value=slack_creds),
            patch("junction.slack.enterprise.validate_enterprise", return_value=False),
        ):
            with pytest.raises(SystemExit) as exc:
                _doctor()
        assert exc.value.code == 1
...
class TestSetupWorkspaceDir:
    ...
    def test_uses_saved_path_as_default(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_setup._workspace_dir_file", lambda: ws_file)
        with patch("builtins.input", return_value=str(custom_dir)) as mock_input:
            from junction.cli_setup import _setup_workspace_dir
...
def _patch_path():
    ...
    return patch("junction.cli_server.Path", return_value=mock_path_inst)
...
def _git_resolvable():
    ...
    """
    with patch(
        "junction.platform.update_capability.trusted_system_bin",
...
class TestUpdateFailures:
    ...
    def test_pip_install_failure_exits(self, tmp_path, monkeypatch):
        ...
        with (
            _patch_path(),
            _git_resolvable(),
            patch("junction.cli_server.shutil.which", return_value=None),
            patch("junction.cli_server.build_frontend_sync"),
            patch("junction.cli._ensure_node"),
            ...
            patch("junction.dep_sync.locked_console_scripts", return_value=[]),
            patch("junction.dep_sync.venv_not_mapped_to", return_value=None),
...
class TestCronCli:
    ...
    def test_cron_update_whitespace_channel_skipped(self, tmp_path, capsys):
        with patch("junction.cli_commands.CronService") as mock_svc_cls:
            mock_svc = mock_svc_cls.return_value
    ...
    def test_cron_update_every_and_cron_exclusive(self, tmp_path, capsys):
        with patch("junction.cli_commands.CronService"):
            args = argparse.Namespace(
                cron_action="update",
                job_id="job1",
                name=None,
    ...
    def test_cli_argparse_cron_add_agent_flag(self) -> None:
        """`junction cron add ... --agent NAME` parses into args.agent."""
        import sys
        ...
        with patch.object(sys, "argv", argv), patch("junction.cli_commands._cron") as mock_cron:
    ...
    def test_cli_argparse_cron_update_agent_flag(self) -> None:
        """`junction cron update <id> --agent NAME` parses into args.agent."""
        import sys
...
class TestPortEnvValidatedAtEntry:
    """`main()` rejects an unusable JUNCTION_PORT before any subcommand runs.

    Type alone is not enough. 70000 parses as an int, so a type-only check let
    `JUNCTION_PORT=70000 junction service install` bake an unbindable port into
    ...
    def test_out_of_range_port_exits_before_dispatch(self, monkeypatch, capsys):
        ...
        for bad in ("70000", "0", "-1"):
            monkeypatch.setenv("JUNCTION_PORT", bad)
            dispatched = []
            with (
                patch.object(sys, "argv", ["junction", "cron", "list"]),
                patch("junction.cli_commands._cron", lambda _ns: dispatched.append(True)),
            ):
                from junction.cli import main
...
class TestSandboxActiveMarkerCleared:
    """cli.main() must drop an INHERITED JUNCTION_SANDBOX_ACTIVE marker.
    ...
    def test_main_clears_inherited_sandbox_active_marker(self, monkeypatch):
        ...
        monkeypatch.setenv("JUNCTION_SANDBOX_LEVEL", "strict")
        ...
        argv = ["junction", "cron", "list"]
        seen = {}

        def _capture(_ns):
            seen["marker"] = os.environ.get("JUNCTION_SANDBOX_ACTIVE")
            seen["level"] = os.environ.get("JUNCTION_SANDBOX_LEVEL")

        with patch.object(sys, "argv", argv), patch("junction.cli_commands._cron", _capture):
            from junction.cli import main

            main()
        assert seen.get("marker") is None
        assert seen.get("level") is None
        assert os.environ.get("JUNCTION_SANDBOX_ACTIVE") is None
        assert os.environ.get("JUNCTION_SANDBOX_LEVEL") is None
...
class TestDirectCliOverrideAttestation:
    def test_agent_command_pins_override_before_jail_gate(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JUNCTION_KIRO_BIN", str(executable))
...
class TestSetupTimezone:
    def test_auto_detect_from_tz_env(self, monkeypatch):
        ...
        from junction.cli_setup import _detect_system_timezone
    ...
    def test_input_or_skip_returns_none_on_empty_and_raises_on_eof(self, monkeypatch):
        ...
        from junction.cli_setup import _input_or_skip, _SetupAborted
    ...
    def test_timezone_retry_eof_propagates_setup_aborted(self, tmp_path, monkeypatch):
        ...
        cfg_file.write_text("{}")
        monkeypatch.setattr("junction.cli_setup.config_path", lambda: cfg_file)

        from junction.cli_setup import _setup_timezone, _SetupAborted
        ...
        with patch("builtins.input", _input):
            with patch("junction.cli_setup._detect_system_timezone", return_value=""):
                with pytest.raises(_SetupAborted):
                    _setup_timezone()
    ...
    def test_auto_detect_from_symlink(self, tmp_path, monkeypatch):
        ...
        from junction.cli_setup import _setup_timezone

        with patch("builtins.input", return_value="") as mock_input:
            with patch(
                "junction.cli_setup._detect_system_timezone",
...
class TestGetAlias:
    ...
    def test_returns_user_env(self, monkeypatch):
        ...
        from junction.cli_setup import _get_alias
...
class TestManifest:
    ...
    def _patch_template(
        self, content="name: Junction-{{ALIAS}}\ndisplay_name: Junction-{{ALIAS}}\n"
    ):
        ...
        return patch("junction.slack_manifest._pkg_files", return_value=mock_resource)

    def test_renders_alias_to_stdout(self, capsys):
        with self._patch_template():
            from junction.cli_setup import _manifest

            _manifest(alias="alice")
        out = capsys.readouterr().out
        assert "Junction-alice" in out
        assert "{{ALIAS}}" not in out

    def test_writes_to_output_file(self, tmp_path):
        out_file = tmp_path / "sub" / "out.yaml"
        with self._patch_template("name: Junction-{{ALIAS}}\n"):
            from junction.cli_setup import _manifest

            _manifest(alias="bob", output=str(out_file))
        assert out_file.exists()
        assert "Junction-bob" in out_file.read_text(encoding="utf-8")
    ...
    def test_exits_when_template_missing(self):
        ...
        with patch("junction.slack_manifest._pkg_files", return_value=mock_resource):
            from junction.cli_setup import _manifest
    ...
    def test_url_flag_prints_creation_link(self, capsys):
        ...
        assert "Junction-alice" in out  # alias substituted
...
class TestLogout:
    ...
    def test_logout_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.read_local_secret", lambda _port: "test-secret")

        from junction.cli_server import _logout
    ...
    def test_logout_gateway_not_running(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.read_local_secret", lambda _port: "")
    ...
    def test_logout_http_error(self, tmp_path, monkeypatch):
        ...
        with patch(
            "junction.cli_server.loopback_urlopen",
...
class TestStatus:
    ...
    def test_status_auth_required(self, capsys):
        ...
        from junction.cli_server import _status
    ...
    def test_status_unexpected_exception(self, capsys):
        ...
        with patch("junction.cli_server.loopback_urlopen", side_effect=RuntimeError("unexpected")):
            _status(self._make_args())
        out = capsys.readouterr().out
        assert "running" in out
...
class TestIsJunctionProcess:
    """Tests for _is_junction_process helper.
    ...
    process whose image name is ``python``/``python.exe`` (the venv junction.exe
    re-exec) is still classified by its command line.
    """

    def _cmdline(self, value):
        return patch(
            "junction.cli_server.platform_compat.process_command_line", return_value=value
        )

    def test_returns_true_for_junction(self):
        from junction.cli_server import _is_junction_process

        with self._cmdline("python3 -m junction.dashboard"):
            assert _is_junction_process(1234) is True

    def test_returns_true_for_junction_binary(self):
        from junction.cli_server import _is_junction_process

        with self._cmdline("/usr/bin/junction start"):
            assert _is_junction_process(1234) is True

    def test_returns_true_for_module_gateway_form(self):
        """Regression: the real service launch form
        ``python -m junction gateway`` must be recognized. Previously the
        matcher only accepted the dotted ``junction.gateway`` form, so
        ``junction stop`` no-op'd on service installs.

        Patched through the cross-platform ``process_command_line`` seam (the
        Windows port routes _is_junction_process through platform_compat rather
        than calling ``subprocess.check_output`` directly)."""
        from junction.cli_server import _is_junction_process

        real = (
            "/Users/x/.toolbox/tools/junction/3.1.0/bin/../python3.10/bin/"
            "python3.10 -m junction gateway\n"
        )
        with self._cmdline(real):
            assert _is_junction_process(54842) is True

    def test_returns_true_for_windows_python_reexec(self):
        # The venv junction.exe re-execs python.exe, so the gateway cmdline reads
        # `python.exe ...\Scripts\junction.exe gateway` (or `-m junction gateway`).
        # The token-parser (_args_look_like_junction) recognizes the console-script
        # basename ("junction"/"junction.exe") and the "-m junction gateway" module
    ...
    def test_returns_false_for_unrelated(self):
        ...
        with self._cmdline("nginx: worker process"):
            assert _is_junction_process(1234) is False

    def test_returns_false_for_broad_match(self):
        """Editing a junction file should NOT match — only gateway entry points."""
        from junction.cli_server import _is_junction_process

        with self._cmdline("vim /tmp/junction-notes.txt"):
            assert _is_junction_process(1234) is False

    def test_returns_false_when_cmdline_unavailable(self):
        # process_command_line returns "" on any failure (dead PID, WMI/ps error);
        # _is_junction_process must then fail closed (False), never raise.
        from junction.cli_server import _is_junction_process
...
class TestArgsLookLikeJunction:
    """Rigorous tests for the pure command-line classifier ``_args_look_like_junction``.
    ...
    matched — a false positive would let ``junction stop`` SIGTERM an unrelated
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "/Users/x/.toolbox/tools/junction/3.1.0/bin/../python3.10/bin/python3.10 -m junction gateway",
            "python3 -m junction gateway",
            "python -m junction dashboard",
            "python3.10 -m junction start",
            ...
            "Python.app/Contents/MacOS/Python -m junction gateway --no-open",
            "Python -m junction gateway",
            # Subcommand followed by trailing flags.
            "python -m junction gateway --no-open --port 7777",
            # Legacy dotted-submodule form.
            "python3 -m junction.gateway",
            "python3 -m junction.dashboard",
            # Console-script wrapper form.
            "/usr/local/bin/junction gateway",
            "/Users/x/.toolbox/bin/junction start",
            "junction dashboard",
    ...
    def test_matches_server_launch_forms(self, args):
        from junction.cli_server import _args_look_like_junction

        assert _args_look_like_junction(args) is True
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "vim /tmp/junction-notes.txt",  # editing a file named junction*
            "cat /var/log/junction_gateway.log",  # reading a junction log file
            "python -m junction",  # bare module, no server subcommand
            "python -m junction run /tmp/spec.md",  # task runner — NOT a port-bound server
            "python -m junction run gateway",  # "gateway" is a file arg to run, not the subcommand
            "python -m junction run start",  # "start" is a file arg to run, not the subcommand
            "python -m junction_other gateway",  # different package named junction_other
            "/usr/bin/junction",  # wrapper with no subcommand
    ...
    def test_rejects_non_server_processes(self, args):
        ...
        assert _args_look_like_junction(args) is False
    ...
    def test_unbalanced_quotes_do_not_raise(self):
        ...
        assert _args_look_like_junction('python -m junction gateway "') is True
    ...
    def test_subcommand_match_is_exact_case(self):
        ...
        assert _args_look_like_junction("python -m junction GATEWAY") is False
...
class TestStop:
    ...
    def _mock_sel(self):
        mock = MagicMock()
        return patch("junction.cli_commands.sel", return_value=mock)
    ...
    def _no_service(self):
        ...
        # test host happens to have ``junction.service`` installed.
        #
        ...
        # host.
        with (
            patch("junction.cli_server.service_controller.stop_service", return_value=False),
            patch(
                "junction.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
        ):
            yield
    ...
    def _ports(self, pids):
        return patch("junction.cli_server.platform_compat.find_listening_pids", return_value=pids)
    ...
    def _tool_absent(self, unpinned_at):
        ...
        return (
            ...
            patch("junction.cli_server.platform_compat.listening_pid_tool", return_value="lsof"),
            patch(
                "junction.cli_server.platform_compat.tool_outside_trusted_dirs",
    ...
    def test_a_tool_outside_the_pin_is_not_reported_as_missing(self, capsys):
        ...
        with (
            patch("junction.cli_server.sel", return_value=mock_sel),
            self._ports([]),
            available,
            tool,
            unpinned,
    ...
    def test_no_junction_process(self, capsys):
        # A listener exists but its cmdline isn't a junction gateway → refuse to kill.
        from junction.cli_server import _stop
    ...
    def test_successful_stop(self, capsys):
        ...
        ctx = [
            self._mock_sel(),
            self._ports([1234]),
            self._cmdline("python3 -m junction.dashboard"),
            patch("time.sleep"),
            patch("junction.cli_server.platform_compat.pid_exists", return_value=False),
        ]
        if sys.platform == "win32":
            ctx.append(
                patch("junction.cli_server.platform_compat.kill_process_tree", return_value=True)
            )
        else:
    ...
    def test_explicit_port_bypasses_service_short_circuit(self, capsys):
        ...
        with (
            self._mock_sel(),
            patch(
                "junction.cli_server.service_controller.stop_service",
                return_value=True,
...
class TestWaitForPidsExit:
    ...
    def test_empty_pid_list_returns_immediately(self):
        from junction.cli_server import _wait_for_pids_exit
    ...
    def test_zero_timeout_still_probes_once(self):
        ...
        with patch("junction.cli_server._pid_exited", return_value=False) as mock_exited:
    ...
    def test_returns_only_the_pids_still_alive(self):
        ...
        with patch("junction.cli_server._pid_exited", side_effect=lambda p: p != 9):
...
class TestRestart:
    ...
    def _fast_restart_ready(self, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server._RESTART_READY_TIMEOUT", 0)
        monkeypatch.setattr(
            "junction.cli_server._wait_gateway_ready",
    ...
    def _mock_sel(self):
        return patch("junction.cli_server.sel", return_value=MagicMock())

    def test_service_active_restarts_via_controller(self, capsys):
        from junction.cli_server import _restart

        with (
            self._mock_sel(),
            patch(
                "junction.cli_server.service_controller.restart_service",
                return_value=True,
            ) as mock_restart,
            patch("junction.cli_server._spawn_detached_gateway") as mock_spawn,
            patch("junction.cli_server.platform_compat.find_listening_pids") as mock_ports,
        ):
            _restart(None)
    ...
    def test_no_service_no_running_gateway_spawns_fresh(self, capsys):
        ...
        # ``junction restart`` after the gateway died, they should still
        ...
        with (
            ...
            patch(
                "junction.cli_server.platform_compat.find_listening_pids",
                return_value=[],
            ),
            patch(
                "junction.cli_server._spawn_detached_gateway",
                return_value=self._fake_proc(4321),
            ) as mock_spawn,
            patch("junction.cli_server._stop") as mock_stop,
        ):
            _restart(None)
    ...
    def test_toctou_stop_systemexit_is_swallowed_so_spawn_proceeds(self, capsys):
        ...
        with (
            ...
            patch("junction.cli_server._stop", side_effect=SystemExit(1)) as mock_stop,
            patch(
                "junction.cli_server._spawn_detached_gateway",
    ...
    def test_waits_for_incumbent_to_exit_before_spawning(self):
        ...
        The incumbent holds the ``JUNCTION_HOME`` flock for its whole graceful
        ...
        with (
            ...
            patch("junction.cli_server._is_junction_process", return_value=True),
            patch("junction.cli_server._stop"),
            patch("junction.cli_server._pid_exited", side_effect=fake_pid_exited),
            patch("junction.cli_server._print_token_url"),
            patch(
                "junction.cli_server._spawn_detached_gateway",
                return_value=self._fake_proc(5678),
            ) as mock_spawn,
    ...
    def test_refuses_to_spawn_when_incumbent_never_exits(self, capsys):
        ...
        with (
            ...
            patch("junction.cli_server._is_junction_process", return_value=True),
            patch("junction.cli_server._stop"),
            patch("junction.cli_server._pid_exited", return_value=False),
            patch("junction.cli_server._RESTART_STOP_TIMEOUT", 0),
            patch("junction.cli_server._spawn_detached_gateway") as mock_spawn,
        ):
            with pytest.raises(SystemExit) as exc:
                cli_server._restart(None)
    ...
    def test_unrelated_port_listener_is_not_waited_on(self):
        """A non-Junction listener must never gate the restart.
        ...
        refuse, so the wait set is filtered by ``_is_junction_process``.
        """
        from junction import cli_server
        ...
        with (
            ...
            patch("junction.cli_server._is_junction_process", return_value=False),
            patch("junction.cli_server._stop") as mock_stop,
            patch("junction.cli_server._pid_exited", return_value=False) as mock_exited,
            patch("junction.cli_server._print_token_url"),
            patch(
                "junction.cli_server._spawn_detached_gateway",
                return_value=self._fake_proc(5678),
        ...
        # _stop still runs (it owns the "not a Junction gateway" diagnostic)...
        mock_stop.assert_called_once_with(None)
    ...
    def test_spawn_detached_gateway_binds_requested_port(self, tmp_path, monkeypatch):
        ...
        """
        from junction.cli_server import _spawn_detached_gateway

        monkeypatch.setattr("junction.cli_server.config_dir", lambda: tmp_path)
        proc = MagicMock(pid=4321)
        with (
            patch("shutil.which", return_value="/usr/local/bin/junction"),
            patch("junction.cli_server.subprocess.Popen", return_value=proc) as mock_popen,
        ):
    ...
    def test_restart_passes_resolved_port_to_spawn(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.config_dir", lambda: tmp_path)
        with (
            patch("junction.cli_server.resolve_client_port", return_value=6776),
            patch("junction.cli_server.service_controller.restart_service", return_value=False),
            patch("junction.cli_server.platform_compat.find_listening_pids", return_value=[]),
            patch(
                "junction.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
    ...
    def test_spawn_detached_gateway_uses_junction_bin(self, tmp_path, monkeypatch):
        # When ``junction`` is on PATH, the detached child must invoke it
        ...
        assert argv == ["/usr/local/bin/junction", "gateway"]
    ...
    def test_spawn_detached_gateway_prefers_own_console_script(self, tmp_path, monkeypatch):
        # Two `junction` scripts can exist at once — an edition composes this
    ...
    def test_spawn_detached_gateway_prefers_own_junction_script(self, tmp_path, monkeypatch):
        ...
        # must respawn it, not a leftover `junction` earlier on PATH.
        from junction.cli_server import _spawn_detached_gateway
    ...
    def test_spawn_detached_gateway_ignores_non_junction_argv0(self, tmp_path, monkeypatch):
        # argv[0] is only trusted when it names the junction console script.
        # Anything else (a test runner, a wrapper, `python -m junction restart`)
        ...
        with (
            ...
            _spawn_detached_gateway()
        assert mock_popen.call_args.args[0] == ["/usr/local/bin/junction", "gateway"]
    ...
    def test_spawn_detached_gateway_ignores_missing_argv0_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.config_dir", lambda: tmp_path)
        monkeypatch.setattr(sys, "argv", [str(tmp_path / "ghost" / "junction"), "restart"])
    ...
    def test_spawn_detached_gateway_absolutizes_relative_argv0(self, tmp_path, monkeypatch):
        # Regression: `cd ~/checkout && .venv/bin/junction restart`. shutil.which()
        ...
        monkeypatch.setattr("junction.cli_server.config_dir", lambda: tmp_path)
        # Use the real per-platform venv console-script layout: `bin/junction` on
        # POSIX, `Scripts\junction.exe` on Windows. shutil.which() only accepts a
        ...
        if sys.platform == "win32":
            ...
            script_name = "junction.exe"
        else:
            venv_bin = tmp_path / ".venv" / "bin"
            script_name = "junction"
        ...
        with patch("junction.cli_server.subprocess.Popen", return_value=proc) as mock_popen:
            _spawn_detached_gateway()
    ...
    def test_spawn_detached_gateway_falls_back_to_python_m(self, tmp_path, monkeypatch):
        # Dev/Brazil-workspace installs may not have ``junction`` on
        # PATH globally. Fall back to ``python -m junction`` so the
        ...
        assert argv[1:] == ["-m", "junction", "gateway"]
...
class TestRestartReadinessVerdict:
    ...
    the ``JUNCTION_HOME`` ownership guard refused (exit 1, milliseconds later)
    ...
    def _drive(self, *, poll, ready_status, marker_pid, timeout=None):
        ...
        stack = [
            ...
            patch("junction.cli_server.run_marker.read_pid", side_effect=marker_pid),
            patch("junction.cli_server._gateway_owns_port", return_value=True),
            patch("junction.cli_server._probe_gateway_ready", return_value=ready_status),
            patch(
                "junction.cli_server._spawn_detached_gateway",
                return_value=MagicMock(pid=4321, poll=MagicMock(return_value=poll)),
            ),
            patch("junction.cli_server._print_token_url"),
            patch("junction.cli_server._RESTART_READY_TIMEOUT", 0 if timeout is None else timeout),
        ]
...
class TestResolveClientPort:
    ...
    def test_cli_flag_wins(self, monkeypatch, tmp_path):
        ...
        from junction.cli_server import resolve_client_port

        monkeypatch.setenv("JUNCTION_PORT", "9999")
        mock_cfg = MagicMock()
        mock_cfg.dashboard.url = "http://localhost:8888"
        with patch("junction.cli_server.JunctionConfig.load", return_value=mock_cfg):
            assert resolve_client_port(12345) == 12345

    def test_env_var_used_when_no_cli(self, monkeypatch):
        """JUNCTION_PORT env var wins over config when no --port passed."""
        from junction.cli_server import resolve_client_port
    ...
    def test_invalid_env_var_falls_through_to_config(self, monkeypatch):
        """A garbage JUNCTION_PORT must not crash; the helper falls through."""
        from junction.cli_server import resolve_client_port
    ...
    def test_config_url_hostname_only_falls_through_to_default(self, monkeypatch):
        ...
        with (
            patch("junction.cli_server.JunctionConfig.load", return_value=mock_cfg),
            # No gateway advertising itself — isolate from the dev box's markers.
            patch("junction.cli_server.run_marker.marker_ports", return_value=[]),
        ):
    ...
    def test_config_load_failure_falls_through_to_default(self, monkeypatch):
        ...
        monkeypatch.delenv("JUNCTION_PORT", raising=False)
        with patch("junction.cli_server.JunctionConfig.load", side_effect=RuntimeError("boom")):
            assert resolve_client_port(None) == 5476
...
class TestResolveClientPortRunMarker:
    ...
    Junction gateway process holds the port, and only when exactly one does.
    """
    ...
    def _cfg(self, url):
        ...
        return patch("junction.cli_server.JunctionConfig.load", return_value=mock_cfg)
    ...
    def _owned(self, ports):
        ...
        return patch(
            "junction.cli_server._gateway_owns_port", side_effect=lambda p: p in set(ports)
        )
    ...
    def test_discovery_failure_falls_through_to_default(self):
        ...
        with (
            self._cfg(""),
            patch(
                "junction.cli_server.run_marker.marker_ports",
...
class TestGatewayOwnsPort:
    ...
    `/tmp/junction gateway`.
    """

    def _sidecar(self, pid):
        return patch("junction.cli_server.run_marker.read_pid", return_value=pid)

    def _listeners(self, pids):
        return patch("junction.cli_server.platform_compat.find_listening_pids", return_value=pids)

    def _owner(self, uid):
        return patch("junction.cli_server.platform_compat.process_owner_uid", return_value=uid)

    def _me(self):
        return os.getuid() if hasattr(os, "getuid") else 0

    def _posix(self, value=True):
        return patch("junction.cli_server.platform_compat.IS_POSIX", value)

    def test_true_when_recorded_pid_holds_the_port_and_is_ours(self):
        from junction.cli_server import _gateway_owns_port
        ...
        with (
            ...
            patch("junction.cli_server._is_junction_process", return_value=True),
            patch("junction.cli_server.os.getuid", return_value=self._me(), create=True),
        ):
    ...
    def test_denies_on_non_posix(self):
        ...
        / JUNCTION_PORT, which is where it was before this fallback existed.
        """
        from junction.cli_server import _gateway_owns_port
    ...
    def test_false_when_pid_is_not_a_junction_process(self):
...
class TestEnsurePrerequisites:
    ...
    def test_returns_true_when_all_satisfied(self):
        from junction.cli_setup import _ensure_prerequisites

        with (
            patch("junction.cli_setup.shutil.which", return_value="/usr/bin/kiro-cli"),
            patch("junction.cli_doctor.subprocess.run", return_value=MagicMock(returncode=0)),
        ):
    ...
    def test_returns_true_when_optional_kiro_absent(self):
        ...
        with patch("junction.cli_setup.shutil.which", return_value=None):
            assert _ensure_prerequisites() is True
...
class TestDoctorStaleProjectDir:
    ...
    def test_doctor_detects_stale_project_dir(self, tmp_path, capsys):
        ...
        agent_file = tmp_path / "junction.json"
        agent_data = {
            "tools": ["@junction-core", "@junction-cron"],
            "allowedTools": ["@junction-core", "@junction-cron"],
            "mcpServers": {
                "junction-core": {"command": "/usr/local/bin/junction", "args": ["mcp-core"]},
                "junction-cron": {"command": "/usr/local/bin/junction", "args": ["mcp-cron"]},
            },
        }
        ...
        with (
            ...
            patch.dict(
                "os.environ",
                {"JUNCTION_PROJECT_DIR": "", "SLACK_APP_TOKEN": "", "SLACK_BOT_TOKEN": ""},
                clear=False,
            ),
        ):
            with pytest.raises(SystemExit):
...
class TestDoctorMcpTools:
    ...
    The helper live-probes only the managed servers (`junction-core`,
    `junction-cron`) via `probe_server`; tests monkey-patch that call so
    ...
    def _mock_probe(self, results: dict[str, tuple[str, list[str], str]]):
        ...
        return patch("junction.cli_doctor.probe_server", side_effect=fake)

    def test_success_shows_tool_counts(self, tmp_path, capsys):
        from junction.cli_doctor import _doctor_mcp_tools
        ...
        with self._mock_probe(
            {
                "junction-core": ("ok", ["spawn_run", "learn_add", "task_run"], ""),
                "junction-cron": ("ok", ["cron_add"], ""),
            }
        ):
            _doctor_mcp_tools(agent_path, issues)
        out = capsys.readouterr().out
        assert "@junction-core: ✅ 3 tools" in out
        assert "@junction-cron: ✅ 1 tool" in out
        assert issues == []
    ...
    def test_failure_shows_error_head_and_indented_stderr(self, tmp_path, capsys):
        ...
        _write_agent_config(
            agent_path,
            tools=["@junction-core", "@junction-cron"],
            allowed=["@junction-core", "@junction-cron"],
            servers={
                "junction-core": {"command": "/bin/junction", "args": ["mcp-core"]},
                "junction-cron": {"command": "/bin/junction", "args": ["mcp-cron"]},
            },
        )
        issues: list[str] = []
        fail_err = (
        ...
        assert "@junction-core: ❌ no response" in out
        assert "      stderr: Directory isn't within a workspace" in out
        assert "FindupException" in out
        assert "@junction-cron: ✅ 0 tools" in out
        assert "@junction-core probe" in issues
        # Healthy server must not pollute the issue list.
        assert "@junction-cron probe" not in issues
    ...
    def test_auto_fix_adds_missing_tools_and_allowed(self, tmp_path, capsys):
        ...
        assert updated["tools"] == ["@junction-cron", "@junction-core"]
        assert updated["allowedTools"] == ["@junction-cron", "@junction-core"]

    def test_auto_fix_never_blanket_allows_computer_use(self, tmp_path, capsys):
        """**Doctor must never add ``@junction-computer`` to ``allowedTools``.**
        ...
        updated = json.loads(agent_path.read_text(encoding="utf-8"))
        assert "@junction-computer" in updated["tools"]
        assert "@junction-computer" not in updated["allowedTools"]
        assert not [t for t in updated["allowedTools"] if t.startswith("@junction-computer/")]
        # The other managed servers are unaffected — the carve-out is scoped.
        assert "@junction-core" in updated["allowedTools"]
        assert "@junction-cron" in updated["allowedTools"]
    ...
    def test_auto_fix_preserves_a_user_made_computer_use_grant(self, tmp_path, capsys):
        ...
        with self._mock_probe({}):
            _doctor_mcp_tools(agent_path, issues)
        updated = json.loads(agent_path.read_text(encoding="utf-8"))
        assert "@junction-computer" in updated["allowedTools"]
    ...
    def test_probe_exception_does_not_crash(self, tmp_path, capsys):
        ...
        issues: list[str] = []
        with patch(
            "junction.cli_doctor.probe_server",
    ...
    def test_only_managed_servers_are_probed(self, tmp_path, capsys):
        ...
        keeps doctor output focused on Junction's own servers and avoids
        ...
        _write_agent_config(
            agent_path,
            tools=["@junction-core", "@junction-cron", "@builder-mcp"],
            allowed=["@junction-core", "@junction-cron", "@builder-mcp"],
            servers={
                "junction-core": {"command": "/bin/junction", "args": ["mcp-core"]},
                "junction-cron": {"command": "/bin/junction", "args": ["mcp-cron"]},
        ...
        with patch("junction.cli_doctor.probe_server", side_effect=recording_probe):
            _doctor_mcp_tools(agent_path, issues)
        assert probed_names == ["junction-cron", "junction-core"]
        out = capsys.readouterr().out
        assert "@builder-mcp" not in out

    def test_malformed_agent_config_does_not_crash(self, tmp_path, capsys):
        """If junction.json is truncated or otherwise unparseable, doctor
        ...
        agent_path.write_text('{"tools": ["@junction-c')
...
class TestDoctorStt:
    ...
    def test_doctor_stt_enabled_all_found(self, tmp_path, capsys):
        from junction.config.loader import JunctionConfig, SttConfig
        ...
        with (
            ...
            patch("junction.cli_doctor.is_local_only", return_value=True),
            patch("junction.cli_doctor.config_dir", return_value=tmp_path),
            patch("junction.cli_doctor._find_whisper", return_value="/usr/local/bin/whisper"),
            patch("junction.cli_doctor.ensure_ffmpeg_in_path"),
            patch("junction.cli_doctor.JunctionConfig.load", return_value=cfg),
            patch("junction.slack.enterprise.validate_enterprise", return_value=True),
            patch("junction.cli_doctor.probe_server", side_effect=_noop_probe_server),
        ):
            try:
                _doctor()
    ...
    def test_doctor_stt_disabled(self, tmp_path, capsys):
        ...
        with (
            ...
            patch("junction.slack.enterprise.validate_enterprise", return_value=True),
            patch("junction.cli_doctor.probe_server", side_effect=_noop_probe_server),
            patch("junction.cli_doctor._find_whisper", return_value=None),
            patch("junction.cli_doctor.ensure_ffmpeg_in_path"),
        ):
            try:
                _doctor()
    ...
    def test_doctor_stt_transcribe_amazon_transcribe_missing(self, tmp_path, capsys, monkeypatch):
        ...
        assert "pip install 'junction[voice]'" in out
...
class TestConfigDirOverride:
    """Tests that CLI functions respect JUNCTION_HOME env var via config_dir()."""
    ...
    def test_project_dir_file_uses_config_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli.config_dir", lambda: tmp_path)

        from junction.cli import _project_dir_file
    ...
    def test_detect_project_dir_reads_from_config_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli.config_dir", lambda: config_home)
        monkeypatch.chdir(tmp_path)  # CWD has no project markers

        from junction.cli import _detect_project_dir
    ...
    def test_detect_project_dir_no_agents_dir(self, tmp_path, monkeypatch):
        ...
        Regression guard: agent config was consolidated into src/junction/config/
        ...
        """
        proj = tmp_path / "Junction"
        (proj / "skills").mkdir(parents=True)
        (proj / "src" / "junction").mkdir(parents=True)
        assert not (proj / "agents").exists()

        monkeypatch.setattr("junction.cli.config_dir", lambda: tmp_path / "cfg")
        (tmp_path / "cfg").mkdir()
    ...
    def test_logout_reads_secret_for_listener_port(self, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.read_local_secret", read_secret)
    ...
    def test_setup_slack_tokens_locks_env_to_owner(self, tmp_path, monkeypatch):
        ...
        import junction.cli_setup as cs
        from junction import platform_compat as pc

        env_file = tmp_path / ".env"
        monkeypatch.setattr("junction.cli_setup.env_path", lambda: env_file)
        inputs = iter(["y", "xapp-test", "xoxb-test", "U12345"])
        monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    ...
    def test_setup_slack_tokens_aborts_when_shared_env_lock_is_held(
        ...
        # Simulate another writer (e.g. `junction secrets import`) holding the
...
class TestSetupChannelGating:
    """`junction setup` runs the Slack steps only with --slack.
    ...
    def _run_setup(self, monkeypatch, tmp_path, **kwargs):
        ...
        monkeypatch.setattr(
            "junction.agent.install_agent", lambda clean=False: tmp_path / "agent.json"
        )
        ...
        monkeypatch.setattr("junction.agent.ensure_junction_on_path", lambda *a, **k: None)
        monkeypatch.setattr("junction.mcp_cleanup.clean_stale_managed_mcp", lambda: [])
        ...
        monkeypatch.setattr(
            cs, "JunctionConfig", MagicMock(load=MagicMock(side_effect=RuntimeError("no config")))
        )
    ...
    def test_the_whatsapp_flag_reaches_setup_from_the_command_line(self):
        ...
        ``junction setup --whatsapp`` exits 2 instead of running anything."""
        import sys

        argv = ["junction", "setup", "--whatsapp"]
        with patch.object(sys, "argv", argv), patch("junction.cli._setup") as mock_setup:
            from junction.cli import main
...
class TestSpawnCliAuth:
    """``junction spawn`` attaches X-Internal-Secret on every gateway call.
    ...
    def test_internal_secret_reads_local_secret_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("junction.cli_commands.read_local_secret", lambda _port: "abc123")

        from junction.cli_commands import _internal_secret

        assert _internal_secret(5476) == "abc123"

    def test_internal_secret_returns_empty_when_missing(self, tmp_path, monkeypatch):
        monkeypatch.setattr("junction.cli_commands.read_local_secret", lambda _port: "")
    ...
    def test_spawn_list_sends_internal_secret_header(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr(
            "junction.cli_commands.read_local_secret", lambda _port: "test-secret-xyz"
        )
        ...
        monkeypatch.setattr("junction.cli_commands.loopback_urlopen", fake_urlopen)

        from junction.cli_commands import _spawn
    ...
    def test_spawn_run_sends_internal_secret_header(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr(
            "junction.cli_commands.read_local_secret", lambda _port: "run-secret-abc"
        )
        ...
        from junction.cli_commands import _spawn_run
...
class TestArtifactCli:
    ...
    def test_save_refuses_sensitive_content_file(
        ...
        from junction.cli_commands import _artifact

        monkeypatch.setattr("junction.cli_commands.is_sensitive_path", lambda _p: True)
...
class TestMcpBuiltinDispatch:
    ...
    def test_mcp_builtin_dispatches_to_module(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["junction", f"mcp-{builtin_name}"])
        with patch("importlib.import_module", return_value=mock_module) as mock_import:
            cli_mod.main()

        mock_import.assert_called_once_with(f"junction.apps.builtins.{builtin_name}.mcp_server")
...
class TestSeedDispatch:
    ...
    def test_seed_calls_seed_cmd(self, monkeypatch):
        ...
        with (
            patch("junction.cli.seed_cmd", mock_seed),
            patch("junction.cli_server._gateway"),
            patch("junction.cli.asyncio.run"),
        ):
            from junction.cli import main
    ...
    def test_seed_nonzero_exits(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["junction", "gateway", "--seed", "bad"])
        mock_seed = MagicMock(return_value=1)
        with patch("junction.cli.seed_cmd", mock_seed):
            from junction.cli import main
    ...
    def test_no_seed_skips_seed_cmd(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["junction", "gateway"])
        mock_seed = MagicMock()
        with (
            patch("junction.cli.seed_cmd", mock_seed),
            patch("junction.cli_server._gateway"),
    ...
    def test_seed_with_replace_flag(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            ["junction", "gateway", "--seed", "demo", "--seed-replace"],
        )
        mock_seed = MagicMock(return_value=0)
        with (
            patch("junction.cli.seed_cmd", mock_seed),
...
class TestDoctorEmbeddings:
    ...
    def _run_doctor(
        ...
        *,
        ...
        agent_file = tmp_path / "junction.json"
        _healthy_agent_file(agent_file)
        import junction.cli_doctor as doc
        ...
        with (
            patch("junction.cli_doctor.KIRO_AGENTS_DIR", tmp_path),
            patch("junction.cli_doctor.subprocess.run", return_value=default_run),
...
class TestWaitGatewayReady:
    ...
    def test_dead_child_short_circuits_with_its_exit_status(self):
        ...
        with (
            patch("junction.cli_server._probe_gateway_ready", probe),
            patch("junction.cli_server.time.sleep") as mock_sleep,
        ):
    ...
    def test_zero_timeout_still_probes_once(self):
        ...
        with (
            patch("junction.cli_server._probe_gateway_ready", probe),
            patch("junction.cli_server._replacement_is_serving", return_value=True),
        ):
    ...
    def test_not_ready_within_deadline_is_a_timeout(self):
        ...
        with patch("junction.cli_server._probe_gateway_ready", return_value=503):
    ...
    def test_polls_until_the_replacement_answers(self):
        ...
        with (
            patch("junction.cli_server._probe_gateway_ready", side_effect=[0, 503, 200]),
            patch("junction.cli_server._replacement_is_serving", return_value=True),
            patch("junction.cli_server.time.sleep") as mock_sleep,
        ):
            verdict = cli_server._wait_gateway_ready(
    ...
    def test_child_that_exits_during_the_last_probe_is_reported_as_died(self):
        ...
        with (
            patch("junction.cli_server._probe_gateway_ready", return_value=503),
    ...
    def test_missing_marker_is_never_the_replacement(self):
        ...
        with (
            patch("junction.cli_server.run_marker.read_pid", return_value=None),
            patch("junction.cli_server.platform_compat.IS_POSIX", False),
        ):
    ...
    def test_unchanged_marker_pid_is_the_old_gateway(self):
        ...
        with patch("junction.cli_server.run_marker.read_pid", return_value=1234):
            assert cli_server._replacement_is_serving(7777, 1234) is False
    ...
    def test_changed_marker_pid_that_owns_the_port_is_the_replacement(self):
        ...
        with (
            patch("junction.cli_server.run_marker.read_pid", return_value=4321),
            patch("junction.cli_server.platform_compat.IS_POSIX", True),
            patch(
                "junction.cli_server.platform_compat.listening_pid_tool_available",
                return_value=True,
            ),
            patch("junction.cli_server._gateway_owns_port", return_value=True) as mock_owns,
        ):
    ...
    def test_non_posix_degrades_to_the_marker_comparison(self):
        ...
        with (
            patch("junction.cli_server.run_marker.read_pid", return_value=4321),
            patch("junction.cli_server.platform_compat.IS_POSIX", False),
            patch("junction.cli_server._gateway_owns_port", return_value=False) as mock_owns,
        ):
            assert cli_server._replacement_is_serving(7777, 1234) is True
    ...
    def test_probe_targets_api_ready_not_api_health(self):
        ...
        with patch("junction.cli_server.loopback_urlopen", return_value=resp) as mock_open:
    ...
    def test_probe_reports_zero_when_unreachable(self):
        ...
        with patch(
            "junction.cli_server.loopback_urlopen", side_effect=urllib.error.URLError("down")
        ):
            assert cli_server._probe_gateway_ready(7777) == 0
    ...
    def test_probe_reports_the_http_status_of_a_not_ready_gateway(self):
        ...
        with patch("junction.cli_server.loopback_urlopen", side_effect=err):
...
class TestPrintTokenUrl:
    ...
    def test_prints_token_on_success(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("junction.cli_server.read_local_secret", lambda _port: "test-secret")
        monkeypatch.setattr(
            "junction.cli_server.JunctionConfig.load",
        ...
        monkeypatch.setattr(
            "junction.cli_server.resolve_dashboard_host",
    ...
    def test_prints_custom_origin(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "junction.cli_server.JunctionConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="http://junction.dev:7777")),
        )
        monkeypatch.setattr(
            "junction.cli_server.dashboard_origin", lambda u: "http://junction.dev:7777"
        )
        ...
        out = capsys.readouterr().out
        assert "http://junction.dev:7777/?token=xyz789" in out
...
class TestInstallPidfdChildWatcher:
    ...
    def _install_then_spawn_in_child(expected_watcher: str) -> None:
        ...
        ``junction gateway`` -- so this deterministically exercises the real
        ...
        code = textwrap.dedent("""
            import asyncio
            from junction.cli import _install_child_watcher
        ...
        # Propagate the runtime's import path so the child can import junction
    ...
    def test_installs_safe_watcher_on_macos(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.cli.sys.platform", "darwin")
        ...
        monkeypatch.setattr("junction.cli.os.pidfd_open", _boom, raising=False)
    ...
    def test_sets_watcher_on_linux(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.cli.sys.platform", "linux")
        ...
        closed = []
        monkeypatch.setattr(
            "junction.cli.os.pidfd_open", lambda pid: opened.append(pid) or 4242, raising=False
        )
        monkeypatch.setattr("junction.cli.os.close", lambda fd: closed.append(fd))
        called_with = []
    ...
    def test_falls_back_to_safe_watcher_on_old_kernel(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.cli.os.pidfd_open", _no_pidfd, raising=False)
        installed = []
    ...
    def test_falls_back_to_safe_watcher_when_pidfd_open_missing(self, monkeypatch) -> None:
        ...
        monkeypatch.delattr("junction.cli.os.pidfd_open", raising=False)
        installed = []
        monkeypatch.setattr(asyncio, "set_child_watcher", lambda w: installed.append(w))
        monkeypatch.setattr(asyncio, "SafeChildWatcher", lambda: "fake-safe-watcher")
...
class TestChildWatcherApiRemoved:
    ...
    thing ``junction gateway`` did was raise ``AttributeError: module 'asyncio'
    ...
    def test_noop_when_child_watcher_api_removed(self, monkeypatch, platform) -> None:
        ...
        monkeypatch.setattr("junction.cli.sys.platform", platform)
        ...
        # stubbing os.close: `junction.cli.os` IS the shared os module, so a
        # stub there would also be seen by asyncio's own internals.
        monkeypatch.setattr(
            "junction.cli.os.pidfd_open",
...
class TestTokenCommand:
    """Tests for the ``junction token`` command handler (``_token``)."""
    ...
    def test_prints_loopback_only(self, tmp_path, capsys, monkeypatch):
        from junction.cli_server import _token
        ...
        monkeypatch.setattr("junction.cli_server.dashboard_origin", lambda u: "")
    ...
    def test_separates_custom_origin_with_blank_line(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "junction.cli_server.JunctionConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="https://junction.dev:7777")),
        )
        monkeypatch.setattr(
            "junction.cli_server.dashboard_origin", lambda u: "https://junction.dev:7777"
        )
        monkeypatch.setattr(
            "junction.cli_server.resolve_dashboard_host",
            lambda local_only=True: "canonical-host.invalid",
        ...
        custom_line = "https://junction.dev:7777/?token=xyz789"
    ...
    # (junction.instances.token_mint.mint_remote_token) over SSH. Error prose on
    ...
    def _stub_token_env(self, tmp_path, monkeypatch, *, secret: bool = True) -> None:
        ...
        monkeypatch.setattr("junction.cli_server.read_local_secret", lambda _port: value)
        monkeypatch.setattr(
            "junction.cli_server.JunctionConfig.load",
            lambda: MagicMock(dashboard=MagicMock(url="")),
        )
    ...
    def test_unreachable_gateway_error_goes_to_stderr(self, tmp_path, capsys, monkeypatch):
        ...
        self._stub_token_env(tmp_path, monkeypatch)
        with patch(
            "junction.cli_server.loopback_urlopen", side_effect=urllib.error.URLError("refused")
        ):
            with pytest.raises(SystemExit) as excinfo:
                _token(argparse.Namespace(ttl="1h", port=7777))
        assert excinfo.value.code == 1
    ...
    def test_empty_token_error_goes_to_stderr(self, tmp_path, capsys, monkeypatch):
        ...
        self._stub_token_env(tmp_path, monkeypatch)
        with patch(
            "junction.cli_server.loopback_urlopen", return_value=self._mock_token_response("")
        ):
            with pytest.raises(SystemExit) as excinfo:
                _token(argparse.Namespace(ttl="1h", port=7777))
        assert excinfo.value.code == 1
...
class TestBannerBranding:
    ...
    def test_banner_is_single_sourced(self):
        ...
        """
        from junction.cli import BANNER as MAIN
        from junction.cli_chat import BANNER as CHAT
        from junction.constants import BANNER as CANON
    ...
    def test_no_reinlined_banner_literal(self):
        ...
        import junction.cli as cli_mod
        import junction.cli_chat as chat_mod
...
class TestChatPermissionRequest:
    """`junction chat` must ANSWER a permission request, and answering one is an
...
"""
    ...
    def test_denial_notice_survives_a_non_utf8_redirected_stderr(self, tmp_path, io_encoding):
        ...
        env["JUNCTION_HOME"] = str(tmp_path / "home")
...
class TestChatProviderShutdown:
    ...
    def _patch(self, monkeypatch, provider):
        ...
        monkeypatch.setattr(JunctionConfig, "load", staticmethod(lambda: JunctionConfig()))
        monkeypatch.setattr(
