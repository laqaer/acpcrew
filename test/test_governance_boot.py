...
from junction.platform.context import CONTRACT_VERSION, PlatformCompositionError
...
def test_standalone_no_policy_is_none(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("junction.platform.governance._policy_home_path", lambda: _nope)
    ctx = build_default_context(JunctionConfig.load())
...
def test_unreadable_policy_aborts_boot(monkeypatch, tmp_path):
    ...
    with pytest.raises(PlatformCompositionError):
        build_default_context(JunctionConfig.load())
...
def test_looser_profile_ordinal_aborts_boot(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("JUNCTION_SECURITY_POLICY", str(p))
    try:
        from junction.platform import bootstrap

        with pytest.raises(PlatformCompositionError):
            bootstrap.bootstrap_context(JunctionConfig.load())
    finally:
        gp.reset_store()
...
def test_profile_within_ceiling_boots(monkeypatch, tmp_path):
    ...
    try:
        ...
        bootstrap.bootstrap_context(JunctionConfig.load())  # no raise
    finally:
        gp.reset_store()
