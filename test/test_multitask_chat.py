...
def _slot_save(side_effect: BaseException | None = None):
    ...
    Bound on `multitask_chat` itself, not on `chat_persistence`: the import is at
    ...
    """
    return patch(
        "junction.multitask_chat.save_slot_off_loop",
...
class TestIngest:
    ...
    async def test_dispatch_warms_the_agent_cache_before_spawning(self) -> None:
        ...
        with patch("junction.multitask_chat.warm_project_agents_for_spawn", new=warm), \
             patch.object(orch, "_post"):
...
class TestGptRoundFive:
    ...
    def test_the_gateway_boot_path_does_not_import_crew(self) -> None:
        ...
        assert imported == "False", (
            "importing the gateway pulled in multitask_chat; a module-level import drifted back"
        )
    ...
    async def test_dispatch_runs_in_the_slots_own_project(self) -> None:
        ...
        warmed: list[str] = []
        with patch("junction.multitask_chat.warm_project_agents_for_spawn",
    ...
    async def test_crew_chat_does_not_import_the_dashboard_handler_tree(self) -> None:
        ...
        prog = (
            "import sys\n"
            "import junction.multitask_chat  # noqa: F401\n"
