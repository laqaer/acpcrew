"""Regression tests for Round-23 findings (Junction PR #6).

F1: the fullstack IAM policy conditions iam:CreateRole on the
    junction-deploy-app-boundary permissions boundary and denies boundary
...
_TPL = _ROOT / "src/junction/deploy/skills/artifact-deploy/templates"
...
def test_f1_create_role_requires_boundary():
    ...
    for st in create_stmts:
        ...
        assert "junction-deploy-app-boundary" in bound
...
def test_f1_boundary_policy_document_is_scoped():
    ...
    assert "junction-deploy-app-*" in body
...
def test_f1_all_template_roles_carry_boundary():
    for name in ("app-apigw.yaml", "app-apigw-ddb.yaml", "app-lambda.yaml"):
        ...
        assert "junction-deploy-app-boundary" in src
