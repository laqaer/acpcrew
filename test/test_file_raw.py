...
from junction.dashboard.handlers import api_file_raw
...
async def test_rejects_missing_file(tmp_path, mock_sel):
    ...
    with patch("junction.dashboard.handlers._validate_dashboard_path", return_value=missing):
        async with TestClient(TestServer(_make_app())) as client:
...
class TestOpenRbNofollow:
    ...
    def test_regular_file_opens_without_o_nofollow(self, tmp_path, monkeypatch):
        from junction.dashboard.handlers.files import _open_rb_nofollow
...
def test_both_endpoints_route_through_the_shared_envelope():
    ...
    from junction.dashboard.handlers import files as mod
...
async def test_the_envelope_still_rejects_a_symlink_for_both(tmp_path):
    ...
    for route in ("file-raw", "file-download"):
        with patch(
            ...
            "junction.dashboard.handlers.files.is_sensitive_path", return_value=False
