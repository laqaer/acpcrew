...
class TestBrandingConfig:
    ...
    def test_to_dict_roundtrip(self):
        cfg = JunctionConfig(dashboard=DashboardConfig(bot_name="Bot", avatar="~/pic.png"))
        d = cfg.to_dict()
...
class TestLogoHandler:
    ...
    async def test_logo_custom_avatar(self, tmp_path):
        from junction.dashboard.handlers import logo

        avatar = tmp_path / "custom.png"
        avatar.write_bytes(b"\x89PNG")
        cfg = JunctionConfig(dashboard=DashboardConfig(avatar=str(avatar)))
        req = MagicMock()
    ...
    async def test_logo_missing_custom_and_default_returns_404(self, tmp_path):
        ...
        cfg = JunctionConfig(dashboard=DashboardConfig(avatar="/nonexistent/path.png"))
        req = MagicMock()
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg), patch(
            "junction.dashboard.handlers._STATIC_DIR", tmp_path
        ):
            resp = await logo(req)
    ...
    async def test_logo_missing_custom_falls_back_to_default(self, tmp_path):
        ...
        default_logo = tmp_path / "junction-logo.png"
    ...
    async def test_logo_sensitive_path_rejected(self, tmp_path):
        ...
        cfg = JunctionConfig(dashboard=DashboardConfig(avatar=str(sensitive)))
        req = MagicMock()
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg), patch(
            "junction.hooks.validate_file_path", return_value=None
        ), patch("junction.dashboard.handlers._STATIC_DIR", tmp_path / "empty"):
            resp = await logo(req)
        assert resp.status == 404
    ...
    async def test_logo_nightly_version_serves_nightly_variant(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "junction-logo.png").write_bytes(b"\x89PNG-default")
        (tmp_path / "junction-logo-nightly.png").write_bytes(b"\x89PNG-nightly")
        monkeypatch.setattr("junction.__version__", "0.1.0-nightly.20260722223306")
        cfg = JunctionConfig(dashboard=DashboardConfig(avatar=""))
        req = MagicMock()
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg), patch(
            "junction.dashboard.handlers._STATIC_DIR", tmp_path
        ):
            resp = await logo(req)
        assert resp.status == 200
        assert resp._path.name == "junction-logo-nightly.png"
    ...
    async def test_logo_nightly_missing_asset_falls_back_to_default(self, tmp_path, monkeypatch):
        ...
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg), patch(
            ...
            resp = await logo(req)
        assert resp.status == 200
        assert resp._path.name == "junction-logo.png"
    ...
    async def test_logo_stable_version_ignores_nightly_variant(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "junction-logo.png").write_bytes(b"\x89PNG-default")
        (tmp_path / "junction-logo-nightly.png").write_bytes(b"\x89PNG-nightly")
        monkeypatch.setattr("junction.__version__", "0.1.0")
        cfg = JunctionConfig(dashboard=DashboardConfig(avatar=""))
        req = MagicMock()
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg), patch(
            "junction.dashboard.handlers._STATIC_DIR", tmp_path
...
class TestBrandingEndpoint:
    ...
    async def test_branding_defaults(self):
        from junction.dashboard.handlers import api_branding
    ...
    async def test_branding_custom_name(self):
        ...
        cfg = JunctionConfig(dashboard=DashboardConfig(bot_name="Jarvis"))
        req = MagicMock()
        with patch("junction.dashboard.handlers.JunctionConfig.load", return_value=cfg):
            resp = await api_branding(req)
        body = json.loads(resp.body)
...
class TestLogoAssetInvariant:
    ...
    ``src/junction/static/junction-logo.png`` (served at ``/logo.png`` for the
    ...
    def test_logo_matches_pwa_512_icon(self):
        ...
        served = repo_root / "src" / "junction" / "static" / "junction-logo.png"
        ...
        assert served_sha == pwa_sha, (
            "junction-logo.png and icon-512.png diverged — update both copies of "
