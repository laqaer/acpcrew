...
``test_issue_radar_steward_routes``.
"""
...
class FoldTest(unittest.TestCase):
    ...
    def test_no_crews_at_all_folds_to_no_items(self):
        ...
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        self.assertEqual(steward_store.fold_fabric(OWNER, REPO, Path(tmp.name)), [])
    ...
    def test_review_round_trip_head_is_last_in_time_not_furthest_reached(self):
        ...
        spine = steward_store.SPINE_PHASES
    ...
    def test_mixed_legacy_and_new_lines_fold_only_the_new_ones(self):
        ...
        steward_store.upsert_work_item(
            OWNER, REPO, self.cid, 5109, {"phase": "selected"}, self.root
        )
    ...
    def test_items_are_newest_progress_first(self):
        ...
        numbers = [it["number"] for it in steward_store.fold_fabric(OWNER, REPO, self.root)]
...
class RouteTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_empty_repo_answers_200_with_no_items(self):
        ...
        self.assertEqual(body["schema"], steward_store.FABRIC_SCHEMA)
        self.assertEqual(body["phases"], list(steward_store.SPINE_PHASES))
...
class TitleHintDegradationTest(unittest.TestCase):
    ...
    def test_an_unreadable_title_cache_still_folds_the_lane(self):
        ...
        with (
            ...
            items = steward_store.fold_fabric(OWNER, REPO, self.root)
