...
def _patch_statfns(monkeypatch, spec, *, uid=4242):
    """Patch os.getuid/os.lstat/os.stat in junction.env for a ccache test."""
    monkeypatch.setattr("junction.env.os.getuid", lambda: uid)
    lstat, stat_fn = _fake_statfns(spec)
    monkeypatch.setattr("junction.env.os.lstat", lstat)
    monkeypatch.setattr("junction.env.os.stat", stat_fn)
...
class TestAugmentedPath:
    ...
    def test_appends_running_interpreter_bin_dir_last(self, monkeypatch) -> None:
        ...
        on ``$PATH``, so ``shutil.which("junction")`` silently returns ``None``
        and every user-configured MCP that spawns the ``junction`` wrapper
        (e.g. ``junction-core``) is dropped. Appending ``sys.executable``'s
...
class TestContributedDirsNeverReachTheTrustedRuntime:
    ...
    def test_kiro_cli_search_dirs_exclude_contributed_dirs(self, tmp_path) -> None:
        from junction.kiro_cli import known_kiro_cli_dirs
...
class TestResolveKrb5Ccname:
    def test_prefers_uid_ccache(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env.sys.platform", "linux")
        monkeypatch.setattr("junction.env.platform_compat.IS_LINUX", True)
    ...
    def test_preserves_kcm_scheme(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.env.sys.platform", "darwin")
        monkeypatch.setattr("junction.env.platform_compat.IS_LINUX", False)
        _patch_statfns(monkeypatch, {"/tmp/krb5cc_4242": ("reg", 4242)})
        env = {"KRB5CCNAME": "KCM:"}
        resolve_krb5_ccname(env)
    ...
    def test_logs_resolved_path_on_success(self, monkeypatch, caplog) -> None:
        ...
        _patch_statfns(monkeypatch, {"/tmp/krb5cc_4242": ("reg", 4242)})
        env: dict[str, str] = {}
        with caplog.at_level(logging.DEBUG, logger="junction.env"):
            resolve_krb5_ccname(env)
    ...
    def test_never_calls_getuid_when_not_linux(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.env.os.getuid", _getuid_boom, raising=False)
        env: dict[str, str] = {}
        resolve_krb5_ccname(env)
        assert calls == []
        assert "KRB5CCNAME" not in env
...
class TestActivateMise:
    def test_noop_when_mise_absent(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: None)
        env: dict[str, str] = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}

    def test_noop_when_disabled_via_env(self, monkeypatch) -> None:
        # JUNCTION_NO_MISE escape hatch short-circuits before mise is invoked.
        called = {"n": 0}
        monkeypatch.setattr("junction.env._mise_bin", lambda: called.__setitem__("n", 1))
        env = {"PATH": "/usr/bin", "JUNCTION_NO_MISE": "1"}
        assert activate_mise(env) == []
        assert called["n"] == 0  # _mise_bin never consulted

    def test_merges_path_and_added_vars(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: "/home/u/.local/bin/mise")
        payload = json.dumps(
            {
        ...
        monkeypatch.setattr("junction.env.subprocess.run", _fake_run(stdout=payload))
        env = {"PATH": "/usr/bin"}
    ...
    def test_skips_unchanged_vars(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: "/m")
    ...
    def test_nonzero_exit_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: "/m")
        monkeypatch.setattr(
            "junction.env.subprocess.run", _fake_run(returncode=1, stderr="boom")
        )
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}
    ...
    def test_unparsable_json_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: "/m")
        monkeypatch.setattr("junction.env.subprocess.run", _fake_run(stdout="not json{"))
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []

    def test_non_dict_json_is_noop(self, monkeypatch) -> None:
        monkeypatch.setattr("junction.env._mise_bin", lambda: "/m")
        monkeypatch.setattr("junction.env.subprocess.run", _fake_run(stdout="[1, 2, 3]"))
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
    ...
    def test_subprocess_failure_is_swallowed(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.env.subprocess.run", _boom)
        env = {"PATH": "/usr/bin"}
        assert activate_mise(env) == []
        assert env == {"PATH": "/usr/bin"}
...
class TestGitBuildInfo:
    """junction.env.git_build_info reports the running checkout's branch+sha."""
    ...
    def test_reads_branch_and_commit(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("junction.env.subprocess.run", _run)
    ...
    def test_fails_open_on_nonzero_exit(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setenv("JUNCTION_PROJECT_DIR", str(tmp_path))
        monkeypatch.setattr(
            "junction.env.subprocess.run", _fake_run(returncode=128, stderr="fatal")
        )
