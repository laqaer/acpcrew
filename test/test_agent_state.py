...
def test_lifts_when_unset():
    config = {"name": "junction", "model_managed": True, "cc_model": "claude-sonnet-4.6"}

    changed = agent_state.lift_and_strip_bookkeeping(config, "junction")
...
def test_non_string_cc_model_discarded_not_lifted(caplog):
    ...
    assert changed is True
    assert "cc_model" not in config
    assert agent_state.get_cc_model("junction") is None
    assert "non-string cc_model" in caplog.text


def test_returns_false_when_neither_key_present():
    config = {"name": "junction", "model": "auto"}

    changed = agent_state.lift_and_strip_bookkeeping(config, "junction")

    assert changed is False
    assert config == {"name": "junction", "model": "auto"}
