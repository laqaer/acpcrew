...
from junction.slack.handler import set_allowed_users, set_owner_id
...
class FakeSlackConfig:
    forward_to_agent_callback: str = "send_to_junction"
...
class FakeOrch:
    def __init__(self, callback="send_to_junction"):
...
class TestHandleMessageShortcut:
    ...
    async def test_opens_modal_for_allowed_user(self, orch):
        ...
        assert view["callback_id"] == "send_to_junction"
    ...
    async def test_modal_metadata_round_trips_into_submission(self, orch):
        ...
        with patch("junction.slack.interactions._orch", orch):
            ...
            with patch("junction.slack.interactions.handle_message", new_callable=AsyncMock):
    ...
    async def test_forwarded_body_is_quarantined_as_untrusted(self, orch):
        ...
        with patch("junction.slack.interactions._orch", orch):
            ...
            built = orch.slack.views_opened[0]["view"]
            with patch(
                "junction.slack.interactions.handle_message", new_callable=AsyncMock
            ) as hm:
                await _handle_shortcut_submission({
                    "user": {"id": "U_OWNER"},
                    "team": {"id": "T_TEAM"},
    ...
    async def test_redacts_exfiltration_urls_and_credentials(self, orch):
        ...
        set_allowed_users({"U_OWNER"})
        with patch("junction.slack.interactions._orch", orch), \
             patch("junction.slack.interactions.redact_exfiltration_urls") as mock_urls, \
             patch("junction.slack.interactions.redact_credentials") as mock_creds:
...
class TestHandleShortcutSubmission:
    ...
    async def test_routes_forwarded_message_to_dm(self, orch):
        ...
        with patch("junction.slack.interactions._orch", orch), \
             patch("junction.slack.interactions.handle_message", new_callable=AsyncMock):
            payload = {
                "user": {"id": "U_OWNER"},
                "team": {"id": "T_TEAM"},
                "view": {
    ...
    async def test_rejects_unauthorized_user_logs_sel(self, orch):
        ...
        set_allowed_users({"U_OWNER"})
        with patch("junction.slack.interactions._orch", orch), \
             patch("junction.slack.interactions.sel") as mock_sel:
...
class TestInit:
    def test_registers_handler_when_callback_configured(self, orch):
        with patch("junction.slack.interactions._orch", orch):
            init(orch)
        assert "send_to_junction" in VIEW_REGISTRY

    def test_no_registration_when_callback_empty(self):
        orch = FakeOrch(callback="")
        with patch("junction.slack.interactions._orch", orch):
            init(orch)
        assert "send_to_junction" not in VIEW_REGISTRY
    ...
    async def test_submission_dispatches_after_live_reconfig(self, orch):
        ...
        with patch("junction.slack.interactions._orch", empty):
            init(empty)
        assert "send_to_junction" not in VIEW_REGISTRY
        ...
        with patch("junction.slack.interactions._orch", orch):  # orch has callback set
            with patch(
                "junction.slack.interactions.handle_message", new_callable=AsyncMock
            ) as hm:
